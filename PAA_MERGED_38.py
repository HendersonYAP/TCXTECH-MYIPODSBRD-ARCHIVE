...
#   MYBook.csv                   — NEWPOR Google Sheet         (SHA-256 delta)
# =============================================================================
...
def _ljson(key: str) -> str:
    return os.path.join(_CACHE_LIVE_DIR, f'{key}.json')
...
DEFAULT_NEWPOR_FILE  = os.path.join(DEFAULT_FOLDER, 'MYBook.csv')   # transaction ledger — mirrors teet.csv pattern
...
class MYIPOApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        self.notebook.add(self.tab_fund_ledger, text='📒  Fund Ledger')
    ...
    def _open_settings(self):
        """Settings dialog — AppLock toggle, cache info, upcoming features."""
        ...
        def _on_toggle():
            s = load_app_settings(); s["applock_enabled"] = applock_var.get(); save_app_settings(s)
        tk.Checkbutton(box, text="🔒  Require AppLock PIN on startup",
                       variable=applock_var, command=_on_toggle,
                       bg='#161b22', fg='#cdd9e5', selectcolor='#161b22',
                       activebackground='#161b22', activeforeground='#58A6FF',
                       font=('Segoe UI', 10), relief='flat', cursor='hand2').pack(anchor='w')
        ...
        # Upcoming: Gamify
        next_box = tk.Frame(win, bg='#161b22', highlightbackground='#30363D',
                            highlightthickness=1, padx=16, pady=12)
        next_box.pack(fill='x', padx=20, pady=(10, 0))
        tk.Label(next_box, text="🎮  NEXT UPDATE — GAMIFY",
                 font=("Segoe UI", 9, "bold"), fg='#D29922', bg='#161b22').pack(anchor='w')
        tk.Label(next_box,
                 text="Gamified scoring system for MYIPO+ and Bursa constituents:\n"
                      "· Strike tables per index (MYIPO+, ACE, Main, Dynamic365)\n"
                      "· Binary instrument P/L tracking with Quant1 × price scoring\n"
                      "· Leaderboard and ranking with zero-decay scoring model\n"
                      "(Planned for the next PAA_MERGED release)",
                 font=("Segoe UI", 8), fg='#888', bg='#161b22',
                 justify='left', wraplength=360).pack(anchor='w', pady=(6, 0))
    ...
    def _build_market_ticker(self, parent):
        """Horizontal strip: KLCI from Yahoo, MYIPO+ series from all_frames."""
        panel = tk.Frame(parent, bg='#0d1117',
                         highlightbackground='#21262d', highlightthickness=1)
        panel.pack(fill='x', pady=(0, 2))

        self._mkt_cells = {}
        for sym, label, _ in self.BURSA_INDICES:
            cell = tk.Frame(panel, bg='#0d1117', padx=10)
            cell.pack(side='left')
            lbl_w = tk.Label(cell, text=label, font=('Segoe UI', 7, 'bold'),
                     fg='#888', bg='#0d1117')
            lbl_w.pack(side='left', padx=(0, 4))
            val_var = tk.StringVar(value='—')
            chg_var = tk.StringVar(value='')
            val_w = tk.Label(cell, textvariable=val_var, font=('Segoe UI', 8, 'bold'),
                     fg='#cdd9e5', bg='#0d1117')
            val_w.pack(side='left')
            chg_lbl = tk.Label(cell, textvariable=chg_var,
                                font=('Segoe UI', 8), bg='#0d1117', fg='#888')
            chg_lbl.pack(side='left', padx=(2, 0))
            self._mkt_cells[sym] = (val_var, chg_var, chg_lbl)
            # Top 10 is clickable → historical in/out board
            if sym == 'MYIPO+ Top 10':
                for w in (cell, lbl_w, val_w, chg_lbl):
                    w.configure(cursor='hand2')
                    w.bind('<Button-1>', lambda e: self._open_top10_board())
                lbl_w.configure(fg='#D29922')   # amber to hint clickability

        tk.Frame(panel, bg='#21262d', width=1).pack(side='left', fill='y', pady=3, padx=6)
        self._mkt_time_var = tk.StringVar(value='')
        tk.Label(panel, textvariable=self._mkt_time_var,
                 font=('Segoe UI', 7), fg='#555', bg='#0d1117').pack(side='left', padx=4)
        tk.Button(panel, text='↻', font=('Segoe UI', 8), bg='#0d1117', fg='#444',
                  relief='flat', cursor='hand2',
                  command=self._refresh_market_ticker).pack(side='right', padx=6)

        self.after(500, self._refresh_market_ticker)

    def _refresh_market_ticker(self):
        """Refresh ticker strip — KLCI from Yahoo, MYIPO+ indices from all_frames."""
        ...
        def _worker():
            ...
            for sym, label, source in self.BURSA_INDICES:
                ...
                try:
                    ...
                    if df is None or df.empty:
                        continue
                    # all_frames stores DataFrames; the index column has the same
                    # name as the key (e.g. df['MYIPO+'] is the price series)
            ...
            if cached and '^KLSE' in cached:
                ...
                try:
                    ...
                    if last:
                        results['^KLSE'] = (float(last), float(prev) if prev else None)
                        cache_set_live_dict('market_indices_klci',
                                            {'^KLSE': {'last': float(last),
    ...
    def _apply_market_data(self, results):
        ...
        for sym, _, __ in self.BURSA_INDICES:
            if sym not in self._mkt_cells:
                continue
            val_var, chg_var, chg_lbl = self._mkt_cells[sym]
            last, prev = results.get(sym, (None, None))
            if last is None:
                val_var.set('N/A'); chg_var.set(''); chg_lbl.configure(fg='#555')
                continue
            val_var.set(f'{last:,.2f}')
            if prev and prev > 0:
                chg = last - prev
                pct = chg / prev * 100
                sgn = '+' if chg >= 0 else ''
                col = '#3FB950' if chg >= 0 else '#F85149'
                chg_var.set(f'{sgn}{chg:,.2f} ({sgn}{pct:.2f}%)')
                chg_lbl.configure(fg=col)
            else:
                chg_var.set('')
    ...
    def _open_wc_inspector(self, event=None):
        ...
        if not vals:
            return
        sym_short = str(vals[0]).strip()
        # comp table shows bare code; reconstruct the .KL symbol
        sym = sym_short if sym_short.endswith('.KL') else f'{sym_short}.KL'
    ...
    def _refresh_fund_events_dropdown(self):
        ...
        if hasattr(self, '_ledger_fund_cb'):
            self._ledger_fund_cb['values'] = FUND_LABELS
            if not self._ledger_fund_var.get() and FUND_LABELS:
                self._ledger_fund_var.set(FUND_LABELS[0])
    ...
    def _build_fund_ledger_tab(self):
        ...
        # Left card: Assets
        asset_card = tk.Frame(sofp_frame, bg='#161b22',
                              highlightbackground='#3FB950', highlightthickness=1,
                              padx=14, pady=8)
        asset_card.pack(side='left', fill='x', expand=True, padx=(0, 4))
        tk.Label(asset_card, text='ASSETS', font=('Segoe UI', 9, 'bold'),
                 fg='#3FB950', bg='#161b22').pack(anchor='w')
        self._sofp_asset_vars = {}
        for field in ('Holdings Value (RM)', 'Cash on Hand (RM)', 'Total Assets (RM)'):
            r = tk.Frame(asset_card, bg='#161b22'); r.pack(fill='x', pady=1)
            tk.Label(r, text=field, font=('Segoe UI', 8), fg='#888',
                     bg='#161b22', width=24, anchor='w').pack(side='left')
            v = tk.StringVar(value='—')
            lbl = tk.Label(r, textvariable=v, font=('Segoe UI', 8, 'bold'),
                           fg='#3FB950' if 'Total' in field else '#cdd9e5',
                           bg='#161b22', anchor='e')
            lbl.pack(side='right')
            self._sofp_asset_vars[field] = v

        # Middle card: Equity / Unit Capital
        equity_card = tk.Frame(sofp_frame, bg='#161b22',
                               highlightbackground='#58A6FF', highlightthickness=1,
                               padx=14, pady=8)
        equity_card.pack(side='left', fill='x', expand=True, padx=4)
        tk.Label(equity_card, text='UNIT CAPITAL (EQUITY)', font=('Segoe UI', 9, 'bold'),
                 fg='#58A6FF', bg='#161b22').pack(anchor='w')
        self._sofp_equity_vars = {}
        for field in ('Units Outstanding', 'NAV per Unit (RM)', 'Total NAV (RM)',
                      'Cum. Distributions/Unit (RM)', 'Total Return/Unit (RM)'):
            r = tk.Frame(equity_card, bg='#161b22'); r.pack(fill='x', pady=1)
            tk.Label(r, text=field, font=('Segoe UI', 8), fg='#888',
                     bg='#161b22', width=28, anchor='w').pack(side='left')
            v = tk.StringVar(value='—')
            lbl = tk.Label(r, textvariable=v, font=('Segoe UI', 8, 'bold'),
                           fg='#58A6FF' if 'Return' in field else '#cdd9e5',
                           bg='#161b22', anchor='e')
            lbl.pack(side='right')
            self._sofp_equity_vars[field] = v

        # Right card: Performance
        perf_card = tk.Frame(sofp_frame, bg='#161b22',
                             highlightbackground='#D29922', highlightthickness=1,
                             padx=14, pady=8)
        perf_card.pack(side='left', fill='x', expand=True, padx=(4, 0))
        tk.Label(perf_card, text='PERFORMANCE', font=('Segoe UI', 9, 'bold'),
                 fg='#D29922', bg='#161b22').pack(anchor='w')
        self._sofp_perf_vars = {}
        for field in ('Inception Date', 'Par NAV (RM)', 'NAV Return %',
                      'Total Return % (incl. dist)', 'N Distributions', 'N Unit Issuances'):
            r = tk.Frame(perf_card, bg='#161b22'); r.pack(fill='x', pady=1)
            tk.Label(r, text=field, font=('Segoe UI', 8), fg='#888',
                     bg='#161b22', width=26, anchor='w').pack(side='left')
            v = tk.StringVar(value='—')
            lbl = tk.Label(r, textvariable=v, font=('Segoe UI', 8, 'bold'),
                           fg='#D29922' if 'Return' in field else '#cdd9e5',
                           bg='#161b22', anchor='e')
            lbl.pack(side='right')
            self._sofp_perf_vars[field] = v
        ...
        cols = ('Date', 'Type', 'Description',
                'Debit (RM)', 'Credit (RM)',
                'Cash Balance (RM)', 'Units Out', 'NAV (RM)', 'Total Assets (RM)')
        self._ledger_tree = ttk.Treeview(frame, columns=cols, show='headings')
        cw = {'Date': 90, 'Type': 160, 'Description': 160,
              'Debit (RM)': 100, 'Credit (RM)': 100,
              'Cash Balance (RM)': 120, 'Units Out': 100,
              'NAV (RM)': 90, 'Total Assets (RM)': 120}
        for col in cols:
    ...
    def _draw_fund_ledger(self):
        ...
        if lbl in frames:
            ...
            aum_last   = aum_series.iloc[-1] if aum_series is not None and not aum_series.empty else 0.0
            units_last = units_s.iloc[-1]   if units_s   is not None and not units_s.empty   else FUND_UNITS_ISSUED
            ...
            nav_ret_pct = (nav_last / FUND_PAR_NAV - 1) * 100
            tr_ret_pct  = (tr_last  / FUND_PAR_NAV - 1) * 100
            ...
            self._sofp_asset_vars['Holdings Value (RM)'].set(f'{holdings_val:,.4f}')
            self._sofp_asset_vars['Cash on Hand (RM)'].set(f'{last_cash:,.4f}')
            self._sofp_asset_vars['Total Assets (RM)'].set(f'{aum_last:,.4f}')
            self._sofp_equity_vars['Units Outstanding'].set(f'{units_last:,.2f}')
            self._sofp_equity_vars['NAV per Unit (RM)'].set(f'{nav_last:,.4f}')
            self._sofp_equity_vars['Total NAV (RM)'].set(f'{aum_last:,.4f}')
            self._sofp_equity_vars['Cum. Distributions/Unit (RM)'].set(f'{cd_last:,.4f}')
            self._sofp_equity_vars['Total Return/Unit (RM)'].set(f'{tr_last:,.4f}')
            self._sofp_perf_vars['Inception Date'].set(inception_date)
            self._sofp_perf_vars['Par NAV (RM)'].set(f'{FUND_PAR_NAV:.4f}')
        ...
        for dt, nav_before, dist_per_unit, nav_after in dist_log:
            ...
            units_at_dist = FUND_UNITS_ISSUED
            cash_at_dist  = 0.0
            for entry in unit_log:
            ...
            if filt in ('All', 'Cash flows only'):
                events.append((dt, 'Distribution Paid',
                               f'RM{dist_per_unit:.4f}/unit × {units_at_dist:,.2f} units',
            ...
            if events:
                ...
                self._ledger_tree.insert('', 'end', tags=('section',),
                    values=(first_dt.strftime('%d/%m/%Y') if hasattr(first_dt, 'strftime') else str(first_dt),
                            '── OPENING ──',
                            f'Fund inception @ RM{FUND_PAR_NAV:.4f} par · {FUND_UNITS_ISSUED} units',
                            '', '', f'{FUND_UNITS_ISSUED * FUND_PAR_NAV:,.4f}',
                            f'{FUND_UNITS_ISSUED:,.2f}', f'{FUND_PAR_NAV:.4f}',
                            f'{FUND_UNITS_ISSUED * FUND_PAR_NAV:,.4f}'))
        ...
        self._ledger_summary_var.set(
            ...
            f'Total debits: RM{total_debits:,.4f}  ·  '
            f'Total credits: RM{total_credits:,.4f}  ·  '
            f'Net cash flow: RM{total_credits - total_debits:+,.4f}'
        )
    ...
    def _pts_load(self) -> dict:
        """Load PTS data from paa_settings.json."""
        s = load_app_settings()
        pts = s.get('pts', {})
        defaults = _DEFAULT_SETTINGS['pts']
        for k, v in defaults.items():
            pts.setdefault(k, v)
        return pts

    def _pts_save(self, pts: dict):
        """Persist PTS data to paa_settings.json."""
        s = load_app_settings()
        s['pts'] = pts
        save_app_settings(s)
    ...
    def _zdws_current_level(self, underlying: str):
        """Return current level for an underlying (internal index or KLCI)."""
        try:
            if underlying == '^KLSE':
                if hasattr(self, '_mkt_cells') and '^KLSE' in self._mkt_cells:
                    raw = self._mkt_cells['^KLSE'][0].get().replace(',', '').strip()
    ...
    def _zdws_auto_settle(self, pts: dict) -> list:
        ...
        for w in pts.get('zdws_active', []):
            ...
            if cur is None or w['entry'] <= 0:
                still_open.append(w); continue   # can't settle yet, keep open
            move_pct = (cur / w['entry'] - 1) * 100
    ...
    def _zdws_market_close(self, warrant_id: int):
        ...
        if cur is None:
            messagebox.showwarning('ZDWS', 'Cannot fetch current level — try again.', parent=self)
            return
        move_pct = (cur / pos['entry'] - 1) * 100
    ...
    def _build_pts_tab(self):
        ...
        # ── ZDWS auto-settle: settle warrants past settlement date ────────────
        ...
        # ── ZDWS settlement results banner ─────────────────────────────────────
        if zdws_msgs:
            zd_banner = tk.Frame(tab, bg='#0a1a2a',
                                 highlightbackground='#58A6FF', highlightthickness=1,
                                 padx=12, pady=8)
            zd_banner.pack(fill='x', padx=12, pady=(0, 6))
            tk.Label(zd_banner, text='⚡ ZDWS AUTO-SETTLEMENT',
                     font=('Segoe UI', 8, 'bold'), fg='#58A6FF',
                     bg='#0a1a2a').pack(anchor='w')
            for m in zdws_msgs[:5]:
                tk.Label(zd_banner, text=m, font=('Segoe UI', 8),
                         fg='#cdd9e5', bg='#0a1a2a').pack(anchor='w')

        # ── ZDWS active positions board ────────────────────────────────────────
        zdws_active = pts.get('zdws_active', [])
        if zdws_active:
            zd_board = tk.Frame(tab, bg=PANEL,
                                highlightbackground='#58A6FF', highlightthickness=1,
                                padx=12, pady=8)
            zd_board.pack(fill='x', padx=12, pady=(0, 6))
            tk.Label(zd_board,
                     text=f'⚡ ZDWS ACTIVE WARRANTS ({len(zdws_active)})',
                     font=('Segoe UI', 9, 'bold'), fg='#58A6FF',
                     bg=PANEL).pack(anchor='w', pady=(0, 4))
            for w_pos in zdws_active:
                cur = self._zdws_current_level(w_pos['underlying'])
                if cur and w_pos['entry']:
                    live_move = (cur / w_pos['entry'] - 1) * 100
                    winning = (live_move >= 0) if w_pos['side'] == 'BULL' else (live_move <= 0)
                    live_txt = f'{live_move:+.2f}%'
                    live_col = GREEN if winning else RED
                    est_pay  = w_pos['stake'] * (1 + abs(live_move) / 100 * 10) if winning else 0
                    est_txt  = f'→ est. {est_pay:,.0f} cr' if winning else '→ est. 0 cr'
                else:
                    live_txt, live_col, est_txt = '—', DIM, ''
                row = tk.Frame(zd_board, bg=PANEL)
                row.pack(fill='x', pady=1)
                side_col = GREEN if w_pos['side'] == 'BULL' else RED
                side_ico = '🔼' if w_pos['side'] == 'BULL' else '🔽'
                tk.Label(row, text=f"{side_ico} {w_pos['side']}",
                         font=('Segoe UI', 8, 'bold'), fg=side_col, bg=PANEL,
                         width=8, anchor='w').pack(side='left')
                tk.Label(row, text=w_pos['underlying'], font=('Segoe UI', 8),
                         fg=FG, bg=PANEL, width=16, anchor='w').pack(side='left')
                tk.Label(row, text=f"entry {w_pos['entry']:,.2f}",
                         font=('Segoe UI', 8), fg=DIM, bg=PANEL).pack(side='left', padx=4)
                tk.Label(row, text=live_txt, font=('Segoe UI', 8, 'bold'),
                         fg=live_col, bg=PANEL).pack(side='left', padx=4)
                tk.Label(row, text=f"{w_pos['stake']:,.0f} cr staked {est_txt}",
                         font=('Segoe UI', 8), fg=GOLD, bg=PANEL).pack(side='left', padx=4)
                tk.Label(row, text=f"settles {w_pos['settle_date']}",
                         font=('Segoe UI', 7), fg=DIM, bg=PANEL).pack(side='left', padx=4)
                tk.Button(row, text='✖ Close Now', font=('Segoe UI', 7),
                          bg='#21262d', fg=RED, relief='flat', padx=6, pady=1,
                          cursor='hand2',
        ...
        for i, m in enumerate(markets):
            short_q = m['question'][:55] + '…' if len(m['question']) > 55 else m['question']
            mkt_labels.append(f"[{m['category'][:8]}] {short_q}")
        ...
        def _update_market_ui(*_):
            sel_idx = mkt_cb.current()
            if sel_idx < 0 or sel_idx >= len(markets):
                return
            m = markets[sel_idx]
            # Update context + question
            ctx_var.set(m.get('context', ''))
            # Build question — replace {level} placeholder if multi-level
            q = m['question']
            if '{level}' in q:
                opt = pos_var.get() if pos_var.get() in m['options'] else m['options'][-1]
                q = q.replace('{level}', opt)
            q_var.set(q)
            # Rebuild position buttons
        ...
        def _pts_settlement_dates():
            """Generate all valid settlement dates from today + 7 days onward.
            Rules:
        ...
        _settle_dates = _pts_settlement_dates()
        ...
        def _submit_prediction():
            ...
            # ── ZDWS route: ZeroDecay Warrants go to the warrant engine ────────
            if cat == 'ZeroDecay Warrant' and 0 <= sel_idx < len(markets):
                meta = markets[sel_idx].get('meta', {})
                underlying = meta.get('zd_underlying')
                pos_sel    = pos_var.get()
                side = 'BULL' if 'LONG' in pos_sel.upper() else 'BEAR'
                ok = self._zdws_open_position(
                    underlying, side, stake, deadline.strftime('%d/%m/%Y'))
                if ok:
                    messagebox.showinfo('ZDWS',
                        f'⚡ ZeroDecay Warrant opened!\n\n'
                        f'{side} on {underlying}\n'
                        f'Entry: {meta.get("zd_entry", 0):,.2f}\n'
                        f'Stake: {stake:,.0f} credits\n'
                        f'Settles: {deadline.strftime("%d/%m/%Y")}\n\n'
                        f'Payout if correct: stake × (1 + |%move| × 10)\n'
                        f'Auto-settles when the date passes.',
                        parent=self)
                    self._build_pts_tab()
                else:
                    messagebox.showwarning('ZDWS',
                        'Could not open warrant — check stake and balance.',
                        parent=self)
                return
    ...
    def _pts_generate_markets(self) -> list:
        ...
        markets = []
        today = _dt.date.today()
        yr_end = f'31/12/{today.year}'

        # ── 1. KLCI ──────────────────────────────────────────────────────────
        try:
            if hasattr(self, '_mkt_cells') and '^KLSE' in self._mkt_cells:
                raw = self._mkt_cells['^KLSE'][0].get().replace(',', '').strip()
                klci_last = float(raw) if raw and raw not in ('—', 'N/A') else None
        ...
        for sym, label, source in getattr(self, 'BURSA_INDICES', []):
            ...
            try:
                if not hasattr(self, '_mkt_cells') or sym not in self._mkt_cells: continue
                raw = self._mkt_cells[sym][0].get().replace(',', '').strip()
                lv = float(raw) if raw and raw not in ('—', 'N/A') else None
        ...
        for idx_key, idx_label in INDEX_PLAY:
            try:
                ...
                markets.append({
                    ...
                    'type':      'Multi-Level',
                    'options':   [f'Yes by {yr_end}',
                                  f'Yes by Q3 {today.year}',
                                  f'No — will be lower'],
                    'source':    f'{idx_key} internal index',
                    'context':   ctx,
                    'suggested_position': 'Yes by ' + yr_end if ytd >= 0 else 'No — will be lower',
                })
        ...
        # ── 8. ZeroDecay Warrants — Long/Short on indices ─────────────────────
        # Zero time decay: P/L based purely on % move from entry to settlement.
        # Payout = stake × (1 + |%move| × 10) if direction correct, 0 if wrong.
        # Settles at each standard period (Friday / month / quarter / year-end).
        ZD_UNDERLYINGS = [
            ('MYIPO+',          'MYIPO+ Index'),
            ('MYIPO+ Main',     'MYIPO+ Main'),
            ('MYIPO+ Ace',      'MYIPO+ ACE'),
            ('MYIPO+ Top 10',   'MYIPO+ Top 10'),
        ]
        for idx_key, idx_label in ZD_UNDERLYINGS:
            try:
                if not hasattr(self, 'all_frames') or idx_key not in self.all_frames:
                    continue
                df_i = self.all_frames[idx_key]
                s = df_i[idx_key].dropna() if idx_key in df_i.columns else \
                    df_i.select_dtypes('number').iloc[:, 0].dropna()
                if s.empty: continue
                cur = float(s.iloc[-1])
                if cur <= 0: continue
                ...
                markets.append({
                    'category':  'ZeroDecay Warrant',
                    'question':  f'ZeroDecay {{level}} on {idx_label} @ {cur:,.2f} — '
                                 f'settle at period end, payout scales with % move',
                    'type':      'Multi-Level',
                    'options':   ['LONG 🔼', 'SHORT 🔽'],
                    'source':    f'{idx_key} ZeroDecay',
                    'context':   (f'Entry: {cur:,.2f} | Day: {day_chg:+.2f}% | '
                                  f'Win payout = stake × (1 + |%move| × 10) | '
                                  f'Zero time decay — pure directional bet'),
                    'meta':      {'zd_entry': cur, 'zd_underlying': idx_key},
                })
            except Exception:
                pass
        ...
        try:
            if hasattr(self, '_mkt_cells') and '^KLSE' in self._mkt_cells:
                raw = self._mkt_cells['^KLSE'][0].get().replace(',', '').strip()
                klci = float(raw) if raw and raw not in ('—', 'N/A') else None
                if klci and klci > 0:
                    markets.append({
                        'category':  'ZeroDecay Warrant',
                        'question':  f'ZeroDecay {{level}} on KLCI @ {klci:,.2f} — '
                                     f'settle at period end, payout scales with % move',
                        'type':      'Multi-Level',
                        'options':   ['LONG 🔼', 'SHORT 🔽'],
                        'source':    'KLCI ZeroDecay',
                        'context':   (f'Entry: {klci:,.2f} | '
                                      f'Win payout = stake × (1 + |%move| × 10) | '
                                      f'Zero time decay'),
                        'meta':      {'zd_entry': klci, 'zd_underlying': '^KLSE'},
                    })
        except Exception:
            pass
...
def _port_display_name(port_key: str) -> str:
    """Convert internal portfolio key to a human-friendly display label.
    SWAY1-MYR → MYR Position,  SWAY1 → USD Position.
    Falls back to the raw key for any other value."""
    return {'SWAY1-MYR': 'MYR Position',
            'SWAY1':     'USD Position'}.get(port_key, port_key)
...
_NEWPOR_LOCAL_CACHE_NAME = 'MYBook.csv'   # stored alongside teet.csv — matches the Google Sheet name "MYBook"

def _mf_fetch_online_chunked(url: str = MYFOLIO_CSV_URL, log_cb=None, chunk_size: int = 200):
    """
    Download MYBook CSV. Tries 4 methods in order until one works:
    ...
    try:
        ...
        r.raise_for_status()
        raw_bytes = r.content
        log(f'✅  MYBook: {len(raw_bytes)/1024:.1f} KB via requests')
    except ImportError:
        pass
    except Exception as e:
        last_error = e
    ...
    if raw_bytes is None:
        try:
            ...
            with urllib.request.urlopen(req, timeout=20, context=ctx) as r:
                raw_bytes = r.read()
            log(f'✅  MYBook: {len(raw_bytes)/1024:.1f} KB via urllib+certifi')
        except Exception as e:
            last_error = e
    ...
    if raw_bytes is None:
        try:
            ...
            with urllib.request.urlopen(req, timeout=20, context=ctx) as r:
                raw_bytes = r.read()
            log(f'✅  MYBook: {len(raw_bytes)/1024:.1f} KB via unverified SSL')
        except Exception as e:
            last_error = e
    ...
    if raw_bytes is None:
        try:
            ...
            with urllib.request.urlopen(req, timeout=20) as r:
                raw_bytes = r.read()
            log(f'✅  MYBook: {len(raw_bytes)/1024:.1f} KB via bare urllib')
        except Exception as e:
            last_error = e
    ...
    if raw_bytes is None:
        ...
        raise Exception(f'MYBook download failed (all methods).\n{err_str}{hint}')
...
def _mf_delta_sync(online_text: str, cache_path: str, log_cb=None) -> tuple[str, bool]:
    """
    Delta-sync strategy for the NEWPOR ledger:
    NEWPOR transactions are always appended chronologically — existing rows
    never change, only new rows are added at the bottom. So instead of
    re-downloading and replacing the entire cache, we:
      1. Compare online row count vs cached row count.
      2. If count matches → no change, use cache as-is.
      3. If online has MORE rows → extract only the NEW rows (the delta),
         append them to the local cache file, done — no full re-write.
      4. If online has FEWER rows → something unusual (row deleted/sheet reset);
    ...
    online_header = online_lines[0]
    online_body   = online_lines[1:]   # data rows only

    # Read existing cache
    cached_body: list[str] = []
    if os.path.exists(cache_path):
        try:
            with open(cache_path, 'r', encoding='utf-8-sig') as f:
                cached_lines = f.read().splitlines()
            if cached_lines:
                cached_body = cached_lines[1:]   # skip header
        except Exception:
            cached_body = []

    n_cached = len(cached_body)
    n_online = len(online_body)

    if n_online == n_cached:
        # Quick hash of last 5 rows to confirm truly identical
        tail_online = '\n'.join(online_body[-5:])
        tail_cached = '\n'.join(cached_body[-5:]) if cached_body else ''
        if hashlib.md5(tail_online.encode()).hexdigest() == \
           hashlib.md5(tail_cached.encode()).hexdigest():
            log(f'✓  NEWPOR unchanged ({n_cached} rows) — using local cache.')
            return '\n'.join([online_header] + cached_body), False
        # Same count but different content — full replace
        log(f'⚠️  NEWPOR row count same ({n_cached}) but content differs — full refresh.')

    elif n_online > n_cached:
        # Verify existing rows haven't changed (first 10 rows spot-check)
        check_n = min(10, n_cached)
        if check_n > 0:
            existing_head_online = '\n'.join(online_body[:check_n])
            existing_head_cached = '\n'.join(cached_body[:check_n])
            if hashlib.md5(existing_head_online.encode()).hexdigest() == \
               hashlib.md5(existing_head_cached.encode()).hexdigest():
                # Existing rows match — only the new rows are genuinely new
                new_rows  = online_body[n_cached:]
                n_new     = len(new_rows)
                log(f'➕  NEWPOR delta: {n_new} new row(s) since last sync '
                    f'(total: {n_cached} → {n_online}).')
                try:
                    # Append delta to cache file — much faster than full rewrite
                    with open(cache_path, 'a', encoding='utf-8') as f:
                        f.write('\n' + '\n'.join(new_rows))
                    merged = '\n'.join([online_header] + cached_body + new_rows)
                    return merged, True
                except Exception as e:
                    log(f'⚠️  Could not append delta to cache ({e}) — falling back to full write.')
            else:
                log(f'⚠️  Existing rows changed — performing full refresh.')
        else:
            # No cached rows yet — fall through to full write
            log(f'📋  NEWPOR first sync: {n_online} rows.')

    else:
        log(f'⚠️  Online has fewer rows ({n_online}) than cache ({n_cached}) — full refresh.')
    ...
    try:
        ...
        log(f'💾  NEWPOR cache fully updated — {n_online} rows.')
    except Exception as e:
...
def _mf_load_csv(url: str = MYFOLIO_CSV_URL, log_cb=None,
                 prefer_online: bool = True,
                 local_file: str = None) -> list:
    """
    Load the NEWPOR transaction ledger. Mirrors how teet.csv works:

      local_file (default: MYBook.csv in same folder as teet.csv)
        — The persistent local copy. Download syncs INTO this file.
        — If it doesn't exist yet, the first download creates it.
        — User can override the path (future: file picker in settings).

    Cache strategy (identical intent to resolve_input / teet_cache.csv):
      1. local_file < 5 min old         → use as-is, zero network.
      2. prefer_online=True             → download Google Sheet, delta-sync:
           · No new rows               → touch mtime, done.
           · New rows appended         → APPEND only new rows to local_file.
           · Rows changed / corrupted  → full replace of local_file.
      3. Network unreachable            → use local_file regardless of age.
    """
    def log(m):
        if log_cb: log_cb(m)

    # Derive local file path — same folder as teet.csv, just like teet_cache.csv
    if local_file is None:
        teet_dir   = os.path.dirname(os.path.abspath(DEFAULT_INPUT_FILE))
        local_file = os.path.join(teet_dir, _NEWPOR_LOCAL_CACHE_NAME)
    ...
    # NOTE: This path runs whether prefer_online is True OR False — if the file
    # is fresh we always use it. The prefer_online flag only controls whether
    # we attempt the online fetch when the file is stale or missing.
    if os.path.exists(local_file):
        try:
            age = time_module.time() - os.path.getmtime(local_file)
            if age < _NEWPOR_TTL:
                log(f'📂  MYBook.csv {age:.0f}s old — using local copy (no network).')
    ...
    if raw_text is None and prefer_online:
        ...
        try:
            ...
            try:
                ...
                pass
            if updated:
                log(f'💾  MYBook.csv updated → {local_file}')
            fetch_error = None
        except Exception as e:
            fetch_error = e
            log(f'⚠️  MYBook online fetch failed: {type(e).__name__}: {e}')
            raw_text = None

    # ── 3. Offline fallback ───────────────────────────────────────────────────
    if raw_text is None:
        if os.path.exists(local_file):
            log(f'📂  Offline — reading MYBook.csv from {local_file}')
            with open(local_file, 'r', encoding='utf-8-sig') as f:
                raw_text = f.read()
        else:
            raise _mf_URLError(
                f'Cannot find MYBook.csv at {local_file}.\n'
                f'Connect to the internet at least once so the app can download\n'
                f'your MYBook Google Sheet and create the local copy.\n'
                f'URL: {url}')

    reader = csv.DictReader(io.StringIO(raw_text))
    rows = []
    # Log the normalised column names so mismatches are immediately visible
    if reader.fieldnames:
        normed = [_mf_norm(k) for k in reader.fieldnames if k]
        log(f'📋  MYBook columns: {normed}')
        ...
        if missing:
            log(f'⚠️  MYBook missing columns: {sorted(missing)} — check sheet headers')
    for row in reader:
...
class LottoStockWindow(tk.Toplevel):
    def __init__(self, parent, preloaded_rows=None, preloaded_portfolios=None,
        ...
        if preloaded_validation is not None:
            ...
            self._on_myfolio_synced(b_own, b_u, e_own, e_u,
    ...
    def _build_ui(self):
        ...
        self.myfolio_link_var = tk.StringVar(value="🔗 MYBook: connecting…")
        tk.Label(link_row, textvariable=self.myfolio_link_var, font=("Courier New", 9),
                 fg="#58A6FF", bg=LOTTO_BG).pack(side="left", padx=6)
        tk.Button(link_row, text="↻ Resync MYBook", font=("Courier New", 8),
        ...
        for text, color in [("● Owned (MYBook) → +0.01", LOTTO_GREEN), ("● Retail → +0.02", "#40c0f0"),
    ...
    def _refresh_myfolio_link(self, initial=False):
        ...
        def worker():
            b_own, b_u, e_own, e_u, ports, validation = lotto_pull_myfolio_state(log_cb=_log)
            self.after(0, lambda: self._on_myfolio_synced(b_own, b_u, e_own, e_u, ports, validation, initial))

        threading.Thread(target=worker, daemon=True).start()

    def _on_myfolio_synced(self, bursa_owned, bursa_units, etf_owned, etf_units,
        ...
        if not validation or not validation.get("ok"):
            ...
            self.myfolio_link_var.set("🔗 MYBook: ❌ validation failed — locked")
            return
        ...
        n_etf   = len(etf_owned)
        link_txt = (f"🔗 MYBook: linked — "
                    f"{n_bursa} Bursa stock{'s' if n_bursa!=1 else ''} · "
                    f"{n_etf} ETF{'s' if n_etf!=1 else ''} owned")
        self.myfolio_link_var.set(link_txt)
        if validation.get("warnings"):
            self.myfolio_link_var.set(link_txt + f"  ⚠ {len(validation['warnings'])} warning(s)")
    ...
    def _build_road_section(self):
        ...
        if head_start > 0:
            tk.Label(prog_frame,
                     text=f"🎁 Head start: {head_start:.4f} units (live from MYBook)",
    ...
    def _confirm_transaction(self, stock):
        ...
        if is_etf:
            ...
            if units > 0:
                lotto_record_etf_buy_to_portfolios(
                    self.myfolio_portfolios, code, units, price, date_str)
...
class SCAWindow(tk.Toplevel):
    ...
    def _load_from_cache_then_refresh(self):
        """Load MYBook.csv from disk for instant display, then check for updates."""
        def _worker():
            try:
                # local-only read — no network, uses MYBook.csv next to teet.csv
    ...
    def _build_header(self):
        ...
        for label, val in [('MYR', 'MY'), ('USD', 'US'), ('⇄ BOTH', 'BOTH')]:
    ...
    def _build_summary_cards(self):
        ...
        # MY MYR card
        self._my_card = self._make_summary_card(cards_row, 'MYR POSITION', self.TEAL, 'MYR')
        self._my_card.grid(row=0, column=0, sticky='nsew', padx=(0, 6))

        # US USD card
        self._us_card = self._make_summary_card(cards_row, 'USD POSITION', self.GREEN, 'USD')
    ...
    def _build_an_table_tab(self):
        ...
        frame.pack(fill='both', expand=True)
        tk.Label(frame, text='Transaction history · all types (Deposit/Withdraw/Buy/Sell/Fees/Dividend) · running cash balance',
                 font=self.FONT, fg=self.DIM, bg=self.BG,
                 anchor='w', padx=4).pack(fill='x', pady=(4, 0))
        cols = ('Date', 'Type', 'Ticker/Asset', 'Units', 'Price',
                'Amount', 'Running Units', 'Cash Balance', 'Running Cost')
        self._an_txn_tree = ttk.Treeview(frame, columns=cols, show='headings',
                                          style='SCA.Treeview', height=12)
        cw = {'Date': 88, 'Type': 80, 'Ticker/Asset': 110, 'Units': 85,
              'Price': 82, 'Amount': 95, 'Running Units': 105,
              'Cash Balance': 112, 'Running Cost': 110}
        for col in cols:
            self._an_txn_tree.heading(col, text=col)
            self._an_txn_tree.column(col, width=cw.get(col, 80),
                                      anchor='w' if col in ('Date', 'Type', 'Ticker/Asset') else 'e',
                                      stretch=False)
        ...
        self._an_txn_tree.tag_configure('buy',     foreground='#58A6FF')  # deposit/buy/dividend
        self._an_txn_tree.tag_configure('sell',    foreground=self.RED)   # sell/withdraw/fees
        self._an_txn_tree.tag_configure('neutral', foreground=self.DIM)
    ...
    def _an_refresh_portfolio_dropdown(self):
        ...
        # Show friendly display names but store internal keys as values
        display_names = [_port_display_name(c) for c in codes]
        self._an_port_cb['values'] = codes   # internal keys for lookup
        # Map raw key → display label for the combobox text
        self._an_port_key_map = dict(zip(display_names, codes))
        self._an_port_cb['values'] = codes
        if codes and self._an_port_var.get() not in codes:
    ...
    def _an_draw_line(self, port):
        ...
        for ticker in all_tickers:
            mkt = 'MY' if ticker.endswith('.KL') else 'US'
            # Always use full history for the line chart — _last_hist_data
            # only has 3 months (optimised for the holdings table initial load).
            # _mf_get_price_history fetches full history and caches it.
            hist = _mf_get_price_history(ticker, mkt, earliest)
    ...
    def _an_draw_txn_table(self, port):
        """Full transaction ledger rebuilt from the raw NEWPOR rows so every
        transaction type is visible — Deposit, Withdraw, Buy, Sell, Fees,
        Dividend — not just Buy/Sell. The per-holding transactions list misses
        cash-flow-only events (deposits, withdrawals, fees, dividends) which
        is the root cause of the running-balance discrepancy the user reported.
        """
        import datetime as _dt
        for row in self._an_txn_tree.get_children():
            self._an_txn_tree.delete(row)

        # Use the raw NEWPOR rows stored on the parent SCAWindow, filtered to
        # the same currency bucket this portfolio belongs to.
        ccy0 = port.get('base_currency', 'MYR')
        raw_rows = getattr(self, '_rows', None) or []
        # Filter to rows matching this portfolio's currency
        port_rows = [r for r in raw_rows
                     if r.get(_NC_CCY, 'USD').upper().strip() == ccy0]

        if not port_rows:
            # Fallback: reconstruct from per-holding transactions (buy/sell only)
            all_txns = []
            for ticker, item in port.get('holdings', {}).items():
                if item.get('_is_cash'): continue
                for txn in item.get('transactions', []):
                    ttype, date_s, unit, price, amount = txn
                    all_txns.append((date_s, ticker, ttype, unit, price, amount))
            port_rows = [{'date': t[0], 'transactiontype': t[2],
                          'assetcode': t[1], 'unit': str(t[3]),
                          'value': str(t[4]), 'amount': str(t[5])}
                         for t in all_txns]
        ...
        port_rows = sorted(port_rows, key=lambda r: _parse(r.get(_NC_DATE, '')))

        if not port_rows: return

        run_units: dict = {}   # ticker -> running units held
        run_cost:  dict = {}   # ticker -> running cost basis
        run_cash   = 0.0       # running cash balance

        # Type colour coding
        TYPE_TAGS = {
            'buy':      'buy',
            'sell':     'sell',
            'deposit':  'buy',    # cash in = same green as buy
            'withdraw': 'sell',   # cash out = same red as sell
            'widthdraw':'sell',
            'fees':     'sell',
            'dividend': 'buy',
            'div':      'buy',
        ...
        for row in port_rows:
            ...
            ru = run_units.get(asset, 0.0)
            rc = run_cost.get(asset, 0.0)

            if asset in _CASH_ASSETS or 'cash' in asset.lower():
                # Cash-flow-only events: Deposit, Withdraw, Fees
                if ttype == 'deposit':
                    run_cash += abs(amount)
                    desc = f'{ccy0}-CASH'
                elif ttype in ('withdraw', 'widthdraw'):
                    run_cash -= abs(amount)
                    desc = f'{ccy0}-CASH'
                elif ttype == 'fees':
                    run_cash -= abs(amount)
                    desc = f'{ccy0}-CASH  (platform fee)'
                else:
                    continue
                self._an_txn_tree.insert('', 'end', tags=(TYPE_TAGS.get(ttype, 'sell'),),
                    values=(date_s, ttype.upper(), desc,
                            f'{abs(unit):+,.4f}' if unit else '—',
            ...
            if ttype == 'buy':
                run_cash -= abs(amount)
                old_s = ru; new_s = old_s + abs(unit)
                if new_s > 0:
                    rc = ((old_s * rc) + (abs(unit) * price)) / new_s
                ru = new_s
                run_cost[asset] = rc * ru   # keep total cost basis
                rc_total = rc * ru
                run_units[asset] = ru
                ...
                sold = abs(unit)
                if ru > 0:
                    cb = (run_cost.get(asset, 0.0) / ru) * sold
                    run_cost[asset] = run_cost.get(asset, 0.0) - cb
                ru -= sold
                if ru <= 1e-8:
                    ru = 0.0
                    run_cost[asset] = 0.0
                run_units[asset] = ru
                rc_total = run_cost.get(asset, 0.0)

            elif ttype in ('dividend', 'div'):
                run_cash += abs(amount)
                rc_total = run_cost.get(asset, 0.0)
                # Dividend doesn't change units or cost basis

            else:
                rc_total = run_cost.get(asset, 0.0)

            self._an_txn_tree.insert('', 'end', tags=(TYPE_TAGS.get(ttype, 'neutral'),),
                values=(date_s, ttype.upper(), asset,
                        f'{unit:+,.4f}' if unit else '—',
                        f'{price:,.4f}' if price else '—',
                        f'{amount:+,.4f}',
                        f'{ru:,.4f}', f'{run_cash:,.4f}',
                        f'{rc_total:,.4f}'))
    ...
    def _on_sheet_error(self, msg):
        ...
        self._show_banner(f'✕ Cannot load NEWPOR sheet: {msg}',
                          sub='Make sure the MYBook Google Sheet is published via File → Share → Publish to web → CSV.\n'
                              'Then connect to the internet and reopen SCA so the local MYBook.csv can be created.')
        self._loading = False
    ...
    def _price_worker(self):
        ...
        # USD/MYR FX rate — check disk cache first (5-min TTL)
        try:
            cached_fx = cache_get_live('usdmyr', _TTL_LIVE)
            if cached_fx is not None:
                self._usdmyr = float(cached_fx); fx_ok = True
            else:
                ...
                if not fx:
                    h = fx_ticker.history(period='2d')
                    fx = float(h['Close'].iloc[-1]) if not h.empty else None
                self._usdmyr = float(fx) if fx else None
                fx_ok = self._usdmyr is not None
                if fx_ok: cache_set_live('usdmyr', self._usdmyr)
        except Exception:
            self._usdmyr = None; fx_ok = False
    ...
    def _render_all(self, price_data, history_data, placeholder=False):
        ...
        # Update summary cards — Cost is local/instant, Value/P/L deferred if placeholder
        ...
        ports_to_show = {}
        if tab in ('ALL', 'MY'):
            ports_to_show['MYR Position'] = ('SWAY1-MYR', 'MYR', self.TEAL)
        if tab in ('ALL', 'US'):
            ports_to_show['USD Position'] = ('SWAY1', 'USD', self.GREEN)
        ...
        for (ticker, port_label, ccy, shares, avg_cost, price, day_chg,
            ...
            ccy_prefix = 'RM' if ccy == 'MYR' else '$'
            # MY stocks: trim trailing zeros (10.0000→10, 4.3900→4.39)
            # US stocks: always 4dp (0.0131 stays 0.0131, not 0.013)
            qty_str = _fmt_units(shares) if ccy == 'MYR' else f'{shares:,.4f}'
...
class SplashScreen(tk.Tk):
    ...
    def _refresh_file_sizes(self):
        """Show local file sizes for teet.csv and MYBook.csv if they exist."""
        parts = []
        try:
        ...
        try:
            nf = os.path.join(os.path.dirname(DEFAULT_INPUT_FILE), _NEWPOR_LOCAL_CACHE_NAME)
            if os.path.exists(nf):
                nz = os.path.getsize(nf)
                ns = f'{nz/1024:.0f} KB' if nz < 1024*1024 else f'{nz/1024/1024:.1f} MB'
                parts.append(f'MYBook.csv: {ns}')
            else:
                parts.append('MYBook.csv: will download')
        except Exception:
            pass
    ...
    def _load_thread(self, path, prefer_online=True, apply_sell_fee=True):
        """
        Download teet.csv, MYBook.csv AND all SCA dividend/EPS data IN PARALLEL.
        ...
        def _fetch_mybook():
            try:
                rows = _mf_load_csv(
                    log_cb=lambda m: self.after(0, lambda msg=m: self._log(f'[MYBook] {msg}')),
                    prefer_online=prefer_online,
                )
                ...
                self.after(0, lambda: self._log(
                    f'[MYBook] ✅  {len(rows)} transactions loaded.'))
                # ── After MYBook is parsed, pre-fetch dividends + EPS for all
