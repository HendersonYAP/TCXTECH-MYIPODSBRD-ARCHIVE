...
class MYIPOApp(tk.Tk):
    ...
    def _draw_fund_events(self):
        ...
        if type_filter in ('All', 'Distribution'):
            for dt, nav_before, dist_per_unit, nav_after in dist_log:
                events.append((dt, 'Distribution', f'NAV {nav_before:.4f}→{nav_after:.4f}',
                               dist_per_unit, None, None))

        if type_filter in ('All', 'Unit Issued (cash shortfall)'):
            for dt, ev_type, sym, cash_amt, units_issued, units_after in unit_log:
                if ev_type == 'entry' and units_issued > 0:
                    ...
                    events.append((dt, 'Unit Issued (cash shortfall)', sym,
                                   -cash_amt if cash_amt else 0.0, units_issued, units_after))

        events.sort(key=lambda e: e[0])

        n_dist = sum(1 for e in events if e[1] == 'Distribution')
        n_issue = sum(1 for e in events if 'Unit Issued' in e[1])
        total_dist = sum(e[3] for e in events if e[1] == 'Distribution' and e[3])
        ...
        for dt, etype, detail, amount, units, units_after in events:
            ...
            tag = 'dist' if etype == 'Distribution' else 'issue'
        ...
        self.fund_events_summary_var.set(
            f'{lbl}  ·  {n_dist} distribution(s) totalling RM{total_dist:.4f}/unit  ·  '
