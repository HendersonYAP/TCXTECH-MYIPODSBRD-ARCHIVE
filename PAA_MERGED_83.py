...
class MYIPOApp(tk.Tk):
    ...
    def _sor_run_and_popup(self):
        ...
        for sym, region, units in holdings_all:
            ...
            if mode == 'custom':
                # Custom mode: one flat units-per-Cr rule, same for every
                # region and symbol (Lotto33 or not) — but PER-USER now:
                # each K-ID's own sor_custom_threshold, falling back to the
                # admin default only if this K-ID never set one.
    ...
    def _rewards_toggle_sor_mode(self):
        ...
        if pts.get('sor_custom_enabled'):
            ...
            val = simpledialog.askfloat('SoR Mode — Custom rate',
                'Set YOUR own Custom SoR rate: 1 Cr for every how many units '
                'held? (applies to every saved position, any market or '
                'Lotto33 status, no minimum-holding gate)\n\n'
            ...
            messagebox.showinfo('SoR Mode',
                f'Custom SoR mode enabled: 1 Cr per {val:g} units held.',
                parent=self)
        self._pts_save(pts)
        self._build_rewards_tab()
...
to earn — incentivised clicking violates ad-network policy.</div>
    ...
    def _build_rewards_tab(self):
        ...
        if custom_on:
            _mode_desc = (f"Custom — your rate: 1 Cr per "
                           f"{pts.get('sor_custom_threshold') or _sor_cfg['custom_threshold']:g} "
                           f"units held (any market).")
        else:
...
class SCAWindow(tk.Toplevel):
    ...
    def _open_sor_settings(self):
        ...
        custom_fields = [
            ('custom_threshold',     'Custom mode: units per 1 Cr (any symbol)'),
        ]
        shared_fields = [
...
_DEFAULT_SETTINGS = {
    ...
    "sor_config": {
        ...
        "custom_threshold":    0.01,   # custom mode: units // this = Cr, any region/symbol alike
