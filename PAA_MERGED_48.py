...
class MYIPOApp(tk.Tk):
    ...
    def _open_sca(self):
        ...
        The splash preloads MYBook before we know whose K-ID is active, so
...
class SCAWindow(tk.Toplevel):
    ...
    def _force_source_setup(self):
        ...
        messagebox.showinfo(
            ...
            f'You can also start with the demo portfolio to look around.',
            parent=self)
    ...
    def _open_source_dialog(self, first_run: bool = False) -> bool:
        ...
        mode_var = tk.StringVar(value='sheet' if first_run else cur.get('mode', 'demo'))
        ...
        def _fetch_mybook():
            ...
            if not sca_has_source(load_app_settings()):
                self.after(0, lambda: self._log(
                    '[MYBook] ⏭  No portfolio connected yet — '
...
def kid_load_recovery_data(settings: dict, username: str):
    ...
    try:
        ...
        if src['mode'] == 'csv' and src.get('csv_path'):
            ...
            rows = _mf_load_csv(url=src.get('url') or MYFOLIO_CSV_URL)
        ports = _mf_build_portfolios(rows)
        return rows, ports
    except Exception:
        return None, None
