...
class MYIPOApp(tk.Tk):
    ...
    FX_QUOTE_LOT = {'JPY': 100, 'KRW': 1_000}

    def _zdws_fx_pair_level(self, code: str):
        """Level for an FX pair when it's the UNDERLYING being traded —
        scaled to the quote-lot convention above (100 JPY, 1,000 KRW, else
        per 1). This is intentionally separate from _zdws_fx_to_myr, which
        must stay a raw per-1-unit rate since it's also used to compound a
        foreign World Index's move with the FX move (e.g. Nikkei 225 in JPY).
        Scaling is consistent at both entry and settlement (both call this
        same function), so P/L math is unaffected — it only changes what the
        level looks like on screen."""
    ...
    def _zdws_underlyings(self) -> list:
        ...
        # 1,000 KRW, else per 1) rather than the raw per-1-unit rate ───────────
