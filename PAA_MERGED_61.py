...
class MYIPOApp(tk.Tk):
    ...
    def _ensure_us_universe_warm(self):
        ...
        prices the first time ZDWS or ZDOS is opened this session, so those
        symbols just appear in the underlying list like everything else — no
        separate 'load' step. One-shot per session (guarded by
        _us_universe_warm_started); runs off the main thread since these are
        network calls, with the UI refresh scheduled back onto the main
        thread via .after() when it completes."""
        if getattr(self, '_us_universe_warm_started', False):
            return
        self._us_universe_warm_started = True
    ...
    def _build_zdws_tab(self):
        ...
        self._ensure_us_universe_warm()   # one-shot per session, runs in background
