...
class MYIPOApp(tk.Tk):
    ...
    def _opie_dash_summary(self) -> str:
        ...
        prefix = 'RM' if base_ccy == 'MYR' else '$'
...
to earn — incentivised clicking violates ad-network policy.</div>
    ...
    # =========================================================================
    # SCA-SMS — ShareMyStrategy Strategy Fund (US market only)
    # SCA is the key: there is no separate fund ledger and no manual trade
    # form. The moment a K-ID opts in, EVERY US-market ticker already held
    # in their SCA/NEWPOR portfolio (the USD bucket) is automatically minted
    # as part of the Strategy Fund — one fund line per US holding, valued at
    # NAV 1.0000 Cr/unit against that holding's own average cost. From then
    # on the fund just mirrors SCA: buy more of a US ticker in SCA (the
    # normal way) and it shows up here; sell it in SCA and it drops out.
    # Nothing is traded from inside this window.
    #   • US market only (the USD portfolio bucket)
    #   • NAV mint 1.0000 Cr/unit; the USD cost value of a holding IS its
    #     Cr stake (1:1) — so fund P/L in Cr is numerically the same as the
    #     position's real unrealized USD P/L
    #   • A holding opened with under 100 Cr of cost is flagged (not
    #     blocked — it's a real SCA trade, the app can't undo it)
    #   • A position is flagged for exit once its P/L moves ±1 Cr
    #   • New US tickers picked up per day is capped at SMS_DAILY_OPEN_LIMIT
    #     — purely informational, since the trade already happened in SCA
    # =========================================================================

    SMS_FUND_NAV_START    = 1.0000    # every fund mints at 1.0000 Cr/unit
    SMS_MIN_BUY_CR         = 100.0    # cost below this on a new US ticker gets flagged
    SMS_EXIT_PL_TRIGGER    = 1.0      # ± this many Cr of P/L flags "ready to exit"
    SMS_DAILY_OPEN_LIMIT   = 10       # new US tickers picked up per K-ID per day (informational)

    def _sms_opt_in_status(self, pts: dict) -> bool:
        return bool(pts.get('sms_opt_in', False))

    def _sms_prompt_opt_in(self, pts: dict) -> bool:
        """One-time confirmation dialog. Returns True if the user is (now)
        opted in, False if they declined."""
        if self._sms_opt_in_status(pts):
            return True
        ans = messagebox.askyesno(
            'ShareMyStrategy (SMS) — Opt-in',
            'Strategy Fund (SCA-SMS) is an opt-in feature.\n\n'
            '• Once you opt in, EVERY US-market ticker already in your SCA\n'
            '  portfolio is automatically minted as part of the fund —\n'
            '  nothing to set up manually\n'
            '• US market only (your SCA USD holdings)\n'
            '• Every position is valued at NAV 1.0000 Cr/unit against its\n'
            '  own SCA cost basis\n'
            f'  — a position opened under {self.SMS_MIN_BUY_CR:,.0f} Cr of cost is flagged\n'
            '• A position is flagged for exit once its P/L moves ±1 Cr\n'
            '• Trade normally in SCA (Buy/Sell a US ticker) — the fund\n'
            '  just mirrors your SCA book from then on\n\n'
            'Opt in as an SMS user now?',
            parent=self)
        if ans:
            pts['sms_opt_in'] = True
            self._pts_save(pts)
    ...
    def _sms_current_price(self, ticker: str):
        """Live/cached price for a US ticker, reusing the same price cache
        every other US-market feature (SP500/SPDR/MSCI) already warms."""
        try:
            s = cache_get_prices(ticker)
    ...
    def _sms_load_ledger_rows(self, log_cb=None):
        """Pull the signed-in K-ID's SCA/NEWPOR ledger (Sheet or CSV,
        merged with any app-recorded local rows) — the same source SCA
        itself reads from. Returns (rows, error_str)."""
        try:
            rows = _mf_load_csv(log_cb=log_cb, prefer_online=False)
            return rows, ''
        except Exception as e:
            return [], str(e)

    def _sms_positions_from_sca(self, rows: list):
        """Auto-mint every US-market holding currently in SCA as an SMS
        fund line. Returns (positions, warnings).

        positions: {ticker: {ticker, units, stake_cr, avg_cost}}
        The fund unit is defined so NAV starts at 1.0000 Cr/unit at each
        holding's own SCA average cost — units = cost_value (USD, treated
        1:1 as Cr). No separate fund book: this is a live mirror, rebuilt
        fresh from SCA every time it's opened/refreshed.
        """
        portfolios = _mf_build_portfolios(rows)
        warnings = []
        positions = {}
        for port in portfolios.values():
            if (port.get('base_currency') or '').upper() != 'USD':
                continue   # US market only
            for ticker, item in port.get('holdings', {}).items():
                if item.get('_is_cash'):
                    continue
                shares = item.get('shares', 0)
                if not shares or shares <= 0:
                    continue
                cost_value = item.get('cost_value', 0.0)
                avg_cost   = item.get('avg_cost', 0.0)
                if cost_value < self.SMS_MIN_BUY_CR:
                    warnings.append(
                        f'{ticker}: cost of {cost_value:,.2f} Cr is below the '
                        f'{self.SMS_MIN_BUY_CR:,.0f} Cr minimum — flagged, not excluded.')
                positions[ticker] = {
                    'ticker':    ticker,
                    'shares':    shares,
                    'avg_cost':  avg_cost,
                    'stake_cr':  cost_value,               # 1:1 USD-cost-as-Cr mint rule
                    'units':     cost_value / self.SMS_FUND_NAV_START,
                    'opened_date': item.get('date', ''),
                }
        return positions, warnings

    def _sms_fund_pl(self, position: dict):
        """Current (price_now, nav_now, pl_cr) for a live SMS position
        mirrored from SCA, priced off that holding's own SCA avg_cost."""
        price_now = self._sms_current_price(position['ticker'])
        if price_now is None or not position.get('avg_cost'):
            return None, None, None
        move_pct = (price_now - position['avg_cost']) / position['avg_cost']
        nav_now  = self.SMS_FUND_NAV_START * (1 + move_pct)
        pl_cr    = (nav_now - self.SMS_FUND_NAV_START) * position['units']
        return price_now, nav_now, pl_cr

    def _sms_ready_to_exit(self, position: dict) -> bool:
        _, _, pl_cr = self._sms_fund_pl(position)
        return pl_cr is not None and abs(pl_cr) >= self.SMS_EXIT_PL_TRIGGER

    def _sms_opens_today(self, all_ledger_rows: list) -> int:
        """Count US tickers whose first Buy in SCA lands today — purely
        informational, since the trade already happened in SCA itself."""
        import datetime as _dt
        today = _dt.date.today().isoformat()
        first_buy_date = {}
        for r in sorted(all_ledger_rows, key=lambda r: r.get(_NC_DATE, '')):
            if (r.get(_NC_TYPE, '') or '').strip().lower() != 'buy':
                continue
            if (r.get(_NC_CCY, '') or '').strip().upper() != 'USD':
                continue
            asset = (r.get(_NC_ASSET, '') or '').strip().upper()
            first_buy_date.setdefault(asset, r.get(_NC_DATE, ''))
        return sum(1 for d in first_buy_date.values() if d.startswith(today))

    def _open_sms_window(self):
        """💹 SCA-SMS — Strategy Fund desk. Auto-mints every US SCA holding
        as a fund line. US market only. Opt-in gated."""
        pts = self._pts_load()
        if not self._sms_prompt_opt_in(pts):
            return
        SMSStrategyFundWindow(self, pts)
...
_NC_PORTFOLIO = "portfolio"      # Col H (Sheets) / optional CSV column — sub-portfolio
                                  # tag, e.g. blank = main, "SMS" = SCA-SMS Strategy Fund
...
def _mf_parse_rows(raw_text: str, log_cb=None) -> list:
    ...
    if reader.fieldnames:
        ...
        if _NC_PORTFOLIO not in normed:
            log('ℹ️  No Portfolio column (Col H) found — all rows treated as main '
                'portfolio; SCA-SMS Strategy Fund rows need a Portfolio column.')
    for row in reader:
...
def sca_record_local_transaction(row: dict, username: str = None) -> bool:
    ...
    clean[_NC_PORTFOLIO] = str(row.get(_NC_PORTFOLIO, '')).strip()   # optional Col H tag
            ...
            if ttype == "buy":
                ...
                if asset not in holdings:
                    holdings[asset] = {
                        ...
                        "portfolio_tag": "",   # Col H — set below if present
                    }
                h = holdings[asset]
                ...
                h["date"]        = date_s
                # Col H "Portfolio" tag — most recent non-blank value on any
                # buy row for this asset wins, so editing Col H in the sheet
                # and re-syncing changes what shows up here.
                tag = (row.get(_NC_PORTFOLIO, "") or "").strip()
                if tag:
                    h["portfolio_tag"] = tag
...
class SMSStrategyFundWindow(tk.Toplevel):
    """💹 SCA-SMS — ShareMyStrategy Strategy Fund desk.

    SCA is the key: there is nothing to set up and no manual trade form
    here. Once opted in, EVERY US-market ticker already held in SCA is
    automatically minted as a fund line (NAV 1.0000 Cr/unit against that
    holding's own SCA cost basis). This window is a live, read-only mirror
    — it rebuilds itself fresh from the SCA ledger every refresh. To trade
    in or out of the fund, buy or sell that US ticker in SCA the normal
    way; it appears here (or drops out) automatically.
    US market only. Min 100 Cr cost flags a thin position. Flagged for
    exit at ±1 Cr P/L. New tickers/day shown is informational.
    """

    BG, PANEL, FG, DIM = '#0d1117', '#161b22', '#e6edf3', '#8b949e'
    GREEN, RED, ACCENT = '#3fb950', '#f85149', '#58a6ff'

    def __init__(self, parent, pts: dict):
        super().__init__(parent)
        self.parent = parent
        self.pts = pts
        self.title('💹 SCA-SMS — Strategy Fund')
        self.configure(bg=self.BG)
        self.geometry('760x560')

        hdr = tk.Frame(self, bg=self.BG); hdr.pack(fill='x', padx=14, pady=(12, 4))
        tk.Label(hdr, text='SCA-SMS  ·  Strategy Fund', font=('Courier New', 15, 'bold'),
                 bg=self.BG, fg=self.FG).pack(side='left')

        info = (f'Auto-mints every US ticker held in SCA  ·  '
                f'US market only  ·  NAV mint 1.0000 Cr/unit  ·  '
                f'min cost {parent.SMS_MIN_BUY_CR:,.0f} Cr  ·  '
                f'exit flag at ±{parent.SMS_EXIT_PL_TRIGGER:.0f} Cr P/L')
        tk.Label(self, text=info, font=('Courier New', 8), bg=self.BG, fg=self.DIM,
                  wraplength=730, justify='left').pack(fill='x', padx=14, pady=(2, 0))
        tk.Label(self, text='Trade normally in SCA (Buy/Sell a US ticker) — the fund '
                             'mirrors your SCA book automatically. Nothing to trade here.',
                 font=('Courier New', 8), bg=self.BG, fg=self.ACCENT,
                 wraplength=730, justify='left').pack(fill='x', padx=14, pady=(2, 6))

        self.opens_var = tk.StringVar()
        tk.Label(self, textvariable=self.opens_var, font=('Courier New', 9, 'bold'),
                 bg=self.BG, fg=self.ACCENT).pack(anchor='w', padx=14, pady=(0, 8))

        # ── Live fund lines, mirrored straight from SCA ────────────────────
        tk.Label(self, text='Strategy Fund Positions (mirrored from SCA)',
                 font=('Courier New', 10, 'bold'),
                 bg=self.BG, fg=self.FG).pack(anchor='w', padx=14)
        cols = ('ticker', 'shares', 'units', 'stake', 'price', 'nav', 'pl', 'flag')
        self.tree = ttk.Treeview(self, columns=cols, show='headings', height=11)
        heads = {'ticker': 'Ticker', 'shares': 'Shares (SCA)', 'units': 'Fund Units',
                 'stake': 'Cost (Cr)', 'price': 'Price now', 'nav': 'NAV now',
                 'pl': 'P/L (Cr)', 'flag': ''}
        widths = {'ticker': 70, 'shares': 90, 'units': 90, 'stake': 90,
                  'price': 90, 'nav': 90, 'pl': 90, 'flag': 100}
        for c in cols:
            self.tree.heading(c, text=heads[c])
            self.tree.column(c, width=widths[c], anchor='center')
        self.tree.pack(fill='both', expand=True, padx=14, pady=(4, 8))

        btn_row = tk.Frame(self, bg=self.BG); btn_row.pack(fill='x', padx=14, pady=(0, 6))
        tk.Button(btn_row, text='↻ Refresh from SCA', font=('Courier New', 9),
                  command=self._refresh).pack(side='left')

        self.warn_var = tk.StringVar()
        tk.Label(self, textvariable=self.warn_var, font=('Courier New', 8),
                 bg=self.BG, fg=self.RED, wraplength=730, justify='left'
                 ).pack(fill='x', padx=14, pady=(0, 4))

        self.status_var = tk.StringVar()
        tk.Label(self, textvariable=self.status_var, font=('Courier New', 9),
                 bg=self.BG, fg=self.DIM, wraplength=730, justify='left'
    ...
    def _refresh(self):
        self.pts = self.parent._pts_load()

        rows, err = self.parent._sms_load_ledger_rows()
        for row in self.tree.get_children():
            self.tree.delete(row)
        if err:
            self.warn_var.set(f'⚠ Could not read SCA ledger: {err}')
            self.opens_var.set('')
            return

        positions, warnings = self.parent._sms_positions_from_sca(rows)
        opens_today = self.parent._sms_opens_today(rows)
        self.opens_var.set(
            f'New US tickers picked up in SCA today: {opens_today} '
            f'(informational — limit {self.parent.SMS_DAILY_OPEN_LIMIT}/day)')
        self.warn_var.set(' · '.join(warnings) if warnings else '')

        if not positions:
            self._set_status('No US-market holdings in SCA yet — buy a US ticker in SCA '
                              'and it will show up here automatically.')
        else:
            self._set_status(f'{len(positions)} US position(s) mirrored from SCA.')

        for ticker, p in sorted(positions.items()):
            price_now, nav_now, pl_cr = self.parent._sms_fund_pl(p)
            price_s = f'{price_now:,.2f}' if price_now is not None else '—'
            nav_s   = f'{nav_now:,.4f}' if nav_now is not None else '—'
            pl_s    = f'{pl_cr:+,.2f}' if pl_cr is not None else '—'
            flag    = '🔔 exit-ready' if (pl_cr is not None and
                        abs(pl_cr) >= self.parent.SMS_EXIT_PL_TRIGGER) else ''
            self.tree.insert('', 'end', iid=ticker, values=(
                ticker, f'{p["shares"]:,.4f}', f'{p["units"]:,.4f}',
                f'{p["stake_cr"]:,.0f}', price_s, nav_s, pl_s, flag))
...
class SCAWindow(tk.Toplevel):
    ...
    def _on_pchart_hover(self, event):
        ...
        prefix = 'RM' if getattr(self, '_pchart_base_ccy', 'MYR') == 'MYR' else '$'
        ann = ax.annotate(
    ...
    def _build_header(self):
        ...
        tk.Button(ctrl, text='💹 SMS', font=self.FONT, bg='#1a2035',
                  fg='#58a6ff', relief='flat', padx=8, pady=4, cursor='hand2',
                  command=lambda: self.master._open_sms_window()
    ...
    def _render_all(self, price_data, history_data, placeholder=False):
        ...
        for port_label, (port_key, ccy, accent) in ports_to_show.items():
            ...
            for ticker, item in sorted(holdings.items()):
                ...
                # PORTFOLIO column: prefer the sheet's own Col H tag for this
                # holding (e.g. "SMS") over the generic currency-bucket label,
                # so editing Col H and re-syncing changes what's shown here.
                row_label = item.get('portfolio_tag') or port_label
                all_rows.append((ticker, row_label, ccy, shares, avg_cost, price,
    ...
    def _on_view_change(self):
        # Keep the body's ALL/MY/US tab in step with the header's USD/MYR/
        # ⇄ALL view buttons — previously these were two independent
        # controls: the header buttons only hid/showed the summary cards,
        # while the holdings table stayed on whatever the tab row was last
        # set to (defaulting to ALL), so clicking "USD" in the header still
        # left both currencies mixed in the table below. Now the two stay
        # synced so either control filters everything together.
        v = self._view_var.get()
        if v in ('MY', 'US'):
            self._tab_var.set(v)
        else:
            self._tab_var.set('ALL')
    ...
    def _on_tab_change(self):
        # Mirror the tab-row choice back into the header view filter (see
        # note in _on_view_change) so both controls always agree.
        t = self._tab_var.get()
        if t in ('MY', 'US') and t in getattr(self, '_view_btns', {}):
            self._view_var.set(t)
        elif t == 'ALL' and 'BOTH' in getattr(self, '_view_btns', {}):
            self._view_var.set('BOTH')
        price_data = {ticker: _mf_get_price(ticker, item.get('market', 'MY'))
                      for port in self._portfolios.values()
                      for ticker, item in port.get('holdings', {}).items()
                      if not item.get('_is_cash')}
...
_DEFAULT_SETTINGS = {
    ...
    "pts": {
        ...
        "sms_opt_in":       False,   # ShareMyStrategy opt-in flag — positions themselves
                                      # live in the SCA ledger (Portfolio=SMS rows), not here
...
class OpieChatWindow(tk.Toplevel):
    ...
    def __init__(self, parent, host, kid_username: str):
        ...
        self._last_context   = None   # {'type': 'stock'|'portfolio', 'ticker': str|None}
        # remembers the last "how much"/"change" answer so a bare follow-up
        # like "what if in MYR" can re-run it in a different currency
        # instead of falling through to the generic help message.
    ...
    def _process(self, text: str):
        ...
        if re.search(r'\b(largest|biggest|smallest)\b', low):
            self._handle_extreme_position(text, low)
            return

        # "what if in MYR" / "in USD?" / "what about MYR" — a currency-only
        # follow-up to the last "how much"/"change" answer. Checked before
        # the generic fallback so it doesn't just repeat the help message.
        if self._last_context and (re.search(r'\bwhat if\b', low) or
                                    re.search(r'\bwhat about\b', low) or
                                    re.fullmatch(r'in\s+\w+\??', low.strip())):
            target = _opie_parse_currency(text)
            if target:
                self._answer_last_context(target)
                return
            self._say('Which currency — e.g. "what if in MYR"?')
            return
        ...
        self._say('I can help with "change" (see your portfolio / a stock), '
                   ...
                   '"stock price" (current price of a stock), "largest"/'
                   '"smallest" (your biggest or smallest position), or '
    ...
    def _start_name_flow(self):
        ...
        if key in names and locked.get(key):
            ...
            self._say(f'Hi, {self._display_name}! What would you like to know — '
                       f'try "change", "how much", "stock price", "largest", or "index".')
            return
    ...
    def _say_stock_value(self, match, target_ccy: str = None):
        ...
        gain   = value - cost
        out_ccy = target_ccy or ccy
        if out_ccy != ccy:
            conv_value = self._convert_amount(value, ccy, out_ccy)
            conv_cost  = self._convert_amount(cost, ccy, out_ccy)
            if conv_value is None or conv_cost is None:
                self._say(f'No exchange rate available for {out_ccy} yet — try MYR or USD.')
                return
            value, cost, out_ccy = conv_value, conv_cost, out_ccy
        gain   = value - cost
        pct    = (gain / cost * 100) if cost > 0 else 0.0
        prefix = 'RM' if out_ccy == 'MYR' else f'{out_ccy} '
        self._last_context = {'type': 'stock', 'ticker': ticker}
        self._say(f'{ticker}: {shares:g} shares, worth '
                   f'{prefix}{value:,.2f}, '
                   f'unrealized {gain:+,.2f} ({pct:+.2f}%).')

    def _convert_amount(self, amount, from_ccy: str, to_ccy: str):
        """Convert amount between currencies via the app's live FX table
        (everything routes through MYR, same as _handle_how_much)."""
        if from_ccy == to_ccy:
            return amount
        fx = _opie_extract_fx(self.host)
        amount_myr = amount if from_ccy == 'MYR' else amount * fx.get(from_ccy, 1.0)
        if to_ccy == 'MYR':
            return amount_myr
        rate = fx.get(to_ccy)
        return (amount_myr / rate) if rate else None

    def _answer_last_context(self, target_ccy: str):
        """Re-run the last 'how much'/'change' answer in a new currency —
        handles a bare follow-up like 'what if in MYR'."""
        ctx = self._last_context
        if not ctx:
            self._say('Which currency — e.g. "what if in MYR"?')
            return
        if ctx['type'] == 'stock':
            match = _opie_match_portfolio_ticker(self.host, ctx['ticker'])
            if not match:
                self._say(f'Lost track of {ctx["ticker"]} — try asking again.')
                return
            self._say_stock_value(match, target_ccy=target_ccy)
        elif ctx['type'] == 'extreme':
            low = 'smallest' if ctx.get('smallest') else 'largest'
            self._handle_extreme_position(text='', low=low, forced_ccy=target_ccy)
        else:
            self._handle_how_much(text=None, forced_ccy=target_ccy)
    ...
    def _handle_how_much(self, text: str, forced_ccy: str = None):
        # If a specific ticker was named ("how much unit i owned in 1171.KL"),
        # answer just that position instead of the whole portfolio — the
        # generic full-summary answer was masking direct per-stock questions.
        if text:
            match = _opie_match_portfolio_ticker(self.host, text)
            if match:
                self._say_stock_value(match, target_ccy=forced_ccy or _opie_parse_currency(text))
                return
        ...
        fx     = _opie_extract_fx(self.host)
        prices = _opie_extract_prices(self.host)
        target = forced_ccy or _opie_parse_currency(text or '')
        ccy_counts = {}
        for port in portfolios.values():
            c = port.get('base_currency', 'USD')
            ccy_counts[c] = ccy_counts.get(c, 0) + 1
        ...
        self._last_context = {'type': 'portfolio', 'ticker': None}
        self._say(f'Full summary in {target}: {positions} position(s), '
                   f'value {prefix}{mv_total:,.2f}, '
                   f'unrealized {prefix}{gain:+,.2f} ({pct:+.2f}%).')

    def _handle_extreme_position(self, text: str, low: str, forced_ccy: str = None):
        """'largest'/'biggest' or 'smallest' keyword — ranks every live
        position by market value (converted to one common currency so a
        mixed MYR+USD portfolio compares fairly) and reports the top/bottom
        one, e.g. 'as on my portfolio, who was the largest'."""
        portfolios = _opie_extract_portfolios(self.host)
        if not portfolios:
            self._say('No portfolio loaded yet — connect a source first.')
            return
        fx     = _opie_extract_fx(self.host)
        prices = _opie_extract_prices(self.host)
        target = forced_ccy or _opie_parse_currency(text) or 'MYR'
        ...
        for port in portfolios.values():
            ...
            for ticker, item in port.get('holdings', {}).items():
                ...
                if not shares or shares <= 0:
                    continue
                price = prices.get(ticker)
                mv_native = shares * price if price else item.get('cost_value', 0.0)
                mv = _to_target(mv_native, ccy)
                if mv is None:
                    continue
                cost = _to_target(item.get('cost_value', 0.0), ccy) or 0.0
                gain = mv - cost
                pct  = (gain / cost * 100) if cost > 0 else 0.0
                ranked.append((ticker, mv, gain, pct))

        if not ranked:
            self._say(f'No exchange rate available for {target} yet — try MYR or USD.')
            return

        want_smallest = 'smallest' in low
        ticker, mv, gain, pct = (min if want_smallest else max)(ranked, key=lambda r: r[1])
        prefix = 'RM' if target == 'MYR' else f'{target} '
        label  = 'smallest' if want_smallest else 'largest'
        self._last_context = {'type': 'extreme', 'ticker': None, 'smallest': want_smallest}
        self._say(f'Your {label} position is {ticker}, worth '
                   f'{prefix}{mv:,.2f} ({len(ranked)} position(s) compared in {target}), '
                   f'unrealized {gain:+,.2f} ({pct:+.2f}%).')
