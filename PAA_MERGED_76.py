...
to earn — incentivised clicking violates ad-network policy.</div>
    ...
    def _stripe_restricted_get(self, path: str, params: str = ''):
        import urllib.request, urllib.error, json as _json
        rkey = (load_app_settings().get('stripe_restricted_key') or '').strip()
        if not rkey:
            return False, 'No restricted key configured.'
        url = f'https://api.stripe.com/v1/{path}'
        if params:
            url += f'?{params}'
    ...
    def _open_admin_dashboard(self):
        ...
        # self-verify their own purchase automatically. Create this in
        # Stripe Dashboard → Developers → API keys → "Create restricted key"
        # with ONLY "Checkout Sessions: Read" permission (leave everything
        # else "None"). Never put the secret key here.
        rkey_row = tk.Frame(sf, bg='#161b22'); rkey_row.pack(fill='x', padx=8, pady=(6, 2))
        tk.Label(rkey_row, text='Restricted key (rk_live_…, read-only, for auto-verify):',
                 font=('Segoe UI', 8), fg='#cdd9e5', bg='#161b22').pack(side='left')
        rkey_var = tk.StringVar(value=s.get('stripe_restricted_key', ''))
        tk.Entry(rkey_row, textvariable=rkey_var, show='•', width=28,
                 font=('Segoe UI', 8), bg='#0d1117', fg='#cdd9e5',
                 insertbackground='#635BFF', relief='flat',
                 highlightbackground='#30363d', highlightthickness=1
                 ).pack(side='left', padx=6)
        def _save_rkey():
            st2 = load_app_settings()
            st2['stripe_restricted_key'] = rkey_var.get().strip()
            save_app_settings(st2)
            messagebox.showinfo('Stripe',
                'Restricted key saved. This ships with the app build, so '
                'every user\'s copy can now auto-verify its own purchase.',
                parent=win)
        tk.Button(rkey_row, text='Save key', font=('Segoe UI', 7, 'bold'),
                  bg='#21262d', fg='#cdd9e5', relief='flat', padx=8, pady=2,
                  cursor='hand2', command=_save_rkey).pack(side='left')
        tk.Label(sf, text='⚠️  Grant ONLY "Checkout Sessions: Read" + "Subscriptions: Read" '
                          'on this key (both read-only) — nothing else. It ships inside the '
                          'app, so it must never be able to write, charge, or refund.',
