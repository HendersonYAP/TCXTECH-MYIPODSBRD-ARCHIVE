...
to earn — incentivised clicking violates ad-network policy.</div>
    ...
    def _build_rewards_tab(self):
        ...
        tk.Label(hdr, text=f'💰 {pts.get("credits", 0):,.0f} Cr',
                 font=('Segoe UI', 11, 'bold'), fg=GOLD, bg=BG).pack(side='right')
...
class SplashScreen(tk.Tk):
    ...
    def _load_thread(self, path, prefer_online=True, apply_sell_fee=True):
        ...
        mybook_error  = [None]
        sp500_result  = [0, 0]   # [n_price, n_divs]
        ...
        def _fetch_sp500():
            # 503-ticker S&P 500 universe — price + dividend data, chunked
            # 100-at-a-time. Runs alongside teet.csv/ledger so the
            # US-Stock ZDWS/ZDOS group and dividend estimator are warm the
            # instant the dashboard opens, instead of a silent post-open fetch.
            if not prefer_online:
                self.after(0, lambda: self._log(
                    '[SP500] ⏭  Offline mode — skipping S&P 500 price/dividend prefetch.'))
                return
            try:
                n_p, n_d = sp500_prefetch_price_and_dividends(
                    log_cb=lambda m: self.after(0, lambda msg=m: self._log(f'[SP500] {msg}')))
                sp500_result[0], sp500_result[1] = n_p, n_d
            except Exception as e:
                self.after(0, lambda msg=str(e): self._log(f'[SP500] ⚠️  {msg}'))
        ...
        # ── Fire all three downloads simultaneously ─────────────────────────
        self.after(0, lambda: self._stage_var.set(
            '⬇  Downloading teet.csv + ledger + S&P 500 data in parallel…'))

        with ThreadPoolExecutor(max_workers=3) as ex:
            f_teet   = ex.submit(_fetch_teet)
            f_mybook = ex.submit(_fetch_mybook)
            f_sp500  = ex.submit(_fetch_sp500)
            for fut in _ac([f_teet, f_mybook, f_sp500]):
                fut.result()
