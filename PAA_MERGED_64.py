...
class MYIPOApp(tk.Tk):
    ...
    def _zdws_level_decimals(self, underlying: str) -> int:
        ...
        The lot-scaled FX pairs (JPY/KRW/IDR) are already brought into a
        normal magnitude by their quote lot, so 2dp reads cleanly there.
        Everything else — per-1 FX pairs (USD, GBP, SGD…), plus indices,
        US stocks and ETFs — shows 4dp per request, since a bare per-1 rate
        or a fine-grained price needs the extra precision to be meaningful."""
        if underlying in self.FX_QUOTE_LOT:      # JPY / KRW / IDR — specialized
            return 2
        return 4
