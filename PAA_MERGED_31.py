...
class SCAWindow(tk.Toplevel):
    ...
    def _refresh_today_activity(self):
        ...
        today_fmts = {
            today.strftime('%m/%d/%Y'),          # 07/13/2026 (zero-padded)
            today.strftime('%-m/%-d/%Y'),         # 7/13/2026  (no leading zeros, Linux)
            f'{today.month}/{today.day}/{today.year}',   # 7/13/2026  (explicit, all platforms)
            today.strftime('%d/%m/%Y'),           # 13/07/2026 (alternate)
            f'{today.day}/{today.month}/{today.year}',   # 13/7/2026
            today.strftime('%Y-%m-%d'),           # 2026-07-13 (ISO)
