...
class MYIPOApp(tk.Tk):
    ...
    def _draw_fund_ledger(self):
        ...
        if lbl in frames:
            ...
            self._sofp_asset_vars['Holdings Value (RM)'].set(f'{ccy}{holdings_val:,.4f}')
            self._sofp_asset_vars['Cash on Hand (RM)'].set(f'{ccy}{last_cash:,.4f}')
            self._sofp_asset_vars['Total Assets (RM)'].set(f'{ccy}{aum_last:,.4f}')
            self._sofp_equity_vars['Units Outstanding'].set(f'{units_last:,.2f}')
            self._sofp_equity_vars['NAV per Unit (RM)'].set(f'{ccy}{nav_last:,.4f}')
            self._sofp_equity_vars['Total NAV (RM)'].set(f'{ccy}{aum_last:,.4f}')
            self._sofp_equity_vars['Cum. Distributions/Unit (RM)'].set(f'{ccy}{cd_last:,.4f}')
            self._sofp_equity_vars['Total Return/Unit (RM)'].set(f'{ccy}{tr_last:,.4f}')
            self._sofp_perf_vars['Inception Date'].set(inception_date)
            self._sofp_perf_vars['Par NAV (RM)'].set(f'{ccy}{par:.4f}')
    ...
    def _zdos_open_position(self, underlying: str, postype: str, tier_num: int,
                             stake: float, settle_date_str: str) -> bool:
    ...
    def _zdos_market_close(self, position_id: int):
    ...
    def _zdws_ccy(self, underlying: str) -> str:
        ...
        2) VFund CEFs — real per-fund currency from VFUND_CONFIG (mixed
           MYR/USD baskets, e.g. MYIPO+ XaaS Fund is MYR, the MSCI-EW ones
           are USD).
        3) KLCI + the internal MYIPO+/Top-Series index family and unit-trust
           FUND_LABELS — always MYR (built from Bursa-listed constituents),
           even though the label itself carries no '.KL' suffix.
        4) Raw ticker symbols (individual stocks, LottoStock, VFund
           constituents) — inferred from suffix: '.KL' = MYR, bare = USD.
        """
    ...
    def _build_zdos_tab(self):
        ...
        # Position type — C/S or S/C: Call-or-Put, Long-or-Short
        tk.Label(left, text='Position', font=FONT, fg=DIM, bg=PANEL).pack(anchor='w')
        ...
        def _write():
            ...
            ok = self._zdos_open_position(key, pos_var.get(), t, stake,
                                          d.strftime('%d/%m/%Y'))
            if ok:
                messagebox.showinfo('ZDOS',
                    f'📐 Option written!\n\n{pos_var.get().replace("_"," ").title()} '
                    f'T{t} on {unders[i][1]}\nEntry: {unders[i][2]:,.2f}\n'
                    f'Stake: {stake:,.0f} credits\nSettles: {d.strftime("%d/%m/%Y")}', parent=self)
                self._build_zdos_tab()
            else:
                messagebox.showwarning('ZDOS',
                    'Could not write — check stake vs balance.', parent=self)

        tk.Button(left, text='📐  WRITE OPTION', command=_write,
        ...
        tk.Label(hist_f, text='📜  SETTLED OPTIONS', font=('Segoe UI', 9, 'bold'),
                 fg=DIM, bg=PANEL).pack(anchor='w', padx=8, pady=(6, 4))
        cols = ('Position', 'Tier', 'Underlying', 'P₀', 'Settled', 'Move %', 'Stake', 'Payout', 'Result')
        htree = ttk.Treeview(hist_f, columns=cols, show='headings', height=7)
        for c, wdt in zip(cols, (85, 35, 120, 65, 65, 60, 60, 65, 130)):
            htree.heading(c, text=c)
            htree.column(c, width=wdt, anchor='center')
        htree.pack(fill='both', expand=True, padx=8, pady=(0, 8))
        htree.tag_configure('win',  foreground=GREEN)
