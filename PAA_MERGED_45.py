...
def _mf_parse_rows(raw_text: str, log_cb=None) -> list:
    """Parse MYBook CSV text into normalised row dicts.
...
def _mf_load_csv(url: str = None, log_cb=None,
    ...
    url=None (default) resolves the SIGNED-IN USER's own source:
      ...
      · mode 'demo'  → the built-in sample sheet
    Pass an explicit url to override.

    Each K-ID gets its own local cache file (MYBook_<user>.csv) so accounts
    ...
    Cache strategy (identical intent to resolve_input / teet_cache.csv):
      ...
      2. prefer_online=True             → download sheet, delta-sync.
    ...
    if url is None:
        ...
        if src and src.get('mode') == 'csv' and src.get('csv_path'):
            ...
            log(f'⚠️  Imported CSV missing: {p} — falling back to demo sheet.')
            url = MYFOLIO_CSV_URL
        else:
            url = (src or {}).get('url') or MYFOLIO_CSV_URL
    ...
    if local_file is None:
        ...
        name = (f'MYBook_{who}.csv' if who else _NEWPOR_LOCAL_CACHE_NAME)
...
class SCAWindow(tk.Toplevel):
    ...
    def _refresh_source_label(self):
        try:
            ...
            icon = {'sheet': '📗', 'csv': '📄', 'demo': '🧪'}.get(src.get('mode'), '●')
    ...
    def _open_source_dialog(self):
        ...
        win = tk.Toplevel(self)
        win.title('SCA — Portfolio Source')
        win.configure(bg=self.BG)
        win.geometry('620x480')
        win.transient(self)
        win.grab_set()

        tk.Label(win, text='📂  PORTFOLIO SOURCE', font=('Courier New', 12, 'bold'),
        ...
        mode_var = tk.StringVar(value=cur.get('mode', 'demo'))
        ...
        # ── Option 3: demo ────────────────────────────────────────────────────
        c3 = tk.Frame(win, bg=self.PANEL, highlightbackground=self.BORDER,
                      highlightthickness=1, padx=12, pady=8)
        c3.pack(fill='x', padx=16, pady=4)
        tk.Radiobutton(c3, text='🧪  Use demo portfolio (sample data)',
                       variable=mode_var, value='demo',
                       font=('Courier New', 9, 'bold'),
                       bg=self.PANEL, fg=self.DIM, selectcolor=self.PANEL,
        ...
        def _save_and_load():
            ...
            if m == 'sheet':
                ...
                sca_save_source(settings, who, 'demo', url=MYFOLIO_CSV_URL,
                                label='Demo portfolio (built-in sample sheet)')
    ...
    def _build_footer(self):
        ...
        # MYBook sheet and is far more flexible to slice there. This panel
        ...
        tk.Label(an_card,
                 text='Charts, pivots and P/L breakdowns live in the MYBook workbook.',
        ...
        def _open_sheet():
            import webbrowser
            try:
                webbrowser.open_new_tab(MYFOLIO_SHEET_EDIT_URL)
            except Exception as e:
                messagebox.showwarning(
                    'SCA', f'Could not open browser:\n{e}\n\n'
                           f'URL:\n{MYFOLIO_SHEET_EDIT_URL}', parent=self)

        def _copy_url():
            try:
                self.clipboard_clear()
                self.clipboard_append(MYFOLIO_SHEET_EDIT_URL)
                messagebox.showinfo('SCA', 'Sheet URL copied to clipboard.', parent=self)
            except Exception:
                pass

        btn_row = tk.Frame(an_card, bg=self.PANEL)
        btn_row.pack(fill='x')
        tk.Button(btn_row, text='📗  Open MYBook Workbook', command=_open_sheet,
    ...
    def _an_refresh_portfolio_dropdown(self):
        """No-op — in-app analytics were replaced by the MYBook workbook link."""
        return
...
APPLOCK_PIN = "1234"   # legacy fallback PIN — used only if no K-ID exists yet
...
def sca_get_source(settings: dict, username: str = None) -> dict:
    ...
    if src and (src.get('url') or src.get('csv_path')):
        return src
    return {'mode': 'demo', 'url': MYFOLIO_CSV_URL, 'csv_path': '',
            'label': 'Demo portfolio (built-in sample sheet)'}
...
class AppLockScreen(tk.Tk):
    ...
    def __init__(self):
        ...
        self._keep_locked_var = tk.BooleanVar(
            value=self._settings.get("applock_enabled", True))
    ...
    def _build_ui(self):
        ...
        tk.Label(box, text="⚙ SETTINGS", font=("Segoe UI", 8, "bold"),
                 fg='#888', bg='#161b22').pack(anchor='w')
        tk.Checkbutton(
            box, text="Require K-ID sign in on startup",
            variable=self._keep_locked_var, command=self._toggle_lock,
            bg='#161b22', fg='#cdd9e5', selectcolor='#161b22',
            activebackground='#161b22', activeforeground='#58A6FF',
            font=("Segoe UI", 8), relief='flat', cursor='hand2'
        ).pack(anchor='w')
    ...
    def _sign_in(self):
        ...
        if kid_verify(self._settings, u, p):
            ...
            self._launch(u)
            return
        # Legacy fallback — no accounts yet, accept the old PIN
        if not self._settings.get('kid_accounts') and p == APPLOCK_PIN:
            self._launch('')
            return
...
if __name__ == '__main__':
    ...
    if _settings.get("applock_enabled", True):
        ...
        # Sign-in disabled — resume as the last K-ID so the right portfolio loads
        CURRENT_KID = _settings.get('kid_last_user', '')
        splash = SplashScreen()
        splash.mainloop()
