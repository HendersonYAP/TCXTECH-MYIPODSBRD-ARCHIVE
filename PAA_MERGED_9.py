...
class MYIPOApp(tk.Tk):
    ...
    def _build_line_tab(self):
        ...
        # ── Hover crosshair (Google-Finance style floating datapoint box) ────
        self._line_hover_series = {}   # label -> pandas Series, refreshed each draw
        self._line_hover_artists = []  # crosshair line/dot/annotation, cleared each move
    ...
    def _on_line_hover(self, event):
        if not self._line_hover_series or event.xdata is None:
            self._clear_line_hover()
            return
        if event.inaxes not in (self.ax_line, getattr(self, '_ax_line_fund', None)):
            self._clear_line_hover()
            return

        import matplotlib.dates as _mdates
        hover_dt = _mdates.num2date(event.xdata).replace(tzinfo=None)

        # Find the closest datapoint, across every plotted series, to the cursor's x position
        best = None  # (distance_days, label, date, value, color, is_fund)
        for lbl, info in self._line_hover_series.items():
            ...
            for p in candidates:
                d = idx_arr[p]
                dist = abs((d.to_pydatetime() - hover_dt).total_seconds())
        ...
        target_ax = self._ax_line_fund if (is_fund and getattr(self, '_ax_line_fund', None)) else self.ax_line
