...
# between the two legs is NOT equal-weight — it's driven by the 'weights'
# dict below instead of 1/N, tilted toward the wrapper's higher
# distribution yield.
#
# NOTE: the intended design (per spec) is a *dynamic* split that moves with
# the live TLTW/TLT-style yield ratio. This engine issues shares once at
# launch (same as every other VFund CEF — see the "CLOSED-END" note above),
# so what's implemented here is a yield-tilted split FIXED AT LAUNCH, not a
# continuously-rebalanced one — wiring live dividend-yield data into the
# historical NAV builder (which currently only consumes price history) is a
# bigger change than a config addition and is flagged separately, not done
# here. Placeholder weights below (60/40 toward the wrapper) should be
# replaced with real trailing-12m distribution yields before this goes live.
#
...
ENHANCED_YIELD_CONFIG = [
    {
        ...
        'members': ['TLTW', 'TLT'],
        'weights': {'TLTW': 0.60, 'TLT': 0.40},
        'note':    'TLTW+TLT, 1:1 base tilted by TLTW/TLT yield (placeholder split)',
    },
    {
        'label':   'MYIPO+ EY High Yield Fund',
        'ccy':     'USD', 'par': 1.0000, 'suffix': '',
        'members': ['HYGW', 'HYG'],
        'weights': {'HYGW': 0.60, 'HYG': 0.40},
        'note':    'HYGW+HYG, 1:1 base tilted by HYGW/HYG yield (placeholder split)',
    },
    {
        'label':   'MYIPO+ EY Investment Grade Fund',
        'ccy':     'USD', 'par': 1.0000, 'suffix': '',
        'members': ['LQDW', 'LQD'],
        'weights': {'LQDW': 0.60, 'LQD': 0.40},
        'note':    'LQDW+LQD, 1:1 base tilted by LQDW/LQD yield (placeholder split)',
    },
    {
        ...
        # within each sleeve the wrapper/underlying split mirrors the
        # single-sleeve funds above.
        'weights': {'TLTW': 0.20, 'TLT': 0.1333,
                    'HYGW': 0.20, 'HYG': 0.1333,
                    'LQDW': 0.20, 'LQD': 0.1334},
        'note':    'Mixed sleeve: 1/3 each of Treasury/HY/IG Enhanced Yield pairs',
    },
    {
        'label':   'MYIPO+ EY Nasdaq 100 Fund',
        'ccy':     'USD', 'par': 1.0000, 'suffix': '',
        'members': ['QQQI', 'QQQM'],
        'weights': {'QQQI': 0.60, 'QQQM': 0.40},
        'note':    'QQQI+QQQM, 1:1 base tilted by QQQI yield (placeholder split)',
    },
    {
        'label':   'MYIPO+ EY S&P 500 Fund',
        'ccy':     'USD', 'par': 1.0000, 'suffix': '',
        'members': ['SPYI', 'SPY'],
        'weights': {'SPYI': 0.60, 'SPY': 0.40},
        'note':    'SPYI+SPY, 1:1 base tilted by SPYI yield (placeholder split)',
    },
    {
        ...
        'members': ['DJIA', 'DIA'],
        'weights': {'DJIA': 0.60, 'DIA': 0.40},
        'note':    'DJIA+DIA, 1:1 base tilted by yield (placeholder split — confirm DJIA ticker)',
    },
    {
        'label':   'MYIPO+ EY Russell 2000 Fund',
        'ccy':     'USD', 'par': 1.0000, 'suffix': '',
        'members': ['IWMI', 'IWM'],
        'weights': {'IWMI': 0.60, 'IWM': 0.40},
        'note':    'IWMI+IWM, 1:1 base tilted by IWMI yield (placeholder split)',
    },
]
