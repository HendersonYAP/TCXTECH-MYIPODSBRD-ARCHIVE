...
def kid_verify(settings: dict, username: str, pin: str) -> bool:
    u = username.strip().lower()
    # ── Admin account: authenticated against its own secret password hash,
    # separate from the normal accounts list. BOOTSTRAP: if no admin password
    # has ever been set, the FIRST login as admin1 sets whatever was entered
    # as the secret (min 6 chars) — nothing is hardcoded in source, so the
    # secret exists only in the admin's head and as a salted hash on disk.
    if u == ADMIN_KID:
        if not pin or len(pin) < 6:
            return False
        stored = settings.get('admin_pw_hash', '')
        if not stored:
            settings['admin_pw_hash'] = kid_hash_pin(ADMIN_KID, pin)
...
class AppLockScreen(tk.Tk):
    ...
    def _sign_in(self):
        ...
        if kid_verify(self._settings, u, p):
            ...
            # Factor 3 is a fixed passphrase checked against its SHA-256 —
            # only the hash lives in source, the phrase itself is never
            # written down anywhere in the code or on disk.
            if u.strip().lower() == ADMIN_KID:
    ...
    ADMIN_RCODE_SHA256 = ('cadaa6c1ee04353db430af922003e16212aacb18f88efb4d4a7cf8f5f1f91be0')

    def _admin_rcode_check(self) -> bool:
        """Third factor for admin1: prompt for the R-Code passphrase in a
        masked dialog and compare SHA-256 against the stored hash."""
