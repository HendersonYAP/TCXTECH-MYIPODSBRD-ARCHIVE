...
class MYFOLIOWindow(tk.Toplevel):
    """
    Portfolio Command Center — 3 views:
      1. Holdings  — per-ticker table with live prices
      2. Total     — P/L summary; currency selector (USD / MYR / Combined)
      3. History   — cumulative portfolio-value line chart from transactions
    """
    _VIEWS = ["Holdings", "Total", "History"]

    def __init__(self, parent):
        super().__init__(parent)
        self.title("MYFOLIO — Portfolio Command Center")
        self.geometry("1300x800"); self.minsize(1060, 660)
        self.configure(bg=_MF_BG)
        ...
        self._cur_tab           = "USD"     # sidebar currency filter
        self._active_view       = "Holdings"
        self._total_ccy_mode    = tk.StringVar(value="Combined")  # USD / MYR / Combined
    ...
    def _build_ui(self):
        ...
        tk.Label(left, text="VIEW", font=("Consolas", 8, "bold"),
                 bg=_MF_PANEL, fg=_MF_SUBTEXT).pack(anchor="w", padx=16, pady=(8, 4))
        self._view_btns = {}
        for v in self._VIEWS:
            btn = tk.Button(left, text=v, font=("Consolas", 8),
                            bg=_MF_ACCENT if v == "Holdings" else _MF_CARD,
                            fg=_MF_BG if v == "Holdings" else _MF_TEXT,
                            relief="flat", bd=0, padx=8, pady=5, anchor="w",
                            command=lambda vv=v: self._switch_view(vv))
            btn.pack(fill="x", padx=12, pady=1)
            self._view_btns[v] = btn

        tk.Label(left, text="DATA SOURCE", font=("Consolas", 7, "bold"),
                 bg=_MF_PANEL, fg=_MF_SUBTEXT).pack(anchor="w", padx=16, pady=(10, 2))
        ...
        # Common header row
        hdr = tk.Frame(self.main, bg=_MF_BG); hdr.pack(fill="x", padx=22, pady=(18, 6))
        self.title_lbl = tk.Label(hdr, font=_MF_FONT_H1, bg=_MF_BG, fg=_MF_TEXT, text="—")
        self.title_lbl.pack(side="left")
        self.cur_badge = tk.Label(hdr, text="● USD", font=("Consolas", 9, "bold"),
                                  bg=_MF_BG, fg=_MF_ACCENT)
        self.cur_badge.pack(side="right", padx=8)
        tk.Button(hdr, text="📊 Charts", font=_MF_FONT_SM,
        ...
        self.kpi_frame.pack(fill="x", padx=16, pady=(0, 4))

        # ── View frames (stacked, only one visible at a time) ─────────────────
        self._view_frames: dict = {}

        # ---- Holdings view --------------------------------------------------
        hv = tk.Frame(self.main, bg=_MF_BG); self._view_frames["Holdings"] = hv

        hv_detail = tk.Frame(hv, bg=_MF_BG); hv_detail.pack(fill="both", expand=True, padx=22, pady=4)
        self.meta_frame = tk.Frame(hv_detail, bg=_MF_PANEL, width=210)
        self.meta_frame.pack(side="left", fill="y", padx=(0, 8)); self.meta_frame.pack_propagate(False)

        hold_outer = tk.Frame(hv_detail, bg=_MF_PANEL)
        hold_outer.pack(side="left", fill="both", expand=True)
        hold_cols = ("Ticker","Market","Ccy","Units","Avg Cost","Cost Value",
                     "Price","Mkt Value","P/L","P/L %","Date")
        self.hold_tree = ttk.Treeview(hold_outer, columns=hold_cols, show="headings")
        cw = {"Ticker":85,"Market":55,"Ccy":50,"Units":75,"Avg Cost":80,
              "Cost Value":88,"Price":80,"Mkt Value":88,"P/L":82,"P/L %":68,"Date":90}
        ...
        sy = ttk.Scrollbar(hold_outer, orient="vertical", command=self.hold_tree.yview)
        self.hold_tree.configure(yscrollcommand=sy.set)
        self.hold_tree.pack(side="left", fill="both", expand=True, padx=(8, 0), pady=8)
        sy.pack(side="left", fill="y", pady=8)

        sty = ttk.Style()
        sty.configure("MF.Treeview", background=_MF_CARD, foreground=_MF_TEXT,
                      fieldbackground=_MF_CARD, font=_MF_FONT_SM, rowheight=24, borderwidth=0)
        sty.configure("MF.Treeview.Heading", background=_MF_BORDER, foreground=_MF_SUBTEXT,
                      font=("Consolas", 8, "bold"))
        sty.map("MF.Treeview", background=[("selected", _MF_ACCENT)],
                foreground=[("selected", _MF_BG)])
        self.hold_tree.configure(style="MF.Treeview")

        # ---- Total view -----------------------------------------------------
        tv = tk.Frame(self.main, bg=_MF_BG); self._view_frames["Total"] = tv

        # Currency mode selector
        ccy_bar = tk.Frame(tv, bg=_MF_PANEL, height=38)
        ccy_bar.pack(fill="x", padx=22, pady=(0, 6)); ccy_bar.pack_propagate(False)
        tk.Label(ccy_bar, text="Show currency:", font=_MF_FONT_SM,
                 bg=_MF_PANEL, fg=_MF_SUBTEXT).pack(side="left", padx=12)
        self._total_ccy_btns = {}
        for opt in ("Combined", "USD", "MYR"):
            col = _MF_ACCENT if opt == "Combined" else _MF_CARD
            fg  = _MF_BG     if opt == "Combined" else _MF_TEXT
            btn = tk.Button(ccy_bar, text=opt, font=("Consolas", 8, "bold"),
                            bg=col, fg=fg, relief="flat", bd=0, padx=12, pady=4,
                            command=lambda o=opt: self._set_total_ccy(o))
            btn.pack(side="left", padx=3)
            self._total_ccy_btns[opt] = btn

        # Total summary treeview
        tot_cols = ("Portfolio","Currency","Type","Cost","Mkt Value","P/L","P/L %","Dep","Fees","Real P/L")
        self.total_tree = ttk.Treeview(tv, columns=tot_cols, show="headings", height=18)
        tot_w = {"Portfolio":110,"Currency":70,"Type":80,"Cost":100,
                 "Mkt Value":100,"P/L":100,"P/L %":80,"Dep":90,"Fees":70,"Real P/L":90}
        for col in tot_cols:
            self.total_tree.heading(col, text=col)
            self.total_tree.column(col, width=tot_w.get(col, 80),
                                   anchor="w" if col in ("Portfolio","Type") else "e")
        sy2 = ttk.Scrollbar(tv, orient="vertical", command=self.total_tree.yview)
        self.total_tree.configure(yscrollcommand=sy2.set)
        self.total_tree.pack(side="left", fill="both", expand=True, padx=22, pady=4)
        sy2.pack(side="left", fill="y", pady=4, padx=(0, 4))
        self.total_tree.configure(style="MF.Treeview")
        self.total_tree.tag_configure("tot_gain",    foreground=_MF_GREEN)
        self.total_tree.tag_configure("tot_loss",    foreground=_MF_RED)
        self.total_tree.tag_configure("tot_cash",    foreground=_MF_YELLOW)
        self.total_tree.tag_configure("tot_grand",   foreground=_MF_ACCENT,
                                      background="#1a2030")
        self.total_tree.tag_configure("tot_section", foreground=_MF_SUBTEXT,
                                      background=_MF_BORDER)

        # ---- History view ---------------------------------------------------
        hisv = tk.Frame(self.main, bg=_MF_BG); self._view_frames["History"] = hisv
        self.hist_canvas_frame = tk.Frame(hisv, bg=_MF_BG)
        self.hist_canvas_frame.pack(fill="both", expand=True, padx=16, pady=8)

        # Show Holdings by default
        self._switch_view("Holdings", init=True)

    # =========================================================================
    # VIEW SWITCHING
    # =========================================================================
    def _switch_view(self, view: str, init: bool = False):
        self._active_view = view
        for v, btn in self._view_btns.items():
            active = (v == view)
            btn.configure(bg=_MF_ACCENT if active else _MF_CARD,
                          fg=_MF_BG     if active else _MF_TEXT)
        for v, frm in self._view_frames.items():
            if v == view:
    ...
    def _switch_currency(self, currency: str):
        ...
        if currency == "USD":
            ...
            self.cur_badge.configure(text="● USD", fg=_MF_ACCENT)
        else:
            self._myr_btn.configure(bg=_MF_GREEN, fg=_MF_BG)
            self._usd_btn.configure(bg=_MF_CARD,  fg=_MF_TEXT)
            self.cur_badge.configure(text="● MYR", fg=_MF_GREEN)
        self._rebuild_list()
        self._refresh_detail()
    ...
    def _portfolios_for_currency(self, currency: str) -> dict:
        ...
        for code, port in self.portfolios.items():
            if port.get("base_currency", "USD").upper() == currency.upper():
                result[code] = port
        return result

    def _set_total_ccy(self, opt: str):
        self._total_ccy_mode.set(opt)
        for o, btn in self._total_ccy_btns.items():
            active = (o == opt)
            btn.configure(bg=_MF_ACCENT if active else _MF_CARD,
                          fg=_MF_BG     if active else _MF_TEXT)
        self._render_total()
    ...
    def _on_error(self, msg: str):
        ...
        _mb.showerror("NEWPOR Load Error",
                      ...
                      "Check the sheet is published: File → Share → Publish to web → CSV")
    ...
    def _refresh_detail(self):
        ...
        self._render_kpi(port)
        if   self._active_view == "Holdings": self._render_holdings(port)
        elif self._active_view == "Total":    self._render_total()
        elif self._active_view == "History":  self._render_history(port)

    # =========================================================================
    # KPI STRIP (common to all views)
    # =========================================================================
    def _render_kpi(self, port: dict):
        ...
        cash  = port["holdings"].get(f"{ccy0}-CASH", {}).get("shares", 0.0)

        # unrealised P/L
        invest_cost = 0.0; invest_mv = 0.0
        for t, item in port["holdings"].items():
            if item.get("_is_cash"): continue
            invest_cost += item["cost_value"]
            p = _mf_get_price(t, item["market"])
            if p is not None: invest_mv += item["shares"] * p
        upl     = invest_mv - invest_cost
        upl_pct = (upl / invest_cost * 100) if invest_cost else 0.0

        self._kpi("Holdings",      str(n_h),                   _MF_PURPLE)
        self._kpi("Deposited",     f"{ccy0} {dep:,.2f}",       _MF_SUBTEXT)
        self._kpi("Cash Position", f"{ccy0} {cash:,.4f}",      _MF_YELLOW)
        self._kpi("Invested",      f"{ccy0} {invest_cost:,.2f}", _MF_SUBTEXT)
        self._kpi("Mkt Value",     f"{ccy0} {invest_mv:,.2f}", _MF_ACCENT)
        self._kpi("Unreal P/L",    f"{ccy0} {upl:+,.2f} ({upl_pct:+.2f}%)",
                  _MF_GREEN if upl >= 0 else _MF_RED)
        self._kpi("Real P/L",      f"{ccy0} {rpl:+,.2f}",
                  _MF_GREEN if rpl >= 0 else _MF_RED)
        self._kpi("Fees",          f"{ccy0} {fees:,.4f}",      _MF_RED)

    def _kpi(self, label: str, value: str, color: str):
        frm = tk.Frame(self.kpi_frame, bg=_MF_CARD, padx=10, pady=8)
        frm.pack(side="left", expand=True, fill="both", padx=4)
        tk.Label(frm, text=label, font=("Consolas", 7), bg=_MF_CARD, fg=_MF_SUBTEXT).pack(anchor="w")
        tk.Label(frm, text=value, font=("Consolas", 10, "bold"), bg=_MF_CARD,
                 fg=color, wraplength=200).pack(anchor="w")

    # =========================================================================
    # VIEW 1 — HOLDINGS
    # =========================================================================
    def _render_holdings(self, port: dict):
        for w in self.meta_frame.winfo_children(): w.destroy()
        for row in self.hold_tree.get_children(): self.hold_tree.delete(row)

        ccy0      = port["base_currency"]
        total_cv  = 0.0; total_mv = 0.0

        for ticker, item in sorted(port["holdings"].items()):
            is_cash  = item.get("_is_cash", False)
            units    = item["shares"]; avg_cost = item["avg_cost"]
            costv    = item["cost_value"]
            market   = item["market"]; currency = item["currency"]

            if is_cash:
                price  = 1.0; mktval = units; pl = 0.0; pl_pct = 0.0; tag = "mf_cash"
                price_s = "1.0000"; pl_s = "CASH"; pct_s = "—"
            else:
                ...
                price_s = _mf_fmt_price(price)
                pl_s   = _mf_fmt_signed(pl) if pl is not None else "N/A"
                pct_s  = _mf_fmt_pct(pl_pct) if pl_pct is not None else "N/A"

            total_cv += costv
            if mktval is not None: total_mv += mktval

            self.hold_tree.insert("", "end", tags=(tag,), values=(
                ticker, market, currency,
                f"{units:,.4f}", f"{avg_cost:,.4f}", _mf_fmt_money(costv),
                price_s, _mf_fmt_money(mktval),
                pl_s, pct_s, item.get("date", ""),
            ))
        ...
        total_pct = (total_pl / total_cv * 100) if total_cv else 0.0

        tk.Label(self.meta_frame, text=" DETAILS", font=_MF_FONT_H3,
                 bg=_MF_PANEL, fg=_MF_ACCENT).pack(anchor="w", padx=10, pady=(10, 4))
        for label, value, col in [
            ("Code",          port["code"],                            _MF_TEXT),
            ("Platform",      port["platform"],                        _MF_TEXT),
            ("Market",        ", ".join(port["markets"]),              _MF_TEXT),
            ("Currency",      ccy0,                                    _MF_TEXT),
            ("Holdings",      str(n_h),                                _MF_PURPLE),
            ("Deposited",     f"{ccy0} {dep:,.2f}",                   _MF_TEXT),
            ("Withdrawn",     f"{ccy0} {wdraw:,.2f}",                  _MF_TEXT),
            ("Fees Paid",     f"{ccy0} {fees:,.4f}",                  _MF_RED),
            ("Cash Position", f"{ccy0} {cash:,.4f}",                  _MF_YELLOW),
            ("Invested",      f"{ccy0} {total_cv:,.2f}",              _MF_SUBTEXT),
            ("Market Value",  f"{ccy0} {total_mv:,.2f}",              _MF_ACCENT),
            ("Unreal P/L",    f"{ccy0} {total_pl:+,.2f} ({total_pct:+.2f}%)",
             _MF_GREEN if total_pl >= 0 else _MF_RED),
            ("Realised P/L",  f"{ccy0} {rpl:+,.2f}",
             _MF_GREEN if rpl >= 0 else _MF_RED),
        ]:
            tk.Label(self.meta_frame, text=label, font=("Consolas", 8, "bold"),
                     bg=_MF_PANEL, fg=_MF_SUBTEXT).pack(anchor="w", padx=12, pady=(5, 0))
            tk.Label(self.meta_frame, text=value, font=_MF_FONT_SM,
                     bg=_MF_PANEL, fg=col, wraplength=200).pack(anchor="w", padx=12)
    ...
    def _render_total(self):
        ...
        mode = self._total_ccy_mode.get()  # "Combined" / "USD" / "MYR"

        def _want(ccy):
            return mode == "Combined" or mode == ccy

        # Grand accumulators
        grand: dict = {}   # ccy → {cost, mv, pl, dep, fees, rpl, cash}

        for code, port in sorted(self.portfolios.items()):
            ccy0 = port["base_currency"]
            if not _want(ccy0): continue

            # Section header row
            self.total_tree.insert("", "end", tags=("tot_section",),
                values=(code, ccy0, "— PORTFOLIO —", "", "", "", "", "", "", ""))

            # Per-ticker rows
            port_cost = 0.0; port_mv = 0.0; port_cash = 0.0
            for ticker, item in sorted(port["holdings"].items()):
                is_cash  = item.get("_is_cash", False)
                costv    = item["cost_value"]
                ...
                if is_cash:
                    mv    = units; pl = 0.0; pl_pct = 0.0
                    port_cash += units
                    self.total_tree.insert("", "end", tags=("tot_cash",),
                        values=(ticker, currency, "CASH",
                                _mf_fmt_money(costv), _mf_fmt_money(mv),
                                "—", "—", "—", "—", "—"))
                else:
                    price = _mf_get_price(ticker, item["market"])
                    mv    = units * price if price is not None else None
                    pl    = (mv - costv)  if mv is not None else None
                    pl_pct = (pl / costv * 100) if pl is not None and costv else None
                    tag   = "tot_gain" if pl is not None and pl >= 0 else "tot_loss"
            ...
            # Portfolio subtotal row
            port_pl  = port_mv - port_cost
            port_pct = (port_pl / port_cost * 100) if port_cost else 0.0
            dep  = port.get("_total_deposited", 0.0)
            fees = port.get("_total_fees",      0.0)
            rpl  = port.get("_realised_pl",     0.0)
            tag  = "tot_gain" if port_pl >= 0 else "tot_loss"
            self.total_tree.insert("", "end", tags=(tag,),
                values=(f"  ↳ SUBTOTAL", ccy0, "",
                        _mf_fmt_money(port_cost),
                        _mf_fmt_money(port_mv),
                        f"{port_pl:+,.2f}",
                        f"{port_pct:+.2f}%",
                        _mf_fmt_money(dep),
                        _mf_fmt_money(fees),
            ...
            if ccy0 not in grand:
                grand[ccy0] = {"cost":0.0,"mv":0.0,"dep":0.0,"fees":0.0,"rpl":0.0,"cash":0.0}
            grand[ccy0]["cost"] += port_cost
            grand[ccy0]["mv"]   += port_mv
            grand[ccy0]["dep"]  += dep
            grand[ccy0]["fees"] += fees
            grand[ccy0]["rpl"]  += rpl
            grand[ccy0]["cash"] += port_cash
        ...
        for ccy, g in sorted(grand.items()):
            ...
            tag  = "tot_grand"
            self.total_tree.insert("", "end", tags=(tag,),
                values=(f"▶ GRAND TOTAL", ccy, f"Cash:{g['cash']:,.4f}",
                        _mf_fmt_money(g["cost"]),
                        _mf_fmt_money(g["mv"]),
                        f"{gpl:+,.2f}",
                        f"{gpct:+.2f}%",
                        _mf_fmt_money(g["dep"]),
                        _mf_fmt_money(g["fees"]),
                        f"{g['rpl']:+,.2f}"))

    # =========================================================================
    # VIEW 3 — HISTORY (cumulative portfolio value line chart)
    # =========================================================================
    def _render_history(self, port: dict):
        for w in self.hist_canvas_frame.winfo_children(): w.destroy()
        ...
        try:
            ...
            tk.Label(self.hist_canvas_frame,
                     text="matplotlib not installed.\npip install matplotlib",
                     font=_MF_FONT_MD, bg=_MF_BG, fg=_MF_RED).pack(expand=True)
            return

        ccy0 = port["base_currency"]

        # ── Reconstruct daily portfolio value from transaction history ────────
        # Step 1: collect all unique dates from every holding's transactions
        from collections import defaultdict
        all_dates = set()
        for item in port["holdings"].values():
            if item.get("_is_cash"): continue
            for txn in item.get("transactions", []):
                ds = txn[1]  # date string e.g. "4/27/2026"
                try:
                    d = _dt.datetime.strptime(ds, "%m/%d/%Y").date()
                    all_dates.add(d)
                except ValueError:
                    pass

        if not all_dates:
            tk.Label(self.hist_canvas_frame, text="No transaction history to chart.",
                     font=_MF_FONT_MD, bg=_MF_BG, fg=_MF_SUBTEXT).pack(expand=True)
            return

        # Step 2: simulate holdings state at each date snapshot
        min_d = min(all_dates); max_d = _dt.date.today()
        # build sorted txn list per ticker: (date, type, units, price, amount)
        txn_by_ticker: dict = defaultdict(list)
        for ticker, item in port["holdings"].items():
            if item.get("_is_cash"): continue
            for txn in item.get("transactions", []):
                ds = txn[1]
                try:
                    d = _dt.datetime.strptime(ds, "%m/%d/%Y").date()
                    txn_by_ticker[ticker].append((d, txn[0], txn[2], txn[3], txn[4]))
                except ValueError:
                    pass
            txn_by_ticker[ticker].sort(key=lambda x: x[0])

        # Step 3: walk day by day, tracking shares and recording cost value
        # We record cost_value (not live price) for the history — live pricing
        # all historical dates would be too slow (hundreds of yf calls).
        # Instead: cost value is the "invested" line; on today's snapshot we
        # add live prices for a "current value" dot.
        sorted_dates = sorted(all_dates)
        snap_dates   = []
        snap_cost    = []    # invested cost at each snapshot date
        snap_cash    = []    # running cash balance

        # replay cash from deposits (approximate — use deposited total spread)
        # We use the actual transaction list stored per holding
        shares_state: dict = defaultdict(float)
        cost_state:   dict = defaultdict(float)   # ticker → cost_value so far

        # collect cash txns too (from portfolio-level data)
        dep = port.get("_total_deposited", 0.0)

        # replay all ticker txns chronologically
        all_txns = []
        for ticker, txns in txn_by_ticker.items():
            for t in txns:
                all_txns.append((t[0], ticker, t[1], t[2], t[3], t[4]))
        all_txns.sort(key=lambda x: x[0])

        running_cost = 0.0
        txn_idx = 0
        for snap_d in sorted_dates:
            # apply all txns up to and including snap_d
            while txn_idx < len(all_txns) and all_txns[txn_idx][0] <= snap_d:
                _, ticker, ttype, unit, price, amount = all_txns[txn_idx]
                if ttype == "buy":
                    old_s = shares_state[ticker]; new_s = old_s + unit
                    shares_state[ticker] = new_s
                    cost_state[ticker]   = cost_state[ticker] + amount
                    running_cost        += amount
                elif ttype == "sell":
                    sold = abs(unit)
                    if shares_state[ticker] > 0:
                        cost_basis = (cost_state[ticker] / shares_state[ticker]) * sold
                        cost_state[ticker]   -= cost_basis
                        running_cost         -= cost_basis
                    shares_state[ticker] -= sold
                    if shares_state[ticker] <= 1e-8:
                        shares_state[ticker] = 0.0
                        cost_state[ticker]   = 0.0
                txn_idx += 1
            snap_dates.append(snap_d)
            snap_cost.append(running_cost)
            snap_cash.append(0.0)  # placeholder — no per-date cash tracking

        # Step 4: compute current live market value (single point at end)
        live_mv = 0.0
        for ticker, shares in shares_state.items():
            if shares <= 0: continue
            p = _mf_get_price(ticker, "MY" if ticker.endswith(".KL") else "US")
            if p is not None: live_mv += shares * p

        # Step 5: draw chart
        fig = _Fig(figsize=(10, 5.2), facecolor=_MF_BG)
        ax  = fig.add_subplot(111)
        ax.set_facecolor(_MF_BG)
        ax.spines[:].set_color(_MF_BORDER)
        ax.tick_params(colors=_MF_TEXT, labelsize=8)
        ax.yaxis.label.set_color(_MF_SUBTEXT)
        ax.xaxis.label.set_color(_MF_SUBTEXT)

        import matplotlib.dates as mdates
        dt_dates = [_dt.datetime.combine(d, _dt.time()) for d in snap_dates]

        # Cost / invested line
        ax.step(dt_dates, snap_cost, where="post",
                color=_MF_SUBTEXT, linewidth=1.8, label="Invested Cost", linestyle="--")
        ax.fill_between(dt_dates, snap_cost, alpha=0.07, color=_MF_SUBTEXT, step="post")

        # Live market value dot at today
        if live_mv > 0 and snap_dates:
            today_dt = _dt.datetime.combine(max_d, _dt.time())
            pl_today  = live_mv - running_cost
            dot_color = _MF_GREEN if pl_today >= 0 else _MF_RED
            ax.plot([today_dt], [live_mv], "o", color=dot_color,
                    markersize=10, label=f"Live MV  {ccy0} {live_mv:,.2f}", zorder=5)
            ax.annotate(f"{ccy0} {live_mv:,.2f}\n({pl_today:+,.2f})",
                        xy=(today_dt, live_mv),
                        xytext=(-60, 18), textcoords="offset points",
                        color=dot_color, fontsize=8,
                        arrowprops=dict(arrowstyle="->", color=dot_color, lw=1))
            # Vertical P/L line from cost to live at today
            ax.vlines(today_dt, running_cost, live_mv,
                      colors=dot_color, linewidth=1.5, linestyles=":")

        ax.set_title(f"Portfolio History — {port['code']}  [{ccy0}]",
                     fontsize=11, color=_MF_TEXT, pad=14)
        ax.set_ylabel(f"Value ({ccy0})", fontsize=9)
        ax.xaxis.set_major_formatter(mdates.DateFormatter("%d %b"))
        ax.xaxis.set_major_locator(mdates.WeekdayLocator(interval=1))
        fig.autofmt_xdate(rotation=30)
        ax.legend(facecolor=_MF_PANEL, edgecolor=_MF_BORDER,
                  labelcolor=_MF_TEXT, fontsize=8)
        ax.grid(axis="y", color=_MF_BORDER, linewidth=0.5, alpha=0.5)

        # Annotate each snapshot with the cost value
        for d, v in zip(dt_dates, snap_cost):
            ax.plot(d, v, ".", color=_MF_ACCENT, markersize=4, zorder=4)

        fig.tight_layout()
        canvas = _FC(fig, master=self.hist_canvas_frame)
        canvas.draw()
    ...
    def _open_charts(self):
        ...
        if not code or code not in self.portfolios:
            from tkinter import messagebox as _mb
            _mb.showinfo("Charts", "No portfolio selected."); return
