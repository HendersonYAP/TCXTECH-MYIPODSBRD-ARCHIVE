...
to earn — incentivised clicking violates ad-network policy.</div>
    ...
    # MYR needs no FX leg (base currency); SGD/HKD go through FX_PAIRS.
    CRYPTO_CROSS_CCY = ['MYR', 'SGD', 'HKD']
    ...
    def _zdws_crypto_cross_level(self, crypto_sym: str, quote_ccy: str):
        """Cross-rate level for a crypto pair quoted in a non-USD currency,
        e.g. BTC/MYR = BTC-USD spot x USD/MYR. Formula per spec:
            BTC/USD x USD/MYR = 50,000.00 x 4.1050 = X
        For SGD/HKD (which are themselves quoted against MYR in FX_PAIRS,
        not against USD directly), this goes through MYR as the common leg:
...
class OpieChatWindow(tk.Toplevel):
    ...
    def _process(self, text: str):
        ...
        self._say('I can help with "change" (see your portfolio / a stock), '
                   ...
                   '"index" (current value of a market index), or '
    ...
    def _start_name_flow(self):
        ...
        if key in names and locked.get(key):
            ...
            self._say(f'Hi, {self._display_name}! What would you like to know — '
                       f'try "change", "how much", "stock price", "index", or "open" a PTS/ZDWS/ZDOS position.')
            return
