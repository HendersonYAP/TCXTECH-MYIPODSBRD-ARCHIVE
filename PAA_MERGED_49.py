...
class MYIPOApp(tk.Tk):
    ...
    def _zdws_world_level(self, symbol: str):
        """Latest level for a world index, cached 5 min."""
        key = f'widx_{symbol.replace("^", "")}'
        cached = cache_get_live(key, _TTL_LIVE)
        if cached:
            return float(cached)
        try:
    ...
    def _zdws_ccy(self, underlying: str) -> str:
        """Native currency of an underlying. Everything Malaysian is MYR."""
