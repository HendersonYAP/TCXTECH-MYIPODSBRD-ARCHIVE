    ...
    def _zdws_warm_batch_cache(self, symbols: list) -> int:
        ...
        for ci in range(0, len(symbols), CHUNK):
            ...
            try:
                ...
                log(f"   ⚠️  Batch warm chunk {ci // CHUNK + 1} failed: {e}")
                continue
            if data is None or len(data) == 0:
                log(f"   ⚠️  Batch warm chunk {ci // CHUNK + 1} returned empty")
                continue
            n_chunk = 0
            for sym in chunk:
                try:
                    ...
                    continue
            n_cached += n_chunk
            log(f"   Batch warm chunk {ci // CHUNK + 1}: {n_chunk}/{len(chunk)} cached")
        return n_cached
