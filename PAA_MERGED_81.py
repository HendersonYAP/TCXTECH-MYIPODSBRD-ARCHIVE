...
class MYIPOApp(tk.Tk):
    ...
    def _sor_run_and_popup(self):
        ...
        for sym, region, units in holdings_all:
            ...
            if mode == 'custom':
                ...
                # region and symbol (Lotto33 or not) — admin-set threshold.
    ...
    def _rewards_toggle_sor_mode(self):
        ...
        if pts.get('sor_custom_enabled'):
            ...
            cfg = self._sor_config()
            ok = messagebox.askyesno('SoR Mode',
                'Switch to Custom SoR mode?\n\n'
                f'Every saved position will instead earn '
                f'1 Cr per {cfg["custom_threshold"]:g} units held, regardless of '
                f'market or Lotto33 status, with no minimum-holding gate.\n\n'
                f'You can switch back to Default at any time.', parent=self)
            if not ok:
                return
            pts['sor_custom_enabled'] = True
            messagebox.showinfo('SoR Mode', 'Custom SoR mode enabled.', parent=self)
        self._pts_save(pts)
        self._build_rewards_tab()
...
to earn — incentivised clicking violates ad-network policy.</div>
    ...
    def _build_rewards_tab(self):
        ...
        tk.Label(left,
                 text=('Custom — using your own units-per-Cr rate.'
                       if custom_on else
                       'Default — automatic preset rates, active for any signed-in K-ID.'),
                 font=('Segoe UI', 8), fg=FG, bg=PANEL,
                 wraplength=260, justify='left').pack(anchor='w', pady=2)
