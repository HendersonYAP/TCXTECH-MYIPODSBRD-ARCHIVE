...
class MYIPOApp(tk.Tk):
    ...
    def _ensure_us_universe_warm(self):
        ...
        if getattr(self, '_us_universe_warm_running', False):
            return
        # Sentinel probe — two tickers from the batch; both cached ⇒ the
        # whole batch is still within TTL (they're written in one pass).
        if (self._zdws_cached_level_only('AAPL') is not None
                and self._zdws_cached_level_only('XLK') is not None):
            return
