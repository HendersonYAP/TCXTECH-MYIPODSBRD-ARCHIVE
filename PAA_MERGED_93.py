...
to earn — incentivised clicking violates ad-network policy.</div>
    ...
    # Opt-in feature: a user launches a personal "fund" tracking a real US
    # ticker (SP500 constituent, sector SPDR, MSCI, or crypto — whatever the
    # US universe already prices). Every fund is minted at a fixed NAV of
    # 1.0000 Cr/unit; buying in costs a minimum of 100 Cr (100.0000 units at
    # mint). Funds can only be opened/funded with credits (never real money),
    # and only after the K-ID has opted in as an SMS user. A fund auto-flags
    # for exit once its running P/L reaches ±1 Cr, but the user decides when
    # to actually close it. Opening is capped at SMS_DAILY_OPEN_LIMIT per day
    # per K-ID, mirroring the AD_DAILY_LIMIT / PTS daily-cap pattern already
    # used elsewhere in the wallet.
    # =========================================================================

    SMS_FUND_NAV_START   = 1.0000     # every fund mints at 1.0000 Cr/unit
    SMS_MIN_BUY_CR        = 100.0     # minimum credits to open/fund a position
    SMS_EXIT_PL_TRIGGER   = 1.0       # ± this many Cr of P/L flags "ready to exit"
    SMS_DAILY_OPEN_LIMIT  = 10        # max new funds opened per K-ID per day
    ...
    def _sms_prompt_opt_in(self, pts: dict) -> bool:
        ...
        ans = messagebox.askyesno(
            ...
            '• Every fund mints at a fixed NAV of 1.0000 Cr/unit\n'
            '• Funded only with credits — minimum 100 Cr to open\n'
            '• A fund is flagged for exit once its P/L moves ±1 Cr\n'
            f'• Limited to {self.SMS_DAILY_OPEN_LIMIT} new fund opens per day\n\n'
    ...
    def _sms_reset_daily_if_needed(self, pts: dict):
        import datetime as _dt
        today = _dt.date.today().isoformat()
        if pts.get('sms_opens_date') != today:
            pts['sms_opens_date']  = today
            pts['sms_opens_today'] = 0

    def _sms_opens_remaining(self, pts: dict) -> int:
        self._sms_reset_daily_if_needed(pts)
        return max(0, self.SMS_DAILY_OPEN_LIMIT - int(pts.get('sms_opens_today', 0)))
    ...
    def _sms_open_fund(self, pts: dict, ticker: str, name: str, stake_cr: float) -> str:
        """Open a new Strategy Fund position. Returns '' on success, else an
        error message. US-only, credits-only, min 100 Cr, daily-open capped."""
        if not self._sms_opt_in_status(pts):
            return 'You must opt in as an SMS (ShareMyStrategy) user first.'
        if self._sms_opens_remaining(pts) <= 0:
            return f'Daily limit reached — max {self.SMS_DAILY_OPEN_LIMIT} fund opens per day.'
        if stake_cr < self.SMS_MIN_BUY_CR:
            return f'Minimum buy-in is {self.SMS_MIN_BUY_CR:,.0f} Cr.'
        if stake_cr > pts.get('credits', 0):
            return 'Insufficient credits.'
        price = self._sms_current_price(ticker)
        if price is None:
            return f'No live/cached US price available for {ticker}.'
        import datetime as _dt
        units = stake_cr / self.SMS_FUND_NAV_START   # NAV mint is always 1.0000 Cr/unit
        pts['credits'] -= stake_cr
        pts.setdefault('sms_funds', []).append({
            'id':          len(pts.get('sms_funds', [])) + len(pts.get('sms_history', [])) + 1,
            'ticker':      ticker,
            'name':        name,
            'opened':      _dt.datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
            'entry_price': price,
            'stake_cr':    stake_cr,
            'units':       units,
            'nav':         self.SMS_FUND_NAV_START,
        })
        pts['sms_opens_today'] = int(pts.get('sms_opens_today', 0)) + 1
        self._credit_log_add(pts, -stake_cr, f'SCA-SMS fund opened: {ticker} ({stake_cr:,.0f} Cr)')
        self._pts_save(pts)
        return ''

    def _sms_fund_pl(self, fund: dict):
        """Current (price_now, nav_now, pl_cr) for an open fund, or
        (None, None, None) if no live price is available."""
        price_now = self._sms_current_price(fund['ticker'])
        if price_now is None or not fund.get('entry_price'):
            return None, None, None
        move_pct = (price_now - fund['entry_price']) / fund['entry_price']
        nav_now  = fund['nav'] * (1 + move_pct)
        pl_cr    = (nav_now - fund['nav']) * fund['units']
        return price_now, nav_now, pl_cr

    def _sms_ready_to_exit(self, fund: dict) -> bool:
        _, _, pl_cr = self._sms_fund_pl(fund)
        return pl_cr is not None and abs(pl_cr) >= self.SMS_EXIT_PL_TRIGGER

    def _sms_close_fund(self, pts: dict, fund_id) -> str:
        """Close (redeem) an open Strategy Fund position at its live NAV."""
        funds = pts.setdefault('sms_funds', [])
        fund  = next((f for f in funds if f['id'] == fund_id), None)
        if fund is None:
            return 'Fund not found.'
        price_now, nav_now, pl_cr = self._sms_fund_pl(fund)
        if price_now is None:
            return f'No live price for {fund["ticker"]} — try again later.'
        import datetime as _dt
        payout = fund['stake_cr'] + pl_cr
        pts['credits'] += payout
        fund['closed']      = _dt.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        fund['exit_price']  = price_now
        fund['exit_nav']    = nav_now
        fund['pl_cr']       = round(pl_cr, 4)
        fund['payout_cr']   = round(payout, 4)
        funds.remove(fund)
        pts.setdefault('sms_history', []).append(fund)
        self._credit_log_add(pts, payout,
            f'SCA-SMS fund closed: {fund["ticker"]} P/L {pl_cr:+.2f} Cr')
        self._pts_save(pts)
        return ''

    def _open_sms_window(self):
        """💹 SCA-SMS — Strategy Fund desk. US market only. Opt-in gated."""
        pts = self._pts_load()
...
class SMSStrategyFundWindow(tk.Toplevel):
    ...
    US market only. Every fund mints at NAV 1.0000 Cr/unit, minimum 100 Cr
    to open, flagged for exit at ±1 Cr P/L, capped at
    MYIPOApp.SMS_DAILY_OPEN_LIMIT new opens per day. Ticker universe is
    whatever the app's US price cache already carries (SP500 constituents,
    sector SPDRs, MSCI, crypto) — the same source ZDWS/ZDOS trade against.
    """
    ...
    def __init__(self, parent, pts: dict):
        ...
        # ── Open a new fund ──────────────────────────────────────────────
        open_row = tk.Frame(self, bg=self.PANEL); open_row.pack(fill='x', padx=14, pady=(0, 10))
        tk.Label(open_row, text='Ticker (US):', bg=self.PANEL, fg=self.FG,
                 font=('Courier New', 9)).grid(row=0, column=0, padx=6, pady=8, sticky='w')
        self.ticker_var = tk.StringVar()
        tk.Entry(open_row, textvariable=self.ticker_var, width=10,
                 font=('Courier New', 9)).grid(row=0, column=1, padx=4)
        tk.Label(open_row, text='Name (optional):', bg=self.PANEL, fg=self.FG,
                 font=('Courier New', 9)).grid(row=0, column=2, padx=6, sticky='w')
        self.name_var = tk.StringVar()
        tk.Entry(open_row, textvariable=self.name_var, width=16,
                 font=('Courier New', 9)).grid(row=0, column=3, padx=4)
        tk.Label(open_row, text='Stake (Cr):', bg=self.PANEL, fg=self.FG,
                 font=('Courier New', 9)).grid(row=0, column=4, padx=6, sticky='w')
        self.stake_var = tk.StringVar(value=str(int(parent.SMS_MIN_BUY_CR)))
        tk.Entry(open_row, textvariable=self.stake_var, width=8,
                 font=('Courier New', 9)).grid(row=0, column=5, padx=4)
        tk.Button(open_row, text='+ Open Fund', font=('Courier New', 9, 'bold'),
                  bg=self.ACCENT, fg='#0d1117', command=self._open_fund
                  ).grid(row=0, column=6, padx=10)

        # ── Open funds list ──────────────────────────────────────────────
        tk.Label(self, text='Open Funds', font=('Courier New', 10, 'bold'),
                 bg=self.BG, fg=self.FG).pack(anchor='w', padx=14)
        cols = ('ticker', 'name', 'stake', 'price', 'nav', 'pl', 'flag')
        self.tree = ttk.Treeview(self, columns=cols, show='headings', height=10)
        heads = {'ticker': 'Ticker', 'name': 'Name', 'stake': 'Stake (Cr)',
                 'price': 'Price now', 'nav': 'NAV now', 'pl': 'P/L (Cr)', 'flag': ''}
        widths = {'ticker': 70, 'name': 150, 'stake': 90, 'price': 90,
                  'nav': 90, 'pl': 90, 'flag': 90}
        for c in cols:
            self.tree.heading(c, text=heads[c])
            self.tree.column(c, width=widths[c], anchor='center')
        self.tree.pack(fill='both', expand=True, padx=14, pady=(4, 8))

        btn_row = tk.Frame(self, bg=self.BG); btn_row.pack(fill='x', padx=14, pady=(0, 12))
        tk.Button(btn_row, text='↻ Refresh', font=('Courier New', 9),
                  command=self._refresh).pack(side='left')
        tk.Button(btn_row, text='Close Selected Fund', font=('Courier New', 9, 'bold'),
                  bg=self.RED, fg='white', command=self._close_selected).pack(side='right')

        self.status_var = tk.StringVar()
        tk.Label(self, textvariable=self.status_var, font=('Courier New', 9),
                 bg=self.BG, fg=self.DIM, wraplength=690, justify='left'
    ...
    def _open_fund(self):
        ...
        name   = self.name_var.get().strip() or ticker
        if not ticker:
        ...
        err = self.parent._sms_open_fund(self.pts, ticker, name, stake)
        if err:
            self._set_status(f'⚠ {err}')
        else:
            self._set_status(f'✓ Opened {ticker} — {stake:,.0f} Cr.')
            self.ticker_var.set(''); self.name_var.set('')
    ...
    def _close_selected(self):
        ...
        if not sel:
            self._set_status('Select a fund to close first.'); return
        fund_id = int(sel[0])
        err = self.parent._sms_close_fund(self.pts, fund_id)
        if err:
            self._set_status(f'⚠ {err}')
        else:
            self._set_status(f'✓ Fund #{fund_id} closed and settled.')
        self._refresh()

    def _refresh(self):
        self.pts = self.parent._pts_load()
        self.credits_var.set(f'💰 {self.pts.get("credits", 0):,.2f} Cr')
        remaining = self.parent._sms_opens_remaining(self.pts)
        self.opens_var.set(f'Opens remaining today: {remaining}/{self.parent.SMS_DAILY_OPEN_LIMIT}')

        for row in self.tree.get_children():
            self.tree.delete(row)
        for fund in self.pts.get('sms_funds', []):
            price_now, nav_now, pl_cr = self.parent._sms_fund_pl(fund)
            ...
            self.tree.insert('', 'end', iid=str(fund['id']), values=(
                fund['ticker'], fund['name'], f'{fund["stake_cr"]:,.0f}',
...
_DEFAULT_SETTINGS = {
    ...
    "pts": {
        ...
        "sms_opt_in":       False,   # ShareMyStrategy opt-in flag
        "sms_funds":        [],      # open Strategy Fund positions (US-only)
        "sms_history":      [],      # closed/settled Strategy Fund positions
        "sms_opens_date":   "",      # 'YYYY-MM-DD' of the daily-open counter below
        "sms_opens_today":  0,       # fund opens used today (max SMS_DAILY_OPEN_LIMIT)
