...
class AppLockScreen(tk.Tk):
    ...
    def _build_ui(self):
        ...
        if accounts and not creating:
            names = [a['username'] for a in accounts] + [ADMIN_KID]
            u_cb = ttk.Combobox(center, textvariable=self._user_var, values=names,
                                font=("Segoe UI", 12), width=18, state='normal')
            u_cb.pack(pady=(2, 8), ipady=3)
            # Only fill an EMPTY field with a default — never stomp what the
            # user typed/selected (the old reset silently replaced anything
            # not in the accounts list, which made typing admin1 impossible).
