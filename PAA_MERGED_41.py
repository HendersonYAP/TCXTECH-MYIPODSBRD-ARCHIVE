...
class MYIPOApp(tk.Tk):
    ...
    def _pts_settlement_dates(self):
    ...
    def _build_zdws_tab(self):
        ...
        tk.Label(hdr, text='  Zero-Decay Warrant System  ·  Θ = 0  ·  MYIPO universe only',
        ...
        if not unders:
            tk.Label(left, text='No MYIPO indices available yet.\nLoad data first.',
        ...
        u_labels = [f'{disp}  —  {lvl:,.2f}' for _, disp, lvl in unders]
        u_var = tk.StringVar()
        u_cb  = ttk.Combobox(left, textvariable=u_var, values=u_labels,
        ...
        settle_pairs  = self._pts_settlement_dates()
        ...
        def _refresh_ctx(*_):
            i = u_cb.current()
            if 0 <= i < len(unders):
                key, disp, lvl = unders[i]
                ctx_var.set(f'Entry P₀ = {lvl:,.2f}   ·   Θ = 0 (no time decay)\n'
