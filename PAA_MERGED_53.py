    ...
    def _draw_pie(self):
        ...
        for pk, port in (self._portfolios or {}).items():
            ...
            for tkr, item in port.get('holdings', {}).items():
                ...
                if mkt == 'US':
                    if not fx:
                        continue        # no FX -> can't mix into a MYR pie
                    val *= fx
                rows.append((tkr.replace('.KL', ''), val))
        ...
        ccy_note = ('MYR' if tab == 'MY' else
                    'USD→MYR' if tab == 'US' else 'MYR (USD converted)')
