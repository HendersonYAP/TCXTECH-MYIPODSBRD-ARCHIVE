...
_TOP10_HISTORY   = []   # [(rebalance record dicts)] — populated by build_indices
    ...
    try:
        ...
        if len(top10_universe) < 11:
            ...
            if not rebalance_dates:
                ...
                index_level      = 100.0
                top10_hist       = []   # [(rb_date, [syms])] — for in/out board
                prev_members     = set()
                ...
                for rb_idx, rb_date in enumerate(rebalance_dates):
                    ...
                    # Record in/out changes for the historical board
                    cur_members = set(syms10)
                    ins  = sorted(cur_members - prev_members)
                    outs = sorted(prev_members - cur_members)
                    top10_hist.append({
                        'date':    rb_date,
                        'members': sorted(cur_members),
                        'in':      ins,
                        'out':     outs,
                    })
                ...
                if not index_values.empty:
                    ...
                    # Store constituent in/out history for the historical board popup
                    global _TOP10_HISTORY
                    _TOP10_HISTORY = top10_hist
...
class MYIPOApp(tk.Tk):
    ...
    def _open_top10_board(self):
        """Historical in/out board for MYIPO+ Top 10."""
        win = tk.Toplevel(self)
        win.title('MYIPO+ Top 10 — Historical In/Out Board')
        win.configure(bg='#0d1117')
        win.geometry('760x640')
        win.transient(self)

        tk.Label(win, text='📋  MYIPO+ TOP 10 — REBALANCE HISTORY',
                 font=('Segoe UI', 12, 'bold'), fg='#D29922',
                 bg='#0d1117').pack(pady=(14, 2))
        tk.Label(win, text='Quarterly rebalance · Score = Quant1 × price · Top 10 by score',
        ...
        canvas.create_window((0, 0), window=body, anchor='nw', width=710)
        body.bind('<Configure>', lambda e: canvas.configure(scrollregion=canvas.bbox('all')))

        hist = list(reversed(_TOP10_HISTORY))
        if not hist:
            tk.Label(body, text='No rebalance history available.\n'
                                'Top 10 index needs 11+ MYIPO+ IPOs to activate.',
        ...
        for i, rec in enumerate(hist):
            ...
            if rec['in']:
                in_names = ', '.join(f"{s.replace('.KL','')} ({name_map.get(s, '')[:14]})"
                                     for s in rec['in'])
            ...
            if rec['out']:
                out_names = ', '.join(f"{s.replace('.KL','')} ({name_map.get(s, '')[:14]})"
                                      for s in rec['out'])
                tk.Label(card, text=f'➖ OUT: {out_names}',
                         font=('Segoe UI', 8), fg='#F85149', bg='#161b22',
                         wraplength=650, justify='left', anchor='w').pack(fill='x', pady=(2, 0))

            mem_str = ', '.join(s.replace('.KL', '') for s in rec['members'])
