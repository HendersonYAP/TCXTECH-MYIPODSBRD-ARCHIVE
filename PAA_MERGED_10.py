...
class MYIPOApp(tk.Tk):
    ...
    def _build_line_tab(self):
        ...
        # value, for every currently-plotted series. Click-based rather than
        # hover-based since motion events can be unreliably intercepted by
        # NavigationToolbar2Tk's pan/zoom bindings on some platforms — a
        # click is unambiguous and the box stays visible until the next
        # click or redraw, so it's easy to verify it's actually working.
    ...
    def _on_line_click(self, event):
        # Ignore clicks that are really toolbar pan/zoom drags (button!=1) or
        # land outside both axes entirely.
        if event.button != 1:
            return
        ...
        try:
            hover_dt = _mdates.num2date(event.xdata)
