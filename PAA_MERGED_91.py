...
class OpieChatWindow(tk.Toplevel):
    ...
    def _answer_stock(self, text: str):
        ...
        for port in portfolios.values():
            ccy = port.get('base_currency', 'USD')
            for ticker, item in port.get('holdings', {}).items():
                if item.get('_is_cash') or query not in ticker.upper():
                    continue
                shares = item.get('shares', 0)
                if not shares or shares <= 0:
                    continue
                ...
                return
        self._say(f'Couldn\'t find "{text}" in your portfolio — try one of the tickers above.')

    def _current_price(self, ticker: str):
        """Live price from whatever's already loaded, falling back to the
        on-disk price cache so this works even for tickers not held."""
        prices = _opie_extract_prices(self.host)
        p = prices.get(ticker)
        if p:
            return p
    ...
    def _handle_price(self, text: str):
        ...
        skip  = {'STOCK', 'STOCKS', 'PRICE', 'PRICES', 'WHAT', 'WHATS', "WHAT'S",
                  'IS', 'THE', 'OF', 'FOR', 'CURRENT', 'ME', 'TELL', 'CAN', 'YOU',
                  'PLEASE', 'A', 'AN'} | _OPIE_TIME_WORDS
        candidates = [w for w in words if w.isalnum() and w not in skip and not w.isdigit()]
        if candidates:
            self._answer_price(candidates[-1])
            return
        portfolios = _opie_extract_portfolios(self.host)
    ...
    def _answer_price(self, text: str):
        self._awaiting_price = False
        query = text.strip().upper()
        price = self._current_price(query)
        if price is None:
            self._say(f'No current price on record for "{text}" — check the ticker and try again.')
            return
        self._say(f'{query} is currently trading at {price:,.4f}.')
