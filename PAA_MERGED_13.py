...
# CORE ENGINE
# =============================================================================
def build_indices(input_file, log_cb=None, prefer_online=True, apply_sell_fee=True):
    def log(msg):
...
class MYFOLIOWindow(tk.Toplevel):
    ...
    def _draw_line_chart(self, port: dict):
        ...
        # ── Current Value = units currently held x CURRENT closing price ─────
        # (Only live/current prices are available — no historical OHLC series
        # is tracked per ticker — so "Current" reflects today's closing price
        # applied to today's unit holdings, not a simulated historical curve.)
        total_units_now = 0.0
        current_value = 0.0
        unit_breakdown = []   # (ticker, units) for the annotation
        for ticker, shares in shares_s.items():
            ...
            total_units_now += shares
            unit_breakdown.append((ticker, shares))
        unit_breakdown.sort(key=lambda x: -x[1])

        import matplotlib.dates as mdates
        if snap_dates:
            dt_dates = [_dt.datetime.combine(d, _dt.time()) for d in snap_dates]
            ax.step(dt_dates, snap_cost, where='post', color='#8B949E',
                    linewidth=1.8, label=f'Total Cost (TC)', linestyle='--')
        ...
        if current_value > 0:
            ...
            pl_today  = current_value - running_cost
            pl_pct    = (pl_today / running_cost * 100) if running_cost else 0.0
            dot_color = '#3FB950' if pl_today >= 0 else '#F85149'
            ax.plot([today_dt], [current_value], 'o', color=dot_color, markersize=11,
                    label=f'Current Value  {ccy0} {current_value:,.2f}', zorder=5)
            # Units held now is shown directly in the annotation for quick
            # reference, alongside the value and P/L — no need to switch tabs.
            ax.annotate(
                f'TC: {ccy0} {running_cost:,.2f}\n'
            ...
            if snap_dates:
                ax.vlines(today_dt, running_cost, current_value,
...
class SplashScreen(tk.Tk):
    ...
    def _start_load(self):
        ...
        # Reset the determinate fill bar for this run. The estimate covers
        # roughly: price-data chunks + 5 base indices + 7 Y-Series + 5 TR-Series
        # + 11 funds + Top-10 + misc build/✅ events seen in a typical run.
