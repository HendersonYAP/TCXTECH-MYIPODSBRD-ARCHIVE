...
to earn — incentivised clicking violates ad-network policy.</div>
    ...
    # SCA is the source of truth: a "fund" is not a separate ledger — it's
    # any transaction row in the user's own SCA/NEWPOR ledger tagged with
    # Portfolio = "SMS" (Col H on Google Sheets; the same column name on an
    # offline CSV). Trading in/out happens the normal SCA way — a Buy/Sell
    # row with that tag — so this module reads the ledger and treats the
    # net position per asset as the fund. US market only (currency must be
    # USD). Every fund is valued at NAV 1.0000 Cr/unit at first buy (the Cr
    # stake IS the ledger amount for SMS-tagged rows); minimum 100 Cr to
    # open a new asset; a fund is flagged for exit once running P/L reaches
    # ±1 Cr; opening NEW assets under the SMS tag is capped at
    # SMS_DAILY_OPEN_LIMIT per K-ID per day. Opt-in gated.
    # =========================================================================

    SMS_TAG               = 'SMS'     # Col H / "Portfolio" value that marks a row as SMS
    SMS_FUND_NAV_START    = 1.0000    # every fund mints at 1.0000 Cr/unit
    SMS_MIN_BUY_CR         = 100.0    # minimum stake to open a NEW asset under SMS
    SMS_EXIT_PL_TRIGGER    = 1.0      # ± this many Cr of P/L flags "ready to exit"
    SMS_DAILY_OPEN_LIMIT   = 10       # max NEW SMS assets opened per K-ID per day
    ...
    def _sms_prompt_opt_in(self, pts: dict) -> bool:
        ...
        ans = messagebox.askyesno(
            ...
            '• Reads straight from your SCA transaction ledger — a row is\n'
            f'  part of the fund when its Portfolio column (Col H) = "{self.SMS_TAG}"\n'
            '• US market only (USD rows)\n'
            '• Every fund is valued at NAV 1.0000 Cr/unit, funded with credits\n'
            f'  — minimum {self.SMS_MIN_BUY_CR:,.0f} Cr to open a new asset\n'
            '• A position is flagged for exit once its P/L moves ±1 Cr\n'
            f'• Limited to {self.SMS_DAILY_OPEN_LIMIT} new SMS assets opened per day\n\n'
    ...
    def _sms_positions_from_ledger(self, rows: list):
        """Build live SMS fund positions from SCA ledger rows tagged
        Portfolio == SMS_TAG. Returns (positions, warnings).

        positions: {asset: {ticker, ccy, units, stake_cr, avg_cost, opened_date}}
        Moving-average costing: each Buy adds units/stake at trade price;
        each Sell removes units at the current average cost (P/L on the
        sold slice isn't tracked here — only the still-open position is,
        matching every other "live P/L" view in the app).
        """
        pos, warnings = {}, []
        rows_sorted = sorted(rows, key=lambda r: r.get(_NC_DATE, ''))
        for r in rows_sorted:
            tag = (r.get(_NC_PORTFOLIO, '') or '').strip().upper()
            if tag != self.SMS_TAG:
                continue
            asset = (r.get(_NC_ASSET, '') or '').strip().upper()
            ccy   = (r.get(_NC_CCY, '') or '').strip().upper()
            ttype = (r.get(_NC_TYPE, '') or '').strip().lower()
            if not asset or ttype not in ('buy', 'sell'):
                continue
            if ccy != 'USD':
                warnings.append(f'{asset}: SMS is US-market only — {ccy} row ignored.')
                continue
            unit   = _mf_strip_currency(r.get(_NC_UNIT, '0'))
            amount = abs(_mf_strip_currency(r.get(_NC_AMOUNT, '0')))
            date_s = r.get(_NC_DATE, '')

            p = pos.get(asset)
            if ttype == 'buy':
                if p is None:
                    if amount < self.SMS_MIN_BUY_CR:
                        warnings.append(
                            f'{asset}: opening buy of {amount:,.2f} Cr is below the '
                            f'{self.SMS_MIN_BUY_CR:,.0f} Cr minimum — flagged, not blocked.')
                    p = pos[asset] = {'ticker': asset, 'ccy': ccy, 'units': 0.0,
                                       'stake_cr': 0.0, 'opened_date': date_s}
                p['units']    += unit
                p['stake_cr'] += amount
            else:  # sell
                if p is None or p['units'] <= 0:
                    warnings.append(f'{asset}: SELL with no open SMS position — ignored.')
                    continue
                frac = min(1.0, unit / p['units']) if p['units'] else 0
                p['stake_cr'] = max(0.0, p['stake_cr'] * (1 - frac))
                p['units']    = max(0.0, p['units'] - unit)

        positions = {a: p for a, p in pos.items() if p['units'] > 1e-9}
        for a, p in positions.items():
            p['avg_cost'] = (p['stake_cr'] / p['units']) if p['units'] else 0.0
        return positions, warnings

    def _sms_fund_pl(self, position: dict):
        """Current (price_now, nav_now, pl_cr) for a live SMS position built
        from the ledger, priced off avg_cost as the entry NAV reference."""
        ...
        pl_cr    = (nav_now - self.SMS_FUND_NAV_START) * (position['stake_cr'] / self.SMS_FUND_NAV_START)
    ...
    def _sms_opens_today(self, positions: dict, all_ledger_rows: list) -> int:
        """Count NEW SMS assets whose first Buy under the SMS tag lands
        today, across the full (including now-closed) ledger history."""
        import datetime as _dt
        today = _dt.date.today().isoformat()
        first_buy_date = {}
        for r in sorted(all_ledger_rows, key=lambda r: r.get(_NC_DATE, '')):
            tag = (r.get(_NC_PORTFOLIO, '') or '').strip().upper()
            if tag != self.SMS_TAG or (r.get(_NC_TYPE, '') or '').strip().lower() != 'buy':
                continue
            asset = (r.get(_NC_ASSET, '') or '').strip().upper()
            first_buy_date.setdefault(asset, r.get(_NC_DATE, ''))
        return sum(1 for d in first_buy_date.values() if d.startswith(today))

    def _sms_record_trade(self, ticker: str, ttype: str, units: float, stake_cr: float) -> str:
        """Record a Buy/Sell into the K-ID's SCA ledger tagged Portfolio=SMS,
        via the app's existing local-transaction overlay — the same
        mechanism the Road-to-Reward / LottoStock buys already use to write
        into SCA. This is how a user 'trades in/out' of a Strategy Fund.
        Returns '' on success, else an error message."""
        if not self._sms_opt_in_status(self._pts_load()):
            return 'You must opt in as an SMS (ShareMyStrategy) user first.'
        ticker = (ticker or '').strip().upper()
        if not ticker:
            return 'Enter a US ticker.'
        if ttype not in ('buy', 'sell'):
            return 'Trade type must be Buy or Sell.'
        if units <= 0 or stake_cr <= 0:
            return 'Units and stake must be positive.'
        price = self._sms_current_price(ticker)
        if price is None:
            return f'No live/cached US price available for {ticker}.'
        rows, err = self._sms_load_ledger_rows()
        if err:
            return f'Could not read SCA ledger: {err}'
        positions, _ = self._sms_positions_from_ledger(rows)
        if ttype == 'buy' and ticker not in positions:
            if self._sms_opens_today(positions, rows) >= self.SMS_DAILY_OPEN_LIMIT:
                return f'Daily limit reached — max {self.SMS_DAILY_OPEN_LIMIT} new SMS assets per day.'
            if stake_cr < self.SMS_MIN_BUY_CR:
                return f'Minimum buy-in for a new SMS asset is {self.SMS_MIN_BUY_CR:,.0f} Cr.'
        pts = self._pts_load()
        if ttype == 'buy' and stake_cr > pts.get('credits', 0):
            return 'Insufficient credits.'
        import datetime as _dt
        row = {
            _NC_DATE:      _dt.date.today().isoformat(),
            _NC_TYPE:      'Buy' if ttype == 'buy' else 'Sell',
            _NC_ASSET:     ticker,
            _NC_CCY:       'USD',
            _NC_UNIT:      f'{units:.4f}',
            _NC_VALUE:     f'{price:.4f}',
            _NC_AMOUNT:    f'{stake_cr:.4f}',
            _NC_PORTFOLIO: self.SMS_TAG,
        }
        if not sca_record_local_transaction(row):
            return 'Could not write to your SCA ledger (are you signed in?).'
        if ttype == 'buy':
            pts['credits'] -= stake_cr
            self._credit_log_add(pts, -stake_cr, f'SCA-SMS buy: {ticker} ({stake_cr:,.0f} Cr)')
        else:
            pts['credits'] += stake_cr
            self._credit_log_add(pts, stake_cr, f'SCA-SMS sell: {ticker} ({stake_cr:,.0f} Cr)')
        self._pts_save(pts)
        return ''

    def _open_sms_window(self):
        """💹 SCA-SMS — Strategy Fund desk. Reads SCA ledger rows tagged
        Portfolio=SMS. US market only. Opt-in gated."""
        pts = self._pts_load()
...
class SMSStrategyFundWindow(tk.Toplevel):
    ...
    SCA is the key: this window does not keep its own fund list — it reads
    the signed-in K-ID's SCA/NEWPOR ledger and treats every row tagged
    Portfolio = "SMS" (Col H on Google Sheets, or a "Portfolio" column on
    an offline CSV) as part of the fund. Trading in/out is a normal SCA
    Buy/Sell; the form below writes exactly that kind of row (via the same
    local-transaction overlay other in-app buys already use), so the fund
    stays 100% reconstructable from the SCA database alone.
    US market only. NAV mint 1.0000 Cr/unit. Min 100 Cr to open a new
    asset. Flagged for exit at ±1 Cr P/L. Max 10 new SMS assets/day.
    """
    ...
    def __init__(self, parent, pts: dict):
        ...
        tk.Label(hdr, textvariable=self.credits_var, font=('Courier New', 11, 'bold'),
                 bg=self.BG, fg=self.GREEN).pack(side='right')

        info = (f'Reads SCA ledger rows tagged Portfolio="{parent.SMS_TAG}" (Col H)  ·  '
                f'US market only  ·  NAV mint 1.0000 Cr/unit  ·  '
                f'min buy {parent.SMS_MIN_BUY_CR:,.0f} Cr  ·  '
                f'exit flag at ±{parent.SMS_EXIT_PL_TRIGGER:.0f} Cr P/L  ·  '
                f'{parent.SMS_DAILY_OPEN_LIMIT} new assets/day')
        tk.Label(self, text=info, font=('Courier New', 8), bg=self.BG, fg=self.DIM,
                  wraplength=730, justify='left').pack(fill='x', padx=14)

        self.opens_var = tk.StringVar()
        tk.Label(self, textvariable=self.opens_var, font=('Courier New', 9, 'bold'),
                 bg=self.BG, fg=self.ACCENT).pack(anchor='w', padx=14, pady=(4, 8))

        # ── Trade in/out (writes a Buy/Sell row into SCA, tagged SMS) ────
        trade_row = tk.Frame(self, bg=self.PANEL); trade_row.pack(fill='x', padx=14, pady=(0, 10))
        tk.Label(trade_row, text='Ticker (US):', bg=self.PANEL, fg=self.FG,
                 font=('Courier New', 9)).grid(row=0, column=0, padx=6, pady=8, sticky='w')
        self.ticker_var = tk.StringVar()
        tk.Entry(trade_row, textvariable=self.ticker_var, width=10,
                 font=('Courier New', 9)).grid(row=0, column=1, padx=4)
        self.side_var = tk.StringVar(value='Buy')
        tk.OptionMenu(trade_row, self.side_var, 'Buy', 'Sell').grid(row=0, column=2, padx=6)
        tk.Label(trade_row, text='Stake (Cr):', bg=self.PANEL, fg=self.FG,
                 font=('Courier New', 9)).grid(row=0, column=3, padx=6, sticky='w')
        self.stake_var = tk.StringVar(value=str(int(parent.SMS_MIN_BUY_CR)))
        tk.Entry(trade_row, textvariable=self.stake_var, width=8,
                 font=('Courier New', 9)).grid(row=0, column=4, padx=4)
        tk.Button(trade_row, text='Submit Trade → SCA', font=('Courier New', 9, 'bold'),
                  bg=self.ACCENT, fg='#0d1117', command=self._submit_trade
                  ).grid(row=0, column=5, padx=10)

        # ── Live SMS positions (derived from SCA) ─────────────────────────
        tk.Label(self, text='Open SMS Positions (from SCA)', font=('Courier New', 10, 'bold'),
                 bg=self.BG, fg=self.FG).pack(anchor='w', padx=14)
        cols = ('ticker', 'units', 'stake', 'price', 'nav', 'pl', 'flag')
        self.tree = ttk.Treeview(self, columns=cols, show='headings', height=9)
        heads = {'ticker': 'Ticker', 'units': 'Units', 'stake': 'Stake (Cr)',
                 'price': 'Price now', 'nav': 'NAV now', 'pl': 'P/L (Cr)', 'flag': ''}
        widths = {'ticker': 70, 'units': 90, 'stake': 90, 'price': 90,
                  'nav': 90, 'pl': 90, 'flag': 100}
        for c in cols:
    ...
    def _submit_trade(self):
        ...
        if not ticker:
            self._set_status('Enter a US ticker first.'); return
        try:
            stake = float(self.stake_var.get())
        except ValueError:
            self._set_status('Stake must be a number.'); return
        price = self.parent._sms_current_price(ticker)
        units = (stake / price) if price else 0
        err = self.parent._sms_record_trade(ticker, side, units, stake)
        if err:
            self._set_status(f'⚠ {err}')
        else:
            self._set_status(f'✓ {side.title()} {ticker} recorded to SCA — {stake:,.0f} Cr.')
            self.ticker_var.set('')
            self.stake_var.set(str(int(self.parent.SMS_MIN_BUY_CR)))
        self._refresh()

    def _refresh(self):
        self.pts = self.parent._pts_load()
        self.credits_var.set(f'💰 {self.pts.get("credits", 0):,.2f} Cr')
        ...
        positions, warnings = self.parent._sms_positions_from_ledger(rows)
        opens_today = self.parent._sms_opens_today(positions, rows)
        remaining = max(0, self.parent.SMS_DAILY_OPEN_LIMIT - opens_today)
        self.opens_var.set(
            f'New SMS assets opened today: {opens_today}/{self.parent.SMS_DAILY_OPEN_LIMIT} '
        ...
        for ticker, p in sorted(positions.items()):
            ...
            self.tree.insert('', 'end', iid=ticker, values=(
                ticker, f'{p["units"]:,.4f}', f'{p["stake_cr"]:,.0f}',
                price_s, nav_s, pl_s, flag))
...
class OpieChatWindow(tk.Toplevel):
    ...
    def _process(self, text: str):
        ...
        self._say('I can help with "change" (see your portfolio / a stock), '
                   ...
                   '"stock price" (current price of a stock), or '
    ...
    def _start_name_flow(self):
        ...
        if key in names and locked.get(key):
            ...
            self._say(f'Hi, {self._display_name}! What would you like to know — '
                       f'try "change", "how much", "stock price", or "index".')
            return
