    ...
    for fund_label, base_label, board_filter, use_d365 in FUND_CONFIG:
        ...
        try:
            if base_label in all_frames and base_label in index_holdings_val:
                ...
                # The fund pays real quarterly cash distributions (dividend
                # pass-through), which permanently reduce NAV. Comparing
                # NAV-only return against a price-only index (which has zero
                # dividend concept) always understates the fund's actual
                # performance by roughly however much was distributed. Total
                # Return = NAV + cumulative distributions paid per unit —
                # the standard convention (matches how the TR-Series already
                # treats dividends for the indices), giving a fair
                ...
                total_return_nav = nav_full + cum_dist_full
