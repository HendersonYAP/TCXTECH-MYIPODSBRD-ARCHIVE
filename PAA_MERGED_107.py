...
# OPIE (local decision-support layer) is temporarily disabled — module removed.
# from opie import OPIEAgent

# Optional WebView backends for the in-app ad area (both degrade gracefully):
#   tkinterweb — pure-Python HTML frame embeddable INSIDE a Tkinter layout
#                (limited JS support, fine for static/sponsored HTML)
#   pywebview  — real native browser window (Edge WebView2 on Windows) for
#                the pop-up ad page; falls back to the system browser if absent
try:
    from tkinterweb import HtmlFrame as _AdHtmlFrame   # pip install tkinterweb
    _HAS_TKINTERWEB = True
except Exception:
    _AdHtmlFrame, _HAS_TKINTERWEB = None, False
try:
    import webview as _pywebview                        # pip install pywebview
    _HAS_PYWEBVIEW = True
except Exception:
    _pywebview, _HAS_PYWEBVIEW = None, False
...
_CACHE_ETF_DIR    = os.path.join(_CACHE_DIR, 'etf_holdings')   # alpha.102 — ETF Top Holdings Intelligence

_TTL_PRICE  = 86_400        # 1 day
_TTL_DIV    = 7 * 86_400    # 7 days
_TTL_LIVE   = 300           # 5 min
_TTL_EPS    = 86_400        # 24 hr
_TTL_ETF    = 86_400        # 24 hr — ETF top-10 holdings / sector weights change slowly

for _d in (_CACHE_DIR, _CACHE_PRICES_DIR, _CACHE_DIVS_DIR, _CACHE_LIVE_DIR, _CACHE_ETF_DIR):
...
def _etfjson(ticker: str) -> str:
    safe = ''.join(ch if (ch.isalnum() or ch in '_-.') else '_' for ch in ticker.upper())
    return os.path.join(_CACHE_ETF_DIR, f'{safe}.json')

def cache_get_etf_holdings(ticker: str, ttl: int = _TTL_ETF) -> dict:
    """Return cached {'holdings': [...], 'sector_weightings': {...}} for an
    ETF ticker, or {} if stale/missing (mirrors the other live/-cache getters)."""
    p = _etfjson(ticker)
    if not _cache_fresh(p, ttl): return {}
    return _cache_rjson(p)

def cache_set_etf_holdings(ticker: str, data: dict):
    _cache_wjson(_etfjson(ticker), data)
...
def _smart_chunk_size(n_total: int, max_single_chunk: int = 100, min_chunk: int = 20,
                       target_chunks: int = 6) -> int:
    """Picks a bulk-download chunk size that scales with how many tickers
    actually need fetching, instead of every call site hardcoding its own
    one-size-fits-all constant:
      - A small batch (<= max_single_chunk) goes out as a single chunk —
        no point splitting a download that's already small.
      - A larger batch is split to aim for roughly `target_chunks` parallel
        requests, clamped between min_chunk and max_single_chunk so any one
        yfinance call stays within a size Yahoo's rate limiter tolerates.
    Every splash-time / bulk price fetch in the app calls this instead of
    picking its own constant, so chunk sizing stays consistent and each
    site only needs to say what its own safe ceiling/floor is.
    """
    if n_total <= 0:
        return 1
    if n_total <= max_single_chunk:
        return n_total
    size = -(-n_total // target_chunks)   # ceil division
    return max(min_chunk, min(max_single_chunk, size))
...
VFUND_CONFIG = [
    ...
    {
        'label':     'MSCI-EW GEA Fund',
        'ccy':       'USD',
        'par':       1.0000,
        'suffix':    '',
        # Great Eastern Asia — China / Korea / Japan / Hong Kong
        'members':   ['MCHI', 'EWY', 'EWJ', 'EWH'],
        'note':      'Equal-weight Greater East Asia MSCI basket (China/Korea/Japan/HK)',
    },
    {
        'label':     'MYIPO+ YieldMax FoF Fund',
        'ccy':       'USD',
        'par':       1.0000,
        'suffix':    '',
        'units':     10_000,   # $10,000 raised at launch, not the usual 1,000-unit default
        # Fund-of-funds — equal-weight across the full YieldMax single-stock/
        # theme covered-call ETF lineup. Bought all equally, per spec.
        'members':   ['AIYY', 'AMDY', 'AMZY', 'APLY', 'BABO', 'BRKC', 'CONY',
                      'CRCO', 'CRSH', 'CVNY', 'DIPS', 'DRAY', 'FBY', 'FIAT',
                      'GMEY', 'GOOY', 'HIYY', 'HOOY', 'INYY', 'JPO', 'MARO',
                      'MRNY', 'MSFO', 'MSTY', 'NFLY', 'NVDY', 'OARK', 'PLTY',
                      'PYPY', 'RBLY', 'RDYY', 'SMCY', 'SNOY', 'TSLY', 'TSMY',
                      'WNTR', 'XOMO', 'XYZY', 'YBIT', 'YSPC'],
        'note':      'YieldMax FoF — equal-weight across the full lineup, $10,000 @ $1.0000/unit',
    },
]

# ── MYIPO+ EY SERIES (Enhanced Yield) — distribution-class CEFs, each
# pairing a plain index/bond ETF with its "wrapper" income/covered-call
# twin (TLTW, HYGW, LQDW, QQQI, SPYI, IWMI…). Same closed-end mechanics as
# VFUND_CONFIG (fixed shares from launch, no rebalance) EXCEPT the split
# between the two legs is a fixed 50:50 CovCall/Underlying (per spec),
# driven by the 'weights' dict below instead of the generic 1/N split.
#
# ── DISTRIBUTION RULES (quarterly, at least) ────────────────────────────
# Ratio = CovCall ETF Distribution Amount / underlying ETF Distribution
# Amount, e.g. TLTW DistAmt / TLT DistAmt — the same wrapper/underlying
# yield ratio the weight split above is meant to track.
#   Ratio > 5.00x  -> payout 67% (of the fund's distributable amount)
#   Ratio < 2.00x  -> payout 33%
# The 2.00x–5.00x band isn't specified — EY_RATIO_PAYOUT_ANCHORS below
# interpolates linearly between the two given anchor points as a documented
# default rather than guessing a third rule; adjust if there's an intended
# step function instead.
#
# NOT WIRED to a live quarterly distribution run yet — that needs actual
# CovCall/underlying distribution-amount data pulled in (the divs/<TICKER>
# cache already in this file covers dividends, but a CovCall ETF's
# distribution amount isn't the same series as a plain dividend and isn't
# fetched anywhere currently). Fabricating a "distribution ran" event off
# invented numbers would show fake payouts as real ones, so this stops at
# the formula — _ey_distribution_payout_pct() below is ready to call once
# that data source exists.
EY_RATIO_PAYOUT_ANCHORS = [(2.00, 0.33), (5.00, 0.67)]   # (ratio, payout_pct)


def _ey_distribution_payout_pct(ratio: float):
    """MYIPO+ EY Series quarterly distribution rule. ratio = CovCall ETF
    DistAmt / underlying ETF DistAmt. Returns the fraction of the fund's
    distributable amount to pay out this quarter, or None if ratio is
    unavailable."""
    if ratio is None:
        return None
    lo_r, lo_p = EY_RATIO_PAYOUT_ANCHORS[0]
    hi_r, hi_p = EY_RATIO_PAYOUT_ANCHORS[-1]
    if ratio <= lo_r:
        return lo_p
    if ratio >= hi_r:
        return hi_p
    frac = (ratio - lo_r) / (hi_r - lo_r)
    return lo_p + frac * (hi_p - lo_p)


ENHANCED_YIELD_CONFIG = [
    {
        'label':   'MYIPO+ EY Treasury Fund',
        'ccy':     'USD', 'par': 1.0000, 'suffix': '',
        'members': ['TLTW', 'TLT'],
        'weights': {'TLTW': 0.50, 'TLT': 0.50},
        'note':    'TLTW+TLT, 50:50 CovCall/Underlying',
    },
    {
        'label':   'MYIPO+ EY High Yield Fund',
        'ccy':     'USD', 'par': 1.0000, 'suffix': '',
        'members': ['HYGW', 'HYG'],
        'weights': {'HYGW': 0.50, 'HYG': 0.50},
        'note':    'HYGW+HYG, 50:50 CovCall/Underlying',
    },
    {
        'label':   'MYIPO+ EY Investment Grade Fund',
        'ccy':     'USD', 'par': 1.0000, 'suffix': '',
        'members': ['LQDW', 'LQD'],
        'weights': {'LQDW': 0.50, 'LQD': 0.50},
        'note':    'LQDW+LQD, 50:50 CovCall/Underlying',
    },
    {
        'label':   'MYIPO+ EY Mixed Fund',
        'ccy':     'USD', 'par': 1.0000, 'suffix': '',
        'members': ['TLTW', 'TLT', 'HYGW', 'HYG', 'LQDW', 'LQD'],
        # Three sleeves (Treasury / High-Yield / IG) at 1/3 capital each;
        # each sleeve is 50:50 CovCall/Underlying internally -> 1/6 per leg.
        'weights': {'TLTW': 0.1667, 'TLT': 0.1667,
                    'HYGW': 0.1667, 'HYG': 0.1667,
                    'LQDW': 0.1666, 'LQD': 0.1666},
        'note':    'Mixed sleeve: 1/3 each of Treasury/HY/IG, 50:50 CovCall/Underlying',
    },
    {
        'label':   'MYIPO+ EY Nasdaq 100 Fund',
        'ccy':     'USD', 'par': 1.0000, 'suffix': '',
        'members': ['QQQI', 'QQQM'],
        'weights': {'QQQI': 0.50, 'QQQM': 0.50},
        'note':    'QQQI+QQQM, 50:50 CovCall/Underlying',
    },
    {
        'label':   'MYIPO+ EY S&P 500 Fund',
        'ccy':     'USD', 'par': 1.0000, 'suffix': '',
        'members': ['SPYI', 'SPY'],
        'weights': {'SPYI': 0.50, 'SPY': 0.50},
        'note':    'SPYI+SPY, 50:50 CovCall/Underlying',
    },
    {
        'label':   'MYIPO+ EY Dow Jones Fund',
        'ccy':     'USD', 'par': 1.0000, 'suffix': '',
        # 'DJIA' as given in spec — not a standard tradeable ticker (the
        # index itself is ^DJI; DIA is the SPDR trust). Left as specified;
        # the builder skips unpriced members gracefully if it doesn't
        # resolve, but this ticker should be confirmed before going live.
        'members': ['DJIA', 'DIA'],
        'weights': {'DJIA': 0.50, 'DIA': 0.50},
        'note':    'DJIA+DIA, 50:50 CovCall/Underlying (confirm DJIA ticker)',
    },
    {
        'label':   'MYIPO+ EY Russell 2000 Fund',
        'ccy':     'USD', 'par': 1.0000, 'suffix': '',
        'members': ['IWMI', 'IWM'],
        'weights': {'IWMI': 0.50, 'IWM': 0.50},
        'note':    'IWMI+IWM, 50:50 CovCall/Underlying',
    },
]

VFUND_CONFIG = VFUND_CONFIG + ENHANCED_YIELD_CONFIG

# ── Fund class / distribution frequency — EVERY fund here is Distribution
# (D) class, never Accumulation: income is paid out as cash (or DRIP'd into
# new units on request), it's never silently rolled into NAV with no ledger
# trace. Default payout is quarterly; the EY series is the one exception —
# per spec it pays monthly.
for _vf in VFUND_CONFIG:
    _vf.setdefault('dist_class', 'D')
    _vf.setdefault('dist_freq', 'quarterly')
for _vf in ENHANCED_YIELD_CONFIG:
    _vf['dist_class'] = 'D'
    _vf['dist_freq']  = 'monthly'
...
for _lbl in FUND_LABELS:
    FUND_META[_lbl] = {'par': FUND_PAR_NAV, 'ccy': 'RM', 'units': FUND_UNITS_ISSUED,
                        ...
                        'dist_class': 'D', 'dist_freq': 'quarterly'}
for _vf in VFUND_CONFIG:
    FUND_META[_vf['label']] = {'par':   _vf['par'],
                                'ccy':   'RM' if _vf['ccy'] == 'MYR' else '$',
                                'units': _vf.get('units', FUND_UNITS_ISSUED),
                                'note':  _vf['note'],
                                'dist_class': _vf.get('dist_class', 'D'),
                                'dist_freq':  _vf.get('dist_freq', 'quarterly')}
...
_CHAMPIONS_HISTORY  = []  # [history contract records] for MYIPO+ Champions
_ANNUAL_WINNERS_HISTORY = []  # [history contract records] for MYIPO+ Annual Winners
_ANNUAL_LOSERS_HISTORY  = []  # [history contract records] for MYIPO+ Annual Losers
...
# Champions is deliberately not another generic Top-N series.  It graduates
# constituents from MYIPO+ Top 100 after a one-year tenure gate, then uses the
# multi-factor score described in the MYIPO+ Index Series methodology.
CHAMPIONS_LABEL = 'MYIPO+ Champions'
CHAMPIONS_MAX_CONSTITUENTS = 50

# Annual Winners — unmanaged "Hall of Fame". On the last trading day of each
# December, the single best full-year performer (by capital gain since IPO
# offer price) is added permanently. Basket grows by 1/year, equal-weighted.
ANNUAL_WINNERS_LABEL = 'MYIPO+ Annual Winners'

# Annual Losers — inverse of Annual Winners. Same mechanics (last trading
# day of December, added permanently, equal-weighted, basket grows by
# 1/year, never removes a prior member) but picks the single WORST full-year
# performer (by capital gain since IPO offer price) among that year's new
# listings instead of the best.
ANNUAL_LOSERS_LABEL = 'MYIPO+ Annual Losers'
...
LINE_STYLE_MAP[CHAMPIONS_LABEL] = (0, (8, 2, 1, 2))  # long dash-dot, elite sub-index
LINE_STYLE_MAP[ANNUAL_WINNERS_LABEL] = (0, (1, 1))     # fine dot, hall-of-fame
LINE_STYLE_MAP[ANNUAL_LOSERS_LABEL]  = (0, (1, 1, 1, 3))  # fine dot-gap, inverse hall-of-fame
for _lbl in FUND_LABELS:
    LINE_STYLE_MAP[_lbl] = '--'
...
def build_indices(input_file, log_cb=None, prefer_online=True, apply_sell_fee=True, drip_distributions=False):
    ...
    CHUNK  = _smart_chunk_size(len(stale_tickers), max_single_chunk=50, min_chunk=10, target_chunks=8)
    ...
    for vf in VFUND_CONFIG:
        ...
        try:
            ...
            px_all = hist_data.loc[launch:, have].ffill()

            # Staggered entry: a constituent doesn't need to be priced on
            # the fund's launch date — it joins the basket on its OWN first
            # priced date on/after launch (its IPO/listing date, effectively
            # — e.g. XaaS Fund's RNG/0465.KL, which listed after the fund's
            # 2026-01-01 launch). Before its entry date its allocated
            # capital sits as flat cash (no gain/loss); from entry date
            # onward it's marked to the constituent's price. A constituent
            # with NO priced data anywhere in the window is excluded
            # entirely (there's genuinely nothing to buy).
            entry_date, entry_px = {}, {}
            for s in have:
                valid = px_all[s][px_all[s] > 0]
                if valid.empty:
                    continue
                entry_date[s] = valid.index[0]
                entry_px[s]   = float(valid.iloc[0])
            tradable = list(entry_date.keys())
            if not tradable:
                log(f"   ⚠️  {label}: none of its constituents ever priced in the window — skipped.")
                continue

            par        = float(vf['par'])
            units_out  = float(vf.get('units', FUND_UNITS_ISSUED))
            capital    = units_out * par            # raised once, at launch
            n          = len(tradable)

            # Equal weight (1/N) unless this VFund defines an explicit
            # per-member 'weights' override (e.g. the Enhanced Yield series,
            # which tilts toward the wrapper ETF instead of splitting 1:1).
            # Missing/unpriced members are dropped and the remaining weights
            # renormalized so capital is still fully allocated.
            raw_weights = vf.get('weights')
            if raw_weights:
                w = pd.Series({m: raw_weights.get(m, 0.0) for m in tradable})
                if w.sum() > 0:
                    w = w / w.sum()
                else:
                    w = pd.Series(1.0 / n, index=tradable)
            else:
                w = pd.Series(1.0 / n, index=tradable)

            per_name = capital * w                   # Series: capital per member
            shares   = {s: per_name[s] / entry_px[s] for s in tradable}

            # Each member contributes flat cash (per_name) before its own
            # entry date, and mark-to-market value (price x shares) from
            # its entry date onward — this is what lets a late-IPO member
            # join the basket without needing launch-date price data.
            basket_val = pd.Series(0.0, index=px_all.index)
            for s in tradable:
                contrib = pd.Series(per_name[s], index=px_all.index)
                post = px_all.index >= entry_date[s]
                contrib.loc[post] = px_all.loc[post, s] * shares[s]
                basket_val = basket_val.add(contrib, fill_value=0.0)

            # NAV = basket value / units. No issuance, so it moves purely
            # with the constituents (net of each member's staggered entry).
            nav        = basket_val / units_out

            # ── Distributions (D class — every VFund pays out, none is
            # Accumulation) ──────────────────────────────────────────────
            # Real per-member dividend cash actually received (divs_map was
            # already fetched for the TR matrix above — reused here, not
            # refetched), scaled by shares held, cumulative from each
            # member's own entry date. Paid out on the fund's own schedule:
            # quarterly by default, monthly for the EY series (dist_freq).
            # CASH mode only for VFund CEFs — no DRIP toggle here (that's a
            # FUND_CONFIG-only feature); distributed cash permanently lowers
            # NAV from that date forward.
            dist_freq = vf.get('dist_freq', 'quarterly')
            div_cash_total = pd.Series(0.0, index=nav.index)
            for s in tradable:
                divs = divs_map.get(s)
                if divs is None or divs.empty:
                    continue
                ...
                for ex_date, amt in divs.items():
                    if ex_date < ent:
                        continue
                    pos = nav.index.searchsorted(ex_date, side='left')
                    if pos >= len(nav.index):
                        continue
                    contrib.iloc[pos:] += float(amt) * sh
                div_cash_total = div_cash_total.add(contrib, fill_value=0.0)

            dist_offset  = pd.Series(0.0, index=nav.index)
            already_paid = 0.0
            vf_dist_log  = []
            idx_list = nav.index
            for i, dt in enumerate(idx_list):
                if dist_freq == 'quarterly':
                    is_dd = (dt.month, dt.day) in FUND_DIST_MONTHS_DAYS
                else:   # monthly — last trading day of the calendar month
                    is_dd = (i == len(idx_list) - 1) or (idx_list[i + 1].month != dt.month)
                if not is_dd:
                    continue
                total_recv = float(div_cash_total.loc[dt])
                dist_total = total_recv - already_paid
                if dist_total <= 0:
                    already_paid = total_recv
                    continue
                dist_per_unit = dist_total / units_out
                nav_before = float(nav.loc[dt] - dist_offset.loc[dt])
                dist_offset.loc[idx_list >= dt] += dist_per_unit
                nav_after = nav_before - dist_per_unit
                vf_dist_log.append((dt, nav_before, dist_per_unit, nav_after))
                already_paid = total_recv

            nav = (nav - dist_offset).clip(lower=0.0)
            all_fund_dist_logs[label] = vf_dist_log

            nav_full = nav.reindex(idx_arr)
            nav_full.loc[idx_arr < launch] = np.nan
            nav_full = nav_full.ffill()

            dist_offset_full = dist_offset.reindex(idx_arr)
            dist_offset_full.loc[idx_arr < launch] = 0.0
            dist_offset_full = dist_offset_full.ffill().fillna(0.0)
            total_return_full = nav_full + dist_offset_full

            all_frames[label] = pd.DataFrame({
                label:                  nav_full.round(4),
                f'{label} Chg%':       (nav_full.pct_change() * 100).round(2),
                f'{label} CumDist':     dist_offset_full.round(4),
                f'{label} TotalReturn': total_return_full.round(4),
            })
            weights_snapshot[label] = w

            if raw_weights:
                note = f"{n} constituents, weighted: " + \
                       ", ".join(f"{m} {w[m]*100:.1f}%" for m in tradable)
            else:
            ...
            log(f"   ✅ {label}: launched {launch.date()} at "
                ...
                f"{len(vf_dist_log)} distribution(s) paid ({dist_freq}).")
        except Exception as e:
    ...
    # ------------------------------------------------------------------
    # MYIPO+ CHAMPIONS — multi-factor seasoned elite sub-index.
    # Graduates from MYIPO+ Top 100 after a >365-day tenure gate, scored on
    # Rank (30%) + Tenure (40%) + Price (20%) + DY (10%, yield-trap adjusted),
    # score-weighted (uncapped), quarterly rebalance (last trading day of
    # Mar/Jun/Sep/Dec). Universal history contract, same shape as Top Series.
    # ------------------------------------------------------------------
    def _build_champions():
        label = CHAMPIONS_LABEL
        universe = df_listed[df_listed['Quant1'] > 0].copy()
        universe = universe.drop_duplicates('Symbol').sort_values('Trade Date')
        if universe.empty:
            log(f"   ⚠️  {label}: no MYIPO+ universe, skipped.")
            return None

        q_ends = pd.date_range(start=pd.Timestamp('2021-03-31'), end=TODAY, freq='QE')
        q_ends = q_ends[q_ends.month.isin([3, 6, 9, 12])]

        def _rank_score(rank_pos):
            if rank_pos <= 10:  return 100.0
            if rank_pos <= 20:  return 75.0
            if rank_pos <= 50:  return 50.0
            return 25.0

        def _tenure_score(months):
            if months > 36: return 100.0
            if months > 24: return 75.0
            if months > 18: return 50.0
            if months > 12: return 25.0
            return 0.0

        def _dy_score(yield_pct):
            if yield_pct <= 0.0:  return 0.0
            if yield_pct <= 3.0:  return 50.0
            if yield_pct <= 7.0:  return 100.0
            return 75.0   # >7% — yield-trap penalty applied

        def _ttm_div_sum(sym, as_of):
            divs = divs_map.get(sym)
            if divs is None or divs.empty:
                return 0.0
            window = divs[(divs.index > as_of - pd.Timedelta(days=365)) & (divs.index <= as_of)]
            return float(window.sum()) if not window.empty else 0.0

        rebalance_dates, snapped_seen = [], set()
        for q_dt in q_ends:
            snapped = _snap_date(q_dt)
            if snapped is not None and snapped not in snapped_seen:
                rebalance_dates.append(snapped)
                snapped_seen.add(snapped)
        if not rebalance_dates:
            log(f"   ⚠️  {label}: no valid rebalance dates, skipped.")
            return None
        ...
        for rb_idx, rb_date in enumerate(rebalance_dates):
            ...
            # Graduation gate: must be Top-100 member as of rb_date AND
            # listing tenure > 365 days (D+365 <= rb_date).
            top100_hist = _TOP_SERIES_HISTORY.get(100, [])
            top100_asof = [h for h in top100_hist if h['date'] <= rb_date]
            if not top100_asof:
                continue
            top100_members = set(top100_asof[-1]['constituents'])
            top100_scores  = top100_asof[-1]['scores']

            elig = universe[universe['Symbol'].isin(top100_members)].copy()
            elig = elig[elig['D+365'].notna() & (elig['D+365'] <= rb_date)]
            elig = elig[elig['Symbol'].isin(hist_data.columns)]
            if elig.empty:
                continue
            ...
            if rb_snap is None:
                continue
            elig['rb_price'] = elig['Symbol'].map(
                lambda s: hist_data.loc[rb_snap, s]
                if s in hist_data.columns and rb_snap in hist_data.index else np.nan)
            elig = elig.dropna(subset=['rb_price'])
            elig = elig[elig['rb_price'] > 0]
            if elig.empty:
                continue

            elig['top100_rank'] = elig['Symbol'].map(
                lambda s: sorted(top100_scores, key=top100_scores.get, reverse=True).index(s) + 1
                if s in top100_scores else 999)
            elig['S_rank'] = elig['top100_rank'].map(_rank_score)

            elig['tenure_months'] = (rb_date - elig['Trade Date']).dt.days / 30.4375
            elig['S_tenure'] = elig['tenure_months'].map(_tenure_score)

            elig['ret_pct'] = (elig['rb_price'] - elig['Purchase Price']) / elig['Purchase Price'] * 100
            r_min, r_max = elig['ret_pct'].min(), elig['ret_pct'].max()
            if r_max > r_min:
                elig['S_price'] = (elig['ret_pct'] - r_min) / (r_max - r_min) * 100
            else:
                elig['S_price'] = 50.0

            elig['ttm_div'] = elig['Symbol'].map(lambda s: _ttm_div_sum(s, rb_date))
            elig['dy_cp']   = np.where(elig['rb_price'] > 0, elig['ttm_div'] / elig['rb_price'] * 100, 0.0)
            elig['dy_op']   = np.where(elig['Purchase Price'] > 0,
                                        elig['ttm_div'] / elig['Purchase Price'] * 100, 0.0)
            elig['dy_blend'] = elig['dy_cp'] * 0.5 + elig['dy_op'] * 0.5
            elig['S_dy'] = elig['dy_blend'].map(_dy_score)

            elig['S_total'] = (0.30 * elig['S_rank'] + 0.40 * elig['S_tenure'] +
                                0.20 * elig['S_price'] + 0.10 * elig['S_dy'])

            top_sel = elig.nlargest(CHAMPIONS_MAX_CONSTITUENTS, 'S_total')
            syms    = top_sel['Symbol'].tolist()
            if not syms:
                continue

            total_score = top_sel['S_total'].sum()
            if total_score <= 0:
                continue
            weights = {s: float(top_sel.loc[top_sel['Symbol'] == s, 'S_total'].iloc[0]) / total_score
                       for s in syms}
            ...
            history.append({
                ...
                'prices':       {s: float(top_sel[top_sel['Symbol'] == s]['rb_price'].iloc[0])
                                 for s in syms},
                'scores':       {s: float(top_sel[top_sel['Symbol'] == s]['S_total'].iloc[0])
                                 for s in syms},
                'in':           sorted(cur_members - prev_members),
                'out':          sorted(prev_members - cur_members),
            })
            ...
            if valid_mask.sum() == 0:
                continue
            w_arr  = np.array([weights.get(s, 0.0) for s in avail_syms])
            w_arr  = np.where(valid_mask, w_arr, 0.0)
            w_sum  = w_arr.sum()
            if w_sum <= 0:
                continue
            w_arr  = w_arr / w_sum
            units  = np.where(valid_mask, w_arr / first_prices, 0.0)
            port_vals = period_prices.dot(units)
            port_norm = port_vals / port_vals[0] * index_level
            index_values.loc[period_days] = port_norm
            index_level = port_norm[-1]
        ...
        last_weights = history[-1]['weights'] if history else {}
        w_snap = (pd.Series(last_weights) if last_weights else pd.Series(dtype=float))
        log(f"   ✅ {label}: {len(rebalance_dates)} rebalances | "
            f"{len(index_values)} days | latest = {index_values.iloc[-1]:.2f}")
        return label, frame, w_snap, history

    # ------------------------------------------------------------------
    # MYIPO+ ANNUAL WINNERS — unmanaged "Hall of Fame". On the last trading
    # day of each December, the #1 full-year performer among that year's new
    # listings (by capital gain since IPO offer price) is added permanently.
    # Basket grows by one member per completed year; equal-weighted (1/N),
    # reset at each annual rebalance. Never removes a prior winner.
    # ------------------------------------------------------------------
    def _build_annual_winners():
        label = ANNUAL_WINNERS_LABEL
        universe = df_listed[df_listed['Quant1'] > 0].copy()
        ...
        years = sorted(universe['Trade Date'].dt.year.dropna().unique().astype(int))
        winners_by_year = {}
        for year in years:
            year_end = pd.Timestamp(year=int(year), month=12, day=31)
            if year_end > TODAY:
                continue   # current/incomplete year not finalised yet
            snapped = _snap_date(year_end)
            if snapped is None:
                continue
            cands = universe[(universe['Trade Date'].dt.year == year) &
                              (universe['Symbol'].isin(hist_data.columns))].copy()
            if cands.empty:
                continue
            cands['close_price'] = cands['Symbol'].map(
                lambda s: hist_data.loc[snapped, s] if snapped in hist_data.index else np.nan)
            cands = cands.dropna(subset=['close_price'])
            cands = cands[(cands['close_price'] > 0) & (cands['Purchase Price'] > 0)]
            if cands.empty:
                continue
            cands['ret'] = (cands['close_price'] - cands['Purchase Price']) / cands['Purchase Price']
            winner = cands.loc[cands['ret'].idxmax()]
            winners_by_year[int(year)] = winner['Symbol']

        if not winners_by_year:
            log(f"   ⚠️  {label}: no completed year with a valid winner, skipped.")
            return None

        sorted_years     = sorted(winners_by_year.keys())
        rebalance_dates  = [_snap_date(pd.Timestamp(year=y, month=12, day=31)) for y in sorted_years]
        ...
        history      = []
        prev_members = set()
        cumulative   = []   # winners added so far, in chronological order

        for i, year in enumerate(sorted_years):
            rb_date = rebalance_dates[i]
            cumulative.append(winners_by_year[year])
            next_rb = rebalance_dates[i + 1] if i + 1 < len(rebalance_dates) \
                      else tdays[-1] + pd.Timedelta(days=1)
            period_days = tdays[(tdays >= rb_date) & (tdays < next_rb)]
            if len(period_days) == 0:
                continue

            syms = [s for s in cumulative if s in hist_data.columns]
            if not syms:
                continue
            eq_w = round(1.0 / len(syms), 6)
            weights = {s: eq_w for s in syms}
            ...
            history.append({
                ...
                'constituents': sorted(cur_members),
                'weights':      weights,
                'prices':       {s: float(hist_data.loc[rb_date, s])
                                 for s in syms if rb_date in hist_data.index},
                'scores':       {s: 1.0 for s in syms},   # equal-weighted, no factor score
                'in':           sorted(cur_members - prev_members),
                'out':          sorted(prev_members - cur_members),
            })
            prev_members = cur_members

            px_period = hist_data.loc[period_days, syms].ffill().bfill()
            if px_period.empty:
                continue
            period_prices = px_period.values
            first_prices  = period_prices[0, :]
            ...
            units   = np.where(valid_mask, 1.0 / (first_prices * n_valid), 0.0)
            port_vals = period_prices.dot(units)
            port_norm = port_vals / port_vals[0] * index_level
            index_values.loc[period_days] = port_norm
            index_level = port_norm[-1]
        ...
        last_weights = history[-1]['weights'] if history else {}
        w_snap = (pd.Series(last_weights) if last_weights else pd.Series(dtype=float))
        log(f"   ✅ {label}: {len(rebalance_dates)} annual rebalances | "
            f"{len(index_values)} days | latest = {index_values.iloc[-1]:.2f}")
        return label, frame, w_snap, history

    # ------------------------------------------------------------------
    # MYIPO+ ANNUAL LOSERS — inverse of Annual Winners. On the last trading
    # day of each December, the #1 WORST full-year performer among that
    # year's new listings (by capital gain since IPO offer price) is added
    # permanently. Same mechanics otherwise: basket grows by one member per
    # completed year, equal-weighted (1/N), never removes a prior member.
    # ------------------------------------------------------------------
    def _build_annual_losers():
        label = ANNUAL_LOSERS_LABEL
        universe = df_listed[df_listed['Quant1'] > 0].copy()
        universe = universe.drop_duplicates('Symbol')
        if universe.empty:
            log(f"   ⚠️  {label}: no MYIPO+ universe, skipped.")
        ...
        for year in years:
            ...
            if cands.empty:
                continue
            cands['ret'] = (cands['close_price'] - cands['Purchase Price']) / cands['Purchase Price']
            loser = cands.loc[cands['ret'].idxmin()]
            losers_by_year[int(year)] = loser['Symbol']

        if not losers_by_year:
            log(f"   ⚠️  {label}: no completed year with a valid loser, skipped.")
            return None

        sorted_years     = sorted(losers_by_year.keys())
        rebalance_dates  = [_snap_date(pd.Timestamp(year=y, month=12, day=31)) for y in sorted_years]
        rebalance_dates  = [d for d in rebalance_dates if d is not None]
        if not rebalance_dates:
            return None
        ...
        history      = []
        prev_members = set()
        cumulative   = []   # losers added so far, in chronological order

        for i, year in enumerate(sorted_years):
            rb_date = rebalance_dates[i]
            cumulative.append(losers_by_year[year])
            next_rb = rebalance_dates[i + 1] if i + 1 < len(rebalance_dates) \
                      else tdays[-1] + pd.Timedelta(days=1)
            period_days = tdays[(tdays >= rb_date) & (tdays < next_rb)]
            if len(period_days) == 0:
    ...
    # ── Top Series fund twins — RM0.25 par, 1,000 units at issue, same as
    # every other unit-trust fund, launched the day its Top-N index itself
    # incepted. Tracks the index 1:1 (price-only — the Top-N index has no
    # TR twin to isolate a dividend-only gap from, unlike the index-twin
    # FUND_CONFIG funds, so no distribution is wired here yet; flagged
    # rather than fabricated, same call as the EY ratio-payout gap above).
    TOP_FUND_LABELS = []
    for _top_n in (10, 20, 50, 100):
        _base_label = f'MYIPO+ Top {_top_n}'
        _fund_label = f'MYIPO+ Top {_top_n} Fund'
        if _base_label not in all_frames:
            continue
        try:
            _idx_series = all_frames[_base_label][_base_label].dropna()
            if _idx_series.empty:
                continue
            _launch_val = float(_idx_series.iloc[0])
            if _launch_val <= 0:
                continue
            _nav = (_idx_series / _launch_val) * FUND_PAR_NAV
            _nav_full = _nav.reindex(idx_arr).ffill()
            all_frames[_fund_label] = pd.DataFrame({
                _fund_label:            _nav_full.round(4),
                f'{_fund_label} Chg%': (_nav_full.pct_change() * 100).round(2),
            })
            weights_snapshot[_fund_label] = weights_snapshot.get(_base_label, pd.Series(dtype=float))
            FUND_META[_fund_label] = {'par': FUND_PAR_NAV, 'ccy': 'RM', 'units': FUND_UNITS_ISSUED,
                                       'note': f'Tracks {_base_label} 1:1 — distribution not yet wired (no TR twin)',
                                       'dist_class': 'D', 'dist_freq': 'quarterly'}
            TOP_FUND_LABELS.append(_fund_label)
            log(f"   ✅ {_fund_label}: launched at RM{FUND_PAR_NAV:.4f}, "
                f"{FUND_UNITS_ISSUED} units · latest NAV RM{_nav_full.iloc[-1]:.4f}")
        except Exception as e:
            log(f"   ⚠️  {_fund_label}: error — {e}")
    global ALL_FUND_LABELS
    ALL_FUND_LABELS = FUND_LABELS + VFUND_LABELS + TOP_FUND_LABELS

    log("   Building MYIPO+ Champions...")
    global _CHAMPIONS_HISTORY
    _CHAMPIONS_HISTORY = []
    try:
        result_ch = _build_champions()
        if result_ch:
            ch_label, ch_frame, ch_wsnap, ch_hist = result_ch
            all_frames[ch_label]       = ch_frame
            weights_snapshot[ch_label] = ch_wsnap
            _CHAMPIONS_HISTORY          = ch_hist
    except Exception as e:
        log(f"   ⚠️  MYIPO+ Champions: error — {e}")

    log("   Building MYIPO+ Annual Winners...")
    global _ANNUAL_WINNERS_HISTORY
    _ANNUAL_WINNERS_HISTORY = []
    try:
        result_aw = _build_annual_winners()
        if result_aw:
            aw_label, aw_frame, aw_wsnap, aw_hist = result_aw
            all_frames[aw_label]       = aw_frame
            weights_snapshot[aw_label] = aw_wsnap
            _ANNUAL_WINNERS_HISTORY     = aw_hist
    except Exception as e:
        log(f"   ⚠️  MYIPO+ Annual Winners: error — {e}")

    log("   Building MYIPO+ Annual Losers...")
    global _ANNUAL_LOSERS_HISTORY
    _ANNUAL_LOSERS_HISTORY = []
    try:
        result_al = _build_annual_losers()
        if result_al:
            al_label, al_frame, al_wsnap, al_hist = result_al
            all_frames[al_label]       = al_frame
            weights_snapshot[al_label] = al_wsnap
            _ANNUAL_LOSERS_HISTORY      = al_hist
    except Exception as e:
        log(f"   ⚠️  MYIPO+ Annual Losers: error — {e}")
...
class SaveForRewards:
    ...
    def calculate_position_cr(
        ...
        """
        # Custom Rule — X credits per Y units held. Y (custom_threshold) is
        # user-adjustable per K-ID; X (custom_credits) is admin-locked, the
        # base credit unit for this localization (e.g. 1 Cr).
        if is_custom and custom_threshold > 0:
            return int(units // custom_threshold) * custom_credits
        # US
        if region == "US":
            # Every 0.01 share = 1 Cr
...
class MYIPOApp(tk.Tk):
    def __init__(self, preloaded=None, csv_path=None):
        ...
        # OPIE disabled for now (module removed).
        self.opie = None
        ...
        # ── US-universe preload at startup — MYIPO+-style ─────────────────────
        # Same pattern as the MYIPO+ data preload above: don't wait for the
        # first ZDWS/ZDOS tab-open to start fetching. The snapshot CSV seeds
        # prices instantly and the paced chunk download runs in the
    ...
    def _build_ui(self):
        ...
        # The selector list can grow beyond a laptop-height window.  Keep the
        # period/summary area fixed while making just this long control area
        # vertically scrollable.
        selector_scroll = tk.Frame(sidebar_outer, bg='#161b22')
        selector_scroll.pack(fill='both', expand=True, side='top')
        selector_canvas = tk.Canvas(selector_scroll, bg='#161b22',
                                    highlightthickness=0, bd=0)
        selector_bar = ttk.Scrollbar(selector_scroll, orient='vertical',
                                     command=selector_canvas.yview)
        selector_canvas.configure(yscrollcommand=selector_bar.set)
        selector_bar.pack(side='right', fill='y', padx=(0, 2), pady=4)
        selector_canvas.pack(side='left', fill='both', expand=True)

        top_block = tk.Frame(selector_canvas, bg='#161b22')
        selector_window = selector_canvas.create_window((0, 0), window=top_block,
                                                        anchor='nw')

        def _resize_selector_scroll(event=None):
            selector_canvas.configure(scrollregion=selector_canvas.bbox('all'))

        def _fit_selector_width(event):
            selector_canvas.itemconfigure(selector_window, width=event.width)

        top_block.bind('<Configure>', _resize_selector_scroll)
        selector_canvas.bind('<Configure>', _fit_selector_width)

        # Dragging/clicking the scrollbar always works.  Mouse-wheel support
        # is deliberately scoped to this canvas so it does not steal scrolling
        # from SCA or other pop-up windows.
        selector_canvas.bind('<MouseWheel>',
            lambda e: selector_canvas.yview_scroll(int(-e.delta / 120), 'units'))
        ...
        _make_section(top_block, 'Top Series (Monthly Rebalanced)',
                      TOP_SERIES_LABELS + [CHAMPIONS_LABEL, ANNUAL_WINNERS_LABEL, ANNUAL_LOSERS_LABEL], accent='#FFD700')
        ...
        # ── OPIE chat trigger — same "Hi, Opie" pattern as SCA, mirrored here
        # so it's reachable from the dashboard without opening SCA first.
        tk.Frame(info_bar, bg='#30363D', width=1).pack(side='right', fill='y', pady=4, padx=(4, 0))
        opie_dash_box = tk.Frame(info_bar, bg='#101820')
        opie_dash_box.pack(side='right', padx=(4, 6))
        self._opie_dash_entry_var = tk.StringVar(value='')
        opie_dash_entry = tk.Entry(opie_dash_box, textvariable=self._opie_dash_entry_var,
                                    font=('Segoe UI', 8), width=20,
                                    bg='#21262d', fg='#cdd9e5', insertbackground='#cdd9e5',
                                    relief='flat')
        opie_dash_entry.insert(0, 'Type: Hi, Opie')
        opie_dash_entry.configure(fg='#555')

        def _clear_ph(event=None):
            if opie_dash_entry.get() == 'Type: Hi, Opie':
                opie_dash_entry.delete(0, 'end')
                opie_dash_entry.configure(fg='#cdd9e5')

        def _restore_ph(event=None):
            if not opie_dash_entry.get().strip():
                opie_dash_entry.insert(0, 'Type: Hi, Opie')
                opie_dash_entry.configure(fg='#555')

        opie_dash_entry.bind('<FocusIn>', _clear_ph)
        opie_dash_entry.bind('<FocusOut>', _restore_ph)
        opie_dash_entry.bind('<Return>', lambda e: self._opie_open_popup())
        opie_dash_entry.pack(side='left')
        tk.Button(opie_dash_box, text='🤖', font=('Segoe UI', 8), relief='flat',
                  bg='#21262d', fg='#00C9FF', cursor='hand2',
                  command=lambda: self._opie_open_popup()).pack(side='left', padx=(3, 0))

        # ── Bursa Market Indices Ticker ───────────────────────────────────────
        self._build_market_ticker(right)

        # OPIE dashboard reply row — hidden until a chat message is answered.
        self._opie_dash_reply_row = tk.Frame(right, bg='#0d1117')
        self._opie_dash_reply_var = tk.StringVar(value='')
        tk.Label(self._opie_dash_reply_row, text='🤖 OPIE:', font=('Segoe UI', 8, 'bold'),
                 fg='#00C9FF', bg='#0d1117').pack(side='left', padx=(8, 4))
        tk.Label(self._opie_dash_reply_row, textvariable=self._opie_dash_reply_var,
                 font=('Segoe UI', 8), fg='#cdd9e5', bg='#0d1117',
                 anchor='w', justify='left', wraplength=900).pack(side='left', fill='x')
        ...
        self.tab_fund_page = tk.Frame(self.notebook, bg='#0d1117')
        self.notebook.add(self.tab_fund_page, text='🏦  Fund Page')
        self._build_fund_page_tab()
        ...
        self.tab_rewards = tk.Frame(self.notebook, bg='#0d1117')
        self.notebook.add(self.tab_rewards, text='💎  Rewards')
    ...
    def _open_sca(self):
        ...
        has_src = sca_has_source(load_app_settings())
        opie_result = self._opie_before_module('sca',
            self.newpor_portfolios if has_src else None)
        self._sca_win = SCAWindow(self,
                  preloaded_rows=self.newpor_rows if has_src else None,
                  preloaded_portfolios=self.newpor_portfolios if has_src else None,
                  opie_context=opie_result)

    def _opie_before_module(self, module: str, portfolios=None) -> dict:
        """Run a fail-safe OPIE pre-processing pass before a module opens."""
        if self.opie is None:
            return {"status": "unavailable", "message": "OPIE is not initialised."}
        try:
            return self.opie.before_module(module, portfolios=portfolios,
                                           wallet=self._pts_load())
        except Exception as exc:
            # OPIE advises; it must never prevent an established workflow from opening.
            return {"status": "unavailable", "message": f"OPIE unavailable: {exc}"}

    def _opie_dash_chat_submit(self, entry_widget):
        """Dashboard-level twin of SCA's OPIE chat trigger — same 'Hi, Opie'
        greeting, reachable without opening SCA first."""
        text = entry_widget.get().strip()
        if not text or text == 'Type: Hi, Opie':
            return
        greeting = text.lower().replace(',', '').replace('!', '').strip()
        if greeting in ('hi opie', 'hello opie', 'hey opie', 'opie', 'hi opie summary'):
            self._opie_dash_reply_var.set(self._opie_dash_summary())
        else:
            self._opie_dash_reply_var.set('Try: "Hi, Opie" for a quick portfolio summary.')
        self._opie_dash_reply_row.pack(fill='x', pady=(0, 4))
        entry_widget.delete(0, 'end')
        self.focus_set()

    def _opie_open_popup(self):
        """Click/Enter on the 🤖 trigger — pop up the OPIE chat window (or
        just focus it if one's already open). The window itself stays idle
        until the user types 'Hi, Opie'."""
        sca = getattr(self, '_sca_win', None)
        host = sca if (sca is not None and getattr(sca, '_portfolios', None)) else self
        _opie_open_chat(self, host, CURRENT_KID)

    def _opie_dash_summary(self) -> str:
        """Prefer SCA's live, FX-forced figures if that window is open with
        prices loaded; otherwise fall back to a cost-basis-only summary from
        whatever's preloaded on the dashboard (no live prices/FX at this
        point unless SCA has already fetched them)."""
        sca = getattr(self, '_sca_win', None)
        if sca is not None and getattr(sca, '_portfolios', None):
            try:
                return sca._opie_lightweight_summary()
            except Exception:
                pass

        portfolios = self.newpor_portfolios
        if not portfolios:
            return "Hi! No portfolio loaded yet — open SCA to connect a source."

        positions, cost_total = 0, 0.0
        ccy_counts = {}
        for port in portfolios.values():
            c = port.get('base_currency', 'USD')
            ...
            for ticker, item in port.get('holdings', {}).items():
                ...
                if not shares or shares <= 0:
                    continue
                positions  += 1
                cost_total += item.get('cost_value', 0.0)   # not FX-forced yet — no prices loaded here

        base_ccy = max(ccy_counts, key=ccy_counts.get) if ccy_counts else 'MYR'
        prefix = ccy_symbol(base_ccy)
        diversification = ('not available' if not positions else
                            'concentrated' if positions < 4 else
                            'developing' if positions < 8 else 'diversified')
        return (f"{positions} active position(s), diversification is {diversification}. "
                f"Cost basis: {prefix}{cost_total:,.2f} "
                f"(open SCA for live value, return % and FX-forced totals).")

    def _dev_quick_switch(self, win=None):
        """🔁 Quick-switch between admin1 and test1 only — both reserved as
        the developer's own accounts, so this skips the PIN screen entirely
        and reloads straight into the other one. Auto-creates test1 (PIN
        0000) the first time if it doesn't exist yet."""
        cur = (CURRENT_KID or '').strip().lower()
        if cur not in (ADMIN_KID, TEST_KID):
            return
        target = TEST_KID if cur == ADMIN_KID else ADMIN_KID
        if not messagebox.askyesno('Quick Switch',
                f'Switch to {target} for testing?\n\n'
                f'admin1 and test1 are both your own reserved dev/test '
                f'accounts — this skips the PIN screen and reloads the app '
                f'straight into {target}.', parent=(win or self)):
            return
        # Flush THIS K-ID's wallet to disk before tearing the window down —
        # every credit mutation already saves immediately via _pts_save, but
        # this guarantees a final round-trip so nothing pending is lost when
        # we destroy(). Always re-load fresh right before writing, so we
        # never save over any other data written elsewhere in the meantime.
        self._pts_save(self._pts_load())
        s = load_app_settings()
        if target == TEST_KID and not kid_find_account(s, TEST_KID):
            kid_create(s, TEST_KID, '0000')
            save_app_settings(s)
        if win is not None:
            win.destroy()
        self.destroy()
    ...
    def _open_settings(self):
        ...
        # ── Dev quick-switch — admin1 ⇄ test1 only. Both are reserved as the
        # developer's own accounts, so this skips the PIN screen entirely
        # and reloads straight into the other one, for fast back-and-forth
        # testing (e.g. checking a rewards/SoR change as a normal user
        # right after setting it up as admin).
        cur_lc = (CURRENT_KID or '').strip().lower()
        if cur_lc in (ADMIN_KID, TEST_KID):
            other = TEST_KID if cur_lc == ADMIN_KID else ADMIN_KID
            tk.Button(box, text=f'🔁 Quick-switch to {other}',
                      font=("Segoe UI", 8, "bold"), bg='#21262d', fg='#58A6FF',
                      relief='flat', padx=10, pady=4, cursor='hand2',
                      command=lambda: self._dev_quick_switch(win)
    ...
    def _refresh_market_ticker(self):
        ...
        def _worker():
            ...
            # TWD has no direct TWDMYR=X pair on Yahoo, only USDTWD=X, so it's
            # derived through the USD leg instead of the generic direct fetch.
            for ccy in sorted({c for _, _, c in self.WORLD_INDICES if c != 'MYR'}):
                if ccy == 'TWD':
                    twd_rate = self._zdws_twd_to_myr()
                    if twd_rate is not None:
                        results['FX:TWD'] = (twd_rate, twd_rate)
                    continue
    ...
    def _open_top10_board(self, top_n: int = 10):
        ...
        def _rebuild():
            ...
            for i, rec in enumerate(hist):
                ...
                mmyyyy = pd.Timestamp(rec['date']).strftime('%m/%Y')
                if rec['in']:
                    in_names = ', '.join(f"{s.replace('.KL','')} ({name_map.get(s, '')[:12]}) "
                                         f"[in {mmyyyy}]" for s in rec['in'])
    ...
    def _build_line_tab(self):
        ...
        tk.Label(self.tab_line, text='💡 Hover (or click) anywhere on the chart to inspect a datapoint\'s date & value',
        ...
        # Hovering the mouse over the chart shows the nearest datapoint's
        # date + value, for every currently-plotted series (click still
        # works too, so the inspector stays pinned once you stop moving).
        self._line_hover_series = {}   # label -> {series, color, is_fund}, refreshed each draw
        self._line_hover_artists = []  # crosshair line/dot/annotation, cleared each hover/click
        self._ax_line_fund = None      # set when a fund twin-axis exists, cleared otherwise

        # Bind BOTH the matplotlib event AND a raw Tkinter event on the canvas
        ...
        # Tk widget guarantees the event is received, and we convert pixel
        ...
        self.canvas_line.mpl_connect('motion_notify_event', self._on_line_hover)
        self.canvas_line.mpl_connect('figure_leave_event', lambda e: self._clear_line_hover())
        self.canvas_line.get_tk_widget().bind('<Button-1>', self._on_line_tk_click, add='+')
        self.canvas_line.get_tk_widget().bind('<Motion>', self._on_line_tk_motion, add='+')
        self.canvas_line.get_tk_widget().bind('<Leave>', lambda e: self._clear_line_hover(), add='+')

    def _line_xdata_at(self, x_px: int, y_px_tk: int):
        """Shared pixel->data-x resolver for both the click and hover Tk
        fallback paths — tries every axes with data (main, then the fund
        twin-axis if present) and returns the first whose data area actually
        contains the point, or None if the pointer isn't over the plot."""
        widget = self.canvas_line.get_tk_widget()
        y_px = widget.winfo_height() - y_px_tk   # Tk y-origin is top-left; mpl's is bottom-left
    ...
    def _on_line_tk_click(self, tk_event):
        ...
        try:
            xdata = self._line_xdata_at(tk_event.x, tk_event.y)
            if xdata is not None:
                self._show_line_inspector(xdata)
        except Exception:
            pass

    def _on_line_tk_motion(self, tk_event):
        """Tkinter-native fallback hover handler — same coordinate path as
        the click fallback, fired continuously as the mouse moves so the
        inspector tracks the cursor instead of needing a click."""
        try:
            xdata = self._line_xdata_at(tk_event.x, tk_event.y)
            if xdata is not None:
                self._show_line_inspector(xdata)
    ...
    def _on_line_hover(self, event):
        """matplotlib-native hover path — mirrors _on_line_click but fires
        on mouse movement (no button press needed)."""
        if event.xdata is None or event.inaxes not in (self.ax_line, self._ax_line_fund):
            self._clear_line_hover()
            return
    ...
    def _topn_membership_status(self, sym):
        """Current: Yes/No against MYIPO+ Top 10/20/50/100 (monthly rebalance).
        Yes  -> which tier(s) it currently sits in, e.g. 'Current: Yes (T50, T100)'.
        No   -> how many distinct monthly rebalances it appeared in any tier."""
        if not sym:
            return ''
        current_tiers = []
        all_in_dates  = set()
        for n in (10, 20, 50, 100):
            ...
            if not hist:
                continue
            for h in hist:
                if sym in h.get('constituents', []):
                    all_in_dates.add(h['date'])
            if sym in hist[-1].get('constituents', []):
                current_tiers.append(f'T{n}')
        if current_tiers:
            return f'Current: Yes ({"/".join(current_tiers)})'
        if all_in_dates:
            return f'Current: No — {len(all_in_dates)} monthly rebalance(s) in Top series historically'
        return 'Current: No — never a Top-series constituent'
    ...
    def _draw_stock_performance(self):
        ...
        topn_status = self._topn_membership_status(sym)
    ...
    def _open_wc_inspector(self, event=None):
        ...
        for n in (10, 20, 50, 100):
            ...
            for rec, in_now, was_in, was_out in reversed(records):
                if was_in or was_out:
                    status, tag = ('➕ ENTERED', 'in') if was_in else ('➖ EXITED', 'out')
                    date_str = pd.Timestamp(rec['date']).strftime('%m/%Y')
                elif in_now:
    ...
    # =========================================================================
    # RETURN CALCULATOR — "if I'd invested $X in <fund>, what would it be
    # worth today?" A historical backtest against the fund's actual NAV
    # series already built into self.all_frames (both unit-trust FUND_LABELS
    # and VFund/Enhanced-Yield CEFs) — NOT a projection or promise of future
    # return. Past performance shown here is not a guarantee of what a new
    # investment made today would earn going forward.
    # =========================================================================
    def _calc_fund_return(self, label: str, amount: float, invest_ccy: str,
                           entry_date=None):
        """Returns a dict with the backtest result, or {'error': msg}."""
        df = self.all_frames.get(label)
        if df is None or df.empty or label not in df.columns:
            return {'error': f'No NAV history available for {label}.'}
        nav_series = df[label].dropna()
        if nav_series.empty:
            return {'error': f'{label} has no priced NAV history yet.'}

        if entry_date:
            ts = pd.Timestamp(entry_date)
            eligible = nav_series[nav_series.index >= ts]
            if eligible.empty:
                return {'error': f'No NAV on/after {entry_date} for {label}.'}
            entry_nav_date = eligible.index[0]
        else:
            entry_nav_date = nav_series.index[0]     # fund launch
        entry_nav  = float(nav_series.loc[entry_nav_date])
        latest_date = nav_series.index[-1]
        latest_nav  = float(nav_series.iloc[-1])
        if entry_nav <= 0:
            return {'error': f'{label} has no valid entry NAV.'}

        # Fund's own currency: RM for unit trusts, MYR/USD for VFunds
        fund_ccy = 'MYR'
        for vf in VFUND_CONFIG:
            if vf['label'] == label:
                fund_ccy = vf['ccy']
        ...
        invest_ccy = (invest_ccy or 'MYR').upper()
        amount = float(amount)

        def _to_myr(v, ccy):
            if ccy == 'MYR':
                return v
            rate = self._zdws_fx_to_myr(ccy)
        ...
        amount_myr = _to_myr(amount, invest_ccy)
        if amount_myr is None:
            return {'error': f'Could not fetch an FX rate for {invest_ccy}.'}
        amount_fund_ccy = amount_myr if fund_ccy == 'MYR' else _from_myr(amount_myr, fund_ccy)
        if amount_fund_ccy is None:
            return {'error': f'Could not fetch an FX rate to convert into {fund_ccy}.'}

        units = amount_fund_ccy / entry_nav
        current_value_fund_ccy = units * latest_nav
        current_value_myr = current_value_fund_ccy if fund_ccy == 'MYR' else _to_myr(current_value_fund_ccy, fund_ccy)
        current_value_invest_ccy = current_value_myr if invest_ccy == 'MYR' else _from_myr(current_value_myr, invest_ccy)
        return_pct = (current_value_fund_ccy / amount_fund_ccy - 1.0) * 100.0

        return {
            'label': label, 'fund_ccy': fund_ccy, 'invest_ccy': invest_ccy,
            'entry_date': entry_nav_date, 'entry_nav': entry_nav,
            'latest_date': latest_date, 'latest_nav': latest_nav,
            'units': units, 'amount_invested': amount,
            'current_value': current_value_invest_ccy,
            'return_pct': return_pct,
        }

    def _open_fund_return_calculator(self):
        win = tk.Toplevel(self)
        win.title('💵 Fund Return Calculator')
        win.configure(bg='#0d1117')
        win.geometry('460x340')

        tk.Label(win, text='💵 RETURN CALCULATOR', font=('Segoe UI', 11, 'bold'),
                 bg='#0d1117', fg='#00E5A0').pack(pady=(12, 2))
        tk.Label(win, text='Historical backtest using actual NAV history — not a\n'
                            'projection or guarantee of future return.',
                 font=('Segoe UI', 8), bg='#0d1117', fg='#888', justify='center').pack(pady=(0, 8))

        row1 = tk.Frame(win, bg='#0d1117'); row1.pack(fill='x', padx=16, pady=4)
        tk.Label(row1, text='Fund:', bg='#0d1117', fg='#cdd9e5', width=12, anchor='w',
                 font=('Segoe UI', 9)).pack(side='left')
        fund_var = tk.StringVar(value=ALL_FUND_LABELS[0] if ALL_FUND_LABELS else '')
        ttk.Combobox(row1, textvariable=fund_var, state='readonly',
                     values=ALL_FUND_LABELS, width=28, font=('Segoe UI', 9)).pack(side='left')

        row2 = tk.Frame(win, bg='#0d1117'); row2.pack(fill='x', padx=16, pady=4)
        tk.Label(row2, text='Amount:', bg='#0d1117', fg='#cdd9e5', width=12, anchor='w',
                 font=('Segoe UI', 9)).pack(side='left')
        amt_var = tk.StringVar(value='10000')
        tk.Entry(row2, textvariable=amt_var, width=16, font=('Segoe UI', 9)).pack(side='left')

        row3 = tk.Frame(win, bg='#0d1117'); row3.pack(fill='x', padx=16, pady=4)
        tk.Label(row3, text='In currency:', bg='#0d1117', fg='#cdd9e5', width=12, anchor='w',
                 font=('Segoe UI', 9)).pack(side='left')
        ccy_var = tk.StringVar(value='MYR')
        ttk.Combobox(row3, textvariable=ccy_var, state='readonly',
                     values=['MYR', 'USD'], width=14, font=('Segoe UI', 9)).pack(side='left')

        row4 = tk.Frame(win, bg='#0d1117'); row4.pack(fill='x', padx=16, pady=4)
        tk.Label(row4, text='Entry date:', bg='#0d1117', fg='#cdd9e5', width=12, anchor='w',
                 font=('Segoe UI', 9)).pack(side='left')
        date_var = tk.StringVar(value='')
        tk.Entry(row4, textvariable=date_var, width=16, font=('Segoe UI', 9)).pack(side='left')
        tk.Label(row4, text='(blank = fund launch)', bg='#0d1117', fg='#666',
                 font=('Segoe UI', 8)).pack(side='left', padx=(6, 0))

        result_var = tk.StringVar(value='')
        result_lbl = tk.Label(win, textvariable=result_var, bg='#0d1117', fg='#cdd9e5',
                               font=('Segoe UI', 9), justify='left', anchor='w')
        result_lbl.pack(fill='x', padx=16, pady=(10, 4))

        def _calc():
            try:
                amt = float(amt_var.get())
            except ValueError:
                result_var.set('⚠️ Enter a valid numeric amount.')
                return
            res = self._calc_fund_return(fund_var.get(), amt, ccy_var.get(),
                                          entry_date=(date_var.get().strip() or None))
            if 'error' in res:
                result_var.set(f"⚠️ {res['error']}")
                return
            sign = '+' if res['return_pct'] >= 0 else ''
            result_var.set(
                f"Entry: {res['entry_date'].date()} @ NAV {res['entry_nav']:.4f} {res['fund_ccy']}\n"
                f"Latest: {res['latest_date'].date()} @ NAV {res['latest_nav']:.4f} {res['fund_ccy']}\n"
                f"Units bought: {res['units']:,.2f}\n"
                f"Current value: {res['current_value']:,.2f} {res['invest_ccy']}\n"
                f"Return: {sign}{res['return_pct']:.2f}%  "
                f"(on {res['amount_invested']:,.2f} {res['invest_ccy']} invested)"
            )

        tk.Button(win, text='Calculate', font=('Segoe UI', 9, 'bold'),
                  bg='#1a2035', fg='#00E5A0', relief='flat', cursor='hand2',
                  command=_calc).pack(pady=(4, 10))
    ...
    def _build_fund_events_tab(self):
        ...
        tk.Button(ctrl, text='💵 Return Calculator', font=('Segoe UI', 9, 'bold'),
                  bg='#1a2035', fg='#00E5A0', relief='flat', cursor='hand2',
                  command=self._open_fund_return_calculator).pack(side='left', padx=(8, 0))

        tk.Button(ctrl, text='🤖 Ask Opie', font=('Segoe UI', 9, 'bold'),
                  bg='#1a2035', fg='#3FB950', relief='flat', cursor='hand2',
                  command=self._opie_open_popup).pack(side='left', padx=(8, 0))
    ...
    def _refresh_fund_events_dropdown(self):
        ...
        # Also refresh the Fund Page dropdown
        if hasattr(self, '_fp_fund_cb'):
            self._fp_fund_cb['values'] = ALL_FUND_LABELS
            if not self._fp_fund_var.get() and ALL_FUND_LABELS:
                self._fp_fund_var.set(ALL_FUND_LABELS[0])
            self._draw_fund_page()
    ...
    def _build_fund_ledger_tab(self):
        ...
        # ── A/D Class — every fund is Distribution (D, pays cash out) by
        # default; toggling to Accumulation (A) shows the same fund with
        # nothing paid out — every distribution is reinvested into NAV on
        # its distribution date instead. No separate simulation needed:
        # D uses the NAV column (already ex-distribution), A uses the
        # TotalReturn column (NAV + cumulative distributions), which is
        # exactly what full reinvestment looks like.
        tk.Label(ctrl, text='Class:', bg='#0d1117', fg='#cdd9e5',
                 font=('Segoe UI', 9)).pack(side='left', padx=(16, 0))
        self._ledger_class_var = tk.StringVar(value='D')
        for val, txt in (('D', 'D — Distribution'), ('A', 'A — Accumulation')):
            tk.Radiobutton(ctrl, text=txt, variable=self._ledger_class_var,
                           value=val, command=self._draw_fund_ledger,
                           bg='#0d1117', fg='#cdd9e5', selectcolor='#0d1117',
    ...
    def _draw_fund_ledger(self):
        ...
        fund_class = self._ledger_class_var.get()
        ...
        if lbl in frames:
            ...
            if fund_class == 'A' and tr_series is not None and not tr_series.empty:
                # Accumulation — nothing paid out, so the fund's own value
                # already includes every distribution reinvested. That's
                # exactly what TotalReturn (NAV + cumulative distributions)
                # represents — use it as this class's NAV.
                nav_series = tr_series.dropna()
            ...
            if fund_class == 'A':
                cd_last = 0.0   # nothing was paid out as cash under Accumulation
        ...
        for dt, nav_before, dist_per_unit, nav_after in dist_log:
            ...
            if fund_class == 'A':
                # Accumulation — reinvested into NAV on this date, nothing
                # paid out. Neutral event, not a cash flow.
                if filt in ('All', 'NAV events only'):
                    events.append((dt, 'Distribution Reinvested',
                                   f'{ccy}{dist_per_unit:.4f}/unit reinvested — no cash paid (A class)',
                                   0.0, 0.0, cash_at_dist, units_at_dist, nav_after))
            else:
                if filt in ('All', 'Cash flows only'):
        ...
        n_events = len(events)
        class_txt = 'A — Accumulation (reinvested)' if fund_class == 'A' else 'D — Distribution (cash paid)'
        self._ledger_summary_var.set(
            f'{lbl}  ·  Class {class_txt}  ·  {n_events} ledger entries  ·  '
    ...
    # =========================================================================
    # FUND PAGE — buy/sell any fund (unit trusts, VFund CEFs, Top Series
    # funds) with the K-ID's own Credits balance. Credits are ALL MYR, so a
    # USD fund's NAV is converted to MYR at the live rate before pricing the
    # purchase — units are still tracked in the fund's own native units,
    # only the credit cost/proceeds are MYR. Every fund is open for
    # purchase, no gating.
    # =========================================================================
    def _fp_fund_nav_myr(self, label):
        """(nav_native, ccy, nav_myr) for a fund label, or (None, None, None)."""
        meta = FUND_META.get(label)
        df   = self.all_frames.get(label) if hasattr(self, 'all_frames') else None
        if not meta or df is None or label not in df.columns:
        ...
        if ccy in ('RM', 'MYR'):
            return nav, 'MYR', nav
        # USD (or other) fund — convert via the same FX path ZDWS/ZDOS use.
        fx = self._zdws_fx_to_myr('USD') if ccy == '$' else self._zdws_fx_to_myr(ccy)
        if not fx:
            return nav, ccy, None
        return nav, ccy, nav * fx

    def _build_fund_page_tab(self):
        tab = self.tab_fund_page

        ctrl = tk.Frame(tab, bg='#0d1117')
        ctrl.pack(fill='x', padx=10, pady=(8, 4))
        tk.Label(ctrl, text='Fund:', bg='#0d1117', fg='#cdd9e5',
                 font=('Segoe UI', 9)).pack(side='left')
        self._fp_fund_var = tk.StringVar(value='')
        self._fp_fund_cb  = ttk.Combobox(ctrl, textvariable=self._fp_fund_var,
                                         state='readonly', width=32, font=('Segoe UI', 9))
        self._fp_fund_cb.pack(side='left', padx=(6, 16))
        self._fp_fund_cb.bind('<<ComboboxSelected>>', lambda e: self._draw_fund_page())

        pts = self._pts_load()
        self._fp_credits_var = tk.StringVar(value=f"Credits: RM{pts.get('credits', 0):,.2f}")
        tk.Label(ctrl, textvariable=self._fp_credits_var, font=('Segoe UI', 9, 'bold'),
                 fg='#D29922', bg='#0d1117').pack(side='left', padx=(0, 0))

        info = tk.Frame(tab, bg='#161b22', highlightbackground='#30363d',
                        highlightthickness=1, padx=14, pady=10)
        info.pack(fill='x', padx=10, pady=(8, 8))
        self._fp_info_var = tk.StringVar(value='Select a fund.')
        tk.Label(info, textvariable=self._fp_info_var, font=('Segoe UI', 9),
                 fg='#cdd9e5', bg='#161b22', justify='left', anchor='w').pack(fill='x')

        buy = tk.Frame(tab, bg='#0d1117')
        buy.pack(fill='x', padx=10, pady=(0, 8))
        tk.Label(buy, text='Amount (Credits, RM):', bg='#0d1117', fg='#cdd9e5',
                 font=('Segoe UI', 9)).pack(side='left')
        self._fp_amount_var = tk.StringVar(value='100')
        tk.Entry(buy, textvariable=self._fp_amount_var, width=12,
                 font=('Segoe UI', 9)).pack(side='left', padx=(6, 10))
        tk.Button(buy, text='🛒 Buy', command=self._fp_buy, bg='#238636', fg='white',
                  relief='flat', padx=14, pady=4, cursor='hand2').pack(side='left')
        tk.Button(buy, text='💵 Sell selected fund (all units)', command=self._fp_sell,
                  bg='#8b2e2e', fg='white', relief='flat', padx=14, pady=4,
                  cursor='hand2').pack(side='left', padx=(6, 0))

        tk.Label(tab, text='MY FUND HOLDINGS', font=('Segoe UI', 9, 'bold'),
                 fg='#cdd9e5', bg='#0d1117', anchor='w', padx=10).pack(fill='x', pady=(4, 0))
        cols = ('Fund', 'Units', 'NAV (native)', 'Value (MYR)')
        self._fp_tree = ttk.Treeview(tab, columns=cols, show='headings', height=8)
        for c, w in zip(cols, (260, 90, 110, 110)):
            self._fp_tree.heading(c, text=c)
            self._fp_tree.column(c, width=w, anchor='center')
        self._fp_tree.pack(fill='both', expand=True, padx=10, pady=(2, 10))

        self._fp_fund_cb['values'] = ALL_FUND_LABELS
        if ALL_FUND_LABELS and not self._fp_fund_var.get():
            self._fp_fund_var.set(ALL_FUND_LABELS[0])
        self._draw_fund_page()

    def _draw_fund_page(self):
        if not hasattr(self, '_fp_info_var'):
            return
        pts = self._pts_load()
        self._fp_credits_var.set(f"Credits: RM{pts.get('credits', 0):,.2f}")

        lbl = self._fp_fund_var.get()
        if lbl:
            nav, ccy, nav_myr = self._fp_fund_nav_myr(lbl)
            meta = FUND_META.get(lbl, {})
            if nav is None:
                self._fp_info_var.set(f'{lbl} — no price data yet.')
            else:
                holdings = pts.get('fund_holdings', {})
                held = holdings.get(lbl, 0.0)
                held_val = held * (nav_myr if nav_myr is not None else 0.0)
                myr_txt = f'  ·  ≈ RM{nav_myr:,.4f}/unit' if ccy != 'MYR' and nav_myr is not None else ''
                self._fp_info_var.set(
                    f"{lbl}  ({meta.get('dist_class', 'D')}-class, {meta.get('dist_freq', 'quarterly')})\n"
                    f"NAV: {ccy}{nav:,.4f}/unit{myr_txt}\n"
                    f"You hold: {held:,.4f} units  ·  ≈ RM{held_val:,.2f}\n"
                    f"{meta.get('note', '')}"
                )

        # ── Holdings table ───────────────────────────────────────────────
        for row in self._fp_tree.get_children():
            self._fp_tree.delete(row)
        holdings = pts.get('fund_holdings', {})
        for flbl, units in sorted(holdings.items()):
            if units <= 0:
                continue
            nav, ccy, nav_myr = self._fp_fund_nav_myr(flbl)
            if nav is None:
                continue
            val_myr = units * (nav_myr if nav_myr is not None else 0.0)
            self._fp_tree.insert('', 'end', values=(
                flbl, f'{units:,.4f}', f'{ccy}{nav:,.4f}', f'RM{val_myr:,.2f}'))

    def _fp_buy(self):
        """Buy units of the selected fund with Credits (MYR). Every fund is
        open for purchase — no gating on class, currency, or tier."""
        lbl = self._fp_fund_var.get()
        if not lbl:
            return
        try:
            amount = float(self._fp_amount_var.get())
        except ValueError:
            messagebox.showwarning('Fund Page', 'Enter a valid Credits amount.', parent=self)
            return
        if amount <= 0:
            messagebox.showwarning('Fund Page', 'Amount must be positive.', parent=self)
            return

        pts = self._pts_load()
        if amount > pts.get('credits', 0):
            messagebox.showwarning('Fund Page',
                f"Not enough Credits — you have RM{pts.get('credits', 0):,.2f}.", parent=self)
            return

        nav, ccy, nav_myr = self._fp_fund_nav_myr(lbl)
        if nav is None or not nav_myr:
            messagebox.showwarning('Fund Page', f'{lbl} has no live NAV yet — try again shortly.',
                                   parent=self)
            return

        units_bought = amount / nav_myr
        pts['credits'] -= amount
        holdings = pts.setdefault('fund_holdings', {})
        holdings[lbl] = holdings.get(lbl, 0.0) + units_bought
        self._credit_log_add(pts, -amount,
                             f'Fund Page — bought {units_bought:,.4f} units of {lbl} @ RM{nav_myr:,.4f}/unit')
        self._pts_save(pts)
        messagebox.showinfo('Fund Page',
            f'Bought {units_bought:,.4f} units of {lbl}\nfor RM{amount:,.2f}.', parent=self)
        self._draw_fund_page()

    def _fp_sell(self):
        """Sell the entire position in the selected fund back to Credits at
        the current NAV."""
        lbl = self._fp_fund_var.get()
        if not lbl:
            return
        pts = self._pts_load()
        holdings = pts.setdefault('fund_holdings', {})
        units = holdings.get(lbl, 0.0)
        if units <= 0:
            messagebox.showinfo('Fund Page', f"You don't hold any {lbl}.", parent=self)
            return
        nav, ccy, nav_myr = self._fp_fund_nav_myr(lbl)
        if nav is None or not nav_myr:
            messagebox.showwarning('Fund Page', f'{lbl} has no live NAV yet — try again shortly.',
                                   parent=self)
            return
        proceeds = units * nav_myr
        pts['credits'] = pts.get('credits', 0) + proceeds
        holdings[lbl] = 0.0
        self._credit_log_add(pts, proceeds,
                             f'Fund Page — sold {units:,.4f} units of {lbl} @ RM{nav_myr:,.4f}/unit')
        self._pts_save(pts)
        messagebox.showinfo('Fund Page',
            f'Sold {units:,.4f} units of {lbl}\nfor RM{proceeds:,.2f}.', parent=self)
        self._draw_fund_page()
    ...
    # ═══ PAA Reward Economy — commercialization layer ═══════════════════════
    # Tier table per the KoFi Supporter proposal. Payment itself (KoFi/Stripe)
    # happens OUTSIDE this app — a desktop Tkinter app can't process cards —
    # so tiers are activated manually here after payment is confirmed
    # externally. Everything else (drip, limits, raffle eligibility) is
    # enforced in-app off the wallet's tier fields.
    PAA_TIERS = {
        'Free': {
            'price_wk': 0.0,  'daily_drip': 0,      'day7_drip': 0,
            'max_stake': 10_000,  'ads_free': False, 'raffle': None,
            'badge': '', 'color': '#8b949e',
        },
        'Kopitiam Kopi': {
            'price_wk': 3.99, 'daily_drip': 5_000,  'day7_drip': 5_000,
            'max_stake': 25_000,  'ads_free': True,  'raffle': None,
            'badge': '☕', 'color': '#D29922',
        },
        'ZusCoffer': {
            'price_wk': 7.99, 'daily_drip': 12_000, 'day7_drip': 12_000,
            'max_stake': 50_000,  'ads_free': True,
            'raffle': 'Weekly RM10 Zus Gift Card',
            'badge': '☕', 'color': '#58A6FF',
        },
        'StarBockers': {
            'price_wk': 10.99, 'daily_drip': 20_000, 'day7_drip': 30_000,
            'max_stake': 80_000,  'ads_free': True,
    ...
    def _pts_check_stake(self, pts: dict, stake: float):
        """Validate a stake against the min + tier max. Returns error string
        or None if OK — single place ZDWS/ZDOS/PTS all call."""
        if stake < self.PTS_MIN_STAKE:
            return f'Minimum stake is {self.PTS_MIN_STAKE:,.0f} credits.'
        mx = self._pts_max_stake(pts)
        if stake > mx:
            return (f'Maximum single play for your tier '
                    f'({pts.get("tier", "Free")}) is {mx:,.0f} credits.')
        if stake > pts.get('credits', 0):
            return 'Insufficient credits.'
        return None

    def _pts_open_custom_prediction(self, question: str, position: str, stake: float,
                                     deadline) -> tuple:
        """Open a custom YES/NO PTS prediction outside the tab's market
        picker — same save logic as the tab's _submit_prediction button,
        factored out so OPIE (or any other caller) can open one directly.
        deadline: datetime.date. Returns (ok: bool, message: str)."""
        import datetime as _dt
        pts = self._pts_load()
        err = self._pts_check_stake(pts, stake)
        if err:
            return False, err
        days_to = (deadline - _dt.date.today()).days
        if days_to < 7:
            return False, f'Minimum resolution period is 7 days (selected date is only {days_to} day(s) away).'
        pred = {
            'id':         len(pts['predictions']) + len(pts['history']) + 1,
            'category':   'Custom',
            'question':   question,
            'type':       'YES/NO',
            'position':   position,
            ...
            'period_lbl': deadline.strftime('%d %b %Y'),
        ...
        pts['predictions'].append(pred)
        pts['credits']    -= stake
        self._credit_log_add(pts, -stake, f'PTS prediction stake: {position}')
        pts['total_made']  = pts.get('total_made', 0) + 1
        self._pts_save(pts)
        return True, (f'Prediction submitted: "{question}"  ·  {position}  ·  '
                       f'{stake:,.0f} credits  ·  settles {pred["deadline"]}  ·  '
                       f'{pts["credits"]:,.0f} credits remaining.')

    # ── Credit ledger ─────────────────────────────────────────────────────
    # Every credit mutation across PTS/ZDWS/ZDOS/SoR/Rewards funnels through
    # here, so the wallet always carries a full, poppable audit trail —
    # surfaced to the user (click the Cr balance), to admin1 (per-K-ID and
    # the cross-account "universe" view), and usable from SoR settings.
    CREDIT_LOG_MAX = 500   # keep the log bounded; oldest entries roll off

    def _credit_log_add(self, pts: dict, delta: float, reason: str):
        import datetime as _dt
        log = pts.setdefault('credit_log', [])
        log.append({
            'ts':      _dt.datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
            'delta':   float(delta),
            'reason':  reason,
            'balance': float(pts.get('credits', 0)),
        })
        if len(log) > self.CREDIT_LOG_MAX:
            del log[:len(log) - self.CREDIT_LOG_MAX]

    def _open_credit_history_popup(self, pts: dict, title_kid: str = None):
        """🧾 Credit History — pop-up ledger for one wallet. Triggered by
        clicking the Cr balance (Rewards tab) or from the admin dashboard
        for any K-ID."""
        title_kid = title_kid or (CURRENT_KID or 'guest')
        win = tk.Toplevel(self)
        win.title(f'🧾 Credit History — {title_kid}')
        win.configure(bg='#0d1117')
        win.geometry('620x460')

        hdr = tk.Frame(win, bg='#0d1117'); hdr.pack(fill='x', padx=12, pady=(10, 4))
        tk.Label(hdr, text=f'🧾  CREDIT HISTORY  ·  {title_kid}',
                 font=('Segoe UI', 12, 'bold'), fg='#FFD700', bg='#0d1117').pack(side='left')
        tk.Label(hdr, text=f'💰 {pts.get("credits", 0):,.0f} Cr',
                 font=('Segoe UI', 10, 'bold'), fg='#FFD700', bg='#0d1117').pack(side='right')

        cols = ('Date/Time', 'Change', 'Balance After', 'Reason')
        tree = ttk.Treeview(win, columns=cols, show='headings', height=16)
        for c, wd, anch in zip(cols, (140, 90, 110, 260), ('center', 'e', 'e', 'w')):
            tree.heading(c, text=c)
            tree.column(c, width=wd, anchor=anch)
        vsb = ttk.Scrollbar(win, orient='vertical', command=tree.yview)
        tree.configure(yscrollcommand=vsb.set)
        tree.pack(side='left', fill='both', expand=True, padx=(12, 0), pady=(0, 10))
        vsb.pack(side='left', fill='y', pady=(0, 10), padx=(0, 12))
        tree.tag_configure('pos', foreground='#3FB950')
        tree.tag_configure('neg', foreground='#F85149')

        log = pts.get('credit_log', [])
        for entry in reversed(log):   # newest first
            ...
            tree.insert('', 'end', tags=(tag,), values=(
                entry.get('ts', '—'),
                f"{delta:+,.0f}",
                f"{entry.get('balance', 0):,.0f}",
                entry.get('reason', ''),
            ))
        if not log:
            tk.Label(win, text='No credit activity recorded yet.',
                     font=('Segoe UI', 9), fg='#8b949e', bg='#0d1117').pack(pady=20)

    def _rewards_daily_engine(self, pts: dict) -> list:
        """Run all once-per-day accruals for this wallet: the 7-day login
        reward cycle, consecutive-day streak bonuses, and the supporter
        tier's daily credit drip (StarBockers gets the bigger day-7 drop).
        Returns human-readable messages for anything credited. Idempotent
        per calendar day — safe to call on every tab build."""
        import datetime as _dt
        today = _dt.date.today()
        today_s = today.isoformat()
        msgs = []
        if pts.get('login_last_date') == today_s:
            return msgs   # already processed today
        # Consecutive-day check: yesterday → streak continues, else reset
        try:
            last = _dt.date.fromisoformat(pts.get('login_last_date', ''))
            consecutive = (today - last).days == 1
        except Exception:
            consecutive = False
        cyc = pts.get('login_streak_day', 0)
        cyc = (cyc % 7) + 1 if consecutive else 1        # 1..7 cycle position
        run = pts.get('login_streak_len', 0) + 1 if consecutive else 1
        reward = self.LOGIN_REWARDS[cyc - 1]
        pts['credits'] += reward
        self._credit_log_add(pts, reward, f'Daily login reward (day {cyc}/7)')
        msgs.append(f'📅 Day {cyc} login reward: +{reward:,} Cr')
        bonus = self.STREAK_BONUS.get(run)
        if bonus:
            pts['credits'] += bonus
            self._credit_log_add(pts, bonus, f'{run}-day login streak bonus')
            msgs.append(f'🔥 {run}-day streak bonus: +{bonus:,} Cr')
        tier = self._pts_tier(pts)
        if tier['daily_drip']:
            drip = tier['day7_drip'] if cyc == 7 else tier['daily_drip']
            pts['credits'] += drip
            self._credit_log_add(pts, drip, f"{pts.get('tier')} tier daily drip")
            msgs.append(f"{tier['badge']} {pts.get('tier')} daily drip: +{drip:,} Cr")
        pts['login_last_date']  = today_s
        pts['login_streak_day'] = cyc
        pts['login_streak_len'] = run
        # New ad-watch day
        if pts.get('ads_date') != today_s:
            pts['ads_date'] = today_s
            pts['ads_watched'] = 0
        return msgs

    def _sor_config(self) -> dict:
        """Admin-tunable SoR rates (⚙ SoR Settings, inside SCA). Falls back
        to _DEFAULT_SETTINGS['sor_config'] for any missing key."""
        s = load_app_settings()
        cfg = dict(_DEFAULT_SETTINGS['sor_config'])
        cfg.update(s.get('sor_config', {}) or {})
        return cfg

    def _sor_save_config(self, cfg: dict):
        s = load_app_settings()
        s['sor_config'] = cfg
        save_app_settings(s)
    ...
    def _sor_run_and_popup(self):
        ...
        import datetime as _dt
        # Prefer the LIVE SCA window's holdings if it's currently open — the
        # window keeps its own working copy (edits there don't write back to
        # the startup preload), so reading the stale preload here is why SoR
        # could see "no holdings" even with real saved positions on screen.
        sca_win = getattr(self, '_sca_win', None)
        if sca_win is not None and sca_win.winfo_exists():
            ports = getattr(sca_win, '_portfolios', None) or {}
        else:
        ...
        if pts.get('sor_last_date') == today_s:
            messagebox.showinfo('🎁 Save for Rewards (SoR)',
                'Already credited for today. Use ⚙ SoR Settings → '
                '🔁 Force Re-run SoR (today) to bypass this while testing.',
                parent=self)
        ...
        if not holdings_all:
            self._pts_save(pts)
            messagebox.showinfo('🎁 Save for Rewards (SoR)',
                'No saved SCA holdings found (0 positions with shares > 0), '
                'so nothing to reward today.\n\n'
                'If you have holdings visible in the Portfolio (SCA) window, '
                'open that window first, then run SoR again — it now reads '
                'live from that window while it\'s open.', parent=self)
            return   # nothing saved in SCA yet — nothing to reward

        cfg = self._sor_config()
        # Default mode is the automatic baseline for every valid K-ID — no
        # opt-in needed. Custom mode only ever applies if THIS K-ID has
        # explicitly switched it on for themselves (see ⚙ SoR Mode).
        mode = 'custom' if pts.get('sor_custom_enabled') else 'default'
        ...
        for sym, region, units in holdings_all:
            ...
            if mode == 'custom':
                # Custom mode: X credits per Y units — Y is THIS K-ID's own
                # sor_custom_threshold (falls back to admin default if never
                # set); X (custom_credits) is always the admin-set value,
                # never user-editable.
                my_threshold = pts.get('sor_custom_threshold') or cfg['custom_threshold']
                cr = SaveForRewards.calculate_position_cr(
                    region, units, is_custom=True,
                    custom_threshold=my_threshold,
                    custom_credits=cfg.get('custom_credits', 1))
            else:
                # Default (preset) mode — a minimum holding applies before
                # any position earns at all, separate per region: MY
                # (Lotto33 + all other MY stocks) needs a much larger stake
                # than US shares/ETFs to qualify.
                min_units = cfg.get('us_min_units', 0) if region == 'US' \
                    else cfg.get('my_min_units', 0)
                if units < min_units:
                    cr = 0
                elif region == 'US':
                    cr = SaveForRewards.calculate_position_cr(
                        region, units, is_custom=True,
                        custom_threshold=cfg['us_share_per_cr'])
                elif region == 'MY' and is_lotto:
                    cr = int(units) * int(cfg['my_lotto_cr_per_unit'])
                elif region == 'MY':
                    # MY stocks outside Lotto33 also earn under default mode —
                    # a separate base-earn rate, distinct from the Road to
                    # Reward diversification bonus below.
                    cr = int(units) * int(cfg.get('my_stock_per_cr', 1))
                else:
                    cr = 0
            if cr > 0:
                base_cr += cr
        ...
        if base_cr <= 0:
            self._pts_save(pts)
            messagebox.showinfo('🎁 Save for Rewards (SoR)',
                f'Found {len(holdings_all)} saved position(s), but none met '
                f'the current SoR rate rules (e.g. below the minimum holding '
                f'threshold), so 0 Cr was earned today.', parent=self)
            return

        tier_name = pts.get('tier', 'Free')
        tier      = self._pts_tier(pts)
        default_mult = cfg['fep_multiplier'] if tier_name == 'Beta Supporter' else (
            cfg['supporter_multiplier'] if tier_name != 'Free' else 1.0)
        mult = tier.get('sor_mult', default_mult)
        total_cr = int(base_cr * mult)
        pts['credits'] = pts.get('credits', 0) + total_cr
        self._credit_log_add(pts, total_cr, f'SoR — saved SCA holdings ({tier_name} {mult:g}x)')
        ...
        road_cr = next((cr for pct, cr in sorted(cfg['road_thresholds'], reverse=True)
                         if target_pct >= pct), 0)
        ...
        if target_pct >= 100 and pts.get('sor_road_claimed') != True:
            ...
            self._credit_log_add(pts, road_cr, 'SoR — Road to Reward 100% bonus')
    ...
    def _rewards_toggle_sor_mode(self):
        """⚙ SoR Mode — each K-ID opts into Custom themselves; Default needs
        no action at all and is what a new/valid K-ID starts on automatically.
        Custom mode's rate (Y units → 1 Cr) is now PER-USER, not admin-only —
        each K-ID sets their own 'sor_custom_threshold' here; admin's
        cfg['custom_threshold'] is only the fallback/starting suggestion."""
        pts = self._pts_load()
        if pts.get('sor_custom_enabled'):
            pts['sor_custom_enabled'] = False
            messagebox.showinfo('SoR Mode', 'Switched back to Default SoR rates.', parent=self)
        else:
            cfg = self._sor_config()
            suggested = pts.get('sor_custom_threshold') or cfg['custom_threshold']
            is_admin = (CURRENT_KID or '').strip().lower() == ADMIN_KID
            from tkinter import simpledialog
            if is_admin:
                x_credits = cfg.get('custom_credits', 1)
                intro = (f'Custom mode pays a fixed {x_credits:g} Cr (X, admin-locked '
                          f'base credit unit) for every Y units you set below.\n\n')
            else:
                # Non-admins never see X's value — only that a fixed credit
                # rate applies and that Y (units) is theirs to set.
                intro = ('Custom mode pays a fixed credit amount per Y units '
                          '(the credit rate is set by the admin) — you only '
                          'set Y below.\n\n')
            val = simpledialog.askfloat('SoR Mode — Custom rate',
                intro +
                f'Set YOUR OWN Y — units per credit — applies to '
                f'every saved position, any market or Lotto33 status, no '
                f'minimum-holding gate.\n\n'
                f'Admin default is {cfg["custom_threshold"]:g}.',
                initialvalue=suggested, minvalue=0.0001, parent=self)
            if val is None:
                return   # cancelled
            pts['sor_custom_threshold'] = val
            pts['sor_custom_enabled'] = True
            confirm_msg = (f'Custom SoR mode enabled: {cfg.get("custom_credits", 1):g} '
                            f'Cr per {val:g} units held.' if is_admin else
                            f'Custom SoR mode enabled: your rate is now {val:g} units per credit.')
            messagebox.showinfo('SoR Mode', confirm_msg, parent=self)
        self._pts_save(pts)
        self._build_rewards_tab()

    def _rewards_watch_ad(self, pts: dict):
        """Claim one ad reward (random 100–999 Cr, max 3/day, free tier only).
        PLACEHOLDER for a real ad network: a local Tkinter app can't serve
        Google Ads, so this simulates the reward flow the future web version
        will wire to actual ad completion callbacks. Returns (ok, message)."""
        import datetime as _dt, random as _rnd
        if self._pts_tier(pts)['ads_free']:
            return False, 'Supporters have an ad-free experience — the tier drip replaces ad rewards.'
        today_s = _dt.date.today().isoformat()
        if pts.get('ads_date') != today_s:
            pts['ads_date'] = today_s
            pts['ads_watched'] = 0
        if pts.get('ads_watched', 0) >= self.AD_DAILY_LIMIT:
            return False, f'Daily ad limit reached ({self.AD_DAILY_LIMIT}/day). Come back tomorrow!'
        reward = _rnd.randint(self.AD_REWARD_MIN, self.AD_REWARD_MAX)
        pts['credits'] += reward
        self._credit_log_add(pts, reward, 'Watched ad reward')
        pts['ads_watched'] = pts.get('ads_watched', 0) + 1
        return True, (f'📺 Ad reward: +{reward:,} Cr  '
                      f'({pts["ads_watched"]}/{self.AD_DAILY_LIMIT} today)')

    # ── WebView ad area ──────────────────────────────────────────────────────
    # POLICY NOTE (important): rewards here are granted for TIME VIEWED, never
    # for clicking ads. Incentivising clicks is an outright AdSense-policy
    # violation (account termination), and AdSense itself is web-only — it may
    # not be served inside desktop software. So this infrastructure is built
    # for the compliant path: the popup can point at a HOSTED web page you own
    # (settings key 'ad_page_url'), where a rewarded-ads product (e.g. Google
    # H5 Games Ads / Ad Manager rewarded) runs in a real browser context.
    # Until that page exists, a bundled local HTML placeholder renders instead
    # so the whole click→view→claim flow is already wired end-to-end.
    AD_VIEW_SECONDS = 15   # minimum view time before the claim unlocks

    def _rewards_ad_page(self) -> str:
        """URL for the ad page. Priority: settings['ad_page_url'] (your hosted
        page with real rewarded ads) → generated local placeholder file."""
        s = load_app_settings()
        url = (s.get('ad_page_url') or '').strip()
        if url:
            return url
        path = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                            'paa_ad_page.html')
        client = (s.get('adsense_client') or 'ca-pub-XXXXXXXXXXXXXXXX').strip()
        slot   = (s.get('adsense_slot')   or '0000000000').strip()
        html = f"""<!DOCTYPE html>
<html><head><meta charset="utf-8"><title>PAA — Sponsored</title>
<style>
  body {{ background:#0d1117; color:#cdd9e5; font-family:Segoe UI,Arial,sans-serif;
         margin:0; padding:24px; text-align:center; }}
  .slot {{ border:1px dashed #30363d; border-radius:8px; padding:40px 16px;
          margin:16px auto; max-width:640px; color:#8b949e; }}
  .note {{ font-size:11px; color:#8b949e; max-width:640px; margin:12px auto; }}
</style>
<!-- Real AdSense markup, inert until a valid publisher ID is configured in
     paa_settings.json ('adsense_client' / 'adsense_slot') AND this page is
     served from an approved domain — AdSense does not fill file:// pages. -->
...
<div class="slot">
  ...
  Ad placeholder — configure 'ad_page_url' in paa_settings.json to point at
  your hosted rewarded-ads page once it exists.
</div>
<div class="note">Reward is granted for viewing time only. Do not click ads
to earn — incentivised clicking violates ad-network policy.</div>
</body></html>"""
        try:
    ...
    def _rewards_open_ad_popup(self):
        """The click-and-earn flow: open the ad page in a pop-up WebView
        (pywebview native window if installed, else the system browser), and
        show a countdown claim dialog — the claim button unlocks only after
        AD_VIEW_SECONDS of view time, then credits via the same
        _rewards_watch_ad gate (daily limit / tier rules unchanged)."""
        url = self._rewards_ad_page()
        opened_via = 'browser'
        if _HAS_PYWEBVIEW:
            def _pw():
                try:
                    _pywebview.create_window('PAA — Sponsored', url,
        ...
        tk.Label(dlg, text='📺  Ad viewing in progress', font=('Segoe UI', 11, 'bold'),
                 fg='#FFD700', bg='#0d1117').pack(pady=(14, 2))
        tk.Label(dlg, text=f'(opened via {opened_via} — keep it open)',
                 font=('Segoe UI', 8), fg='#8b949e', bg='#0d1117').pack()
        cnt_var = tk.StringVar(value=f'{self.AD_VIEW_SECONDS}s')
        tk.Label(dlg, textvariable=cnt_var, font=('Segoe UI', 18, 'bold'),
                 fg='#cdd9e5', bg='#0d1117').pack(pady=4)
        claim_btn = tk.Button(dlg, text='🎁 Claim reward', state='disabled',
                              font=('Segoe UI', 9, 'bold'), bg='#21262d',
                              fg='#FFD700', relief='flat', padx=12, pady=5)
        claim_btn.pack(pady=(2, 10))

        def _claim():
            p = self._pts_load()
            ok, msg = self._rewards_watch_ad(p)
            if ok:
                self._pts_save(p)
            dlg.destroy()
            messagebox.showinfo('Ad Reward', msg, parent=self)
            self._build_rewards_tab()
        claim_btn.config(command=_claim)

        def _tick(remaining):
            if not dlg.winfo_exists():
                return
            if remaining <= 0:
                cnt_var.set('✓ view time met')
                claim_btn.config(state='normal')
            else:
                cnt_var.set(f'{remaining}s')
                dlg.after(1000, _tick, remaining - 1)
        _tick(self.AD_VIEW_SECONDS)

    def _rewards_tier_weeks(self, pts: dict) -> float:
        """How many weeks the current tier has been continuously active."""
        import datetime as _dt
        try:
            start = _dt.date.fromisoformat(pts.get('tier_started', ''))
            return (_dt.date.today() - start).days / 7.0
        except Exception:
            return 0.0
    ...
    def _rewards_raffle_status(self, pts: dict) -> str:
        tier = self._pts_tier(pts)
        if not tier['raffle']:
            return 'No raffle access at this tier.'
        wks = self._rewards_tier_weeks(pts)
        if wks >= self.RAFFLE_MIN_WEEKS:
            return f'🎟️ 1 ticket — eligible for: {tier["raffle"]}'
        return (f'Raffle ({tier["raffle"]}) unlocks after '
                f'{self.RAFFLE_MIN_WEEKS} weeks — {wks:.1f} wk so far.')

    def _rewards_grant_immediate_drip(self, pts: dict, tier_name: str):
        """Catch-up credit for the SAME calendar day a tier is activated.
        _rewards_daily_engine only fires once per day — so if today's daily
        login/drip already ran (as Free, or an old tier) before this upgrade,
        the new tier's drip would otherwise sit unpaid until tomorrow. This
        tops it up immediately, same-day, same as a fresh purchase should
        feel. Guarded by 'tier_drip_date' so it never double-pays even if
        called twice for the same tier on the same day."""
        import datetime as _dt
        today_s = _dt.date.today().isoformat()
        tier = self.PAA_TIERS.get(tier_name, {})
        drip = tier.get('daily_drip', 0)
        if not drip:
            return   # Free / no drip tier — nothing to catch up
        already_ran_today = pts.get('login_last_date') == today_s
        already_topped_up = pts.get('tier_drip_date') == today_s
        if not already_ran_today or already_topped_up:
            return   # engine hasn't run yet today (will pay correctly) or already topped up
        cyc = pts.get('login_streak_day', 0) or 1
        amount = tier['day7_drip'] if cyc == 7 else drip
        pts['credits'] += amount
        self._credit_log_add(pts, amount,
                              f'{tier_name} tier drip (activated mid-day, catch-up)')
        pts['tier_drip_date'] = today_s

    def _rewards_set_tier(self, tier_name: str):
        """Activate/cancel a supporter tier. Manual by design: KoFi/Stripe
        payment happens outside the app; this is the hook you flip once a
        payment is confirmed there."""
        import datetime as _dt
        if tier_name not in self.PAA_TIERS:
            return
        pts = self._pts_load()
        prev = pts.get('tier', 'Free')
        pts['tier'] = tier_name
        if tier_name == 'Free':
            pts['tier_started'] = ''
        elif prev != tier_name:
            pts['tier_started'] = _dt.date.today().isoformat()
            self._rewards_grant_immediate_drip(pts, tier_name)
        self._pts_save(pts)
    ...
    def _raffle_entrants(self, tier_name: str) -> list:
        """All K-IDs currently eligible for `tier_name`'s raffle: active on
        that tier for at least RAFFLE_MIN_WEEKS. One ticket each, per the
        proposal — no weighting. Scans every wallet in pts_by_user."""
        import datetime as _dt
        s = load_app_settings()
        today = _dt.date.today()
        entrants = []
        for kid, w in s.get('pts_by_user', {}).items():
            if w.get('tier') != tier_name:
                continue
            try:
                start = _dt.date.fromisoformat(w.get('tier_started', ''))
            except Exception:
                continue
            if (today - start).days / 7.0 >= self.RAFFLE_MIN_WEEKS:
                entrants.append(kid or '(guest)')
        return entrants

    def _raffle_run_draw(self, tier_name: str):
        """Run one raffle draw for a tier, guarded by its cadence (weekly for
        ZusCoffer, monthly for StarBockers) so it can't be re-rolled early.
        Winner + entrant count are persisted to settings['raffle_history'].
        NOTE: the draw picks a winner; the gift card itself is fulfilled
        manually outside the app (there's no Zus/Starbucks API here)."""
        import datetime as _dt, random as _rnd
        cfg = self.RAFFLES.get(tier_name)
        if not cfg:
            return
        s = load_app_settings()
        hist = s.setdefault('raffle_history', [])
        today = _dt.date.today()
        last = next((h for h in reversed(hist) if h['tier'] == tier_name), None)
        if last:
            try:
                last_d = _dt.date.fromisoformat(last['date'])
                left = cfg['cadence_days'] - (today - last_d).days
                if left > 0:
                    messagebox.showinfo('Raffle',
                        f'{tier_name} draw already ran on {last["date"]} '
                        f'(winner: {last["winner"]}).\nNext draw in {left} day(s).',
                        parent=self)
                    return
            except Exception:
                pass
        entrants = self._raffle_entrants(tier_name)
        if not entrants:
            messagebox.showinfo('Raffle',
                f'No eligible {tier_name} entrants yet — needs an active '
                f'subscription of {self.RAFFLE_MIN_WEEKS}+ weeks.', parent=self)
            return
        winner = _rnd.choice(entrants)
        hist.append({'tier': tier_name, 'date': today.isoformat(),
                     'winner': winner, 'prize': cfg['prize'],
                     'entrants': len(entrants)})
        save_app_settings(s)
        messagebox.showinfo('Raffle',
            f'🎟️ {tier_name} draw complete!\n\n'
            f'Winner: {winner}\nPrize: {cfg["prize"]}\n'
            f'Entrants: {len(entrants)} (1 ticket each)\n\n'
            f'Fulfil the gift card manually to the winner.', parent=self)
        self._build_rewards_tab()

    # ── Admin dashboard (admin1 only) ────────────────────────────────────────

    # ── Stripe subscription sync ─────────────────────────────────────────────
    # Desktop-app reality: Stripe webhooks need a PUBLIC endpoint, which a
    # local Tkinter app doesn't have (the Stripe CLI's `stripe listen
    # --forward-to localhost` is for testing a local dev server, not a
    # deployed desktop app). So instead of webhooks we POLL: the admin's
    # "Sync Stripe" button pulls the subscription list over REST and applies
    # it locally. Setup expected on the Stripe side:
    #   1. One weekly recurring Price per tier: RM3.99 / RM7.99 / RM10.99
    #   2. Payment Links (or Checkout) that set metadata kid=<K-ID> on the
    #      subscription so it can be matched to a local account; if metadata
    #      is absent, the price amount (in sen) is used to infer the tier.
    #   3. Admin's SECRET key (sk_test_… from the sandbox while testing)
    #      entered once in the dashboard — stored in paa_settings.json, so
    #      only do this on the admin's own machine, never a distributed copy.
    def _stripe_api_get(self, path: str, params: str = ''):
        """Minimal Stripe REST GET via urllib (no SDK dependency).
        Returns (ok, parsed_json_or_error_string)."""
        import urllib.request, urllib.error, json as _json
        key = (load_app_settings().get('stripe_api_key') or '').strip()
        if not key:
        ...
        try:
            ...
            try:
                ...
                return False, err.get('error', {}).get('message', str(e))
            except Exception:
                return False, str(e)
        except Exception as e:
            return False, str(e)
    ...
    def _stripe_amount_tier(self, amount_sen: int):
        """Map a Stripe price amount (sen) to a tier name via PAA_TIERS."""
        for name, t in self.PAA_TIERS.items():
            if name != 'Free' and int(round(t['price_wk'] * 100)) == amount_sen:
                return name
        return None

    def _stripe_sync_subscriptions(self):
        """Pull all subscriptions from Stripe and match each to a K-ID.
        HOSTED-PAYMENTS matching (primary): each Payment Link the app opens
        carries ?client_reference_id=<K-ID>; Stripe stores that on the
        Checkout Session, so we fetch sessions first and map
        subscription_id → client_reference_id. metadata.kid (if you ever set
        it) is the fallback. Matched subs are cached per K-ID in
        settings['stripe_subs'] and applied: active/trialing → set the tier
        (stamping tier_started if new); canceled/unpaid → downgrade to Free."""
        import datetime as _dt
        # 1) Checkout sessions → subscription-to-KID reference map
        ref_map = {}
        ok_s, sess = self._stripe_api_get('checkout/sessions', 'limit=100')
        if ok_s:
            for cs in sess.get('data', []):
                sub_id = cs.get('subscription')
                ref    = (cs.get('client_reference_id') or '').strip().lower()
                if sub_id and ref:
                    ref_map[sub_id] = ref
        # 2) Subscriptions
        ok, data = self._stripe_api_get('subscriptions', 'status=all&limit=100')
        if not ok:
            return False, data
        s = load_app_settings()
        wallets = s.setdefault('pts_by_user', {})
        sub_map, unmatched = {}, []
        for sub in data.get('data', []):
            kid = (ref_map.get(sub.get('id'))
                   or (sub.get('metadata', {}).get('kid') or '').strip().lower())
            item = (sub.get('items', {}).get('data') or [{}])[0]
            amount = int(item.get('price', {}).get('unit_amount') or 0)
            tier = self._stripe_amount_tier(amount)
            status = sub.get('status', 'unknown')
            period_end = sub.get('current_period_end')
            end_s = (_dt.datetime.fromtimestamp(period_end).strftime('%Y-%m-%d')
                     if period_end else '—')
            if not kid:
                unmatched.append(f'{sub.get("id", "?")[:14]} (RM{amount/100:.2f}, {status})')
                continue
            sub_map[kid] = {'status': status, 'tier': tier or '?',
                            'period_end': end_s, 'amount': amount}
            # Apply to the local wallet
            w = wallets.get(kid)
            if w is None:
                continue           # Stripe sub for a K-ID that doesn't exist here
            if status in ('active', 'trialing') and tier:
                if w.get('tier') != tier:
                    w['tier'] = tier
                    w['tier_started'] = _dt.date.today().isoformat()
            elif status in ('canceled', 'unpaid', 'incomplete_expired'):
                if w.get('tier') not in (None, 'Free'):
                    w['tier'] = 'Free'
                    w['tier_started'] = ''
        s['stripe_subs'] = sub_map
        s['stripe_last_sync'] = _dt.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        save_app_settings(s)
        msg = f'Synced {len(sub_map)} matched subscription(s).'
        if unmatched:
            msg += (f'\n{len(unmatched)} unmatched (no metadata.kid): '
                    + ', '.join(unmatched[:5]))
        return True, msg

    # ── Auto-verify (self-check, no admin needed) ──────────────────────────
    # Runs for the SIGNED-IN K-ID only, using a RESTRICTED Stripe key (read
    # access to Checkout Sessions + Subscriptions only — safe to ship inside
    # the app, unlike the admin's full secret key). Triggered automatically
    # on app open and right after a Subscribe click; typically resolves in
    # a couple of seconds since it's one small filtered API call, well
    # inside the 90s window after checkout completes.
    def _stripe_restricted_get(self, path: str, params: str = ''):
        import urllib.request, urllib.error, json as _json
        s = load_app_settings()
        mode = s.get('stripe_mode', 'test')  # 'test' while building, 'live' once ready
        rkey = (s.get('stripe_restricted_key_live' if mode == 'live'
                       else 'stripe_restricted_key_test') or '').strip()
        if not rkey:
            return False, f'No restricted key configured for {mode} mode.'
        url = f'https://api.stripe.com/v1/{path}'
        if params:
            url += f'?{params}'
        req = urllib.request.Request(url, headers={'Authorization': f'Bearer {rkey}'})
        try:
    ...
    def _stripe_autoverify_current_user(self, silent: bool = False):
        """Self-check for CURRENT_KID ONLY — never touches any other user's
        wallet, matching strictly on this K-ID's own client_reference_id /
        stored subscription id. Two things happen here, per device:
        1) UPGRADE: find a completed Checkout Session tagged to this K-ID
           that hasn't been applied yet → activate the tier.
        2) DOWNGRADE: if this K-ID already has an active paid tier with a
           known subscription id, re-check its live status → drop to Free
           if canceled/unpaid/past_due-expired. Runs off the UI thread."""
        kid = (CURRENT_KID or '').strip().lower()
        if not kid:
            return
        ...
        def _worker():
            import datetime as _dt
            s = load_app_settings()
            applied = set(s.get('stripe_applied_sessions', []))
            wallets = s.setdefault('pts_by_user', {})
            w = wallets.setdefault(kid, {})
            changed = False
            notify_msg = None

            # ── 1) UPGRADE: scan recent completed sessions for this K-ID ──
            ok, sess = self._stripe_restricted_get(
                'checkout/sessions', 'status=complete&limit=20')
            if ok:
                match = None
                for cs in sess.get('data', []):
                    ref = (cs.get('client_reference_id') or '').strip().lower()
                    if ref == kid and cs.get('id') not in applied \
                            and cs.get('payment_status') == 'paid':
                        match = cs
                        break
                if match:
                    ok2, li = self._stripe_restricted_get(
                        'checkout/sessions/%s/line_items' % match['id'])
                    amount = 0
                    if ok2:
                        items = li.get('data', [])
                        if items:
                            amount = int(items[0].get('price', {}).get('unit_amount') or 0)
                    tier = self._stripe_amount_tier(amount)
                    if tier:
                        if w.get('tier') != tier:
                            w['tier'] = tier
                            w['tier_started'] = _dt.date.today().isoformat()
                            self._rewards_grant_immediate_drip(w, tier)
                            notify_msg = f'✅ Payment confirmed — {tier} tier is now active!'
                        if match.get('subscription'):
                            w['stripe_sub_id'] = match['subscription']
                        applied.add(match['id'])
                        changed = True

            # ── 2) DOWNGRADE: re-check this K-ID's own known subscription ──
            sub_id = w.get('stripe_sub_id')
            if sub_id and w.get('tier') not in (None, 'Free'):
                ok3, sub = self._stripe_restricted_get('subscriptions/%s' % sub_id)
                if ok3:
                    status = sub.get('status', 'unknown')
                    if status in ('canceled', 'unpaid', 'incomplete_expired'):
                        w['tier'] = 'Free'
                        w['tier_started'] = ''
                        notify_msg = ('ℹ️ Your subscription is no longer active — '
                                      'tier reverted to Free.')
                        changed = True

            if changed:
                s['stripe_applied_sessions'] = list(applied)[-200:]  # bounded
                save_app_settings(s)
            if notify_msg and not silent:
                def _notify():
                    messagebox.showinfo('Subscription status', notify_msg, parent=self)
                    if hasattr(self, '_refresh_rewards_ui'):
                        try:
                            self._refresh_rewards_ui()
                        except Exception:
                            pass
                self.after(0, _notify)
        threading.Thread(target=_worker, daemon=True).start()

    # Live Stripe-hosted Payment Links (admin panel entries override these).
    ...
    TIER_BENEFITS = {
        'Kopitiam Kopi': ('Basic Supporter', [
            '35,000 Credits/week (≈5,000 daily)',
            'Ads-Free experience',
            '25K Credit max play per order']),
        'ZusCoffer': ('Premium Supporter', [
            '84,000 Credits/week (≈12,000 daily)',
            'Ads-Free experience',
            '50K Credit max play per order',
            'Weekly RM10 Zus Gift Card raffle eligibility*']),
        'StarBockers': ('VIP Supporter', [
            '140,000 Credits/week + 10,000 Credit streak bonus',
            'Ads-Free experience',
            '80K Credit max play per order',
            'Monthly RM30 Starbucks Gift Card raffle eligibility*']),
    }

    def _stripe_paylink(self, tier_name: str) -> str:
        """Effective Payment Link for a tier: admin-panel override → baked-in
        live default."""
        pl = load_app_settings().get('stripe_paylinks', {})
        return (pl.get(tier_name, '') or
                self.STRIPE_PAYLINK_DEFAULTS.get(tier_name, '')).strip()

    def _open_credit_buy_popup(self):
        """💳 Get Credits — pop-up storefront: the three supporter tiers with
        their benefit lists and hosted-checkout Subscribe buttons, plus the
        free earning routes. All payment happens on Stripe's hosted pages;
        credits arrive via the tier's daily drip once the subscription syncs."""
        dlg = tk.Toplevel(self)
        dlg.title('💳 Get Credits')
        dlg.configure(bg='#0d1117')
        dlg.geometry('460x900')
        dlg.transient(self)

        tk.Label(dlg, text='💳  GET CREDITS', font=('Segoe UI', 13, 'bold'),
                 fg='#FFD700', bg='#0d1117').pack(pady=(12, 0))
        tk.Label(dlg, text='Weekly supporter subscriptions — paid securely on '
                           'Stripe\'s hosted checkout',
                 font=('Segoe UI', 8), fg='#8b949e', bg='#0d1117').pack(pady=(0, 8))

        note = tk.Frame(dlg, bg='#2a1f0a', highlightbackground='#D29922',
                        highlightthickness=1, padx=10, pady=6)
        note.pack(fill='x', padx=14, pady=(0, 8))
        tk.Label(note, text='⚠️  Please note',
                 font=('Segoe UI', 8, 'bold'), fg='#D29922', bg='#2a1f0a').pack(anchor='w')
        tk.Label(note,
                 text='This is a personal, independently-run project — not a registered '
                      'company. Payment is processed by Stripe to the developer\'s own '
                      'personal Stripe account, under their own name, not a business name. '
                      'Subscribe only if you\'re comfortable with that.',
                 font=('Segoe UI', 7), fg='#cdd9e5', bg='#2a1f0a',
        ...
        for name, t in self.PAA_TIERS.items():
            ...
            row = tk.Frame(card, bg='#161b22'); row.pack(fill='x')
            tk.Label(row, text=f"{t['badge']} {name} — {title}",
                     font=('Segoe UI', 10, 'bold'), fg=t['color'],
                     bg='#161b22').pack(side='left')
            tk.Label(row, text=f"RM{t['price_wk']:.2f}/wk",
                     font=('Segoe UI', 10, 'bold'), fg='#cdd9e5',
                     bg='#161b22').pack(side='right')
            for b in bullets:
                tk.Label(card, text=f'  •  {b}', font=('Segoe UI', 8),
                         fg='#cdd9e5', bg='#161b22', anchor='w',
                         justify='left').pack(fill='x')
            if name == cur_tier:
                tk.Label(card, text='✓ Your current tier', font=('Segoe UI', 8, 'bold'),
                         fg='#3FB950', bg='#161b22').pack(anchor='w', pady=(4, 0))
            else:
                tk.Button(card, text='💳  Subscribe on Stripe',
                          font=('Segoe UI', 9, 'bold'), bg='#635BFF', fg='#ffffff',
                          relief='flat', padx=10, pady=4, cursor='hand2',
                          command=lambda n=name: (dlg.destroy(),
                                                  self._rewards_open_subscribe(n))
                          ).pack(anchor='e', pady=(6, 0))

        tk.Label(dlg, text='*Raffle eligibility after 2+ weeks of active subscription.\n'
                           'Credits arrive via the daily tier drip once your subscription '
                           'syncs (admin sync runs periodically).',
                 font=('Segoe UI', 7), fg='#8b949e', bg='#0d1117',
                 wraplength=400, justify='left').pack(padx=14, pady=(6, 2), anchor='w')

        free_f = tk.Frame(dlg, bg='#161b22', highlightbackground='#30363d',
                          highlightthickness=1, padx=12, pady=6)
        free_f.pack(fill='x', padx=14, pady=(4, 12))
        tk.Label(free_f, text='Prefer free?  Daily login (100–700 Cr + streak bonuses) '
                              'and up to 3 ad rewards/day (100–999 Cr each) on the '
                              'Rewards tab.',
                 font=('Segoe UI', 8), fg='#cdd9e5', bg='#161b22',
                 wraplength=380, justify='left').pack(anchor='w')

        # ==========================================================
        # Future Expansion Pack (Beta Supporter)
        # ==========================================================
        ...
        support_frame.pack(fill="x", padx=14, pady=(0, 10))
        fep_description = (
            "Support the future development of PredictTheSharss.\n\n"
            "RM2.99 / Week\n\n"
            "Benefits\n"
        ...
        # Beta Supporter). Lets a user drop feedback/ideas that get
        # saved to paa_settings.json for the admin to review.
        # ==========================================================
        chat_frame = tk.Frame(dlg, bg='#0d1117')
        chat_frame.pack(fill='x', padx=14, pady=(0, 14))
        tk.Button(
            chat_frame,
            text='💬  Suggest a Feature / Chat With Us',
            command=lambda: self._open_suggestion_chat(dlg),
            bg='#1f6feb', fg='white',
        ...
        tk.Label(
            chat_frame,
            text='Supporter perk — Kopitiam Kopi, ZusCoffer, StarBockers '
                 'and Beta Supporter tiers can chat with us directly and '
                 'suggest what gets built next. Free tier can upgrade above '
                 'to unlock it.',
            font=('Segoe UI', 7), fg='#8b949e', bg='#0d1117',
            wraplength=400, justify='left'
        ).pack(anchor='w', pady=(4, 0))

    def _open_suggestion_chat(self, parent=None):
        """💬 Suggestion / feedback box — available to users on ALL tiers
        (Free, Kopitiam Kopi, ZusCoffer, StarBockers, Beta Supporter).
        Messages are stored locally in paa_settings.json under
        'suggestions' so an admin can review them later; there is no
        live network chat involved."""
        pts = self._pts_load() if hasattr(self, '_pts_load') else {}
        cur_tier = pts.get('tier', 'Free') if isinstance(pts, dict) else 'Free'
        kid = (CURRENT_KID or 'guest').strip().lower() if 'CURRENT_KID' in globals() else 'guest'

        if cur_tier == 'Free':
            messagebox.showinfo(
                'Supporter Perk',
                '💬 Chat & Suggestions is a supporter perk.\n\n'
                'Upgrade to Kopitiam Kopi, ZusCoffer, StarBockers or the '
                'Beta Supporter tier to chat with us and suggest features.',
                parent=parent or self)
            return

        box = tk.Toplevel(parent or self)
        box.title('💬 Suggestions & Chat')
        box.configure(bg='#0d1117')
        box.geometry('420x420')
        box.transient(parent or self)

        tk.Label(box, text='💬  SUGGESTIONS & CHAT', font=('Segoe UI', 12, 'bold'),
                 fg='#FFD700', bg='#0d1117').pack(pady=(12, 2))
        tk.Label(box, text=f'Signed in as {kid}  •  Tier: {cur_tier}',
                 font=('Segoe UI', 8), fg='#8b949e', bg='#0d1117').pack()
        tk.Label(box, text='Open to all 4 supporter tiers — tell us what '
                           "you'd like to see next (e.g. HK/SG markets, "
                           'new reward types, UI ideas).',
                 font=('Segoe UI', 8), fg='#cdd9e5', bg='#0d1117',
                 wraplength=380, justify='left').pack(padx=14, pady=(4, 8))

        txt = tk.Text(box, height=8, width=44, bg='#161b22', fg='#cdd9e5',
                      insertbackground='#cdd9e5', wrap='word', relief='flat')
        txt.pack(padx=14, pady=(0, 8), fill='both', expand=True)

        status_lbl = tk.Label(box, text='', font=('Segoe UI', 8),
                              fg='#3FB950', bg='#0d1117')
        status_lbl.pack()
        ...
        def send_suggestion():
            ...
            if not msg:
                status_lbl.config(text='Type a message first.', fg='#F85149')
                return
            import datetime as _dt
            settings = load_app_settings()
            entry = {
                'kid': kid, 'tier': cur_tier, 'message': msg,
                'sent': _dt.datetime.now().strftime('%Y-%m-%d %H:%M'),
            }
            settings.setdefault('suggestions', []).append(entry)
            save_app_settings(settings)
            txt.delete('1.0', 'end')
            status_lbl.config(text='✓ Sent — thanks for the suggestion!', fg='#3FB950')

        tk.Button(box, text='Send Suggestion', command=send_suggestion,
                 bg='#238636', fg='white', activebackground='#2ea043',
                 activeforeground='white', relief='flat', cursor='hand2',
                 font=('Segoe UI', 9, 'bold')).pack(pady=(0, 12), padx=14, fill='x')

    def _open_admin_credit_universe(self):
        """🧾 Credit Universe — every credit event, for every K-ID, in one
        merged/sorted ledger. Admin-only audit trail across the whole
        user base (the "universe" of credit issuance)."""
        if (CURRENT_KID or '').strip().lower() != ADMIN_KID:
            messagebox.showwarning('Admin', 'Admin access only.', parent=self)
            return
        s       = load_app_settings()
        wallets = s.get('pts_by_user', {})

        win = tk.Toplevel(self)
        win.title('🧾 Credit Universe — All K-IDs')
        win.configure(bg='#0d1117')
        win.geometry('820x520')

        hdr = tk.Frame(win, bg='#0d1117'); hdr.pack(fill='x', padx=12, pady=(10, 4))
        tk.Label(hdr, text='🧾  CREDIT UNIVERSE  ·  ALL K-IDS',
                 font=('Segoe UI', 12, 'bold'), fg='#FFD700', bg='#0d1117').pack(side='left')

        rows = []
        total_issued = 0.0
        for kid, w in wallets.items():
            for entry in w.get('credit_log', []):
                delta = entry.get('delta', 0)
                total_issued += delta
                rows.append((entry.get('ts', ''), kid or '(guest)', delta,
                             entry.get('balance', 0), entry.get('reason', '')))
        rows.sort(key=lambda r: r[0], reverse=True)   # newest first

        tk.Label(hdr, text=f'{len(rows):,} event(s)  ·  net {total_issued:+,.0f} Cr across '
                           f'{len(wallets)} account(s)',
                 font=('Segoe UI', 8), fg='#8b949e', bg='#0d1117').pack(side='right')

        cols = ('Date/Time', 'K-ID', 'Change', 'Balance After', 'Reason')
        tree = ttk.Treeview(win, columns=cols, show='headings', height=20)
        for c, wd, anch in zip(cols, (140, 100, 90, 110, 320), ('center', 'center', 'e', 'e', 'w')):
            tree.heading(c, text=c)
            tree.column(c, width=wd, anchor=anch)
        vsb = ttk.Scrollbar(win, orient='vertical', command=tree.yview)
        tree.configure(yscrollcommand=vsb.set)
        tree.pack(side='left', fill='both', expand=True, padx=(12, 0), pady=(0, 12))
        vsb.pack(side='left', fill='y', pady=(0, 12), padx=(0, 12))
        tree.tag_configure('pos', foreground='#3FB950')
        tree.tag_configure('neg', foreground='#F85149')

        for ts, kid, delta, bal, reason in rows:
            tree.insert('', 'end', tags=('pos' if delta >= 0 else 'neg',),
                        values=(ts, kid, f'{delta:+,.0f}', f'{bal:,.0f}', reason))
        if not rows:
            tk.Label(win, text='No credit activity recorded yet across any K-ID.',
                     font=('Segoe UI', 9), fg='#8b949e', bg='#0d1117').pack(pady=20)

    def _tier_days_active(self, w: dict) -> int:
        """Days the wallet's current tier has been active (0 for Free/unset)."""
        import datetime as _dt
        try:
            start = _dt.date.fromisoformat(w.get('tier_started', ''))
            return (_dt.date.today() - start).days
        except Exception:
            return 0

    def _rewards_open_subscribe(self, tier_name: str):
        """Open the tier's Stripe-HOSTED Payment Link in the browser, tagged
        with ?client_reference_id=<K-ID>. The entire payment flow (card entry,
        3DS, renewal) runs on Stripe's hosted page — nothing local handles
        card data and no backend exists on our side. Activation happens when
        the admin's Sync pulls the resulting subscription and matches the
        reference back to this K-ID."""
        import urllib.parse
        kid = (CURRENT_KID or '').strip().lower()
        if not kid:
            messagebox.showwarning('Subscribe',
                'Sign in with a K-ID first — the payment needs to be tagged '
                'to your account.', parent=self)
            return
        link = self._stripe_paylink(tier_name)
        if not link:
            messagebox.showinfo('Subscribe',
                f'No Payment Link configured for {tier_name} yet.\n'
                f'(Admin: create the weekly Payment Link in the Stripe '
                f'Dashboard and paste it in the Admin Dashboard.)', parent=self)
            return
        proceed = messagebox.askokcancel('Before you pay',
            '⚠️  Please note:\n\n'
            'This is a personal, independently-run project — not a registered '
            'company. Your payment is processed by Stripe to the developer\'s '
            'own personal Stripe account, under their own name, not a business '
            'name.\n\n'
            'Click OK to continue to Stripe\'s hosted checkout.', parent=self)
        if not proceed:
            return
        sep = '&' if '?' in link else '?'
        _webbrowser.open(f'{link}{sep}client_reference_id={urllib.parse.quote(kid)}')
        messagebox.showinfo('Subscribe',
            f'💳 Stripe checkout opened in your browser for {tier_name}.\n\n'
            f'Once payment completes, this app will auto-verify and '
            f'activate your tier — no need to do anything else.', parent=self)
        # Poll every 10s for up to 90s so activation happens while the app
        # stays open in the background, not just on next launch.
        for delay in range(10_000, 91_000, 10_000):
            self.after(delay, lambda: self._stripe_autoverify_current_user(silent=False))

    def _admin_emergency_lock(self, parent):
        """🚨 Compromised-secret response: lock admin1 out of the normal
        PIN+R-Code path entirely and mint a single-use recovery code —
        16 hex chars (0-f), cryptographically random via secrets. Only the
        code's SHA-256 is stored; the plaintext is shown exactly once here,
        so copy it somewhere safe — recovering without it means editing
        paa_settings.json by hand."""
        import secrets as _sec, hashlib as _hl
        if not messagebox.askyesno('Emergency Lock',
                'Activate the emergency lock?\n\n'
                '• admin1 login via PIN + R-Code will be DISABLED\n'
                '• A one-time 16-hex recovery code will be generated\n'
                '• Recovery requires that code and sets a NEW R-Code',
                parent=parent):
            return
        code = _sec.token_hex(8)   # 16 hex characters, 0-f
        st = load_app_settings()
        st['admin_emergency_lock']   = True
        st['admin_recovery_sha256']  = _hl.sha256(code.encode()).hexdigest()
        save_app_settings(st)
        dlg = tk.Toplevel(parent)
        dlg.title('🚨 Recovery Code')
        dlg.configure(bg='#0d1117')
        dlg.geometry('380x180')
        dlg.transient(parent); dlg.grab_set()
        tk.Label(dlg, text='🚨  EMERGENCY LOCK ACTIVE', font=('Segoe UI', 11, 'bold'),
                 fg='#F85149', bg='#0d1117').pack(pady=(14, 4))
        tk.Label(dlg, text='Your ONE-TIME recovery code — shown only once, copy it now:',
                 font=('Segoe UI', 8), fg='#cdd9e5', bg='#0d1117').pack()
        ce = tk.Entry(dlg, font=('Consolas', 14, 'bold'), justify='center',
                      bg='#161b22', fg='#FFD700', relief='flat',
                      highlightbackground='#F85149', highlightthickness=1)
        ce.insert(0, code)
        ce.config(state='readonly', readonlybackground='#161b22')
        ce.pack(pady=8, padx=30, fill='x')
        tk.Label(dlg, text='Next admin1 login will demand this code and force a new R-Code.',
                 font=('Segoe UI', 7), fg='#8b949e', bg='#0d1117').pack()
        tk.Button(dlg, text='I saved it', font=('Segoe UI', 9, 'bold'),
                  bg='#21262d', fg='#cdd9e5', relief='flat', padx=14, pady=4,
                  cursor='hand2', command=dlg.destroy).pack(pady=(6, 10))

    def _open_admin_dashboard(self):
        """🛡 Admin-only overview: every K-ID's wallet at a glance (tier,
        credits, streak, last login) with live online status from the
        presence heartbeat, plus the raffle control panel. Gated hard on the
        reserved admin account — the button never renders for anyone else,
        and this method double-checks anyway."""
        if (CURRENT_KID or '').strip().lower() != ADMIN_KID:
            messagebox.showwarning('Admin', 'Admin access only.', parent=self)
            return
        import datetime as _dt
        ...
        win = tk.Toplevel(self)
        win.title('🛡 PAA Admin Dashboard')
        win.configure(bg='#0d1117')
        win.geometry('960x680')

        hdr = tk.Frame(win, bg='#0d1117'); hdr.pack(fill='x', padx=12, pady=(10, 4))
        tk.Label(hdr, text='🛡  ADMIN DASHBOARD', font=('Segoe UI', 13, 'bold'),
                 fg='#F85149', bg='#0d1117').pack(side='left')
        tk.Button(hdr, text='↻ Refresh', font=('Segoe UI', 8, 'bold'),
                  bg='#21262d', fg='#cdd9e5', relief='flat', padx=10, pady=3,
                  cursor='hand2',
                  command=lambda: (win.destroy(), self._open_admin_dashboard())
                  ).pack(side='right')
        tk.Button(hdr, text='🚨 Emergency Lock', font=('Segoe UI', 8, 'bold'),
                  bg='#2a0a0a', fg='#F85149', relief='flat', padx=10, pady=3,
                  cursor='hand2',
                  command=lambda: self._admin_emergency_lock(win)
                  ).pack(side='right', padx=(0, 6))
        tk.Button(hdr, text='🧾 Credit Universe', font=('Segoe UI', 8, 'bold'),
                  bg='#21262d', fg='#FFD700', relief='flat', padx=10, pady=3,
                  cursor='hand2',
                  command=self._open_admin_credit_universe
                  ).pack(side='right', padx=(0, 6))
        ...
        def _status(kid):
            rec = presence.get(kid, {})
            seen_s = rec.get('last_seen', '')
            try:
                seen = _dt.datetime.strptime(seen_s, '%Y-%m-%d %H:%M:%S')
                if (now - seen).total_seconds() <= PRESENCE_ONLINE_SECS:
                    return '🟢 Online', seen_s
                return '⚪ Offline', seen_s
            except Exception:
                return '⚪ Offline', '—'

        n_online = sum(1 for k in wallets
                       if _status(k)[0].startswith('🟢'))
        tk.Label(win, text=f'{len(wallets)} account(s)  ·  {n_online} online now  '
                           f'(online = heartbeat within {PRESENCE_ONLINE_SECS // 60} min)',
                 font=('Segoe UI', 8), fg='#8b949e', bg='#0d1117',
                 anchor='w', padx=12).pack(fill='x')

        # ── Users table ───────────────────────────────────────────────────────
        stripe_subs = s.get('stripe_subs', {})
        uf = tk.Frame(win, bg='#0d1117'); uf.pack(fill='both', expand=True, padx=12, pady=(4, 6))
        cols = ('K-ID', 'Status', 'Tier', 'Sub Days', 'Sub Status', 'Credits',
                'Streak', 'Last Login', 'Last Seen')
        utree = ttk.Treeview(uf, columns=cols, show='headings', height=10)
        for c, wd in zip(cols, (100, 75, 105, 65, 105, 85, 55, 130, 130)):
            utree.heading(c, text=c)
            utree.column(c, width=wd, anchor='center')
        vsb = ttk.Scrollbar(uf, orient='vertical', command=utree.yview)
        utree.configure(yscrollcommand=vsb.set)
        utree.grid(row=0, column=0, sticky='nsew'); vsb.grid(row=0, column=1, sticky='ns')
        uf.rowconfigure(0, weight=1); uf.columnconfigure(0, weight=1)
        utree.tag_configure('on',  foreground='#3FB950')
        utree.tag_configure('off', foreground='#8b949e')
        for kid, w in sorted(wallets.items()):
            st, seen_s = _status(kid)
            rec = presence.get(kid, {})
            w_tier = w.get('tier', 'Free')
            days   = self._tier_days_active(w) if w_tier != 'Free' else 0
            ssub   = stripe_subs.get(kid)
            sub_st = (f"{ssub['status']} → {ssub['period_end']}" if ssub
                      else ('manual' if w_tier != 'Free' else '—'))
            utree.insert('', 'end', tags=('on' if st.startswith('🟢') else 'off',),
                         values=(kid or '(guest)', st, w_tier,
                                 days if w_tier != 'Free' else '—', sub_st,
                                 f"{w.get('credits', 0):,.0f}",
                                 w.get('login_streak_len', 0),
                                 rec.get('last_login', '—'), seen_s))

        tk.Label(win, text='Double-click a row to view that K-ID\'s credit history.',
                 font=('Segoe UI', 8), fg='#8b949e', bg='#0d1117',
                 anchor='w', padx=12).pack(fill='x')

        def _on_row_dclick(event):
            sel = utree.selection()
            if not sel:
                return
            kid = utree.item(sel[0], 'values')[0]
            key = '' if kid == '(guest)' else kid
            w = wallets.get(key)
            if w is not None:
                self._open_credit_history_popup(w, title_kid=kid)
        utree.bind('<Double-1>', _on_row_dclick)

        # ── Raffle control panel ──────────────────────────────────────────────
        rf = tk.Frame(win, bg='#161b22', highlightbackground='#30363d',
                      highlightthickness=1)
        rf.pack(fill='x', padx=12, pady=(0, 10))
        tk.Label(rf, text='🎟️  RAFFLE CONTROL', font=('Segoe UI', 10, 'bold'),
                 fg='#FFD700', bg='#161b22').pack(anchor='w', padx=8, pady=(6, 2))
        for tname, cfg in self.RAFFLES.items():
            row = tk.Frame(rf, bg='#161b22'); row.pack(fill='x', padx=8, pady=2)
            entrants = self._raffle_entrants(tname)
            last = next((h for h in reversed(s.get('raffle_history', []))
                         if h['tier'] == tname), None)
            last_txt = (f"last: {last['date']} → {last['winner']}" if last
                        else 'no draw yet')
            tk.Label(row, text=f'{tname}  ·  {cfg["prize"]}  ·  '
                               f'{len(entrants)} eligible '
                               f'({", ".join(entrants) if entrants else "none"})  ·  {last_txt}',
                     font=('Segoe UI', 8), fg='#cdd9e5', bg='#161b22',
                     wraplength=650, justify='left').pack(side='left')
            tk.Button(row, text='🎲 Draw', font=('Segoe UI', 8, 'bold'),
                      bg='#21262d', fg='#FFD700', relief='flat', padx=10, pady=3,
                      cursor='hand2',
                      command=lambda t=tname: self._raffle_run_draw(t)
                      ).pack(side='right')
        hist = s.get('raffle_history', [])
        if hist:
            tk.Label(rf, text='History: ' + '  |  '.join(
                         f"{h['date']} {h['tier']}: {h['winner']}" for h in hist[-5:][::-1]),
                     font=('Segoe UI', 7), fg='#8b949e', bg='#161b22',
                     wraplength=800, justify='left').pack(anchor='w', padx=8, pady=(2, 6))

        # ── Stripe panel ──────────────────────────────────────────────────────
        sf = tk.Frame(win, bg='#161b22', highlightbackground='#635BFF',
                      highlightthickness=1)
        sf.pack(fill='x', padx=12, pady=(0, 10))
        tk.Label(sf, text='💳  STRIPE SUBSCRIPTIONS', font=('Segoe UI', 10, 'bold'),
                 fg='#635BFF', bg='#161b22').pack(anchor='w', padx=8, pady=(6, 2))
        key_row = tk.Frame(sf, bg='#161b22'); key_row.pack(fill='x', padx=8, pady=2)
        tk.Label(key_row, text='Secret key (sk_test_… in sandbox):',
                 font=('Segoe UI', 8), fg='#cdd9e5', bg='#161b22').pack(side='left')
        key_var = tk.StringVar(value=s.get('stripe_api_key', ''))
        tk.Entry(key_row, textvariable=key_var, show='•', width=34,
                 font=('Segoe UI', 8), bg='#0d1117', fg='#cdd9e5',
                 insertbackground='#635BFF', relief='flat',
                 highlightbackground='#30363d', highlightthickness=1
                 ).pack(side='left', padx=6)
        def _save_key():
            st2 = load_app_settings()
            st2['stripe_api_key'] = key_var.get().strip()
            save_app_settings(st2)
            messagebox.showinfo('Stripe', 'API key saved (this machine only).', parent=win)
        tk.Button(key_row, text='Save key', font=('Segoe UI', 7, 'bold'),
                  bg='#21262d', fg='#cdd9e5', relief='flat', padx=8, pady=2,
                  cursor='hand2', command=_save_key).pack(side='left')

        sync_var = tk.StringVar(value=f"Last sync: {s.get('stripe_last_sync', 'never')}")
        tk.Label(sf, textvariable=sync_var, font=('Segoe UI', 7), fg='#8b949e',
                 bg='#161b22').pack(anchor='w', padx=8)

        # Restricted key — shipped inside the app so EVERY user's copy can
        # self-verify their own purchase automatically. Create TWO of these
        # in Stripe Dashboard → Developers → API keys → "Create restricted
        # key": one while in Test mode (rk_test_…), one in Live mode
        # (rk_live_…). Each needs ONLY "Checkout Sessions: Read" +
        # "Subscriptions: Read" — leave everything else "None". Never put
        # a secret key (sk_…) here. Flip the switch below once you're done
        # testing with sk_test-era flows and ready to take real payments.
        mode_row = tk.Frame(sf, bg='#161b22'); mode_row.pack(fill='x', padx=8, pady=(6, 2))
        tk.Label(mode_row, text='Mode:', font=('Segoe UI', 8, 'bold'),
                 fg='#cdd9e5', bg='#161b22').pack(side='left')
        mode_var = tk.StringVar(value=s.get('stripe_mode', 'test'))
        def _save_mode(*_a):
            st2 = load_app_settings()
            st2['stripe_mode'] = mode_var.get()
            save_app_settings(st2)
        for val, lbl, col in (('test', '🧪 Test (sandbox)', '#D29922'),
                               ('live', '🔴 Live (real payments)', '#F85149')):
            tk.Radiobutton(mode_row, text=lbl, variable=mode_var, value=val,
                           font=('Segoe UI', 8), fg=col, bg='#161b22',
                           selectcolor='#0d1117', activebackground='#161b22',
                           activeforeground=col, command=_save_mode
                           ).pack(side='left', padx=6)

        rkeys = (('stripe_restricted_key_test', 'Test restricted key (rk_test_…):'),
                 ('stripe_restricted_key_live', 'Live restricted key (rk_live_…):'))
        rkey_vars = {}
        for setting_key, label in rkeys:
            rkey_row = tk.Frame(sf, bg='#161b22'); rkey_row.pack(fill='x', padx=8, pady=2)
            tk.Label(rkey_row, text=label, font=('Segoe UI', 8), fg='#cdd9e5',
                     bg='#161b22', width=28, anchor='w').pack(side='left')
            v = tk.StringVar(value=s.get(setting_key, ''))
            rkey_vars[setting_key] = v
            tk.Entry(rkey_row, textvariable=v, show='•', width=26,
                     font=('Segoe UI', 8), bg='#0d1117', fg='#cdd9e5',
                     insertbackground='#635BFF', relief='flat',
                     highlightbackground='#30363d', highlightthickness=1
                     ).pack(side='left', padx=6)
        def _save_rkeys():
            st2 = load_app_settings()
            for setting_key, v in rkey_vars.items():
                st2[setting_key] = v.get().strip()
            save_app_settings(st2)
            messagebox.showinfo('Stripe',
                'Restricted keys saved. Auto-verify uses whichever mode is '
                'selected above — switch to Live once you\'re ready to ship.',
                parent=win)
        tk.Button(sf, text='Save keys', font=('Segoe UI', 7, 'bold'),
                  bg='#21262d', fg='#cdd9e5', relief='flat', padx=8, pady=2,
                  cursor='hand2', command=_save_rkeys).pack(anchor='w', padx=8, pady=(2, 2))
        tk.Label(sf, text='⚠️  Grant ONLY "Checkout Sessions: Read" + "Subscriptions: Read" '
                          'on each key (both read-only) — nothing else. They ship inside the '
                          'app, so they must never be able to write, charge, or refund.',
                 font=('Segoe UI', 7), fg='#D29922', bg='#161b22',
                 wraplength=560, justify='left').pack(anchor='w', padx=8, pady=(0, 4))

        # Hosted Payment Links — one per paid tier, created in the Stripe
        # Dashboard (Products → Payment Links, weekly recurring price).
        # Users pay on Stripe's hosted page; the app only opens the URL.
        pl = s.get('stripe_paylinks', {})
        pl_vars = {}
        for tname, t in self.PAA_TIERS.items():
            if tname == 'Free':
                continue
            prow = tk.Frame(sf, bg='#161b22'); prow.pack(fill='x', padx=8, pady=1)
            tk.Label(prow, text=f'{t["badge"]} {tname} link (RM{t["price_wk"]:.2f}/wk):',
                     font=('Segoe UI', 7), fg='#cdd9e5', bg='#161b22',
                     width=30, anchor='w').pack(side='left')
            v = tk.StringVar(value=pl.get(tname,
                                          self.STRIPE_PAYLINK_DEFAULTS.get(tname, '')))
            pl_vars[tname] = v
            tk.Entry(prow, textvariable=v, font=('Segoe UI', 7), width=46,
                     bg='#0d1117', fg='#cdd9e5', insertbackground='#635BFF',
                     relief='flat', highlightbackground='#30363d',
                     highlightthickness=1).pack(side='left', padx=4)
        def _save_links():
            st2 = load_app_settings()
            st2['stripe_paylinks'] = {k: v.get().strip() for k, v in pl_vars.items()
                                      if v.get().strip()}
            save_app_settings(st2)
            messagebox.showinfo('Stripe', 'Payment Links saved.', parent=win)
        tk.Button(sf, text='Save links', font=('Segoe UI', 7, 'bold'),
                  bg='#21262d', fg='#cdd9e5', relief='flat', padx=8, pady=2,
                  cursor='hand2', command=_save_links).pack(anchor='w', padx=8, pady=(2, 0))
            ...
            def _worker():
                ...
                def _done():
                    if not win.winfo_exists():
                        return
                    if ok2:
                        messagebox.showinfo('Stripe Sync', msg2, parent=win)
                        win.destroy(); self._open_admin_dashboard()
                    else:
                        sync_var.set(f'Sync failed: {msg2}')
                self.after(0, _done)
            threading.Thread(target=_worker, daemon=True).start()
        tk.Button(sf, text='🔄 Sync Subscriptions', font=('Segoe UI', 8, 'bold'),
                  bg='#21262d', fg='#635BFF', relief='flat', padx=10, pady=3,
                  cursor='hand2', command=_sync).pack(anchor='w', padx=8, pady=(2, 2))
        tk.Label(sf, text='Fully Stripe-HOSTED payments: users pay on Stripe\'s own page via the '
                          'links above — no card data or backend on our side. Each opened link '
                          'carries client_reference_id=<K-ID>, which lands on the Checkout '
                          'Session; Sync matches it back and activates the tier. (Webhooks '
                          'would need a public endpoint, hence polling.)',
                 font=('Segoe UI', 7), fg='#8b949e', bg='#161b22',
                 wraplength=800, justify='left').pack(anchor='w', padx=8, pady=(0, 6))

    def _build_rewards_tab(self):
        """💎 Rewards — K-ID reward economy: daily logins, streaks, ad
        rewards, supporter tiers, stake limits, raffle status."""
        tab = self.tab_rewards
        for w in tab.winfo_children():
            w.destroy()
        import datetime as _dt
        BG = '#0d1117'; PANEL = '#161b22'; GOLD = '#FFD700'
        FG = '#cdd9e5'; DIM = '#8b949e'; GREEN = '#3FB950'

        pts = self._pts_load()
        daily_msgs = self._rewards_daily_engine(pts)
        if daily_msgs:
            self._pts_save(pts)
        tier_name = pts.get('tier', 'Free')
        tier = self._pts_tier(pts)

        hdr = tk.Frame(tab, bg=BG); hdr.pack(fill='x', padx=12, pady=(10, 6))
        tk.Label(hdr, text='💎  REWARDS', font=('Segoe UI', 14, 'bold'),
                 fg=GOLD, bg=BG).pack(side='left')
        tk.Label(hdr, text=f'  🆔 {CURRENT_KID or "guest"}   ·   '
                           f'{tier["badge"]} {tier_name}',
                 font=('Segoe UI', 9, 'bold'), fg=tier['color'], bg=BG).pack(side='left')
        tk.Button(hdr, text='💳 Get Credits', font=('Segoe UI', 8, 'bold'),
                  bg='#1a1a2e', fg='#635BFF', relief='flat', padx=10, pady=3,
                  cursor='hand2', command=self._open_credit_buy_popup
                  ).pack(side='left', padx=(14, 0))
        if (CURRENT_KID or '').strip().lower() == ADMIN_KID:
            tk.Button(hdr, text='🛡 Admin Dashboard', font=('Segoe UI', 8, 'bold'),
                      bg='#2a0a0a', fg='#F85149', relief='flat', padx=10, pady=3,
                      cursor='hand2', command=self._open_admin_dashboard
                      ).pack(side='left', padx=(14, 0))
        cr_lbl = tk.Label(hdr, text=f'💰 {pts.get("credits", 0):,.0f} Cr  🧾',
                 font=('Segoe UI', 11, 'bold'), fg=GOLD, bg=BG, cursor='hand2')
        cr_lbl.pack(side='right')
        cr_lbl.bind('<Button-1>', lambda e: self._open_credit_history_popup(pts))

        if daily_msgs:
            ban = tk.Frame(tab, bg='#1a2a0a', highlightbackground=GREEN,
                           highlightthickness=1, padx=12, pady=8)
            ban.pack(fill='x', padx=12, pady=(0, 6))
            tk.Label(ban, text='🎁 TODAY\'S REWARDS CLAIMED', font=('Segoe UI', 8, 'bold'),
                     fg=GREEN, bg='#1a2a0a').pack(anchor='w')
            for m in daily_msgs:
                tk.Label(ban, text=m, font=('Segoe UI', 8), fg=FG,
                         bg='#1a2a0a').pack(anchor='w')

        body = tk.Frame(tab, bg=BG); body.pack(fill='both', expand=True, padx=12, pady=(0, 10))

        # ── Left column: status + ads ──────────────────────────────────────────
        left = tk.Frame(body, bg=PANEL, highlightbackground='#21262d',
                        highlightthickness=1, padx=12, pady=10)
        left.pack(side='left', fill='both', expand=True, padx=(0, 6))

        cyc = pts.get('login_streak_day', 0); run = pts.get('login_streak_len', 0)
        nxt = self.LOGIN_REWARDS[cyc % 7]
        claimed_today = pts.get('login_last_date') == _dt.date.today().isoformat()
        dl_hdr = tk.Frame(left, bg=PANEL); dl_hdr.pack(anchor='w', fill='x')
        tk.Label(dl_hdr, text='📅  DAILY LOGIN', font=('Segoe UI', 10, 'bold'),
                 fg=GOLD, bg=PANEL).pack(side='left')
        tk.Label(dl_hdr, text='✅ Today claimed' if claimed_today else '⏳ Not claimed yet',
                 font=('Segoe UI', 8, 'bold'),
                 fg=GREEN if claimed_today else '#D29922', bg=PANEL
                 ).pack(side='left', padx=(8, 0))
        tk.Label(left, text=f'Cycle day {cyc}/7  ·  streak {run} day(s)\n'
                            f'Tomorrow (Day {(cyc % 7) + 1}): +{nxt:,} Cr'
                            + (f'  (+{self.STREAK_BONUS[run+1]:,} streak bonus)'
                               if (run + 1) in self.STREAK_BONUS else ''),
                 font=('Segoe UI', 8), fg=FG, bg=PANEL, justify='left').pack(anchor='w', pady=(2, 10))

        # ── Weekly credit drip — the supporter tier's daily payout, shown
        # as its weekly total so users can see what the subscription is
        # actually worth (Free has no drip and this section is skipped).
        if tier.get('daily_drip'):
            wk_total = tier['daily_drip'] * 6 + tier['day7_drip']
            today_drip = tier['day7_drip'] if cyc == 7 else tier['daily_drip']
            tk.Label(left, text='🪙  WEEKLY CREDIT DRIP', font=('Segoe UI', 10, 'bold'),
                     fg=GOLD, bg=PANEL).pack(anchor='w')
            tk.Label(left,
                     text=f"{tier['badge']} {tier_name}: +{tier['daily_drip']:,} Cr/day"
                          f" (Day 7: +{tier['day7_drip']:,} Cr)\n"
                          f"Weekly total: {wk_total:,} Cr  ·  "
                          + ("today's drip already paid" if claimed_today
                             else f'today\'s drip: +{today_drip:,} Cr on next open'),
                     font=('Segoe UI', 8), fg=FG, bg=PANEL, justify='left'
                     ).pack(anchor='w', pady=(2, 10))

        tk.Label(left, text='📺  AD REWARDS', font=('Segoe UI', 10, 'bold'),
                 fg=GOLD, bg=PANEL).pack(anchor='w')
        if tier['ads_free']:
            tk.Label(left, text='Ad-free experience — your tier drip replaces ad rewards.',
                     font=('Segoe UI', 8), fg=DIM, bg=PANEL,
                     wraplength=260, justify='left').pack(anchor='w', pady=(2, 10))
        else:
            watched = pts.get('ads_watched', 0)
            tk.Label(left, text=f'{watched}/{self.AD_DAILY_LIMIT} watched today  ·  '
                                f'+{self.AD_REWARD_MIN}–{self.AD_REWARD_MAX} Cr each\n'
                                f'(reward unlocks after {self.AD_VIEW_SECONDS}s view time)',
                     font=('Segoe UI', 8), fg=FG, bg=PANEL, justify='left').pack(anchor='w', pady=2)
            tk.Button(left, text='▶  Watch Ad', command=self._rewards_open_ad_popup,
                      font=('Segoe UI', 9, 'bold'), bg='#21262d', fg=GOLD,
                      relief='flat', padx=12, pady=4, cursor='hand2').pack(anchor='w', pady=(4, 10))

        tk.Label(left, text='🎯  PLAY LIMITS', font=('Segoe UI', 10, 'bold'),
                 fg=GOLD, bg=PANEL).pack(anchor='w')
        tk.Label(left, text=f'Min stake: {self.PTS_MIN_STAKE:,.0f} Cr  ·  '
                            f'Max single play: {self._pts_max_stake(pts):,.0f} Cr\n'
                            f'(applies across PTS, ZDWS and ZDOS)',
                 font=('Segoe UI', 8), fg=FG, bg=PANEL, justify='left').pack(anchor='w', pady=(2, 10))

        tk.Label(left, text='🎟️  RAFFLE', font=('Segoe UI', 10, 'bold'),
                 fg=GOLD, bg=PANEL).pack(anchor='w')
        tk.Label(left, text=self._rewards_raffle_status(pts),
                 font=('Segoe UI', 8), fg=FG, bg=PANEL,
                 wraplength=260, justify='left').pack(anchor='w', pady=2)

        # Draw controls — admin action: runs the cross-wallet draw for each
        # raffle (cadence-guarded, so clicking early just reports next date).
        draw_row = tk.Frame(left, bg=PANEL); draw_row.pack(fill='x', pady=(4, 2))
        for _tname, _cfg in self.RAFFLES.items():
            n_elig = len(self._raffle_entrants(_tname))
            tk.Button(draw_row,
                      text=f'🎲 Draw {_tname} ({n_elig} eligible)',
                      font=('Segoe UI', 7, 'bold'), bg='#21262d', fg=GOLD,
                      relief='flat', padx=6, pady=3, cursor='hand2',
                      command=lambda t=_tname: self._raffle_run_draw(t)
                      ).pack(side='left', padx=(0, 4))

        # Recent winners
        _rh = load_app_settings().get('raffle_history', [])
        if _rh:
            tk.Label(left, text='Recent winners:', font=('Segoe UI', 7, 'bold'),
                     fg=DIM, bg=PANEL).pack(anchor='w', pady=(4, 0))
            for h in _rh[-4:][::-1]:
                tk.Label(left,
                         text=f"{h['date']}  {h['tier']}: {h['winner']} — {h['prize']}",
                         font=('Segoe UI', 7), fg=FG, bg=PANEL,
                         wraplength=260, justify='left').pack(anchor='w')

        # ── SoR mode — per-K-ID, not admin-gated: Default is automatically
        #    active the moment this K-ID is valid; Custom must be switched
        #    on by the user themselves. ─────────────────────────────────────
        tk.Label(left, text='🎁  SoR MODE', font=('Segoe UI', 10, 'bold'),
                 fg=GOLD, bg=PANEL).pack(anchor='w', pady=(10, 0))
        custom_on = bool(pts.get('sor_custom_enabled'))
        _sor_cfg = self._sor_config()
        if custom_on:
            _y = pts.get('sor_custom_threshold') or _sor_cfg['custom_threshold']
            if (CURRENT_KID or '').strip().lower() == ADMIN_KID:
                _mode_desc = (f"Custom — {_sor_cfg.get('custom_credits', 1):g} Cr (admin-locked) "
                               f"per {_y:g} units held (your own Y, any market).")
            else:
                _mode_desc = (f"Custom — your rate: {_y:g} units per credit "
                               f"(credit amount set by admin, any market).")
        else:
            _mode_desc = (f"Default — active for any signed-in K-ID. Min units to earn: "
                           f"MY {_sor_cfg['my_min_units']:g}  ·  US {_sor_cfg['us_min_units']:g}")
        tk.Label(left, text=_mode_desc,
                 font=('Segoe UI', 8), fg=FG, bg=PANEL,
                 wraplength=260, justify='left').pack(anchor='w', pady=2)
        tk.Button(left, text=('↩ Switch back to Default' if custom_on else '⚙ Switch to Custom'),
                  font=('Segoe UI', 7, 'bold'), bg='#21262d', fg=GOLD,
                  relief='flat', padx=6, pady=3, cursor='hand2',
                  command=self._rewards_toggle_sor_mode).pack(anchor='w', pady=(2, 0))

        # ── Right column: supporter tiers ──────────────────────────────────────
        right = tk.Frame(body, bg=BG); right.pack(side='left', fill='both', expand=True)
        tk.Label(right, text='☕  KOFI SUPPORTER TIERS   ·   payment via KoFi/Stripe '
                             'happens outside the app — activate here once confirmed',
                 font=('Segoe UI', 8, 'bold'), fg=DIM, bg=BG,
                 wraplength=420, justify='left').pack(anchor='w', pady=(0, 4))
        for name, t in self.PAA_TIERS.items():
            card = tk.Frame(right, bg=PANEL,
                            highlightbackground=t['color'] if name == tier_name else '#21262d',
                            highlightthickness=2 if name == tier_name else 1,
                            padx=10, pady=6)
            card.pack(fill='x', pady=3)
            row = tk.Frame(card, bg=PANEL); row.pack(fill='x')
            tk.Label(row, text=f"{t['badge']} {name}", font=('Segoe UI', 9, 'bold'),
                     fg=t['color'], bg=PANEL).pack(side='left')
            tk.Label(row, text=('  · current' if name == tier_name else ''),
                     font=('Segoe UI', 8), fg=GREEN, bg=PANEL).pack(side='left')
            if name != tier_name:
                _is_admin = (CURRENT_KID or '').strip().lower() == ADMIN_KID
                if name == 'Free':
                    # Reflect a cancel locally right away (billing itself is
                    # cancelled on Stripe's hosted portal / dashboard).
                    tk.Button(row, text='Downgrade', font=('Segoe UI', 7, 'bold'),
                              bg='#21262d', fg=FG, relief='flat', padx=8, cursor='hand2',
                              command=lambda n=name: (self._rewards_set_tier(n),
                                                      self._build_rewards_tab())
                              ).pack(side='right')
                else:
                    # Hosted payment path for everyone…
                    tk.Button(row, text='💳 Subscribe', font=('Segoe UI', 7, 'bold'),
                              bg='#1a1a2e', fg='#635BFF', relief='flat', padx=8,
                              cursor='hand2',
                              command=lambda n=name: self._rewards_open_subscribe(n)
                              ).pack(side='right')
                    # …manual set is an ADMIN tool only (comping a tier /
                    # fixing a mismatch), so free self-upgrades aren't possible.
                    if _is_admin:
                        tk.Button(row, text='set', font=('Segoe UI', 7),
                                  bg='#21262d', fg='#8b949e', relief='flat', padx=6,
                                  cursor='hand2',
                                  command=lambda n=name: (self._rewards_set_tier(n),
                                                          self._build_rewards_tab())
                                  ).pack(side='right', padx=(0, 4))
            wk = (t['daily_drip'] * 6 + t['day7_drip'])
            desc = (f"RM{t['price_wk']:.2f}/wk  ·  {wk:,} Cr/week"
                    f"  ·  max play {t['max_stake']:,}"
                    + ('  ·  ads-free' if t['ads_free'] else '')
                    + (f"  ·  🎟️ {t['raffle']}" if t['raffle'] else '')) \
                   if name != 'Free' else \
                   (f"Free  ·  logins + ads only  ·  max play {t['max_stake']:,}")
            tk.Label(card, text=desc, font=('Segoe UI', 8), fg=FG, bg=PANEL,
                     wraplength=420, justify='left').pack(anchor='w', pady=(2, 0))
            if name == tier_name and name != 'Free':
                tk.Label(card, text=f'Active {self._rewards_tier_weeks(pts):.1f} week(s)',
                         font=('Segoe UI', 7), fg=DIM, bg=PANEL).pack(anchor='w')

        # ── Embedded ad area (bottom strip) ────────────────────────────────────
        # Inline WebView via tkinterweb when installed; otherwise a styled
        # placeholder with the install hint. Supporters are ads-free, so the
        # strip is simply omitted for them.
        if not tier['ads_free']:
            ad_area = tk.Frame(tab, bg='#161b22', highlightbackground='#30363d',
                               highlightthickness=1)
            ad_area.pack(fill='x', side='bottom', padx=12, pady=(0, 10))
            tk.Label(ad_area, text='SPONSORED', font=('Segoe UI', 6, 'bold'),
                     fg='#8b949e', bg='#161b22').pack(anchor='w', padx=6, pady=(3, 0))
            if _HAS_TKINTERWEB:
                try:
                    hf = _AdHtmlFrame(ad_area, messages_enabled=False, height=120)
                    hf.load_url(self._rewards_ad_page())
                    hf.pack(fill='x', padx=6, pady=(0, 4))
                except Exception:
                    tk.Label(ad_area, text='Ad area — failed to render page',
                             font=('Segoe UI', 8), fg='#8b949e',
                             bg='#161b22', pady=18).pack(fill='x')
            else:
                tk.Label(ad_area,
                         text='Ad area — install tkinterweb to render inline\n'
                              '(pip install tkinterweb)  ·  the ▶ Watch Ad popup works either way',
                         font=('Segoe UI', 8), fg='#8b949e', bg='#161b22',
                         justify='center', pady=18).pack(fill='x')
    ...
    def _pts_load(self) -> dict:
        ...
        presence_touch()   # keep the online heartbeat fresh (60s throttle inside)
    ...
    # ── Crypto ETFs — iShares spot crypto trusts, USD, group 'US-ETF' ───────
    # Separate list (not MSCI-index funds) so future crypto/thematic ETF
    ...
    # ── Crypto spot pairs — the coins themselves (Yahoo BTC-USD etc.),
    # USD-denominated, group 'Crypto'. Distinct from the ETFs above: IBIT
    # trades US market hours at fund NAV, BTC-USD is the 24/7 spot price.
    CRYPTO_PAIRS = [
        ('BTC-USD', 'Bitcoin'),
        ('ETH-USD', 'Ethereum'),
        ('LTC-USD', 'Litecoin'),
    ...
    # Extra settlement currencies a crypto spot pair can be quoted in, on
    # top of its native USD quote — e.g. BTC/MYR = BTC-USD level x USD/MYR.
    # Fiat only (crypto-vs-fiat), not crypto-vs-crypto: MYR needs no FX leg
    # (base currency); SGD/HKD/JPY/KRW go through FX_PAIRS.
    CRYPTO_CROSS_CCY = ['MYR', 'SGD', 'HKD', 'JPY', 'KRW']
    ...
    # Market quoting convention: low-value currencies are quoted per N units
    # rather than per 1, since the raw per-1-unit rate is too small to read
    # (e.g. JPY/MYR raw is ~0.028). Everything not listed here quotes per 1.
    FX_QUOTE_LOT = {'JPY': 100, 'KRW': 1_000, 'IDR': 10_000}

    def _zdws_fx_pair_level(self, code: str):
        """Level for an FX pair when it's the UNDERLYING being traded —
        scaled to the quote-lot convention above (100 JPY, 1,000 KRW,
        10,000 IDR, else per 1). This is intentionally separate from
        _zdws_fx_to_myr, which must stay a raw per-1-unit rate since it's
        also used to compound a foreign World Index's move with the FX move
        (e.g. Nikkei 225 in JPY). Scaling is consistent at both entry and
        settlement (both call this same function), so P/L math is unaffected
        — it only changes what the level looks like on screen."""
        raw = self._zdws_fx_to_myr(code)
        if raw is None:
            return None
        return raw * self.FX_QUOTE_LOT.get(code, 1)

    def _zdws_crypto_cross_level(self, crypto_sym: str, quote_ccy: str):
        """Cross-rate level for a crypto pair quoted in a non-USD FIAT
        currency (crypto-vs-fiat only, not crypto-vs-crypto), e.g.
        BTC/MYR = BTC-USD spot x USD/MYR. Formula per spec:
            BTC/USD x USD/MYR = 50,000.00 x 4.1050 = X
        For SGD/HKD/JPY/KRW (which are themselves quoted against MYR in
        FX_PAIRS, not against USD directly), this goes through MYR as the
        common leg:
            BTC/SGD = (BTC-USD x USD/MYR) / (SGD/MYR)
        """
        usd_lvl = self._zdws_world_level(crypto_sym)
        if not usd_lvl or usd_lvl <= 0:
            return None
        usd_to_myr = self._zdws_fx_to_myr('USD')
        if not usd_to_myr:
            return None
        btc_myr = usd_lvl * usd_to_myr
        quote_ccy = (quote_ccy or 'MYR').upper()
        if quote_ccy == 'MYR':
            return btc_myr
        ccy_to_myr = self._zdws_fx_to_myr(quote_ccy)
        if not ccy_to_myr:
            return None
        return btc_myr / ccy_to_myr

    def _zdws_level_decimals(self, underlying: str) -> int:
        """How many decimal places to show a level at.
        4dp ONLY for per-1-unit FX pairs (USD, GBP, SGD, EUR…) where a rate
        like 4.4731 genuinely needs the precision. Everything else — the
        lot-scaled FX pairs (JPY/KRW/IDR, already brought into a normal
        magnitude by their quote lot), US stocks, ETFs, and indices — reads
        at standard 2dp."""
        if underlying in self.FX_QUOTE_LOT:      # JPY / KRW / IDR — lot-scaled
            return 2
        if underlying in {c for c, _ in self.FX_PAIRS}:   # per-1 FX pairs
            return 4
        return 2                                  # stocks / ETFs / indices

    def _zdws_fmt_level(self, underlying: str, lvl) -> str:
        """Format a level string with the right decimals for `underlying`."""
        if lvl is None:
            return '—'
        return f'{lvl:,.{self._zdws_level_decimals(underlying)}f}'
    ...
    def _zdws_fx_to_myr(self, ccy: str):
        ...
        TWD is a special case: Yahoo has no direct TWDMYR=X pair, only
        USDTWD=X (TWD per USD), so TWD/MYR is derived through the USD leg
        instead: TWD/MYR = (USD/MYR) / (USD/TWD)."""
        ccy = (ccy or 'MYR').upper()
        if ccy == 'MYR':
            return 1.0
        if ccy == 'TWD':
            return self._zdws_twd_to_myr()
    ...
    def _zdws_twd_to_myr(self):
        """TWD/MYR via the USD leg (USDTWD=X x USD/MYR), cached the same way
        as every other FX rate (tick_TWDMYR=X key) — the derivation is an
        implementation detail, callers still just see a normal MYR rate."""
        # 1. Marquee's in-memory value (in case something upstream already
        #    seeded FX:TWD the normal way)
        v = (getattr(self, '_mkt_data', {}) or {}).get('FX:TWD', (None, None))[0]
        if v:
            return float(v)
        # 2. Disk cache
        cached = cache_get_live_dict('tick_TWDMYR=X', _TTL_LIVE)
        if cached and cached.get('last'):
            return float(cached['last'])
        # 3. Cold path — derive via USD/TWD x USD/MYR
        usd_to_myr = self._zdws_fx_to_myr('USD')
        if not usd_to_myr:
            return None
        try:
            import yfinance as _yf
            t  = _yf.Ticker('USDTWD=X')
            fi = t.fast_info
            usdtwd = getattr(fi, 'last_price', None) or getattr(fi, 'lastPrice', None)
            if usdtwd is None:
                h = t.history(period='5d')
                if not h.empty:
                    usdtwd = float(h['Close'].iloc[-1])
            if usdtwd:
                twd_to_myr = usd_to_myr / float(usdtwd)
                cache_set_live_dict('tick_TWDMYR=X',
                                    {'last': twd_to_myr, 'prev': twd_to_myr})
                return twd_to_myr
        except Exception:
            pass
        return None
    ...
    # Display TTL for the big-universe tick cache: accept stale-but-present
    # prices up to 1 hour old so the 503 S&P names don't blink out of the
    # underlying list every time the 5-min live TTL lapses. FRESHNESS is
    # handled separately — _ensure_us_universe_warm probes with the short
    # TTL and re-fetches in the background every ~5 min on tab rebuilds —
    # this long TTL only governs whether an already-known price is still
    # worth SHOWING while the refresh is in flight.
    ...
    def _zdws_cached_level_only(self, symbol: str, ttl: int = None):
        ...
        Returns None if nothing's cached; see _zdws_warm_batch_cache to
        populate the cache in one batched call instead.
        ttl defaults to the 1-hour display TTL above; pass _TTL_LIVE
        explicitly to ask the stricter 'is this actually fresh?' question
        (that's what the re-warm staleness probe does)."""
        v = (getattr(self, '_mkt_data', {}) or {}).get(symbol, (None, None))[0]
        if v:
            return float(v)
        cached = cache_get_live_dict(f'tick_{symbol.replace("^", "")}',
                                     ttl if ttl is not None else self._TTL_TICK_DISPLAY)
        if cached and cached.get('last'):
            return float(cached['last'])
        return None
    ...
    def _extract_batch_closes(data, sym, n_symbols):
        """Pull the Close series for `sym` out of a yfinance batch download,
        tolerating every column layout yfinance produces across versions:
        (ticker, field) MultiIndex, (field, ticker) MultiIndex, or a flat
        single-ticker frame. Returns a Series (possibly empty)."""
        import pandas as _pd
        try:
            if n_symbols == 1 or not isinstance(data.columns, _pd.MultiIndex):
                return data['Close'].dropna()
            if sym in data.columns.get_level_values(0):
                return data[sym]['Close'].dropna()          # group_by='ticker'
            if 'Close' in data.columns.get_level_values(0):
                return data['Close'][sym].dropna()          # field-first layout
        except Exception:
            pass
        return _pd.Series(dtype=float)

    def _zdws_warm_batch_cache(self, symbols: list) -> int:
        """Batch-fetch tickers into the same tick_* cache that
        _zdws_world_level/_zdws_cached_level_only read from — what makes a
        500+ ticker universe practical (a handful of network round trips
        instead of 500). Chunked into 100-ticker calls so one failed or
        malformed response costs at most that chunk, never the whole
        universe — with a single monolithic call, any Yahoo hiccup zeroed
        out all 503 and the US-Stock group simply never appeared. Returns
        how many symbols were successfully cached."""
        if not symbols:
            return 0
        try:
            import yfinance as _yf
        except Exception as e:
            log(f"   ⚠️  yfinance unavailable for batch warm: {e}")
            return 0
        CHUNK = _smart_chunk_size(len(symbols), max_single_chunk=100, min_chunk=25, target_chunks=6)
        PAUSE = 2.0    # seconds between chunks — rapid back-to-back calls trip
                       # Yahoo's rate limiter, which is exactly why only the
                       # FIRST ~100 tickers were showing: chunk 1 landed,
                       # chunks 2+ got throttled and silently cached nothing.
        import time as _time
        n_cached = 0
        self._last_warm_rows = []   # (sym, last, prev) — persisted as the snapshot CSV
        chunks = [symbols[ci:ci + CHUNK] for ci in range(0, len(symbols), CHUNK)]
        for idx, chunk in enumerate(chunks):
            ...
            for attempt in (1, 2):                     # one retry per chunk
                try:
                    ...
                    log(f"   ⚠️  Batch warm chunk {idx + 1} attempt {attempt} failed: {e}")
                    data = None
                if data is None or len(data) == 0:
                    if attempt == 1:
                        _time.sleep(PAUSE * 3)          # longer cooldown, then retry
                        continue
                    break
                for sym in chunk:
                    try:
                        closes = self._extract_batch_closes(data, sym, len(chunk))
                        if closes.empty:
                            continue
                        ...
                        cache_set_live_dict(f'tick_{sym.replace("^", "")}',
                                            {'last': last, 'prev': prev})
                        self._last_warm_rows.append((sym, last, prev))
                        n_chunk += 1
                    except Exception:
                        continue
                break                                   # got data — no retry needed
            n_cached += n_chunk
            log(f"   Batch warm chunk {idx + 1}/{len(chunks)}: {n_chunk}/{len(chunk)} cached")
        return n_cached

    def _sp500_config(self) -> list:
        """S&P 500 universe as (ticker, name) tuples, loaded from sp500.csv
        next to the script — the same external-CSV pattern as the rest of
        the data-driven workflows, so expanding/trimming the universe is a
        spreadsheet edit, not a code change.
        First run: the CSV doesn't exist yet, so it's seeded from the
        baked-in SP500_CONFIG below and then read back — after that the CSV
        is the source of truth and the baked list is only a fallback for a
        missing/corrupt file. Result cached per session."""
        if getattr(self, '_sp500_cache', None) is not None:
        ...
        try:
            if not os.path.exists(path):
                with open(path, 'w', encoding='utf-8', newline='') as f:
                    f.write('ticker,name\n')
                    for t, n in self.SP500_CONFIG:
                        n_esc = f'"{n}"' if ',' in n else n
                        f.write(f'{t},{n_esc}\n')
                log(f"   sp500.csv seeded with {len(self.SP500_CONFIG)} tickers")
            import csv as _csv
            out = []
            ...
            if out:
                self._sp500_cache = out
                return out
        except Exception as e:
            log(f"   ⚠️  sp500.csv load failed ({e}) — using baked-in list")
        self._sp500_cache = list(self.SP500_CONFIG)
        return self._sp500_cache
    ...
    def _us_snapshot_path(self) -> str:
        return os.path.join(os.path.dirname(os.path.abspath(__file__)),
                            self.US_PRICE_SNAPSHOT)

    def _us_snapshot_save(self):
        """Persist the rows from the last successful warm as us_prices.csv —
        the 'pre-chunk' file: next session loads it instantly instead of
        waiting ~15s of paced Yahoo chunks before US-Stock/US-ETF appear."""
        import datetime as _dt
        rows = getattr(self, '_last_warm_rows', [])
        if not rows:
            return
        try:
            with open(self._us_snapshot_path(), 'w', encoding='utf-8', newline='') as f:
                f.write('ticker,last,prev,fetched_at\n')
                stamp = _dt.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                for sym, last, prev in rows:
                    f.write(f'{sym},{last},{prev},{stamp}\n')
            log(f"   us_prices.csv snapshot saved: {len(rows)} tickers")
        except Exception as e:
            log(f"   ⚠️  snapshot save failed: {e}")

    def _us_snapshot_load(self) -> int:
        """Seed the tick cache from us_prices.csv (last session's prices) so
        the US universes render IMMEDIATELY at startup — visibly stale-but-
        usable, per the 1-hour display TTL philosophy — while the background
        warm fetches fresh data and rewrites the snapshot. Returns rows
        seeded (0 if no snapshot yet)."""
        import csv as _csv
        path = self._us_snapshot_path()
        if not os.path.exists(path):
            return 0
        n = 0
        try:
            with open(path, encoding='utf-8') as f:
                for row in _csv.DictReader(f):
                    ...
                    try:
                        ...
                        prev = float(row.get('prev') or last)
                    except ValueError:
                        continue
                    if sym and last > 0:
                        cache_set_live_dict(f'tick_{sym.replace("^", "")}',
                                            {'last': last, 'prev': prev})
                        n += 1
            log(f"   us_prices.csv snapshot loaded: {n} tickers pre-seeded")
        except Exception as e:
            log(f"   ⚠️  snapshot load failed: {e}")
        return n

    def _ensure_us_universe_warm(self):
        """Kick off a background pre-fetch of S&P 500 + SPDR + MSCI + FX
        prices whenever the tick cache has gone stale, so those symbols just
        appear in the underlying list like everything else — no separate
        'load' step.

        NOT one-shot per session: the tick_* cache expires after _TTL_LIVE
        (5 min), so a single session-scoped warm meant whichever of
        ZDWS/ZDOS you opened FIRST got the S&P 500 list, and the tab opened
        after the TTL lapsed silently lost the whole US-Stock group.
        Instead, probe a couple of sentinel tickers: if they're still
        cached, everything from the same batch is too (written together) —
        skip; if stale, re-warm. A running-flag prevents concurrent fetches
        when both tabs rebuild in quick succession."""
        if getattr(self, '_us_universe_warm_running', False):
            return
        # ── Snapshot pre-seed (once per session) ──────────────────────────────
        # Load last session's us_prices.csv into the cache so the US groups
        # render instantly. IMPORTANT: seeding writes fresh cache timestamps,
        # which would fool the sentinel below into thinking a real fetch just
        # happened — so the first pass of a session must skip the sentinel
        # and always kick one true refresh.
        first_pass = not getattr(self, '_us_snapshot_seeded', False)
        if first_pass:
            self._us_snapshot_seeded = True
            self._us_snapshot_load()
        # Sentinel probe — two tickers from the batch; both FRESH (strict
        # 5-min TTL, not the lenient display TTL) ⇒ the whole batch was
        # written recently in one pass — skip. Probing with the display TTL
        # would let prices go an hour stale before re-fetching.
        if (not first_pass
                and self._zdws_cached_level_only('AAPL', ttl=_TTL_LIVE) is not None
                and self._zdws_cached_level_only('XLK', ttl=_TTL_LIVE) is not None):
            return
        self._us_universe_warm_running = True
        tickers = ([t for t, _ in self._sp500_config()] +
                  [t for t, _ in self.SPDR_SELECT_ETFS] +
                  [t for t, _ in self.MSCI_ETF_CONFIG] +
                  [t for t, _ in self.CRYPTO_ETF_CONFIG] +
                  [t for t, _ in self.CRYPTO_PAIRS])

        def _worker():
            try:
                n = self._zdws_warm_batch_cache(tickers)
                # FX pairs use their own smaller per-symbol path (_zdws_fx_to_myr
                # already caches each one) rather than the batch yfinance.download
                # above — only 13 pairs, and several (USD/JPY/GBP/SGD/HKD/KRW)
                # are usually already warm from World Indices anyway.
                n_fx = 0
                for code, _ in self.FX_PAIRS:
                    if self._zdws_fx_to_myr(code) is not None:
                        n_fx += 1
                log(f"   US-Stock/US-ETF universe warmed: {n}/{len(tickers)} symbols cached, "
                   f"{n_fx}/{len(self.FX_PAIRS)} FX pairs cached")
                if n > 0:
                    self._us_snapshot_save()   # next session pre-loads this
                self.after(0, self._redraw_chart)   # refreshes whichever tab is currently open
            finally:
                self._us_universe_warm_running = False
    ...
    def _zdws_current_level(self, underlying: str):
        ...
        try:
            ...
            # FX pair — quote-lot-scaled level (100 JPY, 1,000 KRW, else per 1);
            # see _zdws_fx_pair_level for why this differs from _zdws_fx_to_myr
            if underlying in {c for c, _ in self.FX_PAIRS}:
                return self._zdws_fx_pair_level(underlying)
            # Synthetic crypto cross pair (BTC-MYR / BTC-SGD / BTC-HKD, …) —
            # not a real yfinance ticker, resolve via the USD spot x FX cross.
            if '-' in underlying:
                _coin, _ccy = underlying.split('-', 1)
                if _ccy in self.CRYPTO_CROSS_CCY and any(_coin == s.split('-')[0] for s, _ in self.CRYPTO_PAIRS):
                    return self._zdws_crypto_cross_level(f'{_coin}-USD', _ccy)
    ...
    def _zdws_auto_settle(self, pts: dict) -> list:
        ...
        for w in pts.get('zdws_active', []):
            ...
            if direction_ok:
                ...
                pts['credits'] += payout
                self._credit_log_add(pts, payout, f"ZDWS settle WIN: {w['side']} {w['underlying']}")
                w['payout'] = payout
                w['result'] = 'WIN'
                messages.append(
    ...
    def _zdws_open_position(self, underlying: str, side: str, stake: float,
        ...
        pts = self._pts_load()
        if self._pts_check_stake(pts, stake):   # min / tier max / balance
            return False
        ...
        import datetime as _dt
        pts['credits'] -= stake
        self._credit_log_add(pts, -stake, f'ZDWS {side} {underlying} — stake opened')
    ...
    def _zdws_market_close(self, warrant_id: int):
        ...
        if direction_ok:
            ...
            pts['credits'] += payout
            self._credit_log_add(pts, payout, f"ZDWS early close WIN: {pos['underlying']}")
            ...
            pts['credits'] += refund
            self._credit_log_add(pts, refund, f"ZDWS early close LOSS refund: {pos['underlying']}")
    ...
    def _zdos_auto_settle(self, pts: dict) -> list:
        ...
        for o in pts.get('zdos_active', []):
            ...
            if self._zdos_is_winning(o, move_pct):
                ...
                pts['credits'] += payout
                self._credit_log_add(pts, payout, f"ZDOS settle WIN: {o['postype']} {o['underlying']}")
                o['payout'] = payout
                o['result'] = 'WIN'
                messages.append(
    ...
    def _zdos_open_position(self, underlying: str, postype: str, tier_num: int,
        ...
        import datetime as _dt
        pts['credits'] -= stake
        self._credit_log_add(pts, -stake, f'ZDOS {postype} {underlying} — stake opened')
    ...
    def _zdos_open_strategy(self, underlying: str, strategy: str, base_tier: int,
        ...
        mx = self._pts_max_stake(pts)
        if total_stake > mx:
            return False, (f'Maximum single play for your tier '
                           f'({pts.get("tier", "Free")}) is {mx:,.0f} credits.')
    ...
    def _zdos_market_close(self, position_id: int, silent: bool = False):
        ...
        if self._zdos_is_winning(pos, move_pct):
            ...
            pts['credits'] += payout
            self._credit_log_add(pts, payout, f"ZDOS early close WIN: {pos['underlying']}")
            pos['result'] = 'WIN (early close)'
            pos['payout'] = payout
            ...
            refund = round(pos['stake'] * 0.5, 2)
            pts['credits'] += refund
            self._credit_log_add(pts, refund, f"ZDOS early close LOSS refund: {pos['underlying']}")
            pos['result'] = 'LOSS (early close, 50% refund)'
            pos['payout'] = refund
    ...
    def _build_pts_tab(self):
        ...
        def _submit_prediction():
            ...
            try:
                ...
                return
            _err = self._pts_check_stake(self._pts_load(), stake)
            if _err:
                messagebox.showwarning('PTS', _err, parent=self)
                return
            pts2 = self._pts_load()
            ...
            pts2['credits']    -= stake
            self._credit_log_add(pts2, -stake, f'PTS prediction stake: {pred["position"]}')
            pts2['total_made'] += 1
            self._pts_save(pts2)
        ...
        def _resolve_selected():
            ...
            if correct:
                ...
                pred['correct'] = True
                pts2['credits'] += payout
                self._credit_log_add(pts2, payout, 'PTS prediction WIN payout')
                ...
                if pts2['credits'] - payout < self.PTS_MIN_STAKE:
                    ...
                    self._credit_log_add(pts2, self.PTS_STARTING_CREDITS, 'PTS zero-balance rescue bonus')
                    msg += (f'\n\n🎁 ZERO-BALANCE RESCUE!\n'
        ...
        def _daily_topup():
            ...
            if pts2.get('daily_topup_date', '') == ts:
                ...
                return
            pts2['credits']         += self.PTS_STARTING_CREDITS
            self._credit_log_add(pts2, self.PTS_STARTING_CREDITS, 'PTS daily free top-up')
    ...
    def _pts_grant_daily_login(self):
        """Daily login reward, granted automatically once per day on first
        open of a PTS/ZDWS/ZDOS tab (idempotent — safe to call from all
        three). Separate from the manual 10k Daily Free Top-Up button.

        Scheme: Day N of the streak pays N × 100 Cr (Day 1 = 100 … Day 7 =
        700). Completing Day 7 pays a further +200 Cr streak bonus, then the
        cycle restarts at Day 1 the next day. Missing a day resets the
        streak to Day 1."""
        import datetime as _dt
        pts   = self._pts_load()
        today = _dt.date.today()
        t_str = today.strftime('%Y-%m-%d')
        if pts.get('login_last_date', '') == t_str:
            return                                   # already granted today
        last = pts.get('login_last_date', '')
        try:
            was_yesterday = (last and
                (today - _dt.datetime.strptime(last, '%Y-%m-%d').date()).days == 1)
        except Exception:
            was_yesterday = False
        prev_day = pts.get('login_streak_day', 0)
        # Consecutive day continues the cycle (Day 7 rolls back to Day 1);
        # any gap — or first ever login — starts at Day 1.
        day = (prev_day % 7) + 1 if was_yesterday else 1
        reward = day * 100
        bonus  = 200 if day == 7 else 0
        pts['credits']         += reward + bonus
        self._credit_log_add(pts, reward + bonus, f'PTS daily login (day {day}/7)'
                              + (' + 7-day streak bonus' if bonus else ''))
        pts['login_streak_day'] = day
        pts['login_last_date']  = t_str
        self._pts_save(pts)
        bonus_txt = f'\n🏆 7-day streak complete — bonus +{bonus} Cr!' if bonus else ''
        messagebox.showinfo('Daily Login',
            f'📅 Day {day}/7 login reward: +{reward:,.0f} Cr{bonus_txt}\n'
            f'New balance: {pts["credits"]:,.0f} credits.',
            parent=self)
    ...
    def _zdws_underlyings(self) -> list:
        ...
        Returns list of (key, display, level, group) tuples covering:
          ...
          - S&P 500 constituents, group 'US-Stock' (USD) — auto-warmed in the
            background on first ZDWS/ZDOS open each session (see
            _ensure_us_universe_warm), no manual step needed
        ...
        for sym, name in self.SPDR_SELECT_ETFS + self.MSCI_ETF_CONFIG + self.CRYPTO_ETF_CONFIG:
            lvl = self._zdws_world_level(sym)
            if lvl and lvl > 0:
                out.append((sym, f'{sym} · {name}', lvl, 'US-ETF'))

        # ── Crypto spot pairs, group 'Crypto' — 24/7 USD spot prices ──────────
        for sym, name in self.CRYPTO_PAIRS:
            lvl = self._zdws_world_level(sym)
            if lvl and lvl > 0:
                out.append((sym, f'{name} · {sym.replace("-", "/")}', lvl, 'Crypto'))
            # Same coin, quoted in MYR/SGD/HKD instead of USD — layered on
            # top of the USD spot via the FX cross rate (BTC/USD x USD/ccy).
            coin = sym.split('-')[0]
            for ccy in self.CRYPTO_CROSS_CCY:
                cross_lvl = self._zdws_crypto_cross_level(sym, ccy)
                if cross_lvl and cross_lvl > 0:
                    cross_sym = f'{coin}-{ccy}'
                    out.append((cross_sym, f'{name} · {coin}/{ccy}', cross_lvl, 'Crypto'))

        # ── FX pairs, group 'FX' — quoted per market lot convention (100 JPY,
        # 1,000 KRW, 10,000 IDR, else per 1) rather than the raw per-1-unit rate
        for code, name in self.FX_PAIRS:
            lvl = self._zdws_fx_pair_level(code)
            if lvl and lvl > 0:
                lot = self.FX_QUOTE_LOT.get(code, 1)
                lot_txt = f'{lot:,} ' if lot != 1 else ''
                out.append((code, f'{lot_txt}{code}/MYR · {name}', lvl, 'FX'))

        # ── S&P 500 constituents, group 'US-Stock' ──────────────────────────────
        # 503 tickers — cache-ONLY lookup here (no per-symbol cold fetch, that
        # would hang the UI). _ensure_us_universe_warm() batch-fetches all of
        # them in the background the first time ZDWS/ZDOS is opened each
        # session; whatever's landed in the cache by the time this runs shows
        # up, the rest appears automatically once the background fetch lands
        # (it triggers a redraw when done).
        for sym, name in self._sp500_config():
    ...
    def _build_zdws_tab(self):
        ...
        self._ensure_us_universe_warm()   # background; re-warms whenever cache is stale
        ...
        tk.Button(hdr, text='💳', font=('Segoe UI', 9, 'bold'),
                  bg='#21262d', fg='#635BFF', relief='flat', padx=6, pady=2,
                  cursor='hand2', command=self._open_credit_buy_popup
                  ).pack(side='right', padx=(0, 4))
        ...
        groups = ['All'] + sorted({g for _, _, _, g in unders},
                                  key=lambda x: {'Index': 0, 'World': 1, 'Top Series': 2, 'Stock': 3, 'US-Stock': 4, 'US-ETF': 5, 'FX': 6, 'Crypto': 7, 'LottoStock 33': 8}.get(x, 9))
        ...
        def _apply_group_filter(*_):
            ...
            self._zdws_filtered = [i for i, (_, _, _, grp) in enumerate(unders)
                                   if g == 'All' or grp == g]
            u_cb['values'] = [f'{unders[i][1]}  —  {self._zdws_fmt_level(unders[i][0], unders[i][2])}'
        ...
        def _refresh_ctx(*_):
            ...
            if 0 <= fi < len(self._zdws_filtered):
                ...
                ctx_var.set(f'[{grp}]  Entry P₀ = {self._zdws_fmt_level(key, lvl)}   ·   Θ = 0 (no time decay)\n'
        ...
        def _mint():
            ...
            _err = self._pts_check_stake(self._pts_load(), stake)
            if _err:
                messagebox.showwarning('ZDWS', _err, parent=self); return
            d = settle_map.get(h_var.get())
            if d is None:
            ...
            if ok:
                messagebox.showinfo('ZDWS',
                    ...
                    f'Entry: {self._zdws_fmt_level(unders[i][0], unders[i][2])}\nStake: {stake:,.0f} credits\n'
        ...
        if not za:
            ...
            for w_pos in za:
                ...
                tk.Label(row, text=f"P₀ {self._zdws_fmt_level(w_pos['underlying'], w_pos['entry'])}", font=('Segoe UI', 7),
                ...
                if mv is not None:
                    ...
                    tk.Label(row, text=f"Pₜ {self._zdws_fmt_level(w_pos['underlying'], cur)}", font=('Segoe UI', 7),
                             fg=DIM, bg='#0d1117').pack(side='left', padx=3)
                    if ccy != 'MYR':
        ...
        for w_h in reversed(zh[-40:]):
            ...
            htree.insert('', 'end',
                         ...
                         values=(w_h['side'], w_h['underlying'],
                                 self._zdws_fmt_level(w_h['underlying'], w_h['entry']),
                                 self._zdws_fmt_level(w_h['underlying'], w_h.get('settled_level', 0)),
    ...
    def _build_zdos_tab(self):
        ...
        def _refresh_ctx(*_):
            ...
            if 0 <= fi < len(self._zdos_filtered) and ti >= 0:
                ...
                if strat == 'Single Leg':
                    ...
                    ctx_var.set(f'[{grp}]  Spot {self._zdws_fmt_level(key, lvl)}  →  '
                                f'Strike {self._zdws_fmt_level(key, strike)} '
                                f'({t["pct"]:.0f}% away)   ·   Θ = 0\n'
                    ...
                    for pt, tn in legs:
                        ...
                        lines.append(f'{pt.replace("_"," ").title()} T{tn} @ '
                                    f'{self._zdws_fmt_level(key, strike)} (×{mult:.2f})')
                    ctx_var.set(f'[{grp}]  Spot {self._zdws_fmt_level(key, lvl)}  ·  {len(legs)} legs, '
        ...
        def _write():
            ...
            _err = self._pts_check_stake(self._pts_load(), stake)
            if _err:
                messagebox.showwarning('ZDOS', _err, parent=self); return
            d = settle_map.get(h_var.get())
            if d is None:
            ...
            if strat == 'Single Leg':
                ...
                ok_msg  = (f'📐 Option written!\n\n{pos_var.get().replace("_"," ").title()} '
                          f'T{t} on {unders[i][1]}\nEntry: {self._zdws_fmt_level(unders[i][0], unders[i][2])}\n'
        ...
        for o_h in reversed(oh[-40:]):
            ...
            htree.insert('', 'end',
                         ...
                         values=(o_h['postype'].replace('_', ' ').title(), o_h['tier'],
                                 ...
                                 self._zdws_fmt_level(o_h['underlying'], o_h['entry']),
                                 self._zdws_fmt_level(o_h['underlying'], o_h.get('settled_level', 0)),
...
def _sca_merge_local_transactions(rows: list, username: str = None) -> list:
    """Add the signed-in K-ID's persisted in-app rows to the source ledger."""
    try:
        who = (username if username is not None else CURRENT_KID or '').strip().lower()
        overlay = load_app_settings().get('sca_local_transactions', {}).get(who, [])
        return list(rows or []) + [dict(row) for row in overlay if isinstance(row, dict)]
    except Exception:
        return list(rows or [])


def sca_record_local_transaction(row: dict, username: str = None) -> bool:
    """Immediately persist an app-created transaction for one K-ID.

    Published Google Sheets are read-only here, so this local overlay is merged
    whenever SCA reloads. It survives logout, restart, and quick-switch.
    """
    required = {_NC_DATE, _NC_TYPE, _NC_ASSET, _NC_CCY, _NC_UNIT, _NC_VALUE, _NC_AMOUNT}
    clean = {key: str(row.get(key, '')).strip() for key in required}
    if not clean[_NC_ASSET] or clean[_NC_TYPE].lower() not in {
            'buy', 'sell', 'deposit', 'withdraw', 'fees', 'dividend'}:
        return False
    try:
        settings = load_app_settings()
        who = (username if username is not None else CURRENT_KID or '').strip().lower()
        if not who:
            return False
        settings.setdefault('sca_local_transactions', {}).setdefault(who, []).append(clean)
        save_app_settings(settings)
        return True
    except Exception:
        return False
...
def _mf_load_csv(url: str = None, log_cb=None,
    ...
    if url is None:
        ...
        if src and src.get('mode') == 'csv' and src.get('csv_path'):
            ...
            with open(p, 'r', encoding='utf-8-sig') as f:
                rows = _sca_merge_local_transactions(_mf_parse_rows(f.read(), log_cb=log_cb))
                log(f'✓  NEWPOR loaded: {len(rows)} transactions including local SCA entries')
                return rows
        url = (src or {}).get('url')
        if not url:
            raise ValueError(
    ...
    rows = _sca_merge_local_transactions(_mf_parse_rows(raw_text, log_cb=log_cb))
...
class LottoStockWindow(tk.Toplevel):
    ...
    def open_road_confirm(self):
        ...
        if is_etf:
            ...
            sca_record_local_transaction({
                _NC_DATE: date_str, _NC_TYPE: 'Buy', _NC_ASSET: self.road_target['code'].upper(),
                _NC_CCY: 'USD', _NC_UNIT: f'{units:.4f}',
                _NC_VALUE: f'{price:.4f}', _NC_AMOUNT: f'{units * price:.4f}',
            })
            ...
            lotto_record_buy_to_myfolio(
                self.sca_portfolios, self.road_target["code"], units, price, date_str)
            sca_record_local_transaction({
                _NC_DATE: date_str, _NC_TYPE: 'Buy', _NC_ASSET: f"{self.road_target['code']}.KL",
                _NC_CCY: 'MYR', _NC_UNIT: f'{units:.4f}',
                _NC_VALUE: f'{price:.4f}', _NC_AMOUNT: f'{units * price:.4f}',
            })
    ...
    def _confirm_transaction(self, stock):
        ...
        if is_etf:
            ...
            if units > 0:
                ...
                sca_record_local_transaction({
                    _NC_DATE: date_str, _NC_TYPE: 'Buy', _NC_ASSET: code.upper(),
                    _NC_CCY: 'USD', _NC_UNIT: f'{units:.4f}',
                    _NC_VALUE: f'{price:.4f}', _NC_AMOUNT: f'{units * price:.4f}',
                })
            ...
            lotto_record_buy_to_myfolio(
                self.sca_portfolios, code, units, price, date_str)
            sca_record_local_transaction({
                _NC_DATE: date_str, _NC_TYPE: 'Buy', _NC_ASSET: f'{code}.KL',
                _NC_CCY: 'MYR', _NC_UNIT: f'{units:.4f}',
                _NC_VALUE: f'{price:.4f}', _NC_AMOUNT: f'{units * price:.4f}',
            })
...
class SCAWindow(tk.Toplevel):
    ...
    def __init__(self, parent, preloaded_rows=None, preloaded_portfolios=None,
        ...
        # Live ledger polling.  _refresh() below always bypasses the 5-minute
        # startup cache, so a new source transaction appears on the next tick.
        self._auto_secs    = 15
        ...
        self.opie_context     = opie_context or {}
    ...
    def _load_from_cache_then_refresh(self):
        ...
        self.after(1500, lambda: _maybe_show_scheduled_summary(self))
    ...
    def _redraw_portfolio_chart(self):
        ...
        # ── Currency-forcing rule: SCA shows ONE currency, never mixes them.
        # The base currency is whichever the majority of portfolios use
        # (all-MYR -> force USD into MYR; all-USD -> force MYR into USD;
        # mixed -> majority wins). Every amount below is converted through
        # self._fx_rates (ccy -> MYR rate, MYR:1.0) before it's summed.
        ccy_counts = {}
        for port in self._portfolios.values():
            c = port.get('base_currency', 'USD')
            ccy_counts[c] = ccy_counts.get(c, 0) + 1
        base_ccy = max(ccy_counts, key=ccy_counts.get) if ccy_counts else 'MYR'
        self._pchart_base_ccy = base_ccy
        fx = self._fx_rates or {'MYR': 1.0}

        def _to_base(amount, ccy):
            if ccy == base_ccy:
                return amount
            amount_myr = amount if ccy == 'MYR' else amount * fx.get(ccy, 1.0)
            return amount_myr if base_ccy == 'MYR' else amount_myr / fx.get(base_ccy, 1.0)
        ...
        for port_key, port in self._portfolios.items():
            ...
            prefix = ccy_symbol(ccy)
            for ticker, item in port.get('holdings', {}).items():
                for txn in item.get('transactions', []):
                    ttype, date_s, unit, price, amount = txn
                    try:
                        d = _dt.datetime.strptime(date_s, '%m/%d/%Y').date()
                        all_txns.append((d, ttype, _to_base(amount, ccy), ccy))
                    except ValueError:
                        pass
            ...
            for txn in port.get('_deposits', []):
                try:
                    ...
                    all_txns.append((d, 'deposit', _to_base(txn[1], ccy), ccy))
                except Exception:
                    pass
        ...
        for port_key, port in self._portfolios.items():
            port_ccy = port.get('base_currency', 'USD')
            for ticker, item in port.get('holdings', {}).items():
                if item.get('_is_cash'): continue
                mkt = item.get('market', 'MY')
                ...
                if hist is None or hist.empty:
                    ...
                    if p:
                        total_mv += _to_base(item['shares'] * p, port_ccy)
                    continue
                # Reindex onto full daily calendar with forward-fill
                ph = hist.reindex(full_cal, method='ffill').bfill()
                mv_native = item['shares'] * ph.fillna(0.0)
                if port_ccy == base_ccy:
                    total_mv += mv_native
                else:
                    mv_myr = mv_native if port_ccy == 'MYR' else mv_native * fx.get(port_ccy, 1.0)
                    total_mv += mv_myr if base_ccy == 'MYR' else mv_myr / fx.get(base_ccy, 1.0)
    ...
    def _on_pchart_hover(self, event):
        ...
        ret_pct = (ret_val / dep_val * 100) if dep_val > 0 else 0.0
        ...
        prefix = ccy_symbol(getattr(self, '_pchart_base_ccy', 'MYR'))
        ann = ax.annotate(
            f'{dt_val.strftime("%d %b %Y")}\n'
            f'Total returns after fees\n'
            f'{prefix}{ret_val:+,.2f}  ({ret_pct:+.2f}%)\n\n'
            f'○ {prefix}{mv_val:,.2f}\n'
            f'○ {prefix}{dep_val:,.2f}',
            xy=(dt_val, mv_val),
    ...
    def _build_header(self):
        ...
        # OPIE advisory label removed — replaced by lightweight chat trigger below.
        self._opie_var = tk.StringVar(value='')

        # ── OPIE chat trigger — type "Hi, Opie" for a quick portfolio summary.
        # Self-contained: no external module, reads whatever SCA already has
        # loaded (positions, base currency, cost/value) at the moment asked.
        opie_box = tk.Frame(hdr, bg=self.BG)
        opie_box.pack(side='right', padx=(0, 4))
        self._opie_entry_var = tk.StringVar(value='')
        opie_entry = tk.Entry(opie_box, textvariable=self._opie_entry_var,
                               font=('Courier New', 8), width=22,
                               bg='#1a2035', fg=self.FG, insertbackground=self.FG,
                               relief='flat')
        opie_entry.insert(0, 'Type: Hi, Opie')
        opie_entry.configure(fg=self.DIM)

        def _clear_placeholder(event=None):
            if opie_entry.get() == 'Type: Hi, Opie':
                opie_entry.delete(0, 'end')
                opie_entry.configure(fg=self.FG)

        def _restore_placeholder(event=None):
            if not opie_entry.get().strip():
                opie_entry.insert(0, 'Type: Hi, Opie')
                opie_entry.configure(fg=self.DIM)

        opie_entry.bind('<FocusIn>', _clear_placeholder)
        opie_entry.bind('<FocusOut>', _restore_placeholder)
        opie_entry.bind('<Return>', lambda e: _opie_open_chat(self, self, CURRENT_KID))
        opie_entry.pack(side='left')
        tk.Button(opie_box, text='🤖', font=('Segoe UI', 8), relief='flat',
                  bg='#1a2035', fg=self.TEAL, cursor='hand2',
                  command=lambda: _opie_open_chat(self, self, CURRENT_KID)).pack(side='left', padx=(3, 0))
        ...
        if (CURRENT_KID or '').strip().lower() == ADMIN_KID:
            tk.Button(ctrl, text='🎁 SoR Settings', font=self.FONT, bg='#1a2035',
                      fg='#00e676', relief='flat', padx=8, pady=4, cursor='hand2',
                      command=self._open_sor_settings).pack(side='left', padx=(6, 2))
        tk.Button(ctrl, text='↻', font=self.FONT_B, bg='#1a2035', fg=self.TEAL,
                  relief='flat', padx=8, pady=4, cursor='hand2',
                  command=self._refresh).pack(side='left', padx=(6, 2))

        # OPIE response row — hidden until a chat message is answered.
        self._opie_reply_row = tk.Frame(self, bg=self.BG)
        self._opie_reply_var = tk.StringVar(value='')
        tk.Label(self._opie_reply_row, text='🤖 OPIE:', font=self.FONT_B,
                 fg=self.TEAL, bg=self.BG).pack(side='left', padx=(14, 4))
        tk.Label(self._opie_reply_row, textvariable=self._opie_reply_var,
                 font=self.FONT, fg=self.FG, bg=self.BG,
                 anchor='w', justify='left', wraplength=1100).pack(side='left', fill='x')

    def _opie_chat_submit(self, entry_widget):
        """Handle whatever was typed into the OPIE chat box. Only greeting
        phrases trigger a reply — anything else is quietly ignored so the
        box doesn't feel like a black hole, but also isn't a real chatbot."""
        text = entry_widget.get().strip()
        if not text or text == 'Type: Hi, Opie':
            return
        greeting = text.lower().replace(',', '').replace('!', '').strip()
        if greeting in ('hi opie', 'hello opie', 'hey opie', 'opie', 'hi opie summary'):
            self._opie_reply_var.set(self._opie_lightweight_summary())
            self._opie_reply_row.pack(fill='x', padx=0, pady=(0, 4), after=self.winfo_children()[0])
        else:
            self._opie_reply_var.set("Try: \"Hi, Opie\" for a quick portfolio summary.")
            self._opie_reply_row.pack(fill='x', padx=0, pady=(0, 4), after=self.winfo_children()[0])
        entry_widget.delete(0, 'end')
        self.focus_set()

    def _opie_lightweight_summary(self) -> str:
        """Self-contained portfolio summary — no OPIE module dependency.
        Reads whatever SCA already has loaded: position count, diversification
        (same buckets OPIE used to report), and real value/return figures
        OPIE itself never actually computed, converted into the forced base
        currency so nothing gets mixed across MYR/USD."""
        portfolios = self._portfolios or {}
        if not portfolios:
            return "Hi! No portfolio loaded yet — connect a source first."

        base_ccy = getattr(self, '_pchart_base_ccy', None)
        ccy_counts = {}
        for port in portfolios.values():
            c = port.get('base_currency', 'USD')
            ccy_counts[c] = ccy_counts.get(c, 0) + 1
        ...
        for port in portfolios.values():
            ...
            for ticker, item in port.get('holdings', {}).items():
                ...
                if not shares or shares <= 0:
                    continue
                positions  += 1
                cost_total += _to_base(item.get('cost_value', 0.0), ccy)
                p = price_data.get(ticker)
                mv_total += _to_base(shares * p, ccy) if p else _to_base(item.get('cost_value', 0.0), ccy)

        diversification = ('not available' if not positions else
                            'concentrated' if positions < 4 else
                            'developing' if positions < 8 else 'diversified')
        ret_val = mv_total - cost_total
        ret_pct = (ret_val / cost_total * 100) if cost_total > 0 else 0.0

        return (f"{positions} active position(s), diversification is {diversification} "
                f"({len(ccy_counts)} currency source(s), forced to {base_ccy}). "
                f"Value: {prefix}{mv_total:,.2f} | "
                f"Unrealized: {prefix}{ret_val:+,.2f} ({ret_pct:+.2f}%).")

    def _open_sor_settings(self):
        """🎁 SoR Settings — admin-tunable Save-for-Rewards rates, kept here
        in SCA since this is where saved holdings (the SoR input) live.
        Two modes: Default (preset per-region rates, gated by a minimum
        holding) or Custom (one flat units-per-Cr rule for everything).
        Also links straight into this K-ID's SoR-tagged credit history so
        rate changes can be checked against real payout events."""
        if (CURRENT_KID or '').strip().lower() != ADMIN_KID:
            messagebox.showwarning('SoR Settings', 'Admin access only.', parent=self)
            return
        app = self.master
        cfg = app._sor_config()

        win = tk.Toplevel(self)
        win.title('🎁 SoR Settings')
        win.configure(bg=self.BG)
        win.geometry('460x600')
        win.transient(self); win.grab_set()

        tk.Label(win, text='🎁  SAVE FOR REWARDS (SoR) — RATES',
                 font=('Courier New', 12, 'bold'), fg=self.TEAL, bg=self.BG).pack(pady=(14, 2))
        tk.Label(win, text='Changes apply to the next SoR run (once per calendar day).',
                 font=('Courier New', 7), fg=self.DIM, bg=self.BG).pack(pady=(0, 10))

        body = tk.Frame(win, bg=self.BG); body.pack(fill='both', expand=True, padx=16)

        # ── Mode note ─────────────────────────────────────────────────────
        # Mode itself is no longer a global admin switch — Default is the
        # automatic baseline the moment a K-ID is valid/signed in, and
        # Custom only ever kicks in for a K-ID that has explicitly turned
        # it on for themselves (⚙ SoR Mode, in the Rewards tab). What's
        # tunable here is just the RATES each mode uses when active.
        tk.Label(body, text='ℹ️  Default mode is always-on automatically for every '
                            'valid K-ID. Custom mode only applies to a K-ID that has '
                            'switched it on themselves (⚙ SoR Mode, Rewards tab). '
                            'The rates below apply whichever mode a given K-ID is in.',
                 font=('Courier New', 7), fg=self.DIM, bg=self.BG,
                 wraplength=400, justify='left').pack(anchor='w', pady=(0, 10))

        default_fields = [
            ('my_min_units',         'MY (Lotto33 + all MY stocks): min units to earn'),
            ('us_min_units',         'US stocks/ETFs: min units to earn'),
            ('us_share_per_cr',      'US: shares per 1 Cr'),
            ('my_lotto_cr_per_unit', 'MY StockLotto33: Cr per unit'),
            ('my_stock_per_cr',      'MY non-Lotto33 stocks: Cr per unit'),
        ]
        custom_fields = [
            ('custom_threshold',     'Custom mode: Y — units per credit (user-adjustable default)'),
            ('custom_credits',       'Custom mode: X — Cr per Y units (ADMIN-LOCKED base unit)'),
        ]
        shared_fields = [
            ('fep_multiplier',       'Beta Supporter (FEP) multiplier'),
            ('supporter_multiplier', 'Other paid tiers multiplier'),
        ]

        entries = {}
        row_widgets = {}

        def _add_field(key, label):
            row = tk.Frame(body, bg=self.BG); row.pack(fill='x', pady=4)
            lbl = tk.Label(row, text=label, font=self.FONT, fg=self.FG, bg=self.BG,
                           width=30, anchor='w')
            lbl.pack(side='left')
            var = tk.StringVar(value=str(cfg.get(key, '')))
            ent = tk.Entry(row, textvariable=var, font=self.FONT, width=10,
                            bg=self.PANEL, fg=self.FG, insertbackground=self.FG,
                            relief='flat')
            ent.pack(side='left')
            entries[key] = var
            row_widgets[key] = (row, lbl, ent)

        tk.Label(body, text='— Default mode rates —', font=('Courier New', 8, 'bold'),
                 fg=self.GOLD, bg=self.BG).pack(anchor='w', pady=(4, 0))
        for key, label in default_fields:
            _add_field(key, label)

        tk.Label(body, text='— Custom mode rate —', font=('Courier New', 8, 'bold'),
                 fg=self.GOLD, bg=self.BG).pack(anchor='w', pady=(10, 0))
        for key, label in custom_fields:
            _add_field(key, label)

        tk.Label(body, text='— Supporter multipliers (both modes) —',
                 font=('Courier New', 8, 'bold'), fg=self.GOLD, bg=self.BG
                 ).pack(anchor='w', pady=(10, 0))
        for key, label in shared_fields:
            _add_field(key, label)

        tk.Label(body, text='Road to Reward thresholds (% saved → bonus Cr) are '
                             'a separate diversification bonus, untouched by mode — '
                             'still kept at built-in defaults; edit paa_settings.json '
                             '"sor_config.road_thresholds" directly for finer control.',
                 font=('Courier New', 7), fg=self.DIM, bg=self.BG,
                 wraplength=400, justify='left').pack(anchor='w', pady=(12, 0))

        status_lbl = tk.Label(win, text='', font=self.FONT, bg=self.BG)
        status_lbl.pack(pady=(4, 0))

        def _save():
            new_cfg = dict(cfg)
            try:
                for key, _ in default_fields + custom_fields + shared_fields:
                    new_cfg[key] = float(entries[key].get())
            except ValueError:
                status_lbl.config(text='All rates must be numbers.', fg=self.RED)
                return
            app._sor_save_config(new_cfg)
            status_lbl.config(text='✓ Saved.', fg=self.GREEN)

        btn_row = tk.Frame(win, bg=self.BG); btn_row.pack(fill='x', padx=16, pady=(10, 6))
        tk.Button(btn_row, text='💾 Save', command=_save, bg='#00e676', fg='#0a0e1a',
                  relief='flat', padx=12, pady=4, cursor='hand2',
                  font=('Courier New', 9, 'bold')).pack(side='left')
        tk.Button(btn_row, text='🧾 View My SoR Credit History',
                  command=lambda: app._open_credit_history_popup(app._pts_load()),
                  bg='#1a2035', fg='#FFD700', relief='flat', padx=12, pady=4,
                  cursor='hand2', font=self.FONT).pack(side='left', padx=(8, 0))

        def _force_rerun():
            # Testing aid: SoR pays at most once per calendar day per K-ID
            # (pts['sor_last_date']). If it already ran today — e.g. while
            # your saved portfolio data was wrong/duplicated — every retry
            # today silently no-ops even after you fix the data. This
            # clears today's gate for THIS K-ID only and re-runs SoR
            # immediately, so you don't have to wait until tomorrow.
            pts = app._pts_load()
            pts['sor_last_date'] = ''
            app._pts_save(pts)
            app._sor_run_and_popup()
            status_lbl.config(text='✓ Re-ran SoR for today.', fg=self.GREEN)
        tk.Button(btn_row, text='🔁 Force Re-run SoR (today)', command=_force_rerun,
                  bg='#2a0a0a', fg='#F85149', relief='flat', padx=12, pady=4,
                  cursor='hand2', font=self.FONT).pack(side='left', padx=(8, 0))
    ...
    def _refresh_source_label(self):
        try:
            ...
            label = src.get('label', '')
            # The stored label's txn count is a snapshot from whenever the
            # source was last (re)connected — swap in the LIVE row count so
            # this doesn't go stale between reconnects (auto-refresh updates
            # self._rows every 60s, but never touched this label before).
            live_n = len(self._rows or [])
            if live_n and '(' in label and 'txns)' in label:
                label = label.split('(')[0].rstrip() + f' ({live_n} txns)'
            self._src_var.set(f'{icon} {who} · {label}')
        except Exception:
            self._src_var.set('')
    ...
    def _build_tabs(self):
        ...
        self._tab_btns = {}
        # Driven by MARKET_SPEC (not hardcoded to MYR/USD) so a HKD or SGD
        # portfolio — or any future market registered there — gets its own
        # filter tab instead of being stuck lumped into 'ALL' with no way
        # to view it on its own.
        tab_options = [('⊕ ALL', 'ALL')] + [
            (spec['label'], spec['market']) for spec in MARKET_SPEC.values()
        ]
        for label, val in tab_options:
        ...
        # Combines: allocation by region + by portfolio (ledger), a
        # forced-single-currency summary, and a returns table with 3 modes
        # (% return / native gain-loss / forced-ccy).
        self._portview_open = tk.BooleanVar(value=False)
        self._portview_btn = tk.Button(
            tab_row, text='📊  Portfolio View', font=self.FONT_B,
            bg=self.PANEL, fg=self.GOLD,
            relief='flat', cursor='hand2', padx=14, pady=6,
            command=self._toggle_portview)
        self._portview_btn.pack(side='left', padx=(10, 0))

        self._portview_panel = tk.Frame(self._scroll_body, bg=self.BG)
        self._portview_forced_ccy_var = tk.StringVar(value='')
        self._portview_mode_var       = tk.StringVar(value='PCT')  # PCT / GAINLOSS / FORCED

    def _toggle_portview(self):
        """Show/hide the Portfolio View panel. Built lazily, same pattern as
        the Composition pie — no cost until someone actually opens it."""
        opening = not self._portview_open.get()
        self._portview_open.set(opening)
        if opening:
            anchor = getattr(self, '_holdings_anchor', None)
            if anchor is not None and anchor.winfo_exists():
                self._portview_panel.pack(fill='x', padx=14, pady=(0, 6), before=anchor)
            else:
                self._portview_panel.pack(fill='x', padx=14, pady=(0, 6))
            self._portview_btn.configure(text='📊  Portfolio View  ▲')
            self._draw_portview()
        else:
            self._portview_panel.pack_forget()
            self._portview_btn.configure(text='📊  Portfolio View')

    def _portview_collect(self):
        """One row per priced holding, in native currency plus converted into
        a single forced base currency. Mirrors the same currency-forcing rule
        the value chart/pie already use: majority currency wins unless the
        user has picked one explicitly via the Summary dropdown."""
        portfolios = self._portfolios or {}
        price_data = self._last_price_data or {}
        fx         = self._fx_rates or {'MYR': 1.0}
        ...
        for port in portfolios.values():
            c = port.get('base_currency', 'USD')
            ccy_counts[c] = ccy_counts.get(c, 0) + 1
        majority = max(ccy_counts, key=ccy_counts.get) if ccy_counts else 'MYR'
        base_ccy = self._portview_forced_ccy_var.get() or majority
        ...
        rows = []
        for pk, port in portfolios.items():
            ccy    = port.get('base_currency', 'USD')
            market = MARKET_SPEC.get(ccy, {}).get('market', 'US')
            label  = MARKET_SPEC.get(ccy, {}).get('label', ccy)
            for ticker, item in port.get('holdings', {}).items():
                if item.get('_is_cash'):
                    continue
                shares, cost_v = item.get('shares', 0), item.get('cost_value', 0)
                price = price_data.get(ticker)
                if not price or shares <= 0:
                    continue
                mv_native = shares * price
                pl_native = mv_native - cost_v
                pl_pct    = (pl_native / cost_v * 100) if cost_v else None
                rows.append({
                    'ticker': ticker, 'port_key': pk, 'port_label': label,
                    'region': market, 'ccy': ccy,
                    'mv_native': mv_native, 'cost_native': cost_v,
                    'pl_native': pl_native, 'pl_pct': pl_pct,
                    'mv_base':   _to_base(mv_native, ccy),
                    'cost_base': _to_base(cost_v, ccy),
                    'pl_base':   _to_base(pl_native, ccy),
                })
        return rows, base_ccy, sorted(ccy_counts.keys())

    def _draw_portview(self):
        """Rebuild the whole Portfolio View panel: summary, region/portfolio
        allocation, and the 3-mode returns table."""
        if not self._portview_open.get():
            return
        for w in self._portview_panel.winfo_children():
            w.destroy()

        rows, base_ccy, avail_ccys = self._portview_collect()
        if not rows:
            tk.Label(self._portview_panel, text='No priced holdings to summarize yet.',
                     font=self.FONT, fg=self.DIM, bg=self.BG).pack(pady=10)
            return

        container = tk.Frame(self._portview_panel, bg=self.PANEL,
                             highlightbackground=self.BORDER, highlightthickness=1)
        container.pack(fill='x')

        tk.Label(container, text='PORTFOLIO VIEW', font=self.FONT_B, fg=self.GOLD,
                 bg=self.PANEL, anchor='w', padx=12, pady=(8, 2)).pack(fill='x')

        self._portview_build_summary(container, rows, base_ccy, avail_ccys)
        self._portview_build_allocation(container, rows, 'region',     'By region')
        self._portview_build_allocation(container, rows, 'port_label', 'By portfolio (ledger)')
        self._portview_build_returns(container, rows, base_ccy)

    def _portview_build_summary(self, parent, rows, base_ccy, avail_ccys):
        """Summary mode — everything forced into ONE currency, selectable."""
        sfx          = ccy_symbol(base_ccy)
        total_mv     = sum(r['mv_base']   for r in rows)
        total_cost   = sum(r['cost_base'] for r in rows)
        ...
        tk.Label(row, text='Summary  ·  forced ccy:', font=self.FONT, fg=self.FG2,
                 bg=self.PANEL).pack(side='left')

        ccy_opts = list(avail_ccys) or ['MYR']
        if base_ccy not in ccy_opts:
            ccy_opts = ccy_opts + [base_ccy]
        if not self._portview_forced_ccy_var.get():
            self._portview_forced_ccy_var.set(base_ccy)
        om = tk.OptionMenu(row, self._portview_forced_ccy_var, *ccy_opts,
                           command=lambda _v: self._draw_portview())
        om.configure(font=self.FONT_B, bg=self.PANEL, fg=self.TEAL, relief='flat',
                     highlightthickness=0, activebackground=self.PANEL)
        om.pack(side='left', padx=(4, 0))

        cards = tk.Frame(parent, bg=self.PANEL)
        cards.pack(fill='x', padx=12, pady=(0, 8))
        color = self.GREEN if total_pl >= 0 else self.RED
        for text, val, clr in (
            ('Total value', f'{sfx}{total_mv:,.2f}',   self.FG),
            ('Total cost',  f'{sfx}{total_cost:,.2f}', self.FG2),
            ('Gain / loss', f'{sfx}{total_pl:+,.2f}',  color),
            ('Return %',    f'{total_pl_pct:+.2f}%',   color),
        ):
            c = tk.Frame(cards, bg=self.BG, highlightbackground=self.BORDER,
                        highlightthickness=1, padx=10, pady=6)
            c.pack(side='left', fill='x', expand=True, padx=(0, 6))
            tk.Label(c, text=text, font=('Courier New', 7), fg=self.DIM, bg=self.BG,
                     anchor='w').pack(fill='x')
            tk.Label(c, text=val, font=self.FONT_B, fg=clr, bg=self.BG,
                     anchor='w').pack(fill='x')

    def _portview_build_allocation(self, parent, rows, group_key, title):
        """Allocation breakdown bar — by region (market) or by portfolio
        (the code recorded per-row in the ledger), both in forced ccy."""
        grouped = {}
        for r in rows:
            grouped[r[group_key]] = grouped.get(r[group_key], 0.0) + r['mv_base']
        total = sum(grouped.values())
        if not total:
            return
        tk.Label(parent, text=title, font=self.FONT, fg=self.FG2, bg=self.PANEL,
                 anchor='w', padx=12, pady=(4, 2)).pack(fill='x')
        bar_wrap = tk.Frame(parent, bg=self.PANEL)
        bar_wrap.pack(fill='x', padx=12, pady=(0, 4))
        palette   = ['#00b4d8', '#00e676', '#ffd166', '#ff6b6b', '#a78bfa', '#4a90d9']
        bar_outer = tk.Frame(bar_wrap, bg='#1a2035', height=18)
        bar_outer.pack(fill='x')
        bar_outer.pack_propagate(False)
        legend = tk.Frame(bar_wrap, bg=self.PANEL)
        legend.pack(fill='x', pady=(3, 0))
        for i, (name, val) in enumerate(sorted(grouped.items(), key=lambda x: -x[1])):
            pct   = val / total
            color = palette[i % len(palette)]
            seg = tk.Frame(bar_outer, bg=color, width=max(1, int(pct * 900)), height=18)
            seg.pack(side='left', fill='y')
            tk.Label(legend, text=f'● {name}  {pct*100:.1f}%', font=('Courier New', 7),
                     fg=color, bg=self.PANEL).pack(side='left', padx=(0, 10))

    def _portview_build_returns(self, parent, rows, base_ccy):
        """Returns table — 3 display modes:
          PCT      → % return only
          GAINLOSS → gain/loss in the holding's own (native) currency + %
          FORCED   → value & gain/loss converted into the forced base ccy
        """
        hdr = tk.Frame(parent, bg=self.PANEL)
        hdr.pack(fill='x', padx=12, pady=(6, 2))
        tk.Label(hdr, text='Returns', font=self.FONT, fg=self.FG2, bg=self.PANEL).pack(side='left')

        for label, val in (('% Return', 'PCT'), ('Gain / Loss', 'GAINLOSS'), ('Forced ccy', 'FORCED')):
            active = self._portview_mode_var.get() == val
            tk.Radiobutton(hdr, text=label, variable=self._portview_mode_var, value=val,
                          command=self._draw_portview, font=self.FONT_B, relief='flat',
                          cursor='hand2', bg=self.TEAL if active else self.PANEL,
                          fg='#000' if active else self.FG2, selectcolor=self.TEAL,
                          activebackground=self.PANEL, activeforeground=self.TEAL,
                          indicatoron=False, padx=8, pady=2).pack(side='right', padx=2)

        mode = self._portview_mode_var.get()
        tree_wrap = tk.Frame(parent, bg=self.PANEL)
        tree_wrap.pack(fill='x', padx=12, pady=(2, 10))

        if mode == 'PCT':
            cols = ('TICKER', 'PORTFOLIO', '% RETURN')
        elif mode == 'GAINLOSS':
            cols = ('TICKER', 'PORTFOLIO', 'CCY', 'GAIN/LOSS', '% RETURN')
        else:
            cols = ('TICKER', 'PORTFOLIO', f'VALUE ({base_ccy})', f'GAIN/LOSS ({base_ccy})')

        tv = ttk.Treeview(tree_wrap, columns=cols, show='headings',
                          height=min(10, max(3, len(rows))))
        for c in cols:
            tv.column(c, width=130, anchor='w' if c in ('TICKER', 'PORTFOLIO') else 'e',
                      stretch=True)
            tv.heading(c, text=c)
        tv.configure(style='SCA.Treeview')
        tv.tag_configure('gain', foreground=self.GREEN)
        tv.tag_configure('loss', foreground=self.RED)
        tv.pack(fill='x')

        for r in sorted(rows, key=lambda r: -(r['pl_pct'] or 0)):
            tag = 'gain' if (r['pl_pct'] or 0) >= 0 else 'loss'
            if mode == 'PCT':
                tv.insert('', 'end', tags=(tag,), values=(
                    r['ticker'], r['port_label'],
                    f"{r['pl_pct']:+.2f}%" if r['pl_pct'] is not None else '—'))
            elif mode == 'GAINLOSS':
                sfx = ccy_symbol(r['ccy'])
                tv.insert('', 'end', tags=(tag,), values=(
                    r['ticker'], r['port_label'], r['ccy'],
                    f"{sfx}{r['pl_native']:+,.2f}",
                    f"{r['pl_pct']:+.2f}%" if r['pl_pct'] is not None else '—'))
            else:
                sfx = ccy_symbol(base_ccy)
                tv.insert('', 'end', tags=(tag,), values=(
                    r['ticker'], r['port_label'],
                    f"{sfx}{r['mv_base']:,.2f}", f"{sfx}{r['pl_base']:+,.2f}"))
    ...
    def _update_hold_columns(self):
        """Show/hide the PORTFOLIO and CCY columns depending on what Source
        actually loaded — a ledger that only ever holds one currency (the
        common case: a plain single-market Google Sheet or CSV) doesn't need
        a PORTFOLIO or CCY column repeating the same value on every row.
        Both reappear automatically the moment a second currency/portfolio
        shows up (e.g. the user links a second market), no settings toggle
        needed — it just reflects what's actually in the ledger."""
        portfolios = self._portfolios or {}
        active = [p for p in portfolios.values() if p.get('holdings')]
        ccys = {p.get('base_currency', 'USD') for p in active}
        show_portfolio = len(active) > 1
        show_ccy       = len(ccys) > 1
        cols = self._hold_tree['columns']
        display = [c for c in cols
                   if (c != 'PORTFOLIO' or show_portfolio)
                   and (c != 'CCY' or show_ccy)]
        self._hold_tree['displaycolumns'] = display
    ...
    def _build_holdings_table(self):
        ...
        tk.Button(hdr, text='📒 Holdings Report', font=('Courier New', 8, 'bold'),
                  bg='#1a2035', fg=self.TEAL, relief='flat', bd=0, padx=8,
                  command=self._open_holdings_report_popup).pack(side='right', padx=(0, 10))
    ...
    def _on_sheet_loaded(self, ports, valid, n_before=0, n_after=0):
        self._sheet_ok = True
        self._set_indicator(sheet=True)
        self._refresh_source_label()   # keep the header's txn count live
        if not valid.get('ok'):
    ...
    def _on_prices_loaded(self, price_data, history_data, fx_ok):
        ...
        self._draw_portview()     # no-op while collapsed
        self._update_time()
    ...
    def _render_all(self, price_data, history_data, placeholder=False):
        ...
        # Show/hide holdings-table columns based on what's actually loaded
        # from Source — a single-currency, single-portfolio ledger doesn't
        # need a PORTFOLIO or CCY column cluttering every row with the same
        # repeated value.
        self._update_hold_columns()
    ...
    def _open_holdings_report_popup(self, ticker=None):
        """Explicit trigger for the T80 position-detail popup (ledger +
        price chart) — reachable via the 'Holdings Report' button, a row
        click on the holdings table, or Opie ('show holdings report for
        <ticker>'), instead of only the row-click that existed before.
        With no ticker given, uses the currently selected row, or — if
        nothing is selected and there's exactly one holding — that one;
        otherwise prompts the user to pick a row first."""
        if not ticker:
            sel = self._hold_tree.selection()
            if sel:
                vals = self._hold_tree.item(sel[0], 'values')
                if vals:
                    ticker = vals[0]
        if not ticker:
            all_tickers = [t for port in self._portfolios.values()
                           for t, item in port.get('holdings', {}).items()
                           if not item.get('_is_cash')]
            if len(all_tickers) == 1:
                ticker = all_tickers[0]
        if not ticker:
            messagebox.showinfo('Holdings Report',
                                 'Select a holding row first, then click '
                                 '"📒 Holdings Report" (or click the row directly).')
            return
        self._open_stock_chart(ticker)
    ...
    def _auto_tick(self):
        # Do a real source check rather than reusing the startup cache.  This
        # lets a newly added Google Sheet/CSV transaction flow into SCA while
        # the window is open, even after a quick K-ID switch.
        self._refresh(force_online=True)
        self._schedule_auto()
...
def sp500_prefetch_price_and_dividends(log_cb=None):
    ...
    CHUNK = _smart_chunk_size(len(tickers), max_single_chunk=100, min_chunk=25, target_chunks=6)
                    # scales with universe size — small universes go out as one
                    # chunk; splitting only kicks in once there's enough to
                    # actually parallelize, still capped so no single failed/
                    # throttled call can wipe out the whole universe
...
class SplashScreen(tk.Tk):
    ...
    def _open_dashboard(self):
        ...
        # Auto-verify any pending Stripe purchase for this K-ID right away —
        # this is what makes "pay → reopen app → tier active" automatic
        # without the admin doing a manual Sync.
        app.after(1000, app._stripe_autoverify_current_user)
        app.mainloop()
...
_DEFAULT_SETTINGS = {
    ...
    "sca_sources":   {},
    # App-created SCA transactions are kept locally per K-ID. This protects
    # confirmed trades when the app reloads a read-only published ledger.
    "sca_local_transactions": {},
    "pts": {
        "credits":          10_000.0,
        "predictions":      [],
        "history":          [],
        ...
        "login_last_date":  "",   # 'YYYY-MM-DD' of last daily login reward
        "login_streak_day": 0,    # 1..7 position in the current streak cycle
        "login_streak_len": 0,    # total consecutive days (for streak bonus)
        "tier":             "Free",   # PAA_TIERS key; set after external payment
        "tier_started":     "",   # 'YYYY-MM-DD' current tier activated (raffle clock)
        "ads_date":         "",   # 'YYYY-MM-DD' of the ad-watch counter below
        "ads_watched":      0,    # ads claimed today (max AD_DAILY_LIMIT)
        ...
        "credit_log":       [],   # [{ts, delta, reason, balance}] — full Cr audit trail
        "sor_custom_enabled": False,  # False = Default mode (auto-active for any valid K-ID).
                                       # True only if this K-ID has explicitly opted into Custom.
    },
    # ── SoR (Save for Rewards) admin-tunable rates ───────────────────────────
    # Exposed via a "⚙ SoR Settings" panel inside SCA (admin1 only). Anything
    # not set here falls back to SaveForRewards' built-in defaults.
    "sor_config": {
        "mode":                "default",  # "default" (preset rates) or "custom" (flat units-per-Cr rule)
        "my_min_units":        100.0,  # default mode, MY (Lotto33 + all other MY stocks): min units held to earn
        "us_min_units":        0.01,   # default mode, US stocks/ETFs: min units held to earn
        "us_share_per_cr":     0.01,   # default mode, US: every this-many shares = 1 Cr
        "my_lotto_cr_per_unit": 1,     # default mode, MY StockLotto33: 1 unit = this many Cr
        "my_stock_per_cr":     1,      # default mode, MY non-Lotto33: 1 unit = this many Cr
        "custom_threshold":    0.01,   # custom mode: Y — units per X Cr; user-adjustable per K-ID
        "custom_credits":      1,      # custom mode: X — Cr granted per Y units; ADMIN-LOCKED (base credit unit for this localization)
        "fep_multiplier":      2.0,    # Beta Supporter (FEP) SoR multiplier
        "supporter_multiplier": 1.25,  # any other paid tier's SoR multiplier
        "road_thresholds": [          # [min_percent, cr_reward] — Road to Reward is separate
            [100, 200], [90, 140], [75, 120], [67, 100],  # from base SoR earn above; untouched by mode
            [50, 75], [34, 50], [25, 35], [20, 20], [10, 10],
        ],
    },
}
...
TEST_KID  = 'test1'    # reserved test K-ID — both admin1 and test1 belong to
                        # the developer for testing; quick-switch skips the
                        # PIN screen between just these two.
...
def kid_verify(settings: dict, username: str, pin: str) -> bool:
    u = username.strip().lower()
    # ── Admin account: PIN checked against settings['admin_pw_hash'] when
    # set, else the baked-in default PIN 0000. (The old 6-char minimum was
    # what silently blocked admin1 logins with the 4-digit PIN — removed.)
    if u == ADMIN_KID:
        if not pin:
            return False
        stored = settings.get('admin_pw_hash', '') or ADMIN_DEFAULT_PIN_HASH
        return stored == kid_hash_pin(ADMIN_KID, pin)
...
def kid_create(settings: dict, username: str, pin: str) -> tuple:
    ...
    u = username.strip()
    if u.strip().lower() == ADMIN_KID:
        return False, 'That username is reserved for the administrator — use "Back to sign in" and log in as admin1 instead.'
    if len(u) < 3:
...
# ── OPIE popup chat ───────────────────────────────────────────────────────────
# Stays idle until "Hi, Opie" is typed. Once activated it greets the user,
# resolves a display name (K-ID or a one-time custom alias), and then
# understands two keyword intents:
#   "change"   → lists current portfolio positions, then asks which stock
#                the user wants details on (shares / value / gain / %).
#   "how much" → full summary (value + unrealized %) converted into
#                whichever currency the user names, defaulting to the
#                portfolio's own base currency.
_OPIE_GREETINGS  = ('hi opie', 'hello opie', 'hey opie', 'opie', 'hi opie summary')
# Filler words that carry no ticker/index identity — stripped globally before
# matching a query against tickers or index names so "AAPL now", "KLCI today",
# "current price of TSLA" etc. all resolve the same as the bare name.
_OPIE_TIME_WORDS = {
    'NOW', 'TODAY', 'CURRENT', 'CURRENTLY', 'LATEST', 'RIGHT', 'MOMENT',
    'RECENT', 'RECENTLY', 'REALTIME', 'LIVE', 'TONIGHT', 'THISMORNING',
}
_OPIE_CCY_TOKENS = {
    'RM': 'MYR', 'MYR': 'MYR', 'RINGGIT': 'MYR',
    'USD': 'USD', 'US$': 'USD', 'DOLLARS': 'USD', 'DOLLAR': 'USD',
    'SGD': 'SGD', 'SG$': 'SGD', 'SINGDOLLAR': 'SGD',
    'HKD': 'HKD', 'HK$': 'HKD', 'HONGKONGDOLLAR': 'HKD',
    'EUR': 'EUR', 'EURO': 'EUR',
...
def _opie_infer_ccy(ticker: str) -> str:
    """Currency implied by a ticker's exchange suffix, via the app's own
    MARKET_SPEC registry (.KL→MYR, .HK→HKD, .SI→SGD, ...). A bare ticker
    with no recognized suffix follows the app-wide convention of USD —
    adding a new market to MARKET_SPEC is all that's needed for OPIE to
    pick it up automatically."""
    t = (ticker or '').upper()
    best_ccy, best_len = None, 0
    for ccy, spec in MARKET_SPEC.items():
        suf = spec.get('suffix', '')
        if suf and t.endswith(suf) and len(suf) > best_len:
            best_ccy, best_len = ccy, len(suf)
    return best_ccy or 'USD'


def _opie_suffix_variants(ticker: str):
    """Every plausible fully-qualified ticker to try for a bare code typed
    without an exchange suffix — one per market in MARKET_SPEC (.KL, .HK,
    .SI, bare-USD, ...) plus the ticker exactly as typed. Registering a new
    market in MARKET_SPEC (e.g. Tokyo's .T) is all OPIE needs to start
    resolving it too — nothing here has to change."""
    ...
    if '.' in t:
        return [t]
    variants = [t]
    for spec in MARKET_SPEC.values():
        suf = spec.get('suffix', '')
        if suf:
            variants.append(t + suf)
    return variants
# Generic filler words that show up around a ticker/index name in a natural
# question ("can you tell me about stock price of X", "what's X worth") —
# stripped before candidate extraction so they never get mistaken for the
# ticker itself (e.g. "ABOUT" was previously picked as a fake ticker).
_OPIE_STOPWORDS = {
    'A', 'AN', 'THE', 'IS', 'ARE', 'OF', 'FOR', 'TO', 'IN', 'ON', 'AT', 'AS',
    'AND', 'OR', 'ME', 'MY', 'I', 'YOU', 'YOUR', 'CAN', 'COULD', 'WOULD',
    'WILL', 'TELL', 'SHOW', 'GIVE', 'KNOW', 'WANT', 'ABOUT', 'WHAT', 'WHATS',
    "WHAT'S", 'WHICH', 'DO', 'DOES', 'STOCK', 'STOCKS', 'PRICE', 'PRICES',
    'PLEASE', 'PORTFOLIO', 'LIKE', 'THIS', 'THAT', 'HOW', 'MUCH', 'VALUE',
    'WORTH', 'IT', "IT'S", 'ITS',
} | _OPIE_TIME_WORDS


def _opie_ticker_candidates(text: str):
    """Tokens from free text that could plausibly be a ticker/index name —
    filler words stripped, but internal '.' and '-' (e.g. '5398.KL') kept
    so real tickers survive instead of being rejected by a plain isalnum
    check."""
    out = []
    for raw in text.split():
        w = raw.strip('.,!?;:()').upper()
        if not w or w in _OPIE_STOPWORDS:
            continue
...
def _opie_best_candidate(candidates):
    """Prefer the most ticker-like token (contains a '.' or mixes letters
    and digits, e.g. '5398.KL' or 'AAPL2'); otherwise fall back to the last
    remaining candidate, since that's usually the one actually being asked
    about ("...tell me about 5347.KL")."""
    if not candidates:
        return None
    for w in reversed(candidates):
        if '.' in w or (any(c.isalpha() for c in w) and any(c.isdigit() for c in w)):
            return w
    return candidates[-1]


def _opie_match_portfolio_ticker(host, text: str):
    """Match free text against actual portfolio holdings — exact ticker
    match first, then loose substring match ignoring punctuation. Returns
    (ticker, item, base_ccy) or None."""
    candidates = _opie_ticker_candidates(text)
    if not candidates:
        return None
    portfolios = _opie_extract_portfolios(host)
    holdings = []
    for port in portfolios.values():
        ccy = port.get('base_currency', 'USD')
        for ticker, item in port.get('holdings', {}).items():
            ...
            if not shares or shares <= 0:
                continue
            holdings.append((ticker, item, ccy))
    for cand in candidates:
        for ticker, item, ccy in holdings:
            if cand == ticker.upper():
                return ticker, item, ccy
    for cand in candidates:
        cand_bare = cand.replace('.', '').replace('-', '')
        for ticker, item, ccy in holdings:
            t_bare = ticker.upper().replace('.', '').replace('-', '')
            if cand_bare and (cand_bare == t_bare or cand in ticker.upper() or ticker.upper() in cand):
                return ticker, item, ccy
    return None


def _opie_extract_portfolios(host) -> dict:
    return getattr(host, '_portfolios', None) or getattr(host, 'newpor_portfolios', None) or {}


def _opie_extract_fx(host) -> dict:
    return getattr(host, '_fx_rates', None) or {'MYR': 1.0}


def _opie_extract_prices(host) -> dict:
    return getattr(host, '_last_price_data', None) or {}


def _build_portfolio_summary_text(host, ccy: str = 'MYR') -> str:
    """Standalone whole-portfolio summary (value, P/L, recent buys) usable
    outside a chat session — shared by Opie's 'summary' answer and the
    scheduled/on-open auto-summary popup."""
    portfolios = _opie_extract_portfolios(host)
    if not portfolios:
        return 'No portfolio loaded yet.'
    fx     = _opie_extract_fx(host)
    prices = _opie_extract_prices(host)

    def _to_ccy(amount, c):
        if c == ccy:
            return amount
        myr = amount if c == 'MYR' else amount * fx.get(c, 1.0)
        if ccy == 'MYR':
            return myr
        rate = fx.get(ccy)
        return (myr / rate) if rate else amount

    cost_total, mv_total, positions = 0.0, 0.0, 0
    all_buys = []
    for port in portfolios.values():
        p_ccy = port.get('base_currency', 'USD')
        for ticker, item in port.get('holdings', {}).items():
            if item.get('_is_cash'):
                continue
            shares = item.get('shares', 0)
            if not shares or shares <= 0:
                continue
            positions += 1
            cost_total += _to_ccy(item.get('cost_value', 0.0), p_ccy)
            price = prices.get(ticker)
            mv = shares * price if price else item.get('cost_value', 0.0)
            mv_total += _to_ccy(mv, p_ccy)
            for t in item.get('transactions', []):
                if t[0] == 'buy':
                    all_buys.append((ticker, t))
    gain = mv_total - cost_total
    pct  = (gain / cost_total * 100) if cost_total > 0 else 0.0
    prefix = 'RM' if ccy == 'MYR' else f'{ccy} '
    recent_buys = sorted(all_buys, key=lambda x: x[1][1], reverse=True)[:5]
    buys_txt = '\n'.join(f'  • {tk}  +{t[2]:g} @ {t[3]:,.4f}  ({t[1]})' for tk, t in recent_buys) \
               or '  (none on record)'
    return (f'Portfolio summary ({ccy})\n'
            f'{positions} position(s)  ·  value {prefix}{mv_total:,.2f}  ·  '
            f'unrealized {prefix}{gain:+,.2f} ({pct:+.2f}%)\n\n'
            f'Recently bought:\n{buys_txt}')


def _maybe_show_scheduled_summary(sca_window):
    """Checked once per SCAWindow open. Respects a per-K-ID setting
    (configurable via Opie: 'set summary on open' / 'set summary time
    9:00am' / 'turn off summary schedule') for whether/when to pop up an
    automatic portfolio summary — either every time the app opens, or once
    per day at/after a fixed time."""
    import datetime as _dt
    kid = globals().get('CURRENT_KID') or ''
    if not kid:
        return
    settings = load_app_settings()
    prefs = settings.get('opie_summary_prefs', {}).get(kid.lower(), {})
    mode  = prefs.get('schedule_mode', 'off')
    if mode == 'off':
        return
    today = _dt.date.today().isoformat()
    if prefs.get('last_shown_date') == today:
        return
    if mode == 'fixed_time':
        fixed = prefs.get('fixed_time', '09:00')
        try:
    ...
    ccy = prefs.get('currency', 'MYR')
    text = _build_portfolio_summary_text(sca_window, ccy)
    prefs['last_shown_date'] = today
    settings.setdefault('opie_summary_prefs', {})[kid.lower()] = prefs
    save_app_settings(settings)

    pop = tk.Toplevel(sca_window)
    pop.title('📋 Your Portfolio Summary')
    pop.configure(bg='#0a0e1a')
    pop.geometry('440x320')
    tk.Label(pop, text=text, font=('Courier New', 9), fg='#e2e8f0',
             bg='#0a0e1a', justify='left', anchor='nw', wraplength=410,
             padx=14, pady=14).pack(fill='both', expand=True)
    tk.Button(pop, text='Close', command=pop.destroy, bg='#1a2035',
              fg='#00b4d8', relief='flat', padx=10).pack(pady=(0, 10))


def _opie_extract_mkt_data(host) -> dict:
    """Live index/FX snapshot lives on the dashboard (MYIPOApp); SCAWindow's
    host needs to hop up to its master to find it."""
    data = getattr(host, '_mkt_data', None)
    if data:
        return data
    master = getattr(host, 'master', None)
    return getattr(master, '_mkt_data', None) or {}


def _opie_dashboard_app(host):
    """Resolve up to the MYIPOApp dashboard instance, which is where
    all_frames (every index/fund NAV series) and _calc_fund_return live —
    SCAWindow's host needs to hop up to its master to find it."""
    if hasattr(host, 'all_frames'):
        return host
    master = getattr(host, 'master', None)
    if master is not None and hasattr(master, 'all_frames'):
        return master
    return host


def _opie_fund_catalog(host):
    """List of (label, ccy) for every fund OPIE can look up — both
    unit-trust FUND_LABELS (always MYR) and VFund/Enhanced-Yield CEFs
    (MYR or USD, per VFUND_CONFIG)."""
    out = [(lbl, 'MYR') for lbl in FUND_LABELS]
    out += [(vf['label'], vf['ccy']) for vf in VFUND_CONFIG]
    return out


_OPIE_MYIPO_INDEX_LABELS = (
    'MYIPO+', 'MYIPO+ Momentum', 'MYIPO+ Ace', 'MYIPO+ Main', 'MYIPO+ Dynamic365',
    'MYIPO+ Y2020', 'MYIPO+ Y2021', 'MYIPO+ Y2022', 'MYIPO+ Y2023',
    'MYIPO+ Y2024', 'MYIPO+ Y2025', 'MYIPO+ Y2026',
    'MYIPO+ Top 10', 'MYIPO+ Top 20', 'MYIPO+ Top 50', 'MYIPO+ Top 100',
    'MYIPO+ Champions', 'MYIPO+ Annual Winners', 'MYIPO+ Annual Losers',
)


def _opie_index_catalog(host):
    """List of (symbol, display name, currency) for every index OPIE can
    look up — the Bursa benchmark, every world index the dashboard already
    tracks, and the full internal MYIPO+ index family (marked with the
    'internal' symbol tag so _say_index_value knows to read it straight
    out of all_frames instead of the live world-index mkt-data snapshot)."""
    app = host if hasattr(host, 'WORLD_INDICES') else getattr(host, 'master', None)
    world = list(getattr(app, 'WORLD_INDICES', []) or [])
    myipo = [('internal', lbl, 'MYR')
             for lbl in _OPIE_MYIPO_INDEX_LABELS
             if lbl in getattr(app, 'all_frames', {})]
    return [('^KLSE', 'FBM KLCI', 'MYR')] + world + myipo


def _opie_parse_currency(text: str):
    """Best-effort currency code detected in free text, or None."""
    t = text.upper()
    for token, code in _OPIE_CCY_TOKENS.items():
        if token in t:
            return code
    return None


# ── ETF Top Holdings Intelligence (alpha.102) ────────────────────────────────
# Feature 1 (Top 10 Holdings) + Feature 2 (Opie Smart Summary) from the
# ETF Top Holdings Intelligence proposal, reached through the existing OPIE
# chatbox (Feature 3 — no separate window/module; OPIE is currently disabled
# as a full AI agent, so the "summary" is generated by a local, deterministic
# rule engine reusing the same disk-cache pattern as the rest of the app).
_OPIE_ETF_KEYWORDS = ('top holding', 'top 10 holding', 'holdings', 'etf summary',
                      'etf holdings', 'etf breakdown')


def _opie_fetch_etf_holdings(ticker: str) -> dict:
    """Top-10 holdings + sector weightings for an ETF ticker, via yfinance's
    funds_data, disk-cached for _TTL_ETF like every other network call in
    this app. Returns {} if the ticker isn't a fund or the fetch fails."""
    ticker = (ticker or '').upper().strip()
    if not ticker:
        return {}
    cached = cache_get_etf_holdings(ticker)
    if cached:
        return cached
    try:
        ...
        if top_df is None or top_df.empty:
            return {}
        holdings = []
        for sym, row in top_df.head(10).iterrows():
            pct = row.get('Holding Percent', row.iloc[0] if len(row) else 0) or 0
            holdings.append({
                'symbol': str(sym),
                'name':   str(row.get('Name', sym)),
                'pct':    float(pct) * (100.0 if float(pct) <= 1 else 1.0),
            })
        sectors = {}
        try:
            raw_sectors = getattr(fd, 'sector_weightings', None) or {}
            for sec, w in raw_sectors.items():
                w = float(w)
                sectors[str(sec)] = w * 100.0 if w <= 1 else w
        except Exception:
            sectors = {}
        data = {'ticker': ticker, 'holdings': holdings, 'sector_weightings': sectors}
        if holdings:
            cache_set_etf_holdings(ticker, data)
        return data
    except Exception:
        return {}


def _opie_format_holdings_table(data: dict) -> str:
    """Plain-text Top 10 Holdings table, mirroring the proposal's example
    layout ('1. Apple Inc.   6.72%')."""
    lines = [f"{data.get('ticker', '?')} — Top 10 Holdings", '']
    for i, h in enumerate(data.get('holdings', [])[:10], 1):
        lines.append(f"{i:>2}. {h['name']:<28} {h['pct']:>5.2f}%")
    return '\n'.join(lines)


def _opie_generate_etf_summary(data: dict) -> str:
    """Rule-based stand-in for the proposal's 'Opie Smart Summary' — no live
    AI call (OPIE-the-agent is disabled), just a deterministic write-up from
    the holdings/sector data already fetched, in the same tone as the
    proposal's example summary."""
    ticker   = data.get('ticker', 'This ETF')
    holdings = data.get('holdings', [])
    sectors  = data.get('sector_weightings', {})
    if not holdings:
        return f"I couldn't retrieve holdings for {ticker} — check the ticker or try again shortly."

    top3 = [h['name'] for h in holdings[:3]]
    top3_str = ', '.join(top3[:-1]) + (f' and {top3[-1]}' if len(top3) > 1 else top3[0])
    top1_pct = holdings[0]['pct']

    ranked_sectors = sorted(sectors.items(), key=lambda kv: kv[1], reverse=True)
    largest_sector = ranked_sectors[0][0].replace('_', ' ').title() if ranked_sectors else None
    largest_sector_pct = ranked_sectors[0][1] if ranked_sectors else None
    other_sectors = [s.replace('_', ' ').title() for s, _ in ranked_sectors[1:4]]

    concentration = ('a fairly concentrated' if top1_pct >= 8
                      else 'a moderately concentrated' if top1_pct >= 4
                      else 'a broadly diversified')

    parts = [f"{ticker} counts {top3_str} among its largest holdings."]
    if largest_sector:
        parts.append(f"The largest sector allocation is {largest_sector} "
                      f"(~{largest_sector_pct:.1f}%), "
                      f"suggesting a strong tilt toward that space.")
    if other_sectors:
        parts.append(f"Remaining exposure spans {', '.join(other_sectors)}, "
                      f"rounding out the fund's sector mix.")
    parts.append(f"With its top holding at {top1_pct:.2f}%, {ticker} looks like "
                 f"{concentration} fund overall.")
    return ' '.join(parts)


class OpieChatWindow(tk.Toplevel):
    """Pop-up OPIE chat box. Idle until 'Hi, Opie' is typed; only then does
    it greet the user, confirm a display name, and start answering."""

    def __init__(self, parent, host, kid_username: str):
        super().__init__(parent)
        self.host = host
        self.kid  = (kid_username or '').strip()
        self.title('🤖 OPIE')
        self.configure(bg='#0d1117')
        ...
        self._awaiting_stock = False
        self._awaiting_price = False
        self._awaiting_index = False
        self._awaiting_fund  = False
        self._pending_invest = None   # (amount, ccy) waiting on a fund name
        self._awaiting_summary_currency = False
        self._pending_summary = None   # ('ticker', match) | ('portfolio', None) awaiting a currency reply
        self._display_name   = None
        # Continuation mode: remembers the last ticker/index actually
        # resolved so a follow-up like "how much is it worth" or "what
        # about the price" can be answered without repeating the name.
        self._last_subject   = None   # ('ticker', ticker, item, ccy) | ('index', match)
        # Order-opening (PTS / ZDWS / ZDOS) conversation state.
        self._awaiting_permission = False
        self._pending_order_text  = None
        self._order               = None   # in-progress order dict
        self._awaiting_order_field = None  # which field the next reply fills

        tk.Label(self, text='🤖  OPIE', font=('Segoe UI', 12, 'bold'),
                 fg='#3FB950', bg='#0d1117').pack(pady=(10, 2))

        self._log = tk.Text(self, height=18, width=44, font=('Segoe UI', 9),
                             bg='#161b22', fg='#cdd9e5', wrap='word',
                             relief='flat', state='disabled')
        self._log.pack(padx=10, pady=6, fill='both', expand=True)

        row = tk.Frame(self, bg='#0d1117')
        row.pack(fill='x', padx=10, pady=(0, 10))
        self._entry_var = tk.StringVar()
        entry = tk.Entry(row, textvariable=self._entry_var, font=('Segoe UI', 10),
                          bg='#21262d', fg='#cdd9e5', insertbackground='#cdd9e5',
                          relief='flat')
        entry.pack(side='left', fill='x', expand=True, ipady=4)
        entry.bind('<Return>', lambda e: self._submit())
        entry.focus_set()
        tk.Button(row, text='Send', font=('Segoe UI', 9, 'bold'), bg='#3FB950',
                  fg='#0d1117', relief='flat', padx=10, cursor='hand2',
                  command=self._submit).pack(side='left', padx=(6, 0))

        self._say('Type "Hi, Opie" to start.')

    # ── log helpers ──────────────────────────────────────────────────────
    def _say(self, text: str):
        self._log.configure(state='normal')
        self._log.insert('end', f'🤖 OPIE: {text}\n\n')
        self._log.configure(state='disabled')
        self._log.see('end')

    def _say_user(self, text: str):
        self._log.configure(state='normal')
        self._log.insert('end', f'You: {text}\n')
        self._log.configure(state='disabled')
    ...
    def _process(self, text: str):
        ...
        # Squash all whitespace/punctuation so "Hi,Opie", "Hi, Opie!", "hi   opie"
        # etc. all normalize the same way for the activation check.
        squashed = ''.join(ch for ch in low if ch.isalnum())

        if not self._activated:
            if squashed in {g.replace(' ', '') for g in _OPIE_GREETINGS}:
                self._activated = True
                self._start_name_flow()
            else:
                self._say('Type "Hi, Opie" to start.')
            return

        if self._awaiting_name:
            self._resolve_name(text)
            return

        if self._awaiting_stock:
            self._answer_stock(text)
            return

        if self._awaiting_price:
            self._answer_price(text)
            return

        if self._awaiting_index:
            self._answer_index(text)
            return

        if self._awaiting_fund:
            self._answer_fund(text)
            return

        if self._awaiting_summary_currency:
            self._answer_summary_currency(text)
            return
        ...
        if self._awaiting_order_field:
            self._continue_order(text)
            return

        # Log every real question from here on — greetings and mid-flow
        # answers above are excluded since they aren't questions in
        # themselves.
        self._log_recent_asked(text)

        if re.search(r'\bopen\b', low) and re.search(r'\b(pts|zdws|zdos|order|position|warrant|option|prediction)\b', low):
            self._handle_open_order(text)
            return

        # "invest $10,000 in <fund>" / "if I put RM5000 into <fund>" — a
        # historical backtest of what that would be worth today, using the
        # fund's real NAV history (same engine as the Return Calculator
        # button on the VFund Events tab, just reachable from chat too).
        if re.search(r'\binvest(ed|ing)?\b|\bput\b.*\bin(to)?\b', low) and \
           re.search(r'\b\d', low):
            self._handle_invest(text)
            return

        if any(kw in low for kw in _OPIE_ETF_KEYWORDS):
            self._handle_etf_holdings(text)
            return

        # 'trend BOTZ 5d', 'MYIPO+ trend YTD', 'how has S&P 500 trended all time'
        if re.search(r'\btrend(ed|ing)?\b', low):
            self._handle_trend(text)
            return

        # 'summary settings' / 'set summary on open' / 'set summary time
        # 9:00am' / 'turn off summary schedule' — config, checked before
        # the data-summary query below.
        if re.search(r'\bsummary\b', low) and \
           re.search(r'\b(setting|schedule|on\s*open|turn off|disable|fixed|daily)\b', low):
            self._handle_summary_settings(text)
            return

        # 'summary' — checked before 'holdings'/'weight' etc since it's a
        # broader ask; continuation-aware (ticker vs whole portfolio).
        if re.search(r'\bsummary\b', low):
            self._handle_summary(text)
            return

        if re.search(r'\b(avg|average)\s*cost\b|\bbreakeven\b', low):
            self._handle_avg_cost(text)
            return

        # 'weight'/'allocation'/'% of portfolio' — new FAQ, not ETF-related.
        if re.search(r'\b(weight|allocation|% of (my )?portfolio)\b', low):
            self._handle_weight(text)
            return

        # 'holdings report', 'position report/detail for BOTZ' — opens the
        # T80 ledger+chart popup from chat, same as the button/row-click.
        if re.search(r'\b(holdings?|position)\b', low) and re.search(r'\b(report|detail|ledger|popup)\b', low):
            self._handle_holdings_report(text)
            return

        if re.search(r'\bfund[s]?\b', low):
            self._handle_fund(text)
            return

        # Continuation mode: "it"/"that"/"this one" etc. with no ticker/index
        # of its own named in the message refers back to whatever was last
        # resolved, so a follow-up question doesn't need the name repeated.
        if self._last_subject and re.search(r"\b(it|that|this( one)?|its)\b", low) and \
           not _opie_ticker_candidates(text) and not self._match_index(self.host, text):
            if re.search(r'\bchange[ds]?\b', low) and not re.search(r'\bexchange\b', low):
                self._handle_change()
                return
            if re.search(r'\btrend(ed|ing)?\b', low):
                self._handle_trend(text)
            ...
            if re.search(r'\b(avg|average)\s*cost\b|\bbreakeven\b', low):
                self._handle_avg_cost(text)
                return
            # ETF ("top holdings") and Holdings Report continuation are
            # deliberately excluded here — those stay explicit-trigger only.
            if 'how much' in low or re.search(r'\bprices?\b', low) or \
               re.search(r'\b(index|indexes|indices)\b', low):
                self._reply_last_subject()
                return
        ...
        if 'how much' in low:
            self._handle_how_much(text)
            return

        if re.search(r'\bprices?\b', low):
            self._handle_price(text)
            return

        if re.search(r'\b(index|indexes|indices)\b', low):
            self._handle_index(text)
            return

        # Last resort: no keyword matched, but the message may just be a bare
        # ticker/index left over from an earlier prompt (e.g. "5347.KL?"
        # typed after a failed match) — resolve it directly instead of
        # forcing the user to re-trigger a keyword.
        match = _opie_match_portfolio_ticker(self.host, text)
        if match:
            self._say_stock_value(match)
            return
        idx = self._match_index(self.host, text)
        if idx:
            self._say_index_value(idx)
            return
        cand = _opie_best_candidate(_opie_ticker_candidates(text))
        if cand:
            resolved, price = self._current_price(cand)
            if price is not None:
                self._last_subject = ('price', resolved, price)
                self._say(f'{resolved} is currently trading at {price:,.4f} {_opie_infer_ccy(resolved)}.')
                return

        self._say('I can help with "change" (see your portfolio / a stock), '
                   '"how much" (value & % in a currency of your choice), '
                   '"stock price" (current price of a stock), '
                   '"index" (current value of a market index — KLCI, world '
                   'indices, or any MYIPO+ index), "trend" (e.g. "trend BOTZ '
                   '5d", "MYIPO+ trend YTD" — 1d/5d/1mo/3mo/6mo/ytd/1y/all), '
                   '"summary" (ticker or whole-portfolio, incl. recent buys — '
                   '"set summary on open" / "set summary time 9:00am" to '
                   'schedule an auto-popup), "avg cost" (cost basis / '
                   'breakeven on a stock), '
                   '"holdings report" (opens the ledger + chart popup), '
                   '"weight" (% of your portfolio a stock represents), '
                   '"top holdings" of an ETF (e.g. "top holdings VOO"), or '
                   '"open" a PTS/ZDWS/ZDOS position.')

    def _reply_last_subject(self):
        """Continuation-mode follow-up ("how much is it worth now?") — repeat
        whatever's already stored as the last resolved subject."""
        kind = self._last_subject[0]
        if kind == 'stock':
            self._say_stock_value(self._last_subject[1])
        elif kind == 'index':
            self._say_index_value(self._last_subject[1])
        elif kind == 'fund':
            self._say_fund_value(self._last_subject[1])
        elif kind == 'etf':
            data = _opie_fetch_etf_holdings(self._last_subject[1])
            if data.get('holdings'):
                self._say(_opie_format_holdings_table(data))
                self._say(_opie_generate_etf_summary(data))
        elif kind == 'price':
            _, ticker, price = self._last_subject
            self._say(f'{ticker} is currently trading at {price:,.4f} {_opie_infer_ccy(ticker)}.')

    def _recent_asked_line(self):
        """Formats this K-ID's 'Recently Asked' list for display right after
        the Opie greeting, or '' if there's no history yet."""
        settings = load_app_settings()
        recent = settings.get('opie_recent_asked', {}).get(self.kid.lower(), [])
        if not recent:
            return ''
        shown = recent[-5:][::-1]   # most recent first
        bullets = '\n'.join(f'  • {q}' for q in shown)
        return f'\n\nRecently asked:\n{bullets}'

    def _start_name_flow(self):
        settings = load_app_settings()
        names  = settings.get('opie_display_names', {})
        locked = settings.get('opie_name_locked', {})
        key = self.kid.lower()
        if key in names and locked.get(key):
            self._display_name = names[key]
            self._say(f'Hi, {self._display_name}! What would you like to know — '
                       f'try "change", "how much", "stock price", "index", "top holdings" of an ETF, or "open" a PTS/ZDWS/ZDOS position.'
                       f'{self._recent_asked_line()}')
            return
        self._awaiting_name = True
        self._say(f'Hi there! Want me to call you by your K-ID ("{self.kid}"), '
                   f'or type a custom name instead — this can only be set once.')

    def _resolve_name(self, text: str):
        settings = load_app_settings()
        names  = settings.setdefault('opie_display_names', {})
        locked = settings.setdefault('opie_name_locked', {})
        key = self.kid.lower()
        choice = text.strip()
        name = self.kid if choice.lower() in ('kid', 'k-id', self.kid.lower()) else choice
        names[key]  = name
        locked[key] = True
        save_app_settings(settings)
        self._display_name  = name
        self._awaiting_name = False
        self._say(f'Got it, {name}! What would you like to know — '
                   f'try "change", "how much", "stock price", "index", "top holdings" of an ETF, or "open" a PTS/ZDWS/ZDOS position.'
                   f'{self._recent_asked_line()}')

    def _handle_change(self):
        portfolios = _opie_extract_portfolios(self.host)
        if not portfolios:
            self._say('No portfolio loaded yet — connect a source first.')
            return
        lines = []
        for port in portfolios.values():
            for ticker, item in port.get('holdings', {}).items():
        ...
        if not lines:
            self._say('Your portfolio has no active positions right now.')
            return
        self._awaiting_stock = True
        self._say('Here\'s your portfolio: ' + ', '.join(lines) +
                   '. Which stock do you want to know about?')

    def _answer_stock(self, text: str):
        self._awaiting_stock = False
        match = _opie_match_portfolio_ticker(self.host, text)
        if not match:
            self._say(f'Couldn\'t match "{text}" to a ticker in your portfolio — '
                       f'try typing just the ticker, e.g. 5398.KL.')
            return
        self._say_stock_value(match)

    def _say_stock_value(self, match):
        ticker, item, ccy = match
        self._last_subject = ('stock', match)
        prices = _opie_extract_prices(self.host)
        shares = item.get('shares', 0)
        cost   = item.get('cost_value', 0.0)
        price  = prices.get(ticker)
        if not price:
            _, price = self._current_price(ticker)
        value  = shares * price if price else cost
        gain   = value - cost
        pct    = (gain / cost * 100) if cost > 0 else 0.0
        prefix = 'RM' if ccy == 'MYR' else f'{ccy} '
        self._say(f'{ticker}: {shares:g} shares, worth '
                   f'{prefix}{value:,.2f}, '
                   f'unrealized {gain:+,.2f} ({pct:+.2f}%).')

    def _current_price(self, ticker: str):
        """Live price from whatever's already loaded, falling back to the
        on-disk price cache. Bare codes with no exchange suffix (e.g. a
        Hong Kong or Singapore code typed without .HK/.SI) are tried
        against every market in MARKET_SPEC, so this keeps working as more
        exchanges get registered there — no changes needed here."""
        prices = _opie_extract_prices(self.host)
        variants = _opie_suffix_variants(ticker)
        ...
        for cand in variants:
            try:
                s = cache_get_prices(cand)
                if s is not None and not s.empty:
                    return cand, float(s.iloc[-1])
            except Exception:
                pass
        return None, None

    def _handle_price(self, text: str):
        """'price' keyword: if a ticker was named in the same message, answer
        directly; otherwise list the portfolio and ask which stock."""
        cand = _opie_best_candidate(_opie_ticker_candidates(text))
        if cand:
            self._answer_price(cand)
            return
        portfolios = _opie_extract_portfolios(self.host)
        tickers = sorted({t for port in portfolios.values()
                           for t, item in port.get('holdings', {}).items()
                           if not item.get('_is_cash')})
        self._awaiting_price = True
        if tickers:
            self._say('Which stock\'s price do you want — e.g. ' +
                       ', '.join(tickers[:8]) + '?')
        else:
            self._say('Which stock\'s price do you want to know?')

    def _answer_price(self, text: str):
        self._awaiting_price = False
        query = text.strip().upper()
        resolved, price = self._current_price(query)
        if price is None:
            cand = _opie_best_candidate(_opie_ticker_candidates(text))
            if cand and cand != query:
                resolved, price = self._current_price(cand)
        if price is None:
            self._say(f'No current price on record for "{text}" — check the ticker and try again.')
            return
        self._last_subject = ('price', resolved, price)
        self._say(f'{resolved} is currently trading at {price:,.4f} {_opie_infer_ccy(resolved)}.')

    def _match_index(self, host, query: str):
        """Match free text against the index catalog by symbol or name,
        ignoring time-filler words, whichever way the substring runs (a bare
        name like "KLCI" or a full sentence like "what's the S&P 500 at")."""
        words = [w.strip('.,!?') for w in query.split()
                 if w.strip('.,!?').upper() not in _OPIE_TIME_WORDS]
        squashed_q = ''.join(ch for ch in ' '.join(words).upper() if ch.isalnum())
        if not squashed_q:
            return None
        for sym, name, ccy in _opie_index_catalog(host):
            sym_bare = ''.join(ch for ch in sym.replace('^', '').upper() if ch.isalnum())
            name_sq  = ''.join(ch for ch in name.upper() if ch.isalnum())
            if (name_sq and (name_sq in squashed_q or squashed_q in name_sq)) or \
               (sym_bare and (sym_bare == squashed_q or sym_bare in squashed_q)):
                return sym, name, ccy
        return None

    def _handle_index(self, text: str):
        """'index'/'indices' keyword: if a specific index was named in the
        same message, answer directly; otherwise list what's trackable and
        ask which one the user wants."""
        match = self._match_index(self.host, text)
        if match:
            self._say_index_value(match)
            return
        catalog = _opie_index_catalog(self.host)
        names = [name for _, name, _ in catalog]
        self._awaiting_index = True
        self._say('Which index do you want the current value for — ' +
                   ', '.join(names) + '?')

    def _answer_index(self, text: str):
        self._awaiting_index = False
        match = self._match_index(self.host, text)
        if not match:
            self._say(f'Couldn\'t match "{text}" to a tracked index — try one of the names above.')
            return
        self._say_index_value(match)

    def _say_index_value(self, match):
        sym, name, ccy = match
        self._last_subject = ('index', match)
        if sym == 'internal':
            app = _opie_dashboard_app(self.host)
            df = getattr(app, 'all_frames', {}).get(name)
            if df is None or df.empty or name not in df.columns:
                self._say(f'No data for {name} yet.')
                return
            s = df[name].dropna()
            if s.empty:
                self._say(f'No priced level for {name} yet.')
                return
            last = float(s.iloc[-1])
            chg_pct = None
            if len(s) > 1:
            ...
            chg_txt = f', {chg_pct:+.2f}% vs previous close' if chg_pct is not None else ''
            self._say(f'{name} ({ccy}) is currently at {last:,.2f}{chg_txt}.')
            return
        mkt = _opie_extract_mkt_data(self.host)
        vals = mkt.get(sym)
        if not vals or vals[0] is None:
            self._say(f'No live value for {name} yet — open the dashboard so it can load.')
            return
        last, prev = vals
        chg_pct = ((last - prev) / prev * 100) if prev else 0.0
        self._say(f'{name} ({ccy}) is currently at {last:,.2f}, '
                   f'{chg_pct:+.2f}% vs the previous close.')

    def _match_fund(self, host, query: str):
        """Match free text against the fund catalog (unit-trust + VFund
        labels) by name, same style as _match_index."""
        words = [w.strip('.,!?') for w in query.split()
                 if w.strip('.,!?').upper() not in _OPIE_TIME_WORDS]
        squashed_q = ''.join(ch for ch in ' '.join(words).upper() if ch.isalnum())
        if not squashed_q:
            return None
        best = None
        for label, ccy in _opie_fund_catalog(host):
            name_sq = ''.join(ch for ch in label.upper() if ch.isalnum())
            if name_sq and (name_sq in squashed_q or squashed_q in name_sq):
                # Prefer the longest matching label — avoids a short/generic
                # name matching every close variant equally well.
                if best is None or len(name_sq) > len(best[0]):
                    best = (name_sq, label, ccy)
        return (best[1], best[2]) if best else None

    # ── TREND — 1d/5d/1mo/3mo/6mo/ytd/1y/all, for SCA tickers, funds,
    #    the MYIPO+ index family, and world indices ─────────────────────
    _TREND_PERIODS = [
        (r'\b1\s*d(ay)?\b',            '1d',  1),
        (r'\b5\s*d(ay)?s?\b',          '5d',  5),
        (r'\b(1\s*mo(nth)?|30\s*d)\b', '1mo', 30),
        (r'\b3\s*mo(nth)?s?\b',        '3mo', 90),
        (r'\b6\s*mo(nth)?s?\b',        '6mo', 182),
        (r'\bytd\b',                   'ytd', None),
        (r'\b1\s*y(ea)?r?\b',          '1y',  365),
        (r'\ball\b',                   'all', None),
    ]

    def _parse_trend_period(self, low: str):
        for pat, label, _days in self._TREND_PERIODS:
            if re.search(pat, low):
                return label
        return None

    def _trend_series(self, text: str):
        """Resolve free text to (label, ccy, pandas Series) for trend, in
        priority order: MYIPO+/world index catalog -> tracked fund -> SCA
        portfolio ticker (fetched live via the same history helper the
        Holdings Report popup chart uses). World indices get their history
        loaded on demand if it isn't cached yet."""
        idx = self._match_index(self.host, text)
        if idx:
            sym, name, ccy = idx
            app = _opie_dashboard_app(self.host)
            if name not in getattr(app, 'all_frames', {}) and hasattr(app, '_load_world_history'):
                try:
                    app._load_world_history(symbols=[sym])
                except Exception:
                    pass
            df = getattr(app, 'all_frames', {}).get(name)
            if df is not None and not df.empty and name in df.columns:
                return name, ccy, df[name].dropna()
            return name, ccy, None

        fund = self._match_fund(self.host, text)
        if fund:
            label, ccy = fund
            app = _opie_dashboard_app(self.host)
            df = getattr(app, 'all_frames', {}).get(label)
            if df is not None and not df.empty and label in df.columns:
                return label, ccy, df[label].dropna()
            return label, ccy, None

        match = _opie_match_portfolio_ticker(self.host, text)
        if match:
            ticker = match[0]
            mkt = 'US' if _opie_infer_ccy(ticker) == 'USD' else 'MY'
            hist = None
            try:
                hist = _mf_get_price_history(ticker, mkt)
            except Exception:
                pass
            ccy = _opie_infer_ccy(ticker)
            return ticker, ccy, (hist if hist is None else hist.dropna())

        return None

    def _handle_trend(self, text: str):
        low = text.lower()
        period = self._parse_trend_period(low) or '1mo'
        resolved = self._trend_series(text)
        if not resolved and self._last_subject:
            kind = self._last_subject[0]
            if kind == 'index':
                resolved = self._trend_series(self._last_subject[1][1])  # index name
            elif kind in ('stock', 'fund'):
                resolved = self._trend_series(self._last_subject[1][0])
            elif kind == 'price':
                resolved = self._trend_series(self._last_subject[1])
        if not resolved:
            self._say('Which stock, fund, or index — try "trend BOTZ 5d" or "MYIPO+ trend YTD".')
            return
        label, ccy, s = resolved
        if s is None or s.empty:
            self._say(f'No price history for {label} yet.')
            return
        self._last_subject = ('price', label, float(s.iloc[-1]))
        import datetime as _dt
        last = float(s.iloc[-1])
        if period == 'all':
            start_val = float(s.iloc[0])
            span_txt = f'since {s.index[0].date()}'
        elif period == 'ytd':
            this_year = s.index[s.index.year == _dt.date.today().year]
            if this_year.empty:
                start_val = float(s.iloc[0])
            else:
                start_val = float(s.loc[this_year].iloc[0])
            span_txt = 'YTD'
        else:
            days = dict((lbl, d) for _, lbl, d in self._TREND_PERIODS if d)[period]
            cutoff = pd.Timestamp(_dt.date.today() - _dt.timedelta(days=days))
            window = s[s.index >= cutoff]
            start_val = float(window.iloc[0]) if not window.empty else float(s.iloc[0])
            span_txt = period
        if not start_val:
            self._say(f'No baseline price to compute the {span_txt} trend for {label}.')
            return
        chg = last - start_val
        chg_pct = chg / start_val * 100
        arrow = '▲' if chg >= 0 else '▼'
        self._say(f'{label} {span_txt} trend: {start_val:,.4f} → {last:,.4f} {ccy} '
                   f'({arrow} {chg_pct:+.2f}%).')

    def _handle_holdings_report(self, text: str):
        """'holdings report' / 'position detail' — opens the same T80 popup
        (ledger + price chart) the button and row-click open, for a named
        ticker or the last one discussed."""
        opener = getattr(self.host, '_open_holdings_report_popup', None)
        if opener is None:
            master = getattr(self.host, 'master', None)
            opener = getattr(master, '_open_holdings_report_popup', None)
        if opener is None:
            self._say('Holdings Report is only available from the SCA portfolio window — open it from there.')
            return
        match = _opie_match_portfolio_ticker(self.host, text)
        ticker = match[0] if match else None
        if not ticker and self._last_subject and self._last_subject[0] in ('stock', 'price'):
            ticker = self._last_subject[1] if self._last_subject[0] == 'price' else self._last_subject[1][0]
        opener(ticker)
        self._say(f'Opened the holdings report{f" for {ticker}" if ticker else ""}.')

    def _handle_weight(self, text: str):
        """'weight'/'allocation'/'% of portfolio' — what share of the total
        portfolio a given stock represents. Falls back to the last resolved
        stock subject if none is named in this message."""
        match = _opie_match_portfolio_ticker(self.host, text)
        if not match and self._last_subject and self._last_subject[0] == 'stock':
            match = self._last_subject[1]
        if not match:
            self._say('Which stock — try "what\'s my weight in 5398.KL?"')
            return
        ticker, item, ccy = match
        self._last_subject = ('stock', match)
        portfolios = _opie_extract_portfolios(self.host)
        ...
        if not price:
            _, price = self._current_price(ticker)
        this_value = shares * price if price else item.get('cost_value', 0.0)

        fx = _opie_extract_fx(self.host)
        def _to_myr(amount, c):
            return amount if c == 'MYR' else amount * fx.get(c, 1.0)

        total_myr = 0.0
        for port in portfolios.values():
            p_ccy = port.get('base_currency', 'USD')
            for t, it in port.get('holdings', {}).items():
                if it.get('_is_cash'):
                    continue
                sh = it.get('shares', 0)
                if not sh or sh <= 0:
                    continue
                p = prices.get(t)
                mv = sh * p if p else it.get('cost_value', 0.0)
                total_myr += _to_myr(mv, p_ccy)

        if not total_myr:
            self._say('No portfolio value on record yet to compute weight from.')
            return
        this_myr = _to_myr(this_value, ccy)
        weight_pct = this_myr / total_myr * 100
        prefix = 'RM' if ccy == 'MYR' else f'{ccy} '
        self._say(f'{ticker} is {weight_pct:.2f}% of your total portfolio '
                   f'({prefix}{this_value:,.2f} of the whole book).')

    def _handle_avg_cost(self, text: str):
        """'avg cost'/'average cost'/'breakeven' — cost basis for a named
        ticker, or the last one discussed."""
        match = _opie_match_portfolio_ticker(self.host, text)
        if not match and self._last_subject and self._last_subject[0] == 'stock':
            match = self._last_subject[1]
        if not match:
            self._say('Which stock — try "what\'s my avg cost on 5398.KL?"')
            return
        ticker, item, ccy = match
        self._last_subject = ('stock', match)
        shares   = item.get('shares', 0)
        cost_val = item.get('cost_value', 0.0)
        avg_cost = item.get('avg_cost', (cost_val / shares) if shares else 0.0)
        prefix = 'RM' if ccy == 'MYR' else f'{ccy} '
        prices = _opie_extract_prices(self.host)
        price = prices.get(ticker)
        if not price:
        ...
        live_txt = ''
        if price:
            gap = (price - avg_cost) / avg_cost * 100 if avg_cost else 0.0
            live_txt = f', live is {prefix}{price:,.4f} ({gap:+.2f}% vs breakeven)'
        self._say(f'{ticker}: avg cost {prefix}{avg_cost:,.4f} over {shares:g} shares '
                   f'(total cost {prefix}{cost_val:,.2f}){live_txt}.')

    def _handle_etf_holdings(self, text: str):
        """alpha.102 — 'top holdings <TICKER>' / 'ETF summary <TICKER>'.
        Prints the Top 10 holdings table (Feature 1) followed by Opie's
        rule-based summary (Feature 2), reusing the same disk-cache and
        ticker-matching helpers as the rest of the chat."""
        cand = _opie_best_candidate(_opie_ticker_candidates(text))
        ticker = None
        if cand:
            ticker = cand
        elif self._last_subject and self._last_subject[0] == 'etf':
            ticker = self._last_subject[1]
        if not ticker:
            self._say('Which ETF? Try "top holdings VOO" or "top holdings QQQ".')
            return
        data = _opie_fetch_etf_holdings(ticker)
        if not data.get('holdings'):
            self._say(f'Couldn\'t find ETF holdings for "{ticker}" — double-check the '
                       f'ticker (e.g. VOO, QQQ, VNQ) and try again.')
            return
        self._last_subject = ('etf', data['ticker'])
        self._say(_opie_format_holdings_table(data))
        self._say(_opie_generate_etf_summary(data))

    def _handle_fund(self, text: str):
        match = self._match_fund(self.host, text)
        if match:
            self._say_fund_value(match)
            return
        self._awaiting_fund = True
        labels = [lbl for lbl, _ in _opie_fund_catalog(self.host)]
        self._say('Which fund do you mean — ' + ', '.join(labels) + '?')

    def _answer_fund(self, text: str):
        self._awaiting_fund = False
        match = self._match_fund(self.host, text)
        if not match:
            self._say(f'Couldn\'t match "{text}" to a tracked fund — try one of the names above.')
            return
        if self._pending_invest:
            amount, ccy = self._pending_invest
            self._pending_invest = None
            app = _opie_dashboard_app(self.host)
            self._say_invest_result(app, match[0], amount, ccy)
            return
        self._say_fund_value(match)

    def _say_fund_value(self, match):
        label, ccy = match
        self._last_subject = ('fund', match)
        app = _opie_dashboard_app(self.host)
        df = getattr(app, 'all_frames', {}).get(label)
        if df is None or df.empty or label not in df.columns:
            self._say(f'No NAV data for {label} yet.')
            return
        s = df[label].dropna()
        if s.empty:
            self._say(f'No priced NAV for {label} yet.')
            return
        last = float(s.iloc[-1])
        chg_pct = None
        if len(s) > 1:
            ...
            if prev:
                chg_pct = (last - prev) / prev * 100
        chg_txt = f', {chg_pct:+.2f}% vs previous close' if chg_pct is not None else ''
        self._say(f'{label} NAV is currently {last:,.4f} {ccy}{chg_txt}.')

    def _handle_invest(self, text: str):
        """'invest $10,000 in <fund>' — historical backtest via the
        dashboard's _calc_fund_return (same engine as the Return Calculator
        button on the VFund Events tab). Explicitly framed as a backtest,
        not a forecast — see _say_invest_result."""
        app = _opie_dashboard_app(self.host)
        if not hasattr(app, '_calc_fund_return'):
            self._say("Return calculator isn't available from here yet.")
            return
        m = re.search(r'([\d,]+(?:\.\d+)?)', text)
        if not m:
            self._say('How much would you like to invest, and in which fund?')
            return
        amount = float(m.group(1).replace(',', ''))
        ccy = _opie_parse_currency(text) or 'MYR'
        match = self._match_fund(self.host, text)
        if not match:
            self._pending_invest = (amount, ccy)
            self._awaiting_fund = True
            labels = [lbl for lbl, _ in _opie_fund_catalog(self.host)]
            self._say('Which fund do you want to invest in — ' + ', '.join(labels) + '?')
            return
        self._say_invest_result(app, match[0], amount, ccy)

    def _say_invest_result(self, app, label, amount, ccy):
        res = app._calc_fund_return(label, amount, ccy)
        if 'error' in res:
            self._say(f"⚠️ {res['error']}")
            return
        sign = '+' if res['return_pct'] >= 0 else ''
        self._say(
            f"Backtest, not a forecast: {amount:,.2f} {ccy} invested in {label} "
            f"at launch ({res['entry_date'].date()}, NAV {res['entry_nav']:.4f} "
            f"{res['fund_ccy']}) would be worth {res['current_value']:,.2f} {ccy} "
            f"today ({res['latest_date'].date()}) — {sign}{res['return_pct']:.2f}%."
        )

    # ── SUMMARY — ticker or whole-portfolio, currency-aware, remembers
    #    what was recently asked ──────────────────────────────────────────
    def _summary_currency_pref(self):
        settings = load_app_settings()
        return settings.get('opie_summary_prefs', {}).get(self.kid.lower(), {}).get('currency')

    def _set_summary_currency_pref(self, ccy: str):
        settings = load_app_settings()
        prefs = settings.setdefault('opie_summary_prefs', {}).setdefault(self.kid.lower(), {})
        prefs['currency'] = ccy
        save_app_settings(settings)

    def _log_recent_asked(self, text: str):
        """Keep the last 10 non-trivial things this K-ID has asked Opie, so
        summaries (and Opie itself) can reference 'what you usually ask'."""
        settings = load_app_settings()
        recent = settings.setdefault('opie_recent_asked', {}).setdefault(self.kid.lower(), [])
        recent.append(text.strip())
        del recent[:-10]
        save_app_settings(settings)

    def _handle_summary(self, text: str):
        """'summary' — continuation-aware: a named/last-discussed ticker
        gets a per-position summary, otherwise the whole portfolio.
        Currency: uses one already in the message (existing
        _opie_parse_currency), else the saved preference, else asks once
        and remembers the answer for next time."""
        match = _opie_match_portfolio_ticker(self.host, text)
        if not match and self._last_subject and self._last_subject[0] == 'stock':
            match = self._last_subject[1]
        target = ('ticker', match) if match else ('portfolio', None)

        ccy = _opie_parse_currency(text) or self._summary_currency_pref()
        if not ccy:
            self._pending_summary = target
            self._awaiting_summary_currency = True
            self._say('Which currency should I show the summary in — MYR or USD?')
            return
        self._say_summary(target, ccy)

    def _answer_summary_currency(self, text: str):
        self._awaiting_summary_currency = False
        ccy = _opie_parse_currency(text)
        if not ccy:
            self._say('Sorry, I didn\'t catch a currency — try MYR or USD.')
            self._awaiting_summary_currency = True
            return
        self._set_summary_currency_pref(ccy)
        target, self._pending_summary = self._pending_summary, None
        self._say_summary(target, ccy)

    def _say_summary(self, target, ccy: str):
        kind, match = target
        prefix = 'RM' if ccy == 'MYR' else f'{ccy} '
        if kind == 'ticker':
            ticker, item, item_ccy = match
            self._last_subject = ('stock', match)
            fx = _opie_extract_fx(self.host)
            def _to_ccy(amount, c):
                if c == ccy:
            ...
            shares = item.get('shares', 0)
            cost   = item.get('cost_value', 0.0)
            price  = prices.get(ticker) or self._current_price(ticker)[1]
            value  = shares * price if price else cost
            gain   = value - cost
            pct    = (gain / cost * 100) if cost > 0 else 0.0
            recent_buys = sorted(
                [t for t in item.get('transactions', []) if t[0] == 'buy'],
                key=lambda t: t[1], reverse=True)[:3]
            buys_txt = '; '.join(f'{t[1]}: +{t[2]:g} @ {t[3]:,.4f}' for t in recent_buys) or 'none on record'
            self._say(f'{ticker} summary — {shares:g} shares, avg cost '
                       f'{prefix}{_to_ccy(cost / shares if shares else 0, item_ccy):,.4f}, '
                       f'worth {prefix}{_to_ccy(value, item_ccy):,.2f}, '
                       f'unrealized {prefix}{_to_ccy(gain, item_ccy):+,.2f} ({pct:+.2f}%). '
                       f'Recent buys: {buys_txt}.')
            return
        ...
        pct  = (gain / cost_total * 100) if cost_total > 0 else 0.0
        recent_buys = sorted(all_buys, key=lambda x: x[1][1], reverse=True)[:5]
        buys_txt = '; '.join(f'{tk} {t[1]}: +{t[2]:g} @ {t[3]:,.4f}' for tk, t in recent_buys) or 'none on record'
        self._say(f'Portfolio summary ({ccy}): {positions} position(s), '
                   f'value {prefix}{mv_total:,.2f}, '
                   f'unrealized {prefix}{gain:+,.2f} ({pct:+.2f}%). '
                   f'Recently bought: {buys_txt}.')

    def _handle_summary_settings(self, text: str):
        """'summary settings' / 'set summary on open' / 'set summary time
        9:00am' / 'turn off summary schedule' — configures the per-K-ID
        auto-summary popup shown when SCA opens."""
        low = text.lower()
        settings = load_app_settings()
        prefs = settings.setdefault('opie_summary_prefs', {}).setdefault(self.kid.lower(), {})

        if re.search(r'\boff\b|\bstop\b|\bdisable\b', low):
            prefs['schedule_mode'] = 'off'
            save_app_settings(settings)
            self._say('Turned off the auto-summary popup.')
            return

        if re.search(r'\bon\s*open\b|\bevery time\b|\bwhen(ever)? i open\b', low):
            prefs['schedule_mode'] = 'on_open'
            save_app_settings(settings)
            self._say('Got it — I\'ll show a portfolio summary every time you open SCA.')
            return

        m = re.search(r'\b(\d{1,2}):?(\d{2})?\s*(am|pm)?\b', low)
        if m and re.search(r'\btime\b|\bdaily\b|\bfixed\b|\bam\b|\bpm\b|:\d{2}', low):
            h = int(m.group(1)); mnt = int(m.group(2) or 0)
            if m.group(3) == 'pm' and h < 12:
                h += 12
            if m.group(3) == 'am' and h == 12:
                h = 0
            prefs['schedule_mode'] = 'fixed_time'
            prefs['fixed_time'] = f'{h:02d}:{mnt:02d}'
            save_app_settings(settings)
            self._say(f'Got it — I\'ll show a portfolio summary once a day, '
                       f'the first time you open SCA at or after {prefs["fixed_time"]}.')
            return

        mode = prefs.get('schedule_mode', 'off')
        detail = f' at {prefs.get("fixed_time", "09:00")}' if mode == 'fixed_time' else ''
        self._say(f'Auto-summary is currently "{mode}"{detail}. Try "set summary on open", '
                   f'"set summary time 9:00am", or "turn off summary schedule".')

    def _handle_how_much(self, text: str):
        portfolios = _opie_extract_portfolios(self.host)
        if not portfolios:
            self._say('No portfolio loaded yet — connect a source first.')
            return
        fx     = _opie_extract_fx(self.host)
        prices = _opie_extract_prices(self.host)
        target = _opie_parse_currency(text)
        ccy_counts = {}
        for port in portfolios.values():
            c = port.get('base_currency', 'USD')
            ccy_counts[c] = ccy_counts.get(c, 0) + 1
        if not target:
            target = max(ccy_counts, key=ccy_counts.get) if ccy_counts else 'MYR'

        def _to_target(amount, ccy):
            if ccy == target:
                return amount
            amount_myr = amount if ccy == 'MYR' else amount * fx.get(ccy, 1.0)
            if target == 'MYR':
                return amount_myr
            rate = fx.get(target)
            return (amount_myr / rate) if rate else None
        ...
        for port in portfolios.values():
            ...
            for ticker, item in port.get('holdings', {}).items():
                ...
                if not shares or shares <= 0:
                    continue
                positions += 1
                cost = _to_target(item.get('cost_value', 0.0), ccy)
                price = prices.get(ticker)
                mv = _to_target(shares * price, ccy) if price else cost
                if cost is None or mv is None:
                    unresolved = True
                    continue
                cost_total += cost
                mv_total   += mv

        if unresolved and positions and cost_total == 0:
            self._say(f'No exchange rate available for {target} yet — try MYR or USD.')
            return

        gain = mv_total - cost_total
        pct  = (gain / cost_total * 100) if cost_total > 0 else 0.0
        prefix = 'RM' if target == 'MYR' else f'{target} '
        self._say(f'Full summary in {target}: {positions} position(s), '
                   f'value {prefix}{mv_total:,.2f}, '
                   f'unrealized {prefix}{gain:+,.2f} ({pct:+.2f}%).')

    # ── PTS / ZDWS / ZDOS order opening ─────────────────────────────────
    def _trade_host(self):
        """The MYIPOApp instance that actually owns the PTS/ZDWS/ZDOS wallet
        and open_position methods — self.host is that instance directly on
        the dashboard, or its master when opened from inside SCA."""
        if hasattr(self.host, '_zdws_open_position'):
            return self.host
        master = getattr(self.host, 'master', None)
        if master is not None and hasattr(master, '_zdws_open_position'):
            return master
        return None

    def _handle_open_order(self, text: str):
        """'open' + PTS/ZDWS/ZDOS/order/position keyword — gate on a
        one-time permission grant, then start (or continue) the guided
        order-builder."""
        settings = load_app_settings()
        granted = settings.get('opie_trade_permission', {}).get(self.kid.lower(), False)
        if not granted:
            self._awaiting_permission = True
            self._pending_order_text  = text
            self._say('I can open PTS / ZDWS / ZDOS positions for you, but I need '
                       'your permission first (one-time) — reply "yes" to grant it, '
                       'or "no" to skip.')
            return
        self._order = self._order or {}
        self._continue_order(text)

    def _resolve_permission(self, text: str):
        self._awaiting_permission = False
        low = text.strip().lower()
        grant = bool(re.search(r'\b(yes|y|ok|okay|sure|yeah|yep|grant|granted|allow)\b', low))
        deny  = bool(re.search(r'\b(no|nope|nah|never|skip|deny|cancel)\b', low))
        if grant and not deny:
            settings = load_app_settings()
            perms = settings.setdefault('opie_trade_permission', {})
            perms[self.kid.lower()] = True
            save_app_settings(settings)
            self._say('Permission granted — I can open orders for you from now on.')
            pending, self._pending_order_text = self._pending_order_text, None
            self._order = self._order or {}
            if pending:
                self._continue_order(pending)
        else:
            self._pending_order_text = None
            self._order = None
            self._say('No problem — I won\'t open any orders. Just ask again anytime.')

    def _continue_order(self, text: str):
        """Fill the in-progress order one field at a time: system → side →
        underlying → stake → settlement horizon → confirm. Each step tries
        to parse straight out of free text before falling back to asking."""
        order = self._order if self._order is not None else {}
        self._order = order
        low = text.lower()

        if not order.get('system'):
            if re.search(r'\bzdws\b', low):
                order['system'] = 'ZDWS'
            elif re.search(r'\bzdos\b', low):
                order['system'] = 'ZDOS'
            elif re.search(r'\bpts\b', low):
                order['system'] = 'PTS'
        if not order.get('system'):
            self._awaiting_order_field = 'system'
            self._say('Which desk — PTS, ZDWS, or ZDOS?')
            return

        if order['system'] == 'PTS':
            self._continue_pts_order(text, order)
        else:
            self._continue_market_order(text, order)

    def _continue_market_order(self, text: str, order: dict):
        """ZDWS (BULL/BEAR warrant) or ZDOS (CALL/PUT option) field collection."""
        low = text.lower()
        is_zdos = order['system'] == 'ZDOS'

        if not order.get('side'):
            if is_zdos:
                if re.search(r'\blong\s*call\b', low) or (re.search(r'\bcall\b', low) and re.search(r'\blong\b', low)):
                    order['side'] = 'LONG_CALL'
                elif re.search(r'\bshort\s*call\b', low) or (re.search(r'\bcall\b', low) and re.search(r'\bshort\b', low)):
                    order['side'] = 'SHORT_CALL'
                elif re.search(r'\blong\s*put\b', low) or (re.search(r'\bput\b', low) and re.search(r'\blong\b', low)):
                    order['side'] = 'LONG_PUT'
                elif re.search(r'\bshort\s*put\b', low) or (re.search(r'\bput\b', low) and re.search(r'\bshort\b', low)):
                    order['side'] = 'SHORT_PUT'
            else:
                if re.search(r'\bbull\b', low):
                    order['side'] = 'BULL'
                elif re.search(r'\bbear\b', low):
                    order['side'] = 'BEAR'
        if not order.get('side'):
            self._awaiting_order_field = 'side'
            opts = 'LONG_CALL, SHORT_CALL, LONG_PUT, or SHORT_PUT' if is_zdos else 'BULL or BEAR'
            self._say(f'Which side — {opts}?')
            return

        if not order.get('underlying'):
            match = _opie_match_portfolio_ticker(self.host, text)
            idx   = self._match_index(self.host, text)
            if match:
                order['underlying'] = match[0]
            elif idx:
                order['underlying'] = idx[0]
            elif self._last_subject and re.search(r"\b(it|that|this)\b", low):
                if self._last_subject[0] == 'stock':
                    order['underlying'] = self._last_subject[1][0]
                elif self._last_subject[0] == 'index':
                    order['underlying'] = self._last_subject[1][0]
                elif self._last_subject[0] == 'price':
                    order['underlying'] = self._last_subject[1]
            else:
                cand = _opie_best_candidate(_opie_ticker_candidates(text))
                if cand:
                    order['underlying'] = cand
        if not order.get('underlying'):
            self._awaiting_order_field = 'underlying'
            self._say('Which underlying — a ticker or an index name?')
            return

        if order.get('tier') is None and is_zdos:
            m = re.search(r'\btier\s*(\d+)\b', low) or re.search(r'\bt(\d+)\b', low)
            if m:
                order['tier'] = int(m.group(1))
            elif re.search(r'\bat[- ]?the[- ]?money\b|\batm\b', low):
                order['tier'] = 0
        if order.get('tier') is None and is_zdos:
            self._awaiting_order_field = 'tier'
            self._say('Which strike tier (e.g. 0 for at-the-money, 1, 2, -1)?')
            return

        if not order.get('stake'):
            m = re.search(r'(\d[\d,]*\.?\d*)\s*(cr|credits?)?\b', low.replace(',', ''))
            if m:
                try:
                    order['stake'] = float(m.group(1))
                except ValueError:
                    pass
        if not order.get('stake'):
            self._awaiting_order_field = 'stake'
            self._say('How many credits do you want to stake?')
            return

        if not order.get('days'):
            m = re.search(r'\b(\d+)\s*day', low)
            if 'daily' in low:
                order['days'] = 1
            elif m:
                order['days'] = int(m.group(1))
        if not order.get('days'):
            self._awaiting_order_field = 'days'
            self._say('How many days out should it settle — e.g. "daily", "7", "30"?')
            return

        self._awaiting_order_field = None
        self._execute_market_order(order)

    def _execute_market_order(self, order: dict):
        host = self._trade_host()
        if host is None:
            self._say('Trading isn\'t available from this window right now — open OPIE from the dashboard instead.')
            self._order = None
            return
        settle_pairs = host._pts_settlement_dates(include_daily=(order['system'] == 'ZDWS'))
        wanted = order['days']
        best_label, best_date, best_diff = None, None, None
        import datetime as _dt
        today = _dt.date.today()
        for lbl, d in settle_pairs.items():
            diff = abs((d - today).days - wanted)
            if best_diff is None or diff < best_diff:
                best_label, best_date, best_diff = lbl, d, diff
        if best_date is None:
            self._say('Couldn\'t find a valid settlement date for that — try again with a different horizon.')
            self._order = None
            return
        settle_str = best_date.strftime('%d/%m/%Y')
        if order['system'] == 'ZDWS':
            ok = host._zdws_open_position(order['underlying'], order['side'], order['stake'], settle_str)
            msg = (f'ZDWS opened: {order["side"]} {order["underlying"]}  ·  '
                   f'{order["stake"]:,.0f} credits  ·  settles {settle_str}.') if ok else \
                  'Couldn\'t open that ZDWS position — check the underlying has a live price and you have enough credits.'
        else:
            ok = host._zdos_open_position(order['underlying'], order['side'], order['tier'],
                                           order['stake'], settle_str)
            msg = (f'ZDOS opened: {order["side"].replace("_"," ").title()} T{order["tier"]} '
                   f'{order["underlying"]}  ·  {order["stake"]:,.0f} credits  ·  '
                   f'settles {settle_str}.') if ok else \
                  'Couldn\'t open that ZDOS position — check the underlying has a live price and you have enough credits.'
        self._say(msg)
        self._order = None

    def _continue_pts_order(self, text: str, order: dict):
        """PTS custom YES/NO prediction field collection."""
        low = text.lower()

        if not order.get('question'):
            # Whatever's left after stripping the trigger words is treated
            # as the prediction question, if there's enough of it.
            stripped = re.sub(r'\b(open|pts|please|can|you|help|me)\b', '', low).strip(' ?.!')
            if len(stripped) > 6:
                order['question'] = text.strip()
        if not order.get('question'):
            self._awaiting_order_field = 'question'
            self._say('What\'s the prediction question?')
            return

        if not order.get('position'):
            if re.search(r'\byes\b', low):
                order['position'] = 'YES'
            elif re.search(r'\bno\b', low):
                order['position'] = 'NO'
        if not order.get('position'):
            self._awaiting_order_field = 'position'
            self._say('YES or NO?')
            return
        ...
        if not order.get('days'):
            self._awaiting_order_field = 'days'
            self._say('How many days until it resolves (minimum 7)?')
            return
        ...
        if host is None:
            ...
            return
        import datetime as _dt
        deadline = _dt.date.today() + _dt.timedelta(days=max(order['days'], 7))
        ok, msg = host._pts_open_custom_prediction(order['question'], order['position'],
                                                     order['stake'], deadline)
        self._say(msg)
        self._order = None


def _opie_open_chat(parent, host, kid_username: str):
    """Open (or focus) the single OPIE popup chat window for this parent."""
    win = getattr(parent, '_opie_chat_win', None)
    if win is not None and win.winfo_exists():
        win.lift()
        win.focus_set()
        return
    win = OpieChatWindow(parent, host, kid_username)
    parent._opie_chat_win = win
...
CURRENT_OPIE = None  # authenticated user's OPIE coordinator; set by AppLockScreen
...
def presence_touch(login: bool = False):
    """Record that CURRENT_KID is active right now. Writes at most once per
    60s (unless it's the login event itself) — called from login and from
    every wallet load, so any gameplay keeps the heartbeat fresh."""
    global _presence_last_write
    import time as _t, datetime as _dt
    kid = (CURRENT_KID or '').strip().lower()
    if not kid:
        return
    now = _t.time()
    if not login and now - _presence_last_write < 60:
        return
    _presence_last_write = now
    try:
        s = load_app_settings()
        rec = s.setdefault('presence', {}).setdefault(kid, {})
        stamp = _dt.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        rec['last_seen'] = stamp
        if login:
...
class AppLockScreen(tk.Tk):
    ...
    def __init__(self):
        ...
        # Field starts EMPTY by design — prefilling the last-used K-ID would
        # hand back the valid-username knowledge the dropdown removal hides.
    ...
    def _build_ui(self):
        ...
        # Default to Create on a fresh install (no accounts yet), but honor an
        # EXPLICIT switch to sign-in even with zero accounts — the admin
        # account (admin1) exists implicitly without ever being created, so
        # sign-in must always be reachable. Previously `or not accounts`
        # forced Create unconditionally and 'Back to sign in' just bounced
        # straight back, locking admin1 out of a fresh install entirely.
        creating = (self._mode == 'create') or (
            not accounts and not getattr(self, '_mode_explicit', False))
        ...
        # Username — always a plain typed entry, NO dropdown. Deliberate
        # security choice: the old dropdown enumerated every K-ID on the
        # machine (and advertised admin1's existence), handing an attacker
        # half the credential for free. Now knowing a valid username is part
        # of what a login attempt must supply.
    ...
    def _toggle_mode(self):
        ...
        self._mode_explicit = True   # user chose — don't force Create anymore
        self._pin_var.set("")
        self._build_ui()
    ...
    def _sign_in(self):
        ...
        # ── Emergency lock: while active, admin1 can ONLY get in via the
        # 16-hex recovery code (normal PIN+R-Code path is disabled) and must
        # set a brand-new R-Code secret on the way through.
        if (u.strip().lower() == ADMIN_KID
                and self._settings.get('admin_emergency_lock')):
            if self._admin_recovery_flow():
                self._settings['kid_last_user'] = u
                save_app_settings(self._settings)
                self._launch(u)
            else:
                self._error_var.set("Recovery failed. Admin remains locked.")
                self._pin_var.set("")
            return
        if kid_verify(self._settings, u, p):
            # ── Admin 3FA: username + password + R-Code passphrase ──────────
            if u.strip().lower() == ADMIN_KID:
                if not self._admin_rcode_check():
                    self._error_var.set("R-Code incorrect. Access denied.")
                    self._pin_var.set("")
                    return
            self._settings['kid_last_user'] = u
            save_app_settings(self._settings)
    ...
    # SHA-256 of the R-Code passphrase (HYDEV001). A rotated secret set via
    # emergency recovery lives in settings['admin_rcode_sha256'] and takes
    # priority over this baked-in default.
    ADMIN_RCODE_SHA256 = ('7e23f29f13be8c3431f50d6c92fe6b4eedf651cfa0cc16aece474e67bdd5128f')

    def _admin_rcode_hash(self) -> str:
        return (self._settings.get('admin_rcode_sha256', '')
                or self.ADMIN_RCODE_SHA256)

    def _admin_rcode_check(self) -> bool:
        """Third factor for admin1: prompt for the R-Code passphrase in a
        masked dialog and compare SHA-256 against the effective hash."""
        ...
        dlg.transient(self); dlg.grab_set()
        tk.Label(dlg, text='🔐  Admin R-Code', font=('Segoe UI', 11, 'bold'),
                 fg='#F85149', bg='#0d1117').pack(pady=(14, 2))
        tk.Label(dlg, text='Enter the R-Code passphrase (3rd factor)',
                 font=('Segoe UI', 8), fg='#8b949e', bg='#0d1117').pack()
        rv = tk.StringVar()
        e = tk.Entry(dlg, textvariable=rv, show='•', font=('Segoe UI', 10),
                     bg='#161b22', fg='#cdd9e5', insertbackground='#F85149',
                     relief='flat', highlightbackground='#30363d',
                     highlightthickness=1, justify='center')
        e.pack(pady=6, padx=24, fill='x'); e.focus_set()
        result = {'ok': False}
        def _submit(_=None):
            result['ok'] = (_hl.sha256(rv.get().encode()).hexdigest()
                            == self._admin_rcode_hash())
            dlg.destroy()
        e.bind('<Return>', _submit)
        tk.Button(dlg, text='Verify', font=('Segoe UI', 9, 'bold'),
                  bg='#21262d', fg='#F85149', relief='flat', padx=14, pady=4,
                  cursor='hand2', command=_submit).pack(pady=(0, 10))
        self.wait_window(dlg)
        return result['ok']

    def _admin_recovery_flow(self) -> bool:
        """Emergency recovery: verify the one-time 16-hex recovery code, then
        force setting a NEW R-Code secret (entered twice). On success the
        lock clears and the recovery hash is burned — each recovery code is
        single-use."""
        import hashlib as _hl
        stored = self._settings.get('admin_recovery_sha256', '')
        if not stored:
            messagebox.showerror('Recovery',
                'Emergency lock is active but no recovery code exists — '
                'restore paa_settings.json from backup.', parent=self)
            return False
        dlg = tk.Toplevel(self)
        dlg.title('Emergency Recovery')
        dlg.configure(bg='#0d1117')
        dlg.geometry('340x250')
        dlg.transient(self); dlg.grab_set()
        tk.Label(dlg, text='🚨  EMERGENCY RECOVERY', font=('Segoe UI', 11, 'bold'),
                 fg='#F85149', bg='#0d1117').pack(pady=(12, 2))
        tk.Label(dlg, text='Enter the 16-character recovery code:',
                 font=('Segoe UI', 8), fg='#8b949e', bg='#0d1117').pack()
        code_v = tk.StringVar()
        tk.Entry(dlg, textvariable=code_v, font=('Consolas', 11), justify='center',
                 bg='#161b22', fg='#cdd9e5', insertbackground='#F85149',
                 relief='flat', highlightbackground='#30363d',
                 highlightthickness=1).pack(pady=4, padx=24, fill='x')
        tk.Label(dlg, text='New R-Code secret (enter twice):',
                 font=('Segoe UI', 8), fg='#8b949e', bg='#0d1117').pack(pady=(8, 0))
        new1_v, new2_v = tk.StringVar(), tk.StringVar()
        for v in (new1_v, new2_v):
            tk.Entry(dlg, textvariable=v, show='•', font=('Segoe UI', 10),
                     justify='center', bg='#161b22', fg='#cdd9e5',
                     insertbackground='#F85149', relief='flat',
                     highlightbackground='#30363d', highlightthickness=1
                     ).pack(pady=2, padx=24, fill='x')
        result = {'ok': False}
        def _submit():
            code = code_v.get().strip().lower()
            if _hl.sha256(code.encode()).hexdigest() != stored:
                messagebox.showerror('Recovery', 'Recovery code incorrect.', parent=dlg)
                return
            if len(new1_v.get()) < 4 or new1_v.get() != new2_v.get():
                messagebox.showerror('Recovery',
                    'New secrets must match (min 4 chars).', parent=dlg)
                return
            self._settings['admin_rcode_sha256'] = _hl.sha256(
                new1_v.get().encode()).hexdigest()
            self._settings['admin_emergency_lock'] = False
            self._settings['admin_recovery_sha256'] = ''   # burn the code
            save_app_settings(self._settings)
            result['ok'] = True
            dlg.destroy()
            messagebox.showinfo('Recovery',
                '✅ New R-Code set, emergency lock cleared.\n'
                'The recovery code is now burned (single-use).', parent=self)
        tk.Button(dlg, text='Recover', font=('Segoe UI', 9, 'bold'),
                  bg='#21262d', fg='#F85149', relief='flat', padx=14, pady=4,
                  cursor='hand2', command=_submit).pack(pady=(8, 10))
        self.wait_window(dlg)
        return result['ok']

    def _launch(self, username: str):
        global CURRENT_KID, CURRENT_OPIE
        CURRENT_KID = username
        # OPIE disabled for now (module removed) — role-aware agent init skipped.
        CURRENT_OPIE = None
        presence_touch(login=True)
        self.destroy()
