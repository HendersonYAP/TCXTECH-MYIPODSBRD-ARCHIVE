...
def lotto_derive_owned_from_portfolios(ports: dict):
    ...
    IMPORTANT: owned is filtered to ONLY the 33 hardcoded LOTTO_STOCK_LIST
    codes. IPO stocks and any other MYFOLIO holdings that aren't in the 33
    are intentionally excluded — the owned tag and pickmax bonus only apply
    to stocks the lottery actually knows about.
    """
    if not ports:
        return set(), {}
    ...
    # Build a set of bare codes that are in the hardcoded 33 for fast lookup
    ...
    for ticker, item in myr_port["holdings"].items():
        ...
        # Only include if this code is one of the 33 lottery stocks
...
class LottoStockWindow(tk.Toplevel):
    ...
    def _build_road_section(self):
        ...
        if head_start > 0:
            tk.Label(prog_frame, text=f"🎁 Head start: {head_start:.4f} units (live from MYR Position)",
...
class SCAWindow(tk.Toplevel):
    ...
    def _apply_portfolios(self, ports):
        ...
        # ── Instant render from disk-cached prices ────────────────────────────
    ...
    def _open_stock_chart(self, ticker):
        ...
        def _fetch_div_data():
            ...
            def _get_dividends():
                try:
                    # Use same key as _prefetch_sca_dividends so cache hits
                    yf_sym = _mf_ticker_for_market(ticker, mkt)
                    # 1. In-memory (prefetched at splash)
                    if yf_sym in _MF_DIV_CACHE:
                        divs = _MF_DIV_CACHE[yf_sym]
                    else:
                        # 2. Disk parquet (7-day TTL, written by prefetch)
                        disk = cache_get_divs(yf_sym)
                        if disk is not None:
                            _MF_DIV_CACHE[yf_sym] = disk
                            divs = disk
                        else:
                            # 3. Live fetch only if both caches miss
                            divs = yf_t.dividends
                            if divs is not None and not divs.empty:
                                if divs.index.tz is not None:
                                    divs.index = divs.index.tz_localize(None)
