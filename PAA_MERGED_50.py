...
class MYIPOApp(tk.Tk):
        ...
        def _draw_copy(x0, tag):
            ...
            for si, (header, items) in enumerate(self._ticker_sections()):
                ...
                for sym, label in items:
                    ...
                    is_top10 = (sym == 'MYIPO+ Top 10')
                    x_start  = x

                    iid = c.create_text(x, y, text=label, font=F_LBL,
                                        fill='#D29922' if is_top10 else '#888',
                                        anchor='w', tags=(tag,))
                    x = c.bbox(iid)[2] + 5
                    ...
                    if is_top10:
                        self._mkt_hitboxes.append((x_start, x, self._open_top10_board))
    ...
    def _ticker_sections(self):
        ...
        """
        secs = [('🇲🇾 MALAYSIA', [(s, l) for s, l, _ in self.BURSA_INDICES])]
        world = [(s, l) for s, l, _ in self.WORLD_INDICES]
        if world:
            secs.append(('🌏 WORLD', world))
        secs.append(('💱 FX', [(f'FX:{c}', f'{c}MYR') for c in ('USD', 'HKD')]))
        return secs
